"""C-R2-03: drawn cell tt_digits / w4 / cpu_ttl / numpy / none (draw seed 13374394572435975906).

Cohort C builds the drawn cell; it does not choose it. Posted on the bus before the run.

  pressure  cpu_ttl, SAME definition as C-R2-01 (not re-tuned): training stops when client process CPU
            time (time.process_time, all threads) spent in the loop reaches --ttl-cpu-s = 1.0.
            A warm-up rollout runs before the clock. Held-out scoring and oracles are outside it.
  substrate numpy: E7's own rollout (NpEncounter + family forward), not the fused kernel.
  setup     E7b exactly otherwise: G7 tt_digits + codebook, LuaArchive, batch 128, E6's 8 train seeds,
            top-16 on E6's 64 held-out seeds, RNG PCG64([710, run_seed, 4, family index]).
  loophole  Redis server-side Lua CPU is not charged; the redis-server used_cpu delta is reported.
  primary   held64_median over run seeds 0-7 >= 62.01 - 0.5 * 5.84 = 59.09
            (round 1 baseline w4 tt_digits train8_held64, E9)
  ...
  python -m primordial.cohorts.c.r2_03_cpu_ttl_numpy [--ttl-cpu-s 1.0] [--run-seeds 0-7]
"""
...
GS, FAM, BATCH = 4, "tt_digits", 128
CELL = {"representation": "tt_digits", "world": "w4", "pressure": "cpu_ttl", "substrate": "numpy", "channel": "none"}
BASE_MEDIAN, BASE_IQR = 62.01, 5.84
...
def main() -> None:
    ...
    p.add_argument("--max-gens", type=int, default=200)   # E7's budget is the ceiling; TTL should bind first
    a = p.parse_args()
    ...
    with RowWriter(qd_ledger.CELLS, EXP, commit_every_s=10**9) as q:
        q.write({"cell": CELL, "mechanism": f"closed_loop_tt_digits_codebook_numpy_cpu_ttl_{a.ttl_cpu_s:g}s",
