"""Master Smith cycle 1 — candidate v1 for MINT-0001 (vacuous_truth).
...
def _parse_claim(claim: str):
    ...
    if m:
        return "universal", _split_domain(m.group(1))[0], m.group(2)
    m = re.match(r"all\s+(.+?)\s+are\s+(.+)$", c)
    if m:
        return "universal", _split_domain(m.group(1))[0], m.group(2)
    m = re.match(r"no\s+(.+?)\s+is\s+(.+)$", c)
    if m:
        return "neg_universal", _split_domain(m.group(1))[0], m.group(2)
    m = re.match(r"none of the\s+(.+?)\s+are\s+(.+)$", c)
    if m:
        return "neg_universal", _split_domain(m.group(1))[0], m.group(2)
    m = re.match(r"(?:if|whenever)\s+an?\s+(.+?)\s+is\s+(.+?),\s*(?:then\s+)?it\s+is\s+(.+)$", c)
    if m:
        dom = _split_domain(m.group(1))[0] + " that is " + m.group(2)
        return "conditional", dom, m.group(3)
    m = re.match(r"(?:some|at least one)\s+(.+?)\s+is\s+(.+)$", c)
    if m:
        return "existential", _split_domain(m.group(1))[0], m.group(2)
    m = re.match(r"there is an?\s+(.+?)\s+that is\s+(.+)$", c)
    if m:
        return "existential", _split_domain(m.group(1))[0], m.group(2)
    return None
...
_EMPTY_PATTERNS = [
    ...
    r"(?:^|[.,]\s*)no\s+(.+?)\s+in\s+[^,.]+?\s+is\s+(.+?)[.,]",      # 'No X in C is Q.' -> domain 'X that is Q'
]
...
def _domain_facts(text: str, dom_key: frozenset, pred_key: frozenset):
...
def op_vacuous_truth(state):
    ...
    if not parsed:
        return state
    q, dom, pred = parsed
    n_dom, n_sat = _domain_facts(text, _key(dom), _key(pred))
