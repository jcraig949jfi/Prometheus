...
| 008 | **bool() coercion turns every vector program into a witness** — the closure gauntlet's coerced comparison cast outputs to `bool`; for vector-valued targets every candidate "matched" on the probes AND on the 1,290-point exhaustive domain, so an OPERATOR wall would have read SEARCH_ROUTING with thousands of "mechanism-bearing" witnesses | evaluation (instrument artifact) | `hephaestus/src/closure_test.py` before 2026-09-11; specimen 3 run 1 would have been it | a positive control that the instrument can say OPERATOR, run BEFORE the instrument is declared standard; spec-declared equality (`COERCE`) |
| 009 | **Six-probe aliases** — programs that agree with the target on the world's six probes and fail on the other 1,290 vectors: 4 A0 aliases beside 1 witness on `p09(X, p07(X))`, 1 beside 4 on `p01(p03(X))` (specimen 3 CONTROL set, run 2); none on the 20 LOST targets | evaluation (verification domain too small) | `hephaestus/closure_results/q045_lost_class.json`, `arms.A0.alias` | the exhaustive column; never quote a "witness" from a probe-only match |
