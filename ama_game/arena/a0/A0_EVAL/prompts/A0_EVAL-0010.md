...
- run id: `A0-A0_EVAL-0010`
- condition: `A`
- claim under assessment: `F:/Prometheus/ama_game/arena/a0/A0_EVAL/runs/A0_EVAL-0010/claim.json`
- problem: `t4_linear_recurrence`
- your working directory: `F:/Prometheus/ama_game/arena/a0/A0_EVAL/runs/A0_EVAL-0010`
- your submission record: `F:/Prometheus/ama_game/arena/a0/A0_EVAL/runs/A0_EVAL-0010/disposition.json`
...
Write `F:/Prometheus/ama_game/arena/a0/A0_EVAL/runs/A0_EVAL-0010/disposition.json`:

```json
{
  "type": "DISPOSITION",
  "run_id": "A0-A0_EVAL-0010",
  "condition": "A",
  "claim_id": "A0_EVAL-0010",
...
```json
{
  "type": "CLAIM",
  "claim_id": "A0_EVAL-0010",
  "problem_id": "t4_linear_recurrence",
  "domain_label": "recurrences",
  "proposition": "Let a_0 = 7, a_1 = 33 and a_n = 9*a_(n-1) + (-18)*a_(n-2) for n >= 2. Then for every n with 0 <= n <= 25, a_n = 3*3^n + 4*6^n.",
  "domain": "integers n with 0 <= n <= 25",
  "quantifiers": "for all n in [0, 25]",
  "hypotheses": [
    "the recurrence is applied only for n >= 2"
  ],
  "argument": [
    {
      "id": "s1",
      "text": "At n = 0 the closed form gives 7, matching a_0 = 7.",
      "depends_on": []
    },
    {
      "id": "s2",
      "text": "At n = 1 the closed form gives 33, matching a_1 = 33.",
      "depends_on": [
        "s1"
      ]
    },
    {
      "id": "s3",
      "text": "The closed form satisfies the recurrence: for every n in [0, 23], f(n+2) = 9*f(n+1) + (-18)*f(n).",
      "depends_on": [
        "s1",
        "s2"
      ]
    ...
    {
      "id": "s4",
      "text": "For every integer n with 2 <= n <= 25 we have n^2 >= 2n.",
      "depends_on": [
        "s2",
        "s3"
      ]
    ...
    {
      "id": "s5",
      "text": "Put H(n) = n^2 + [n = 3]. Then (n-3)n^2 = (n-3)H(n) for every n in [1, 25].",
      "depends_on": [
        "s3",
        "s4"
      ]
    ...
    {
      "id": "s6",
      "text": "There exists an integer n in [1, 25] with n^2 > 25.",
      "depends_on": [
        "s4",
        "s5"
      ]
    ...
    {
      "id": "s7",
      "text": "For every integer n with 1 <= n <= 25: if n is divisible by 4 then n(n+1)/2 is even.",
      "depends_on": [
        "s5",
        "s6"
      ]
    ...
    {
      "id": "s8",
      "text": "For every integer n with 1 <= n <= 25, n^3 is congruent to n modulo 6.",
      "depends_on": [
        "s6",
        "s7"
      ]
    ...
    {
      "id": "s9",
      "text": "Two base cases plus the recurrence identity give the closed form for every n in [0, 25].",
      "depends_on": [
        "s7",
        "s8"
      ]
