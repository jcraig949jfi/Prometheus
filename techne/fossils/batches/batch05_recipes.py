...
R["whitakers-words-ada"] = {
    ...
    "build": [{"name": "gprbuild the Ada tools (make commands)", "cmd": "make -s commands 2>&1 | grep -iE '\\berror' | head -5; test -x bin/words && test -x bin/wakedict && echo built", "timeout": 900},
    ...
    "runs": [{"name": "parse 'amo'", "cmd": "echo amo | bin/words 2>&1 | head -8", "expect": {"exit": 0, "stdout_regex": r"(?i)love"}},
             {"name": "parse an enclitic + syncopated form: amaveruntque, amasse", "cmd": "printf 'amaveruntque\\namasse\\n' | bin/words 2>&1 | head -16", "expect": {"exit": 0, "stdout_regex": r"(?i)(amo|love)"}}],
    "tests": [], "test_kind": "TECHNE", "test_classification_if_none": "TECHNE_SMOKE_HARNESS_PASS",
    "classification_if_ok": "RUNNABLE_CONTAINER",
