# DISPLAY variant of the PDP-1 world (TECHNE-87b, 2026-09-16): the same pinned open-simh and macro1,
# built with libsdl2 present so simh compiles its video layer and the Type 30 DPY device exists;
# xvfb provides a framebuffer so a scope program can run and be screenshotted headless.
#   docker build -t prometheus-fossil-pdp1:display -f fossil-pdp1-display.Dockerfile .
# The headless world's own description follows unchanged.
...
RUN apt-get update && apt-get install -y --no-install-recommends \
        build-essential git ca-certificates libpcre3-dev libedit-dev expect file procps python3 libsdl2-dev xvfb xauth imagemagick \
