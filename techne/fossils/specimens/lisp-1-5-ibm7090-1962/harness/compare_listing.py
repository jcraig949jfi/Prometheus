...
Prints the counts; exits 1 if any (location, word) pair differs, so the recipe gate is the
oracle and not the assembler's exit status. Usage: python3 compare_listing.py ref.lst new.lst
"""
import re
import sys
...
diff = sorted(k for k in ref if k in new and new[k] != ref[k])
only_ref = sorted(k for k in ref if k not in new)
only_new = sorted(k for k in new if k not in ref)
print("reference words=%d generated words=%d identical=%d differ=%d only_in_reference=%d only_in_new=%d "
      "(duplicate locations ref=%d new=%d)" % (len(ref), len(new), same, len(diff), len(only_ref), len(only_new), dref, dnew))
for k in diff[:12]:
