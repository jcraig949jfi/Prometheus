...
def compile_genome(norm: dict, cycle: int, request_url: Optional[str] = None) -> tuple:
    ...
    g = ResearchGenome(
        ...
        fulltext_available=False,           # we hold abstracts; say so
            ...
            if frontier["kind"] == "citation_expand":
                ...
                for work in results:
                    ...
                    if domain.is_rejected(dstat):
                        ...
                        rec.sources_rejected += 1
                        continue
                    if not norm.get("abstract"):
                        # No abstract: record the rejection so the same node is not re-fetched
                        # every cycle, and so the corpus's abstract-coverage rate is auditable.
                        store.append("rejected", {"source_id": norm["source_id"],
