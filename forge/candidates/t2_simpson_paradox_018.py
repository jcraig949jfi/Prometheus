...
class ReasoningTool:
    """Control Theory x pgmpy_acids - simpson_paradox"""
    ...
    def _reason(self, structure):
        ...
        model = build_bn([(entity, "better") for entity in entities])
        # Compute P(better | entity) for each entity
        probabilities = {}
        for entity in entities:
            probabilities[entity] = conditional_query(model, ["better"], {entity: True})
        # The answer is a SPECIFIC ENTITY NAME from the prompt
        best_entity = max(probabilities, key=probabilities.get)
        return {"answer": best_entity, "confidence": 0.8, "reasoning": "Computed from data"}
    ...
    def _score(self, candidates, computed_answer):
        ...
        for c in candidates:
            ...
            if computed_answer.lower() in c.lower():
                ...
                score = 1.0 / (1.0 + _ncd(computed_answer, c))
            results.append({"candidate": c, "score": score})
        return results

    def _calibrate(self, scored):
        # Apply control theory to adjust scores based on system stability
        calibrated = []
        for result in scored:
            # Use entropy to measure system stability
            stability = entropy([result["score"]])
            # Adjust score based on stability
            calibrated_score = result["score"] * (1 - stability)
            calibrated.append({"candidate": result["candidate"], "score": calibrated_score})
        return calibrated