...
RUNS = {
    "v2_d2 (50/50 Run A)": APOLLO_DIR / "run_v2d2",
    "v2_d2b (50/50 Run B)": APOLLO_DIR / "run_v2d2b",
}
...
def check_deepseek_balance():
    """Check DeepSeek API balance. Returns dict or None on failure."""
    try:
        import requests
        resp = requests.get(
...
def save_and_push_report(analysis, data, tokens_used=None, balance=None):
    ...
    est_str = est_now.strftime("%A, %B %d, %Y at %I:%M %p %Z")
    ...
    if tokens_used:
        report += f"\n## API Usage\n"
        report += f"- Analysis tokens: {tokens_used.get('prompt_tokens', 0)} in / {tokens_used.get('completion_tokens', 0)} out / {tokens_used.get('total_tokens', 0)} total\n"
    if balance:
        if "balance_usd" in balance:
            report += f"- DeepSeek balance: ${balance['balance_usd']:.2f} USD remaining\n"
        elif "error" in balance:
            report += f"- Balance check failed: {balance['error']}\n"
...
def main():
    ...
    if balance and "balance_usd" in balance:
        print(f"  Balance: ${balance['balance_usd']:.2f} USD")
    else:
    ...
    tokens_used = None
    try:
        analysis, tokens_used = call_deepseek(prompt)
        print(f"Analysis received. Tokens: {tokens_used.get('total_tokens', '?')}")
    except Exception as e:
    ...
    save_and_push_report(analysis, data, tokens_used=tokens_used, balance=balance)
    print("Done.\n")
