...
def produce_offspring(population: list, compiled_sources: list,
                      ...
                      aos=None, use_batch: bool = True) -> list:
...
def run_apollo(smoke_test: bool = False, config_path: str = None,
    ...
    while generation < max_gen:
        ...
        if aos is not None and not is_warmup:
            # Check which offspring Pareto-improve over parents
            parent_arrays = [fv.as_array() for fv in fitness_vectors]
            for child_fv, op_name in zip(child_fitness, operator_names):
                child_arr = child_fv.as_array()
                # Reward = 1 if child dominates any parent, else 0
                reward = 0.0
                for pa in parent_arrays:
                    if np.all(child_arr >= pa) and np.any(child_arr > pa):
                        reward = 1.0
                        break
                aos.update(op_name, reward)
