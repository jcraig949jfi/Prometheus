"""V0.5 fresh-seed replication of the V0.4 crucible and its null controls. One arm per invocation.

    python proteus/v0_5/run_crucible.py NC1B|NC2|NC3|NC4|NC5|V0_5
...
Refuses to run against any grammar or runtime other than the ones in PREREG_V0_5.json.
...
def run_arm(arm: str, pre: dict):
    ...
    cap = d["tape_words"] // IW
    root = SplitMix64(seed_from("proteus.v0_5.crucible", pre["seed"], arm, grammar.GRAMMAR_HASH))
    ...
    nc4_lengths = None
    if arm == "NC4":
        with open(os.path.join(HERE, "RESULT_V0_5.json"), encoding="utf-8") as f:
            v3 = json.load(f)
...
def main():
    arm = sys.argv[1] if len(sys.argv) > 1 else "V0_5"
    if arm not in ("NC1B", "NC2", "NC3", "NC4", "NC5", "V0_5"):
        raise SystemExit("arm must be NC1B, NC2, NC3, NC4, NC5 or V0_5")
    pre = load_prereg()
...
if __name__ == "__main__":
    ...
    assert_not_canonical("run run_crucible.py")
    sys.exit(main())
