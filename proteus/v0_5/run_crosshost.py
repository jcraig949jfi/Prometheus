...
the battery coordinates, the null controls' kernels, and a diffusion segment.
    ...
    python proteus/v0_5/run_crosshost.py [outfile]
...
from proteus.v0_5.run_crucible import per_lineage_coords  # noqa: E402
...
def main():
    ...
    art["structural_audit"] = {
        "v0_4": _nc5.audit_transitions("v0_4"),
        "nc5": _nc5.audit_transitions("nc5"),
        "symmetry_proof": _nc5.paired_tape_symmetry_proof("v0_4"),
    }
    # V0.5: the measured structural kernel on a small closed subspace, plus its equilibrium
    # quantities, so the Part II numbers are themselves replicated across runtimes.
    from proteus.v0_5 import kernel as _K
    _st = _K.state_space((16, 32), 8)
    _P, _oc, _tr = _K.measure_kernel(_st, 4000, 20260905, "crosshost")
    _pi, _it, _dl = _K.stationary(_P, _st)
    _cur = _K.currents(_P, _pi, _st)
    _ep = _K.entropy_production(_P, _pi, _st)
    art["v0_5_kernel_probe"] = {
        "states": [list(s) for s in _st],
        "kernel": {repr(list(k)): {repr(list(j)): repr(v) for j, v in row.items()}
                   for k, row in _P.items()},
        "pi": {repr(list(k)): repr(v) for k, v in _pi.items()},
        "max_abs_current": repr(max(abs(r["J"]) for r in _cur)),
        "entropy_production_sigma": repr(_ep["sigma"]),
        "one_way_edges": _ep["one_way_edges"],
    }
...
if __name__ == "__main__":
    ...
    assert_not_canonical("run run_crosshost.py")
    sys.exit(main())
