...
**CRITICAL LESSON FROM 136 CHALLENGES — read this before proposing:**
The instrument's track record reveals a sharp divide:
- **Within-domain problems FIND SIGNAL.** Arithmetic↔arithmetic (mod-p fingerprints, Hecke congruences, Galois image detection, enrichment slopes) and topology↔topology consistently produce measurable constants with z-scores > 3.
- **Cross-domain problems (arithmetic↔topology) FIND NULLS.** Every attempt to bridge number theory and topology at the coefficient level has produced z < 2. The scalar layer between domains is empty after prime detrending. The six classical sequence functors all gave zero cross-domain matches.
- **The sweet spot is STRUCTURAL bridges within a domain family**, or transformation detection (Layer 3) within algebraic families. Problems that ask "measure X within the Hecke algebra" or "detect Y within genus-2 curves" succeed. Problems that ask "correlate knot invariants with elliptic curve conductors" fail.

**FOCUS YOUR PROBLEMS ON:**
- Within-arithmetic measurements (Hecke algebras, Galois representations, L-functions, modular forms)
- Within-topology measurements (knot polynomials, lattice theta series, Betti numbers, persistence)
- Within-physics measurements (crystal symmetries, particle spectra, CMB structure)
- Layer 3 transformation detection (twists, lifts, dualities, functors) within a domain
- Metric/geometric structure of mathematical spaces (like the Gamma metric finding)

**AVOID:** Cross-domain coefficient correlations, number↔topology bridges, any "correlate X from domain A with Y from domain B" at the scalar level.
...
4. At least 1 use physics data (CODATA, PDG, Planck CMB, crystal structures)
...
7. Priority: WITHIN-DOMAIN problems where success reveals a NEW DIMENSION of structure the 30+ existing constants cannot see
8. DO NOT re-propose any of the 21 confirmed dead ends listed above
9. DO NOT propose cross-domain scalar correlations (arithmetic↔topology) — these are confirmed dead
