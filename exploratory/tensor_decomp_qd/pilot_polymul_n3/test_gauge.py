"""Canonicalizer unit tests for polymul-3 tensor over F_2.

These MUST pass before MAP-Elites runs. Each is a hard kill criterion
for the canonicalizer design.

Run: python -m exploratory.tensor_decomp_qd.pilot_polymul_n3.test_gauge
"""
...
from .gauge import (
    GAUGE_ACTIONS, GAUGE_SIZE, apply_gauge, canonicalize, orbit_size,
    stabilizer_order, effective_rank,
)
...
def test_gauge_size():
    print(f"\n[1] Gauge group size = 12 (D_3 x Z_2 from <SUB, REV, SWAP>)")
    _assert(GAUGE_SIZE == 12, f"|G| = 12 (got {GAUGE_SIZE})")
...
def test_orbit_equivalence_collapses():
    ...
    for seed in range(n_trials):
        # Apply gauge transform + column permutation.
...
def test_near_miss_distinct():
    ...
    _assert(collisions == 0,
            f"10 single-bit-flip variants distinct from Karatsuba canonical "
...
def run_all():
    print("=" * 72)
    print("Canonicalizer unit tests for polymul n=3 over F_2")
    print("=" * 72)
    tests = [
        test_gauge_size,
