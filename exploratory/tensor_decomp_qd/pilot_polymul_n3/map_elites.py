"""MAP-Elites on CP decompositions of the polymul tensor (n=3) over F_2.

Genome: (U, V, W) with U, V each (3, R_MAX) and W (5, R_MAX), uint8 over
{0, 1}. Effective rank = #non-zero columns after gauge-canonicalization.

Fitness: binary — 1 iff (U, V, W) reconstructs POLYMUL_T.

Archive cell: (rank, sparsity_bin, stabilizer_bin) on canonical form.

Forbidden-cell discipline: any candidate that canonicalizes to rank
< RANK_MIN_HARD raises ForbiddenCellViolation. Hard kill — never seen
in correct runs.
"""
    ...
    def submit(self, U: np.ndarray, V: np.ndarray, W: np.ndarray):
        ...
        if r_eff < self.rank_min_hard:
            raise ForbiddenCellViolation(
                f"canonical rank {r_eff} < {self.rank_min_hard} (polymul-3 F_2 lower bound). "
...
def random_genome(rng: np.random.Generator, r: int = R_MAX, density: float = 0.5):
    """Random dense genome at fixed rank r over F_2."""
...
def pad_genome(U: np.ndarray, V: np.ndarray, W: np.ndarray, r: int = R_MAX):
    """Pad a genome to R_MAX columns with zero columns."""
    cur = U.shape[1]
    if cur >= r:
