"""One-off evaluator for fire-015 v2 responses (decomposition_mode aware)."""
...
def main() -> int:
    probes = json.loads(Path("ergon/learner/diagnostics/fire_015_probes.json").read_text(encoding="utf-8"))
    responses_v2 = json.loads(Path("ergon/learner/diagnostics/fire_015_responses.json").read_text(encoding="utf-8"))
    ...
    Path("ergon/learner/diagnostics/fire_015_eval.json").write_text(
    ...
    for e in evaluations:
        ...
        print(f"    severity: {e['severity'][:140]}")
    return 0
