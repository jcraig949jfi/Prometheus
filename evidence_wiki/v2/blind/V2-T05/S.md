...
- **Ergon Gen-1A (2026-09-01, section 7)**: The MDE-exceeds-bound signature was discovered and acted upon prospectively (n=5 → n=30 redesign). This changed the hypothesis from "can the signature predict doom?" (arm) to "does prospectively applying the signature as an early-abort trigger actually save resources in practice?" (arm). The 1.5-hour Phase 0 cost vs. 27-hour run-time savings motivated focusing Arm B on operational deployment, not post-hoc validation.
