...
- The coupling tensor over NF/Artin/EC/MF/knots measured **feature geometry, not
  object-level coupling**, confirmed across all three scorers ([REF]).
- Re-encoding knots with the mathematically correct features (Mahler measure,
  ...
  does not manufacture a bridge ([REF]).
- The one concrete bridge hypothesis in this space that was actually executed —
  ...
  reverse-substitution: 245,280 evaluations, 0 hits at 10^-40 tolerance ([REF]).
- The NF "backbone" bond structure was killed by a permutation null: z = 0.0, proving the
  structure was distributional, not object-level ([REF]).
- The tensor's own latent-rank claims (effective dim ≤5, SVT rank 12-16) were retracted:
  ...
  ([REF]) — i.e. the instrument manufactures structure from population imbalance.
- Separately, on a different substrate: **generic** operators found zero relations over
  ...
  already LMFDB-recorded, so none were discoveries ([REF], [REF]). A
  ...
  not a general native-operator win ([REF], retrieved as counterevidence to
  [REF]).
- A ranking-gain precedent directly on point for validation method: a state-conditioned
  ...
  ([REF]). This is the direct template for this proposal's mandatory null.
- Data-integrity caution: the elliptic-curve catalog's `object_id` column was NULL on
  ...
  identity key, and the repair watermark is 134475 ([REF]). Any feature vector
...
- A stratification precedent: an abc/Szpiro decrease was first confounded by a
  ...
  count ([REF]) — the same conductor/discriminant-scale confound is expected to
...
- Elliptic curves: conductor, rank, discriminant, j-invariant, trace-of-Frobenius
  ...
  [REF]; verify watermark 134475 before any bulk pull).
...
2. **Stratify by conductor/discriminant decade** before computing any cross-domain
   similarity, to pre-empt the prime-conductor selection effect ([REF]).
...
- **Conductor/discriminant scale confound:** all cross-domain comparisons stratified by
  conductor/discriminant decade (per [REF]); no un-stratified comparison is
...
native-vocabulary-only path (per [REF]) becomes the sole surviving track.
...
1. **Feature-geometry recurrence (most likely, per [REF] and [REF]):**
...
2. **Null disqualification despite high raw significance** (per [REF]): a
...
3. **Dedup wipeout** (per [REF]): every candidate surviving the statistical
...
4. **Object-identity corruption silently biasing the EC arm** (per [REF]) if
   the watermark/lmfdb_label check is skipped — would make any EC-side candidate
   unusable regardless of downstream gates.
5. **Conductor-confounded false positive** (per [REF] pattern) if
