...
feature geometry, not object-level coupling, on all three scorers ([REF]), confirmed
independently by a permutation null on the NF backbone landing at z=0.0 ([REF]); a concrete
attempt to fix a similarity/coupling approach for knots by re-encoding with the mathematically correct
features (Mahler measure, root-of-unity evaluations) still found no signal ([REF]); and a
...
tolerance ([REF]). Separately, a generic-vs-native operator comparison on elliptic curves found
that seven generic operators produced zero relations over 294,909,843 reachable triples while one
native verb (quadratic twist) found 4,476 ([REF], [REF]) — direct evidence that
...
3. Any apparent positive signal in either arm will substantially shrink (>=50% of its materiality) under
   the marginal-shuffle "feature geometry" control, reproducing [REF]/[REF].
4. The naive (within-set) shuffled-label null, if anyone is tempted to reuse it, will again land
   meaningfully above chance (repeating [REF]'s 0.5903 vs 0.5029); this design preregisters the
...
field roots (12,963 available per [REF]'s run). Number fields: NF catalog keyed by
...
distinct triples under isogeny-class dedup — [REF]); (b) the McMullen K3 negative anchor
(expect exactly 0 hits at 10^-40 tolerance over the same 5 Salem polynomials x 12,963 knot roots —
[REF]). Any deviation (missed positive anchor, or a nonzero hit on the negative anchor) means
...
1. **Corrected null (replaces the vacuous naive null).** Across-set stratified conditional permutation:
   ...
   strata, preserving availability and hold-count structure — the exact fix [REF]'s cycle
...
2. **Feature-geometry / marginal-shuffle control.** Recompute the ranking after replacing each object's
   ...
   marginals, destroys cross-catalog joint structure). This directly operationalizes [REF] and
   [REF] as an executable control rather than a citation.
...
5. **Class-level deduplication.** Any "novel" hit must be deduplicated by isogeny-class/equivalence-class
   before being counted, per [REF]'s 3,896 -> 12 collapse.
6. **Catalog identity/integrity control.** EC joins via `lmfdb_label`, watermark check per
   [REF]; refuse any row where `object_id` is NULL or below 134475 without explicit repair
   verification.
7. **Prime/conductor stratification.** Any conductor- or discriminant-indexed feature is stratified by
   ...
   ([REF]), guarding against the Layer-1 "96% is prime factorization" confound.
8. **No SVD/low-rank shortcut.** The ranking/coupling scores are never passed through SVT-style
   low-rank factorization; [REF] showed this is invalid for sparse ordinal MNAR data of this
...
- **Availability confound:** the stratified null in control 1 exists specifically because a
  ...
  than genuine correspondence ([REF]).
...
- **Vocabulary confound:** a null generic-arm result must not be read as "no bridge exists" — it may
  only mean the vocabulary is wrong ([REF] / [REF]); this is why the native arm is
...
cell — mirroring the exact "2 of 4 controls failed -> terminal REDESIGN" bar [REF] already
...
- [REF] -> mandated an executable marginal-shuffle "feature geometry" control (Controls #2),
  not merely a citation of the risk.
- [REF] -> the design does not treat "use the mathematically correct features" as sufficient;
  it requires a native-vs-generic split with the generic arm expected to fail regardless.
- [REF] -> repurposed as the Stage 0 negative calibration anchor (must reproduce exactly 0 hits
  at 10^-40) rather than a hypothesis to re-test.
- [REF] -> reinforced Controls #2 as mandatory (distributional-vs-object-level distinction).
- [REF] -> added the explicit SVD/low-rank ban (Controls #8).
- [REF] / [REF] -> produced the two-arm generic-vs-native design itself; set F4's
  0-hits generic threshold and F6's dedup-collapse threshold and the Stage 0 positive-anchor numbers
  (4,476 / 95% / 12-of-12).
- [REF] -> replaced the naive shuffled-label null with the corrected across-set stratified
  conditional permutation (Controls #1) and set the "2 of N controls fail -> terminal" stopping bar.
- [REF] -> added the EC catalog identity/integrity control (`lmfdb_label`, watermark 134475).
- [REF] -> added the prime/conductor stratification confound defense.
