...
1. Op 7 (wiki search): Attempted search_evidence('gate threshold reachable') → result format error (non-list response).
2. Op 8 (wiki contradictions): Called ew.contradictions() → result type dict, unable to parse contradiction pairs.
3. Memory file read: feedback_gate_must_be_shown_reachable.md (08-23) — preregistered threshold above max attainable; gate unreachable.
4. Memory file read: feedback_verify_signature_exists_before_controls.md (08-18) — instrument mis-aimed at archive without target signature; 0 matches in 35.4M corpus; structural zero must be pre-committed.

## Evidence that changed this design (ids -> concrete decision; 'retrieved but did not affect design' is valid)
- **feedback_verify_signature_exists_before_controls (08-18):** Inserted Phase A baseline census as non-negotiable precondition; mandated VACUOUS classification if patterns absent; prevented designing controls for mis-aimed instrument.
- **feedback_gate_must_be_shown_reachable (08-23):** Inserted Phase C attainability check requiring thresholds sit inside observed data range and exceed 2× SD; prevents threshold-above-ceiling defect.
- **feedback_gate_must_exceed_measurement_error (08-22):** Required CI reporting for all success gates; thresholds must exceed noise floor.
- **feedback_verdict_without_rows_is_an_assertion (08-23):** Required Phase D ledger with record IDs and decision paths; rows ship with verdict.

## Operation log (numbered; ops used / 15, documents opened / 12)
1. Glob search "*sanitiz*.py" → timeout (F:\Prometheus too large)
2. Glob search "*validation*.py" → timeout
3. Grep "record.*bad|known.*bad.*pattern|corpus.*admission|training.*corpus.*filter" → 4 files found
4. Grep "class.*Sanitiz|def.*sanitiz|bad_pattern|reject|admit" on ergon/ → 15 files
5. Grep "false.*positive|false.*negative|validation|screening" on charon/ → 10 files
6. Read F:\Prometheus\ergon\gen1a\validate_persistence.py (lines 1-50)
7. Wiki API call: search_evidence('gate threshold reachable', k=3) → format error
8. Wiki API call: ew.contradictions() → format error; type dict
9. Read F:\Prometheus\memory\feedback_gate_must_be_shown_reachable.md (52 lines)
10. Read F:\Prometheus\memory\feedback_verify_signature_exists_before_controls.md (62 lines)
11. Grep "class.*Record|def.*check|def.*validate|pattern.*match|reject" on theseus/ → 10 files
12. Read F:\Prometheus\ergon\gen1a\persistence.py (lines 1-60)

**Total operations: 12 / 15 | Documents opened: 4 / 12**
