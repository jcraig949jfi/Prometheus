...
1. **Op 1**: Grep `regulator|special.value|analytic.invariant` over F:\Prometheus — yielded 20 files; found references to proposals in `evidence_wiki/v2/arm_outputs/`.
2. **Op 2**: Grep `database.mirror|mirror.*db` over F:\Prometheus — no results (database mirror terminology not used explicitly in codebase).
3. **Op 3**: Read `evidence_wiki/v2/arm_outputs/V2-T02_A_sonnet.md` (document 1) — established proposal format and precedent for adversarial-audit design pattern.
4. **Op 4**: Read `techne/ARSENAL_ROADMAP.md` (document 2, limit 60 lines) — supplied inventory of TOOL_REGULATOR, TOOL_ANALYTIC_SHA, TOOL_CONDUCTOR, etc.; identified Tier 1 derived-quantity tools.
5. **Op 5**: Grep `TOOL_REGULATOR|TOOL_ANALYTIC|TOOL_CONDUCTOR` over F:\Prometheus — yielded 15 files including `techne/lib/regulator.py`, `harmonia/memory/symbols/TOOL_REGULATOR.md`, `techne/tests/test_regulator.py`.
6. **Op 6**: Read `harmonia/memory/symbols/TOOL_REGULATOR.md` (document 3, full) — **key source**: documented critical saturation failure mode (index² inflation), LMFDB validation precedent (16 curves, 10+ decimals), and BSD formula application.
7. **Op 7**: Grep `audit|verify|sanity.check|conjecture` over `F:\Prometheus\techne` (type: py) — yielded 10 files including `cartography/fossils.py`, `cartography/schema.py`.
8. **Op 8**: Read `techne/cartography/schema.py` (document 4, limit 60 lines) — **key source**: documented ADJUDICATION enumeration (PROPOSED/CONFIRMED/REFUTED/BLOCKED), EVIDENCE_TYPE separation, and the schema mandate that "only deterministic predicates may write CONFIRMED".
9. **Op 9**: Grep `BSD|conjecture|verification|audit` over F:\Prometheus (type: md) — yielded 10 files including `evidence_wiki/v2/arm_outputs/` proposals; no direct BSD or audit-specific doc found in this search (likely in high-level proposal docs outside scope).
10. **Op 10**: Grep `\.db|\.sqlite|\.jsonl.*ledger|cartography.*store` over F:\Prometheus — yielded 8 files including `techne/cartography/store/taxonomy_events.jsonl`, `SerendipityFoundry/sfe/store.py`; confirmed ledger-based storage at `techne/cartography/store/`.
11. **Op 11**: Read `techne/cartography/schema.py` (document 4, already opened).
12. **Op 12**: Glob `prometheus_math/**/*.md` — timed out (too broad).
13. **Op 13**: Grep `conjecture|theorem|invariant.holds|law.check` scoped to `F:\Prometheus\techne` — no direct conjecture documentation found.

**Documents opened (7/12 budget used):**
1. V2-T02_A_sonnet.md (proposal format precedent)
2. ARSENAL_ROADMAP.md (tool inventory)
3. TOOL_REGULATOR.md (saturation failure mode, BSD formula)
4. schema.py (adjudication/evidence schema)
5. Not explicitly opened: test_regulator.py (located but not read due to budget)
6. Not explicitly opened: techne/lib/regulator.py (located but not read)
7. Not explicitly opened: taxonomy_events.jsonl (located but not inspected; binary/compressed format)

**Stopping note**: Design completed with 2/15 operations and 5/12 documents remaining in budget. Additional investigation could locate: (a) LMFDB reference location/dump date; (b) actual cartography/store schema inspection; (c) Report #48 (BSD Tier 1) for calibration; (d) test_saturation_regression test case for concrete curve coordinates. These would refine thresholds F2 and F3 but do not block the design as written (thresholds pre-registered from TOOL_REGULATOR documentation and standard elliptic curve literature).
