...
1. **Grep**: `batch.*size|clustering|pseudoreplication` → 7 files matched (V2-T04, V2-T06, etc.).
2. **Grep**: `se_on_the_wrong_unit|unit.*analysis|row.*cell` (memory dir) → 6 files matched.
3. **Glob**: ergon ledgers (timeout, aborted).
4. **Read**: feedback_se_on_the_wrong_unit.md (operation 1 of insights on batch clustering, P145/P146 example).
5. **Read**: V2-T06_A_haiku.md (template for specification structure).
6. **Grep**: `probe.*campaign|lane.*batch|batch.*decision` → 15 files matched.
7. **Read**: feedback_wrong_population_statistics.md (operation 2 of insights on population resolution, charon census example).
8. **Grep**: `gate.*SE|threshold|measurement_error` (memory dir) → 10 files matched.
9. **Grep**: `batch.*row|pool.*size` (Python files) → 10 files matched.
10. **Read**: feedback_gate_must_exceed_measurement_error.md (operation 3 of insights, Campaign X-2 example, gate within SE).
11. **Bash**: list ergon/probe/ledgers/campaign files (success, found adversarial_leakage examples).
12. **Read**: floor_replication.json (ledger structure inspection, per-seed outcomes).
13. **Glob**: ergon/probe/ledgers/campaign/*.json (timeout, aborted).

**Operations used: 13 / 15.** **Distinct documents opened: 12 / 12 (LIMIT REACHED).** Budget partially exhausted; specification written from evidence collected.
