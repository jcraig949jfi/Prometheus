...
1. Grep "learner|procedural.*world|goal.*state" over F:\Prometheus (md files) — found 15 references.
2. Glob **/{roles,charon,ludus,aporia}/*.md — timed out.
3. Read F:\Prometheus\roles\Ludus\ROLE.md (doc 1) — world-supply seat, GATE-W1, depth-gap metric, admission criteria.
4. Read F:\Prometheus\evidence_wiki\v2\arm_outputs\PILOT-1_A_sonnet.md (doc 2) — paired within-task design, repeated measures, confound defenses, preregistered falsifiers.
5. Read F:\Prometheus\SerendipityFoundry\D10\review\DESIGN_BRIEF.md (doc 3) — environment design, information boundary, arms, controls, D-10's procedural task generation.
6. Read F:\Prometheus\SerendipityFoundry\D8\agent_d8\REPORT.md (doc 4, lines 1–200) — verdict, baseline suite (M0a/b/c), hoard/organization, controls (H-BAG, H-SHUFFLE, H-RANDOM), falsifiers.
7. Read F:\Prometheus\SerendipityFoundry\SerendipityFoundryEngine\docs\SERENDIPITY_FOUNDRY_GEN2_ARCHITECTURE.md (doc 5, lines 1–150) — Foundry substrate, atomicity, world isolation, object model.
8. Grep "baseline|random.*floor|chance|ceiling" over F:\Prometheus\SerendipityFoundry (md files) — found 10 references.
9. Grep "solve.*rate|success.*rate|performance" over F:\Prometheus\ludus (py files) — no files matched.

**Operations used: 9/15. Documents opened: 5/12.** Six operations and seven document-opens of budget remain unused; stopped once the primary substrate (D-8, D-10, Ludus, PILOT-1 methodology) and control/baseline patterns were grounded. Unread resources (task-manifest.json, probe timing ledgers, additional baseline variants, detailed Foundry API) are deferred as implementation details in Phase 0, not design essentials.
