...
- C-eb3e53602cce, C-45bd5ff28211, C-98702f29ab81, C-b5c1a85cca8b -> Phase 0 reachability precondition (oracle + generic-navigator far-stratum check, disqualification rule F1, and the borrowed N2/N4/d1<=0.10/1,200-budget conventions).
- C-82fe472469ca -> heuristic floor added as a required, empirically-computed second chance bar (Phase 1), not assumed at a formula value.
- C-353ec1eb022a -> Confound defense requiring an explicit ceiling-vs-floor (Jensen) check before reading any "excess over X" as an effect size.
- C-750f2b6fc3ac -> shuffled-goal null pre-check (F3): a null must be shown to perturb the axis before being used as evidence.
- C-31e6f516f20b -> random-pairing/common-reference chance-floor construction and treatment of zero-count results (rule-of-three framing carried into F1).
- C-b3b4b28a3a62 -> frozen common reference distribution (Phase 1) so exposure cannot be conflated with competence.
- C-305b3854fef0 -> difficulty strata defined by oracle shortest-path distance, never by a classifier over the generator's own output categories.
...
1. Read `F:\Prometheus\evidence_wiki\v2\packs\V2-T03_pack.json` (free, not counted against budget).
2. Glob `**/agent_d4_blind/VERDICT-PHASE1.md` — not found at that relative pattern.
3. Glob `**/agent_d5_blind/VERDICT.md` — not found at that relative pattern.
4. Glob `**/agent_d3_blind/VERDICT-PHASE1.md` — not found at that relative pattern.
5. Grep `agent_d4_blind` over `F:\Prometheus` — located `F:\Prometheus\agent_d4_blind\VERDICT-PHASE1.md`.
6. Grep `accessibility_geometry|far-stratum|N2 pooled` over `F:\Prometheus` — confirmed relevant doc locations (also surfaced files under `evidence_wiki`, correctly not opened).
7. Read `F:\Prometheus\agent_d4_blind\VERDICT-PHASE1.md` (document 1/12) — grounded N2/N4/far-stratum/budget/ablation-z methodology used throughout this design.
8. Grep `agent_d5_blind` over `F:\Prometheus` — no non-evidence_wiki hits; no further file opened.
9. Grep `random-pairing chance floor|random_pairing` over `F:\Prometheus` — surfaced only evidence_wiki arm outputs (not opened, per rules) and one unrelated cartography script (not opened, low relevance).

Ops used: 8 / 15 (excluding the free pack read). Documents opened: 1 / 12.
