...
- Selective-vs-random-eviction is explicitly OUT of scope here — already settled REFUTED (C-fff52fca02a0); this experiment only tests whether evicting either one is safe, not which one to keep.
...
C-f8f06a6e21ca (Gen-1A MR test) supplied both the hypothesis's numeric anchor (tau=0.50, prior median MR=0.333, 23% redundancy rate, edit distance 20/24) and the falsifier thresholds directly reused above. C-a553b0e69795 revealed that behavior-fingerprint deduplication is *already* the live admission policy on at least one substrate, which reframed the task from "should we start deduplicating" to "is an existing, unaudited policy safe" — raising the stakes of Step 0's inventory pass. C-fff52fca02a0 (REFUTED) scoped the "which one to keep" question out of this design entirely, so the falsifiers above test only whether evicting either twin is safe. The recorded contradiction (R-e68c9331eca2, D-5 program-ecology SUPPORTED vs. a different "foundry ecology" substrate NO_EFFECT for accumulated-history value) added the explicit substrate-transfer flag and the PROXY_EVIDENCE labeling requirement in the Confound defenses section.
...
1. search_evidence("byte-identical output deduplication functional equivalence artifacts", k=8) → surfaced C-f8f06a6e21ca, C-a553b0e69795, C-6c7e06892e46, C-9d2f9bf2e064, C-090bbd91ee42, C-e3c149ca4f7e, C-b287aa6823b0, C-96a0e90f4eeb.
2. get_claim("C-f8f06a6e21ca") → full evidence record (MR median 0.333, tau 0.50, 23.0% redundant, edit distance 20/24, n=6,505 pairs, 834 artifacts).
3. get_counterevidence("C-f8f06a6e21ca") → empty (no recorded counter-relations or negative evidence against this claim).
4. search_evidence("behavioral deduplication safe discard redundant", k=6, status='REFUTED') → C-fff52fca02a0.
5. contradictions() → R-e68c9331eca2 (C-3a1c49fa5a78 CONTRADICTS C-3d12c440f087, substrate-dependent executable-history effect), R-2dc413ddca43 (unrelated FAILS_TO_REPLICATE pair, not used).
6. get_claim("C-3d12c440f087") → +10.95pp CFR finding, used only to characterize the contradiction's substrate-dependence, not as direct dedup evidence.
7. get_claim("C-a553b0e69795") → confirms D-5 admission already dedupes on behavior/output fingerprints.

## Evidence that changed this design (ids -> concrete decision; 'retrieved but did not affect design' is valid)
- C-f8f06a6e21ca -> set the hypothesis's numeric anchors and all preregistered falsifier thresholds (tau=0.50, MR median comparisons, edit-distance expectation).
- C-a553b0e69795 -> reframed the experiment as an audit of an already-live policy (Step 0's inventory requirement, and the framing in Motivating evidence).
- C-fff52fca02a0 -> scoped OUT "which duplicate to keep" from Controls/Preregistered falsifiers; this design only tests "evict at all," not "evict which."
- R-e68c9331eca2 / C-3d12c440f087 -> added the substrate-transfer flag and PROXY_EVIDENCE labeling requirement in Confound defenses and Unresolved uncertainty.
- C-9d2f9bf2e064, C-090bbd91ee42, C-e3c149ca4f7e, C-b287aa6823b0, C-96a0e90f4eeb, R-2dc413ddca43 -> retrieved but did not affect design (off-topic to dedup safety: packet-gate whitespace defect, blind-band verdicting, Zaremba spectrum bug, wormhole certification, quadratic-twist relations, an unrelated replication-failure pair).

## Operation log (numbered; ops used / 15, documents opened / 12)
1. search_evidence (initial query) — wiki search, not a document open.
2. get_claim C-f8f06a6e21ca — document opened (1/12).
3. get_counterevidence C-f8f06a6e21ca — wiki query, not a document open.
4. search_evidence (status='REFUTED' query) — wiki search, not a document open.
5. contradictions() — wiki query, not a document open.
6. get_claim C-3d12c440f087 — document opened (2/12).
7. get_claim C-a553b0e69795 — document opened (3/12).
8. Grep "Serendipity Foundry" across F:\Prometheus — repo search, not a document open.
9. Read GEN2.1_RELEASE_PACKET.md (F:\Prometheus\SerendipityFoundry\SerendipityFoundryEngine\) — document opened (4/12).

Total operations used: 9/15. Total documents opened: 4/12. Early stop — remaining budget not needed once the central precedent (C-f8f06a6e21ca) and its immediate context (admission policy, eviction-selectivity null, substrate-transfer contradiction, and SFE's actual current architecture) were confirmed.
