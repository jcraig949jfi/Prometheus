...
1. Grep `F:\Prometheus` for `bridges|bridge` (*.md) — files_with_matches.
2. Grep `F:\Prometheus` for `feature[- ]vector similarity|cross-domain` —
   files_with_matches. (Note: both op 1 and op 2 returned many hits under
   the cwd-relative `evidence_wiki` tree, which were excluded from all
   further consideration per the out-of-scope rule.)
3. Grep `F:\Prometheus` for `canonicality hypothesis|candidate bridge|bridge
   candidate|feature vector similarity` — files_with_matches.
4. Grep `F:\Prometheus` for `knots.*number field|number field.*knot|elliptic
   curve.*knot|zeros.*knot|object catalog` — files_with_matches.
5. Read `F:\Prometheus\cartography\v2\knot_bridge.py` (document 1).
6. Read
   `F:\Prometheus\aporia\docs\deep_research_reports\2026-05-21\00074_isl_02_knot_theory_number_theory_bridges_2025.md`
   (document 2).
7. Read `F:\Prometheus\cartography\shared\scripts\v2\layer2\signature_matcher.py`
   (document 3).
8. Read `F:\Prometheus\cartography\shared\scripts\v2\moonshine_oeis_bridge.py`
   (document 4).
9. Grep `F:\Prometheus\cartography` (*.md) for `bridge_candidates|
   signature_matcher|Kill #11|tier3_confirmed|cross-domain bridge` —
   files_with_matches.
10. Grep `F:\Prometheus\cartography\convergence` for
    `signature_matcher|bridge_candidates|SURVIVES|KILLED` — timed out
    (20s ripgrep timeout over a large directory); no results obtained,
    not retried (budget discipline).
11. Read `F:\Prometheus\cartography\docs\cross_domain_falsification_protocol.md`
    (document 5).
12. Grep `F:\Prometheus\cartography\docs\all_findings_classified.md` for
    `bridge|signature|cosine|survive|killed` (content mode, head_limit 80)
    (document 6).

Ops used: 12/15. Documents opened: 6/12. Unused budget: 3 ops, 6 documents —
left unspent deliberately once the falsification-protocol document and the
transport-matrix finding gave a materially load-bearing prior result;
further spend would likely have gone to the three items listed under
"Unresolved uncertainty" rather than to anything that would have changed the
hypothesis or falsifiers.
