...
## Motivating evidence
- Procedurally generated (or authored) worlds can instantiate an apparently strategic domain while containing essentially no strategic decision: a one-ply greedy heuristic picks the optimal action in 85-100% of states and four plies of trivial search solves the world (Ludus, C-ba882ad5cc7e). If this world class were used as-is, a learner matching greedy performance would look "successful" while having learned nothing.
- A naive chance/null floor is not automatically informative or well-separated from real performance: an independent re-evaluation found a non-promoted survivor population re-evaluating TRUE at 45.9% against a 46.1% random-pairing chance floor — indistinguishable from chance despite looking like a real result on its face (Harmonia, C-31e6f516f20b). This is the direct cautionary precedent for what happens if "better than chance" is asserted without characterizing the floor's own uncertainty.
- Baseline/comparator choice is known to be substrate-sensitive and must be frozen before seeing learner data, not selected post hoc to flatter either arm (contradiction R-e68c9331eca2: Daedalus vs Aporia disagree on outcome for the same abstract claim across two substrates, traced to differing frozen comparator rules, not to differing learner competence).
- The "attainable-without-reasoning ceiling" pattern recurs across substrates (graph edge SAME_MECHANISM linking C-ba882ad5cc7e to C-82fe472469ca, a coprime-heuristic-vs-one-ply-greedy world ceiling held out during V0 benchmarks) — evidence this is a general confound class for PCG-world evaluations, not a one-off.
...
## Confound defenses
- **World triviality** (Ludus C-ba882ad5cc7e): defended by the mandatory pre-registered admissibility gate (F1) run before learner scoring; if it fails, the evaluation is aborted and the generator is redesigned, not silently re-interpreted as a learner result.
- **Chance floor sitting at/near real performance** (Harmonia C-31e6f516f20b): defended by computing B3's SE empirically via permutation rather than asserting a nominal chance value (e.g. 50% or 1/branching-factor), and requiring the learner's margin over B2 to exceed 3x B2's own SE (feedback_gate_must_exceed_measurement_error), not just be numerically positive.
...
- C-ba882ad5cc7e materially changed the design: without it, this proposal would have used a naive random-chance baseline; instead it adds a mandatory pre-registered world-triviality admissibility gate (F1) and makes shallow fixed-depth search (B2), not uniform-random chance, the primary comparator.
- C-31e6f516f20b materially changed the design: it added the requirement to empirically characterize the chance floor's own SE via a random-pairing permutation null (B3) and to require the learner's margin over B2 to exceed 3x B2's SE (F2) rather than merely be positive — directly modeled on the observed 45.9%-vs-46.1% near-miss.
- Contradiction R-e68c9331eca2 materially changed the design: it reinforced fixing the comparator rule (B1/B2 as primary, B0/B3 as reported-not-primary) before any learner data is seen, rather than choosing post hoc whichever baseline makes the learner look best.
- C-82fe472469ca (related_findings graph edge, SAME_MECHANISM) did not change a concrete design decision beyond confirming the "attainable-without-reasoning ceiling" is a recognized recurring pattern across substrates, which supported naming and framing B2 this way with more confidence.
...
1. search_evidence("procedurally generated world learner goal states better than chance baseline", k=5) → C-b57f0217986c, C-b287aa6823b0, C-ba882ad5cc7e, C-f8fd488fda5b, C-b3b4b28a3a62
2. search_evidence("baseline comparison bar random chance null navigability failed", k=5, status='REFUTED') [negative-evidence query] → C-31e6f516f20b
3. contradictions() → R-e68c9331eca2 (C-3a1c49fa5a78 vs C-3d12c440f087), R-2dc413ddca43 (C-1d99d0adac44 vs C-7d559fe50c7a)
4. get_claim('C-31e6f516f20b') → full claim + evidence detail
5. get_claim('C-ba882ad5cc7e') → full claim + evidence detail
6. related_findings('C-ba882ad5cc7e') → graph: C-82fe472469ca (SAME_MECHANISM, hop 1), C-353ec1eb022a (hop 2); semantic neighbors C-a9fc01aa3892, C-5a1e687671e3, C-3d12c440f087, C-eb3e53602cce, C-5168060736c5, C-6509907fc169, C-065a321e0808, C-fff52fca02a0, C-b5c1a85cca8b, C-0a1e16161517

## Evidence that changed this design (ids -> concrete decision; 'retrieved but did not affect design' is valid)
- C-ba882ad5cc7e -> added F1 (world-triviality admissibility gate, 85%/90% thresholds) and made shallow search (B2) the primary comparator instead of uniform-random.
- C-31e6f516f20b -> added B3 (random-pairing permutation null) and F3 (floor-characterization falsifier), and the "margin must exceed 3x SE" clause in F2.
- R-e68c9331eca2 (contradiction) -> reinforced pre-registering the comparator rule (B1/B2 primary, B0/B3 reported) before learner data is seen.
- C-82fe472469ca -> retrieved but did not change a concrete decision beyond confirming the naming/framing of the "attainable-without-reasoning ceiling" concept; treated as corroborating context, not a design driver.
- C-b57f0217986c, C-b287aa6823b0, C-f8fd488fda5b, C-b3b4b28a3a62 -> retrieved via the first search but did not materially change this design (they concern experienced-vs-naive learner acquisition cost and history-conditioned synthesis, not baseline/comparator construction for a fresh learner's first evaluation); noted as adjacent literature, not incorporated.
- C-353ec1eb022a and the ten semantic-neighbor ids from related_findings were retrieved but not opened individually (budget discipline); not used to justify any design decision.

## Operation log (numbered; ops used / 15, documents opened / 12)
1. search_evidence("procedurally generated world learner goal states better than chance baseline", k=5) — wiki API call
2. search_evidence(..., status='REFUTED') — wiki API call (negative-evidence query, required)
3. contradictions() — wiki API call (required)
4. get_claim('C-31e6f516f20b') — wiki API call / document opened (1)
5. get_claim('C-ba882ad5cc7e') — wiki API call / document opened (2)
6. related_findings('C-ba882ad5cc7e') — wiki API call
7. Grep repo (F:\Prometheus) for "random-pairing|chance floor|one-ply greedy|attainable-without-reasoning" — repo search

Ops used: 7 / 15. Documents opened: 2 / 12.
