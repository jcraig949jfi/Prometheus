...
1. `ls F:\Prometheus` (directory survey) — not a document open.
2. Grep `success probability|will succeed|predict success|success likelihood|attempt.{0,20}judge`
   over F:\Prometheus — search op, no document opened.
3. Grep `fine-tun|finetun|LoRA` over F:\Prometheus — search op, no document opened.
4. Read `F:\Prometheus\ergon\gen1b\PREREG_GEN1_2026-09-01.txt` — document 1.
5. Read `F:\Prometheus\ergon\gen1a\power_analysis_2026-09-01.json` — document 2.
6. Read `F:\Prometheus\ignis\RESULTS.md` — document 3.
7. Grep `falsification.routing|routing.residue|oracle feature|causal feature` over
   F:\Prometheus — search op, no document opened.
8. Read `F:\Prometheus\aporia\docs\deep_research_reports\2026-05-23\00356_erg_02_substrate_alternatives_for_falsification_routing_lear.md`
   — document 4.
9. Grep `falsification.routing learner|routing learner` in
   `F:\Prometheus\roles\Aporia\RESPONSIBILITIES.md` — search op, no match, no document opened.
10. Read `F:\Prometheus\aporia\docs\EVIDENCE_2026-08-18_falsification_first_external.md` —
    document 5.
11. Bash `wc -l` + `head -c 2000` on
    `F:\Prometheus\ergon\probe\ledgers\campaign\campaign_log.jsonl` — document 6 (partial read
    used to check whether probe ledgers are a viable "attempt record" source; found to be
    predominantly infra-level events, see Unresolved uncertainty).
12. `ls F:\Prometheus\evidence_wiki\v2\arm_outputs` — output-directory check, not a document open
    for research purposes.

Ops used: 12 of 15 (3 unused, not spent). Documents opened: 6 of 12 (6 unused, not spent). All
retrieval was ordinary repository search/read over F:\Prometheus, excluding
F:\Prometheus\evidence_wiki per instruction; no local network services, databases, or web access
were used.
