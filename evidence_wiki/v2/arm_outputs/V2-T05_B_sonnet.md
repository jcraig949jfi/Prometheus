...
1. `search_evidence("failure signature early abort doomed attempt", k=8)` -> top-ranked hit `C-ff8811fa0ac7` (sigma-kernel OBSTRUCTION_SHAPE, REFUTED cross-family), plus 7 other lower-relevance claims scanned (Ergon prepass-loader defect, Aporia R==E, Hephaestus fail-dangerous estimator, Ergon truncation-confound, Ergon D-5 retraction, Ergon warm-start collab completion, Aporia D-3 NO_BASIS_PASSED).
2. `get_claim("C-ff8811fa0ac7")` -> full claim text/mechanism/status, linked evidence `E-1db21f0d3b4d`, experiment `X-fa82b3913349`.
3. `get_counterevidence("C-ff8811fa0ac7")` -> negative_evidence `E-1db21f0d3b4d` (0 strict matches in 91/142; family-specific 54x lift survives).
4. `contradictions()` -> global contradiction list scanned (`R-e68c9331eca2` D-5 vs D-8 accumulated-history effect; `R-2dc413ddca43` FAILS_TO_REPLICATE pair); neither bears directly on the OBSTRUCTION_SHAPE claim.
5. `related_findings("C-ff8811fa0ac7")` -> graph neighbor `C-e5e726a050c1` (SAME_FAILURE_CLASS: cross-family anti-transfer of a locally predictive structure), `C-a36c7e9fe323`, `C-01f913ae81af`; semantic neighbors including `C-5a1e687671e3` (D-3 NO_BASIS_PASSED) and 9 others.

## Evidence that changed this design (ids -> concrete decision; 'retrieved but did not affect design' is valid)
- `C-ff8811fa0ac7` / `E-1db21f0d3b4d` -> set Preregistered Falsifiers F2 (strict-match=0 at N>=90 as an actual, already-observed falsification threshold, not a hypothetical) and F4 (its recorded claim_ceiling "family-specific... promotion remains blocked" became this SPEC's default program-wide promotion bar).
- `C-e5e726a050c1` (SAME_FAILURE_CLASS: "cross-family anti-transfer of a locally predictive structure") -> retrieved but did not affect design beyond confirming this is a recognized recurring failure class in the program; motivates treating an early kill in the Stopping Rule as high-value rather than continuing to search for a pass.
- `C-a36c7e9fe323`, `C-01f913ae81af`, `C-5a1e687671e3`, and the remaining semantic-neighbor ids from `related_findings` -> retrieved but did not affect design (not opened in detail; retrieval budget reserved for the directly on-point claim).
- Global `contradictions()` rows `R-e68c9331eca2`, `R-2dc413ddca43` -> retrieved but did not affect design (neither concerns this signature or domain).

## Operation log (numbered; ops used / 15, documents opened / 12)
1. `ew.search_evidence("failure signature early abort doomed attempt", k=8)` — wiki API call.
2. `ew.get_claim("C-ff8811fa0ac7")` — wiki API call; document opened (1/12).
3. `ew.get_counterevidence("C-ff8811fa0ac7")` — wiki API call.
4. `ew.contradictions()` — wiki API call.
5. `ew.related_findings("C-ff8811fa0ac7")` — wiki API call.
6. `Grep "OBSTRUCTION_SHAPE"` over `F:\Prometheus` (repo search) — located 73 candidate files.
7. `Read F:\Prometheus\aporia\scouting\04_obstruction_shape_oeis_a150_a151.md` — document opened (2/12).
8. `Read F:\Prometheus\sigma_kernel\README.md` — document opened (3/12).

Ops used: 8/15. Documents opened: 3/12.
