...
| R-7 | per-agent READ+WRITE identity `vivarium` (Vivarium outbox deliverer; G4 of the Campaign 4 launch gate): token issued 2026-09-17 on M2, value held in the vivarium-consumer pinned worktree's untracked config.local.json (pew_token) and ~/.prometheus/ew_agent_tokens.json; committed `config.json` carries only its sha256 and scopes `[read, write]` | issued; not in git; not in the queue | Mnemosyne (issuer) / Vivarium (holder) | ISSUED 2026-09-17 |
