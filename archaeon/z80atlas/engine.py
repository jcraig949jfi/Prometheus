...
def run(spec: dict, seed: int, progress=None) -> dict:
    ...
    for i in range(N):
        if transplant:
            ...
            if i < len(tapes):
                genomes[i] = bytes.fromhex(tapes[i])[:G].ljust(G, bytes(1))
        elif rng.randbelow(100) < F["init_fill_pct"]:
            genomes[i] = repl if (seeded and rng.randbelow(100) < F["seed_fraction_pct"]) else bytes(rng.randbelow(256) for _ in range(G))
        if genomes[i] is not None:
            energy[i] = F["energy_init"]; oid[i] = next_id; lin[i] = next_id; parent[i] = 0; age[i] = rng.randbelow(F["max_age"] // 2); next_id += 1
    ...
    def snapshot(reason: str):
        ...
        snapshots.append({"epoch": epoch, "reason": reason, "tasks": list(W.tasks), "params": [p["K"] for p in W.params],
                          "organisms": [{"cell": i, "id": oid[i], "lineage": lin[i], "parent": parent[i], "born": born[i], "niche": W.niche[i], "score": round(score[i], 3), "energy": round(energy[i], 1), "tape": genomes[i].hex()} for i in range(N) if genomes[i] is not None]})

    def place(j: int, child: bytes, p: int, mech: str, fid: float, mig: bool = False):
        ...
        if mech != "EXTERNAL": births_endo += 1
        if len(events) < F["event_cap"]:
            events.append({"e": epoch, "k": "birth", "mech": mech, "p": parent[j], "c": oid[j], "cell": j, "fid": round(fid, 3), "mig": mig})
    ...
    while epoch < max_epochs and steps_total < budget:
        ...
        for i in order:
            ...
            if phys in ENDO and res0 is not None and res0["writes_nbr"] > 0:
                ...
                if ok and (genomes[j] is None or phys in ("OVERWRITE", "ENDOGENOUS_COPY", "ENDOGENOUS_PARTIAL", "PAIR_EXECUTION")):
                    ...
                    place(j, child, i, phys, fidelity(child, genomes[i]))
        ...
        if phys == "EXTERNAL":
            ...
            for _ in range(nb):
                ...
                if j == p: continue
                child = mutate(genomes[p], mut, rng, F["mutation_rate"])
                if "recombination" in press and len(occ) > 1:
                    q = occ[rng.randbelow(len(occ))]; x = rng.randbelow(G); child = child[:x] + genomes[q][x:]
                place(j, child, p, "EXTERNAL", fidelity(child, genomes[p]))
    ...
    signals = {"final_pop_frac": round(last["pop"] / N, 3), "mean_pop_frac": round(sum(r["pop"] for r in telemetry) / max(1, len(telemetry)) / N, 3),
               ...
               "spontaneous_replication": bool(spec["init"] == "random" and phys in ENDO and births_endo >= F["spont_births"] and last["pop"] / N >= F["spont_pop_frac"] and (last["fid"] or 0) >= F["spont_fid"]),
    ...
    final_population = [{"cell": i, "id": oid[i], "lineage": lin[i], "parent": parent[i], "born": born[i], "niche": W.niche[i], "score": round(score[i], 3), "tape": genomes[i].hex()} for i in occ]
