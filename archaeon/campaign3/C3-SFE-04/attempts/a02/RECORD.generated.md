...
## A. STARTUP (preregistration; sealed sha256:6d8c0b6b42b0550e1eb6bec5d391b56936625d6758d13ce7d3c0c25a6fd05b51)
...
- CLAIM CEILING: a sparse map at n=1 per edge and G=6: which edges show a measurable initialization advantage over a matched baseline, with the permuted control as the capability test; no claim that the advantage transfers structure rather than search time
...
- decl (machine-read by archaeon.wse.states): {"battery": [{"name": "permuted_control_gives_no_advantage", "passed": null}, {"name": "advantage_survives_excluding_directly_solved_edges", "passed": null}, {"name": "advantage_is_not_only_generation_0", "passed": null}], "n_min": 3, "primary": {"control": "baseline", "metric": "foothold", "min_effect": 0.25, "treatment": "init_mature"}}
...
- timings (s): direct_s=0.81, harvest_s=1.08, phase1_s=1.9, records_s=0.0, search_s=6.89, total_s=9.2
...
- primary table:
    arm / competence_heldout      s1    mean    n
    baseline                 0.062   0.062    1
    init_control             0.021   0.021    1
    init_mature              0.229   0.229    1

    arm / first_foothold_gen      s1    mean    n
    baseline                     -       -    0
    init_control                 -       -    0
    init_mature                  -       -    0

    arm / first_shelf_gen       s1    mean    n
    baseline                     -       -    0
    init_control                 -       -    0
    init_mature                  -       -    0

    arm / first_summit_gen      s1    mean    n
    baseline                     -       -    0
    init_control                 -       -    0
    init_mature                  -       -    0

    arm / summit                s1    mean    n
    baseline                     0   0.000    1
    init_control                 0   0.000    1
    init_mature                  0   0.000    1
...
- disposition candidate (machine): WEAK_POSITIVE -- effect >= min_effect but n < 10 or a declared falsification attack failed or was not run
    evidence: {"control_mean": 0.0, "effect": 0.6667, "min_effect": 0.25, "n_control": 5, "n_treatment": 3, "paired": 1, "paired_wins": 0, "treatment_mean": 0.6667}
    battery: {"attacked": 3, "declared": 3, "survived": 2}
- claim ceiling (machine): weak; not for propagation; preregistered ceiling: a sparse map at n=1 per edge and G=6: which edges show a measurable initialization advantage over a matched baseline, with the permuted control as the capability test; no claim that the advantage transfers structure rather than search time
