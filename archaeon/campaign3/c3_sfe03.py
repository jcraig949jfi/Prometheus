...
def main(argv=None) -> int:
    ...
    reach = X.reachability_for([(RUNGS[0], a.N, a.rung_gens, a.E, "E0"), (RUNGS[3], a.N, 60, a.E, "E0")])
    X.seal({
        "question": "On the delay ladder W0 -> d1 -> d2 -> d4 (%d generations per rung, revisit share p=%s), at which generation and on which rung does the elite "
                    "first score >= %.2f on every rung (delay generality), does it appear abruptly or gradually, does it survive later transitions, and do "
                    "different seeds find the same corridor?" % (a.rung_gens, a.p, GENERAL_MIN),
        ...
        "assay_capability_requirement": "the population reaches rung-0 competence >= 0.5 within the first rung in >= 6 of %d seeds (else POSITIVE_CONTROL_FAILED: nothing to climb from)" % len(a.seeds),
        "positive_control": "rung-0 (W0 4-bit) reached within %d generations (table: W0 solved 11/21 by G30)" % a.rung_gens,
        ...
        "budget": {"N": a.N, "E": a.E, "rung_gens": a.rung_gens, "G": G_, "p": a.p, "seeds": a.seeds, "probe_dense": 5, "probe_sparse": 5, "probe_episodes": 24,
                   "general_min": GENERAL_MIN, "rungs": [r.knobs() for r in RUNGS]},
        "primary_observable": "general_gen (first generation with elite competence >= %.2f on all four rungs) per seed; rung_at_general; r3_rise_gens; general_survives; "
                              "held-out per rung at the end" % GENERAL_MIN,
        ...
        "falsification_condition": "general_heldout in < 4 of %d seeds => the ladder is not a reliable corridor at this budget (the C2 4/6 was luck or budget)" % len(a.seeds),
        ...
        "machine_changes_exercised": ["D probe_plan + transition_events", "C corridor ladder rows", "G step API with per-step spec/episodes", "I"],
    ...
    jobs = [{"arm": "p%s" % a.p, "p": a.p, "seed": s, "N": a.N, "E": a.E, "rung_gens": a.rung_gens} for s in a.seeds]
    ...
    for r in rows:
        ...
        X.corridor_row(arm=r["arm"], source_cell="ladder:W0>W1_d1>W1_d2>W1_d4(p=%s)" % a.p, target_cell="W1_d4", kind="ladder", source_maturity={"route": True},
                       source_foundry=fid, target_foundry=fid, target_budget={"N": a.N, "G": G_, "E": a.E, "rung_gens": a.rung_gens},
                       init={"level": lvl, "seed": r["seed"], "first_summit_gen": r["events"]["R3"]["appeared"], "general_gen": r["general_gen"],
                             "heldout_r3": r["heldout_by_rung"]["R3"], "general_survives": r["general_survives"]},
    ...
    t0 = time.time()
    for r in rows:
        X.record(wid, r, {"experiment": X.ID, "arm": r["arm"], "seed": r["seed"], "N": a.N, "E": a.E, "rung_gens": a.rung_gens, "p": a.p, "prereg_digest": X.prereg["prereg_digest"]},
    ...
    summ = {"general_gen": [r["general_gen"] for r in rows], "rung_at_general": [r["rung_at_general"] for r in rows], "r3_rise_gens": [r["r3_rise_gens"] for r in rows],
            ...
            "adapt": [r["adapt_gens"] for r in rows], "persist": [r["persist"] for r in rows], "instr": [r["elite_summary"]["instr"] for r in rows],
