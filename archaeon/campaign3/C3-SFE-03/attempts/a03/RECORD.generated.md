...
## A. STARTUP (preregistration; sealed sha256:db386ed382e0ff04af89b136e2f43590bf126d3e0fd3d6c655ad59d72f5963d9)

- experiment ID: C3-SFE-03
- parents: C2-SFE-06
- QUESTION: On the delay ladder W0 -> d1 -> d2 -> d4 (rung 0 HELD until the elite reaches 0.5 training reward or 30 generations, then 6 generations per rung, revisit share p=0.1), at which generation and on which rung does the elite first score >= 0.75 on every rung (delay generality), does it appear abruptly or gradually, does it survive later transitions, and do different seeds find the same corridor?
...
- ASSAY CAPABILITY REQUIREMENT: the population reaches rung-0 competence >= 0.5 within the rung-0 hold (<= 30 generations) in >= 6 of 2 seeds (else POSITIVE_CONTROL_FAILED: nothing to climb from)
- POSITIVE CONTROL: rung-0 (W0 4-bit) reached within the hold of 30 generations (table: W0 pooled 13/21 by G100; a02 measured 5/12 within 25)
...
- attempts: 3 (of record: a02); resumed_from: None; replayed steps on the attempt of record: 0
...
- timings (s): ladder_s=7.99, records_s=0.0, total_s=8.2
...
- primary table:
    arm / general_gen           s7      s9    mean    n
    p0.1                         -       -       -    0

    arm / final_r3              s7      s9    mean    n
    p0.1                     0.000   0.167   0.083    2

    arm / final_r0              s7      s9    mean    n
    p0.1                     0.083   0.042   0.062    2

    arm / r3_rise_gens          s7      s9    mean    n
    p0.1                         -       -       -    0

    arm / general_heldout       s7      s9    mean    n
    p0.1                         0       0   0.000    2

- typed states fired: ['POSITIVE_CONTROL_FAILED']
    POSITIVE_CONTROL_FAILED  {"arm": "p0.1", "metric": "reached_r0_by_rung_end", "min": 1, "rows": 2, "rows_meeting": 0, "values": [0, 0]}
