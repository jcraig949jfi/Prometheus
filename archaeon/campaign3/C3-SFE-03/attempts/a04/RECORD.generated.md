...
## A. STARTUP (preregistration; sealed sha256:73a70972b43a6a97e73e50f11a204fb7e9edf62148ed7c8215f035f8d6566183)

- experiment ID: C3-SFE-03
- parents: C2-SFE-06
- QUESTION: On the delay ladder W0 -> d1 -> d2 -> d4 (rung 0 HELD until the elite reaches 0.5 training reward or None generations, then 6 generations per rung, revisit share p=0.1), at which generation and on which rung does the elite first score >= 0.75 on every rung (delay generality), does it appear abruptly or gradually, does it survive later transitions, and do different seeds find the same corridor?
...
- ASSAY CAPABILITY REQUIREMENT: the population reaches rung-0 competence >= 0.5 within the rung-0 hold (<= None generations) in >= 6 of 1 seeds (else POSITIVE_CONTROL_FAILED: nothing to climb from)
- POSITIVE CONTROL: rung-0 (W0 4-bit) reached within the hold of None generations (table: W0 pooled 13/21 by G100; a02 measured 5/12 within 25)
...
- decl (machine-read by archaeon.wse.states): {"n_min": 1, "positive_control": {"arm": "p0.1", "metric": "reached_r0_by_rung_end", "min": 1, "min_rows": 6}, "primary": {"control": "p0.1", "metric": "general_heldout", "min_effect": 0.0, "treatment": "p0.1"}}
...
- timings (s): ladder_s=1.36, records_s=0.0, total_s=1.6
...
- primary table:
    arm / general_gen           s7    mean    n
    p0.1                         -       -    0

    arm / final_r3              s7    mean    n
    p0.1                     0.083   0.083    1

    arm / final_r0              s7    mean    n
    p0.1                     0.042   0.042    1

    arm / r3_rise_gens          s7    mean    n
    p0.1                         -       -    0

    arm / general_heldout       s7    mean    n
    p0.1                         0   0.000    1

- typed states fired: ['POSITIVE_CONTROL_FAILED']
    POSITIVE_CONTROL_FAILED  {"arm": "p0.1", "metric": "reached_r0_by_rung_end", "min": 1, "rows": 1, "rows_meeting": 0, "values": [0]}
- disposition candidate (machine): POSITIVE_CONTROL_FAILED -- assay precondition failed; the scientific question was not posed
- claim ceiling (machine): none; preregistered ceiling: an instrument reading at n=1: the ladder reaches W1_d4 competence in k/1 seeds with a measured timescale; no mechanism is claimed
