...
## D-27 (Archaeon 2026-09-11 evening, on Atalanta #86/#97; the operator's reading quoted in #97) Base rule 10 ADOPTED: a loop may not outlive its own usefulness silently

Bound on CONSECUTIVE NON-PRODUCTIVE ticks keyed on the rule-8 productivity signal and never on emission; park at N with one message to a named accountable seat; no bound or no seat, no launch. Text in roles/base-role/RESPONSIBILITIES.md rule 10; registry columns BOUND and ACCOUNTABLE SEAT on every row of roles/base-role/MONITORS.md (12 ACTIVE rows UNDECLARED at adoption; the self-test ratchets that number down only, so a new row must declare both); archaeon/tests/test_base_role.py carries the checks. Per-loop enforcement is each owner's migration; Archaeon's own is ARCH-33 (ArchaeonTick, N = 96 ticks). Conflict of interest carried from the proposer: rule 10 would have stopped the seat that proposed it, which biases toward making it look necessary; adopted on the operator's reading, not on that seat's numbers.

## D-28 (OPENED 2026-09-11 evening, NOT RULED; operator decision) The producer-declaration invariant

The operator's formulation (via Atalanta #97): "a consumer cannot launch against an output location/interface that the producer has not explicitly declared." Evidence: agents/atalanta/daemon.py:63-67 and :222-226 (three author-chosen literal paths; liveness = "is there a directory at this string"); Talos invented the same phantom apollo/runs the same week with no contact. Rule 9 does not close it (an address can exist, be empty, and be someone else's). What a ruling must settle: where a declaration lives and what it contains; the consumer is the party refused at launch when it is absent; whether existing producers block their consumers immediately or after a window. Blast radius: every producer. ARCHAEON'S RECOMMENDATION, for the operator: adopt in REGISTRY form, no new code path -- a producer's MONITORS row declares its output (location or table plus the freshness source it already names); a consumer row's INPUT column must name a producer row or a comms message kind; the base self-test ratchets UNDECLARED inputs downward exactly as rule 10 does; a consumer whose input names nothing declared is not launched. Cost: one column and one test, plus each owner's migration. Not adopted until the operator says so.

## D-29 (Archaeon 2026-09-11 evening, on Eos #113, WORKING_CONTRACT s10) Code every seat must import cannot be gitignored

CLAUDE.md mandates `from keys import get_key`; .gitignore:9 ignores keys.py; D-23 puts every seat in a linked worktree where the file does not exist; 28 tracked files therefore carry their own dotenv resolver (Eos's census, predicate in agents/eos/src/credential_census.py). Ruled as a constitution defect: a mandated import is part of the tracked tree or the mandate is void; the mandate is SUSPENDED (not violated) until the operator's one-minute look at the single literal Eos's counts-only scan flagged (P0-a); no seat copies keys.py into a worktree by hand. Text: WORKING_CONTRACT.md s11. The un-ignore itself is the operator's act; the per-lane migration to one resolver (P0-c) follows it.

## D-24 amendment 2 (Archaeon 2026-09-11 evening, on Hermes #106) Failure convergence: promoted narrowly as a library under comms; incident home fixed

Hermes's probe: 2 of 5 historical classes converge deterministically (configuration/environment, broken interface -- the classes that RAISE); the other three (silent no-op, gitignored write, success-looking violation) cannot be signed because nothing exists at the point of failure to key on, so signature coverage is downstream of instrument coverage. Ruling: PROMOTE AS A LIBRARY, no boot step, no incident role, no workflow. Home: comms/incidents/ (one Markdown file per signature, appended, in git) and comms.identity.INCIDENT_DIR points there; Hermes moves the primitive (record/split) beside comms/identity.py in one commit and the placeholder roles/Hermes/incidents is retired. The record() call is the lookup; a seat that hits a raised failure calls it and journals one line with the prior count. Falsifiers stand as Hermes wrote them; the cheapness claim is unmeasured until a second author uses it.

## D-30 (Archaeon 2026-09-11 evening, on Hypatia HYPATIA-27 addendum 6ce12265f) Seat trees are allowlisted against blanket content-type ignores

Four instances in eleven days of the same shape (Vivarium journals, Ergon verdict rows, Hypatia season rows, Skopos reports), each found by a different seat and each patched by a per-seat negation; the fourth (**/reports/, line 28) arrived while the third (**/results/, line 27) was being patched. Ruled: `!roles/**` at the end of .gitignore, followed by an EXPLICIT list of what stays ignored beneath a seat tree (__pycache__, *.pyc, databases, pickles/arrays/binaries/archives, *.log, .env, config.local.json, keys.py, per-process state files). test_base_role asserts ten kept and ten dropped paths for a synthetic seat. A seat that needs a *.log or *.db committed negates it in its own row of the list, with a size stated. The five earlier per-seat negations are left in place as history; they are now redundant, not wrong.

## D-24 amendment 3 (Archaeon 2026-09-11 evening, on Harmonia #154) One seat, many instances

Measured defect: comms.agents keyed on the seat alone, so a second instance's boot overwrote the first's presence silently; receipts marked a message seen FOR THE SEAT, so the second instance's sync returned "0 new" for a message it was the one to act on; claim() printed nothing to the instance that lost the race. Three Harmonia instances wrote under one name on 2026-09-11. Ruled: a seat MAY be several instances (that is the operator's practice across M1/M2, not a policy to forbid), so the fix is Harmonia's additive proposal, adopted: an instance tag derived from <machine label>-<8 of the harness session id> on every comms call (never chosen; no session id -> <machine>-nosession, the old behaviour); comms.agent_instances (one row per instance, `who` lists them beneath the seat, a seat is online when any instance is); comms.receipt_instances (seen per instance) with UNSEEN judged per instance: never seen by the seat, or not seen by me and arrived after I first booted (so a live sibling cannot swallow a message and a fresh boot does not replay history); claim() RETURNING with CLAIMED / LOST (held by <tag>) / NOT_QUEUED, and task_queue.claimed_by; `--from Seat[tag]` stored as sender + sender_instance. The seat-level tables keep their keys so older code keeps working through the migration; live schema migrated and 31 seat rows backfilled with one instance each at 20:2x UTC. Tests: comms/tests/test_instances.py (positive: two rows, per-instance unseen, one winner; negative: no history replay, untagged unchanged; cheat: the loser is told). Harmonia's roles/Harmonia/instance.py tag convention is compatible by construction (same derivation).
