...
### Original class: causal_structure_learning

#### `causal_discovery_pc.v0`  

*Causal Discovery* — report `10_model_and_law_discovery.md` — kind `pc_algorithm_skeleton_v0` — mechanism `prune_hypothesis_space` — route R-BACKEND; R-EXECUTOR; R-REPAIR — capabilities C-4 (R-BACKEND: causal-learn / pcalg / Tetrad); C-5 if the sampling distribution is assembled across specs — proxy class **static_structured**
...
- **Must survive.** a data object in the world (PC consumes a sample, not a seed), a real conditional-independence hypothesis test with a stated null, and a planted ground-truth graph so the skeleton can be scored
...
### Original class: continuous_dynamics_physics_informed

#### `sciml_pinn_residual.v0`  

*Scientific Machine Learning* — report `10_model_and_law_discovery.md` — kind `train_physics_informed_nn_v0` — mechanism `fit_model_to_constraint` — route R-BACKEND; R-COMPOSE; R-SUBSTRATE — capabilities C-4 (a training run behind one returned scalar; EXPANSION_REQUESTS #16 asks for an autodiff executor); C-5 (the residual-AND-error conjunction must be assembled from two fossils downstream; unavoidable, not incidental) — proxy class **numeric_calibration**
...
- **Must survive.** scoring a candidate by how much it violates a LAW with no target data, the residual evaluated at sampled points, an explicit boundary term with stated weighting, a reference solution so residual can be checked against accuracy, and several seeds per cell
- **Can change.** the function class (a three-parameter polynomial on du/dt = -u, u(0)=1 reproduces both traps with a closed-form truth; Herakles proposes 64 collocation points, boundary weight 1.0, 20 seeds as NEW DESIGN CHOICES), the optimiser (descent performed by the producer across specs)
...
- **Retained route.** PINN residual minimisation is a continuous-optimisation experiment over a declared PDE/ODE; the residual-only rule is vacuously satisfiable by the zero function on the bench.
- **Bench proxy.** random_walk_v0 as a numerical integrator of a diffusion: the law is Var(displacement after n steps) = n * step_scale^2 / 3, and the residual is the deviation of the ensemble variance from that law computed downstream across seeds (R-COMPOSE over specs that run today)
- **Proxy would lose.** the network, automatic differentiation, the optimisation, and the inverse-problem half of the paper; the bitstring scorer is the OPPOSITE of residual scoring (distance to a stored answer) so mapping onto it would be a gesture
...
### Original class: cyber_physical_falsification

#### `falsification_walk.v0`  

*Falsification-Based Search* — report `07_experimental_design_and_falsification.md` — kind `random_walk_v0` — mechanism `accumulate_trajectory` — route R-REPAIR; R-COMPOSE; R-EXECUTOR; R-BACKEND — capabilities C-5; C-4; C-2 (nearest listed capability to a trace-returning executor; a full trace return is not in C-0..C-6) — proxy class **numeric_calibration**
...
- **Must survive.** Stochastic, deliberately incomplete search; robustness as a continuous quantity guiding an outer loop; the asymmetry that FALSIFIED is conclusive and SURVIVED is not; the counterexample stored as a replayable input; the budget recorded.
- **Can change.** The dynamics model, the property, the optimiser, the budget, the envelope threshold.
...
- **Retained route.** Temporal-logic falsification searches a real system's input space for a spec violation; the walk is a surrogate whose envelope law is analytic.
- **Bench proxy.** Monte Carlo baseline on random_walk_v0: steps 1000, step_scale 0.05 (NEW DESIGN CHOICES), outcome rule displacement >= 3.0 -> FALSIFIED else SURVIVED, 500 repeats sha256_index state reset; sweep threshold and step_scale for a rate-versus-budget curve. Guided arm (R-COMPOSE): producer treats 3.0 minus displacement as robustness and runs cross-entropy across specs, comparing samples-to-first-falsification. Compute the attainable crossing probability from steps and step_scale BEFORE fixing the threshold.
- **Proxy would lose.** Temporal logic entirely (only a final-time predicate survives, since random_walk_v0 never returns the trace, so a mid-flight excursion is invisible); hybrid dynamics and discrete mode switches, which make real falsification hard.
...
### Original class: decision_theoretic_experiment_selection

#### `bayesian_utility.v0`  

*Bayesian Experimental Design* — report `07_experimental_design_and_falsification.md` — kind `bayesian_utility_v0` — mechanism `compute_closed_form_criterion` — route R-EXECUTOR; R-BACKEND; R-COMPOSE — capabilities  — proxy class **numeric_calibration**
...
- **Must survive.** Information gain as the utility, a design as the judged object, and the distinction between pre-experimental expectation (analytic) and realised distribution (measured).
- **Can change.** Prior and likelihood family, estimator (closed form vs Monte Carlo, with estimator bias reported), sample size, prior variance.
...
- **Retained route.** Expected-utility experiment selection; needs a prior, a likelihood and a utility.
- **Bench proxy.** evaluate_bitstring: design = (query length L, query count R); fixed candidate, repeat count R under sha256_index; matching count is Binomial(L, 1/2), so mutual information between one score and the hidden target is computable from the observed distribution and compared to its analytic value; sweep L in {8,16,32,64}, R = 200 (Herakles NEW DESIGN CHOICES).
- **Proxy would lose.** A user-supplied prior (fixed to uniform over 2^L targets), a noisy likelihood (noiseless, degenerate), and any way to misspecify the prior, so the robustness question stays untouchable.
...
### Original class: dynamical_systems_equation_discovery

#### `bacon_equation_discovery.v0`  

*Automated Scientific Discovery* — report `06_automated_discovery_systems.md` — kind `discover_equation_v0` — mechanism `fit_model_to_data` — route R-BACKEND; R-COMPOSE; R-EXECUTOR — capabilities C-4; C-5 — proxy class **numeric_calibration**
...
- **Must survive.** A real generating process with a planted law known exactly in advance, measurement noise, term composition by ratio/product, and a constancy test at declared tolerance; the discovered expression must be recorded.
- **Can change.** Identity of the data source, number of variables, heuristic depth, tolerance.
...
- **Retained route.** Equation discovery from measured trajectories (BACON): the faithful route is a symbolic-regression / law-induction executor over a declared dynamical system with noise; the walk proxy calibrates only the instrument.
- **Bench proxy.** random_walk_v0 as data generator: grid steps in {10,30,100,300,1000} x step_scale in {0.1,1,10}, many seeds under sha256_index; downstream apply BACON's ratio heuristic to (steps, step_scale, displacement^2) and test recovery of the constant 1/3 (Herakles NEW DESIGN CHOICE: grid above, depth 3).
- **Proxy would lose.** The richness of a multi-variable physical table: the discoverable law is a single scaling relation and the recursion never passes depth 2.
...
*Computational Models of Scientific Discovery* — report `14_evolutionary_epistemology.md` — kind `bacon_heuristic_search.v0` — mechanism `search_over_candidates` — route R-COMPOSE; R-EXECUTOR; R-REPAIR — capabilities C-5 (R-COMPOSE: recursion lives in the producer, statistic downstream) — proxy class **symbolic_execution**
...
- **Must survive.** search over DERIVED TERMS rather than a fit of a pre-chosen functional form, the constancy tolerance, a data table in the world, and a known planted law so a discovered term can be scored right or wrong
- **Can change.** the number of variables, the noise level and distribution, the recursion depth bound, the exact heuristic set (BACON.1 vs later symmetry/common-divisor heuristics)
...
- **Retained route.** Same phenomenon as bacon_equation_discovery; retained separately because its design question (law simplicity vs fit) differs.
- **Bench proxy.** evaluate_bitstring across specs: encode an integer as the bitstring (independent variable), take score as dependent variable, build the table across specs and run BACON's heuristics downstream; the hash target guarantees no law exists, so the result is a calibrated false-discovery rate vs table size and tolerance
- **Proxy would lose.** the planted law and therefore any true-positive rate: on the hash landscape no smooth law exists by construction, so the bench proxy is one-sided (can only show over-reporting)
...
*Equation Discovery* — report `10_model_and_law_discovery.md` — kind `sparse_regression_sindy_v0` — mechanism `fit_model_to_data` — route R-BACKEND; R-COMPOSE; R-EXECUTOR — capabilities C-4; C-5 — proxy class **numeric_calibration**
...
- **Must survive.** A genuine trajectory from a genuine process, derivative estimation with real numerical error, the library and thresholding machinery, and a known ground truth; the surviving-term identity must be recorded.
- **Can change.** Library size, threshold, sampling rate, noise level, the process identity.
...
- **Retained route.** Sparse identification of nonlinear dynamics needs a real ODE trajectory library and a sparsity oracle; the walk proxy has one trivial law and is analytically solved.
...
- **Proxy would lose.** Any test of RECOVERY: a 1-D driftless walk has no nonlinear dynamics, coupling or attractor, so nothing is there to be found.
...
### Original class: experimental_design_optimality
...
- **Must survive.** The determinant criterion, which is irreducibly non-additive because it measures how far the chosen points are from collinear.
...
### Original class: interacting_landscapes

#### `adam_yeast_growth.v0`  

*Robot Scientists* — report `06_automated_discovery_systems.md` — kind `simulate_yeast_knockout_v0` — mechanism `score_fixed_candidate` — route R-COMPOSE; R-BACKEND; R-EXECUTOR — capabilities C-5 (R-COMPOSE: interaction adjudicated downstream); C-4 (R-BACKEND: flux-balance / metabolic simulator) — proxy class **interacting_landscapes**
...
- **Must survive.** a genotype that is a structured object with removable parts, an environment that is a set of supplied resources, and a metabolism connecting them so that removing a part can be compensated by supplying a resource; the 2x2 factorial rescue logic with all four cells sharing one environment
...
- **Must survive.** Test difficulty as a moving function of current host competence; host/test asymmetry; a held-out iid evaluation set not drawn from the adversarial selection relation.
...
- **Class rationale.** Each population IS the other's fitness landscape; the coupling is the phenomenon.

#### `hide_and_seek_autocurriculum.v0`  

*Autocurricula* — report `04_open_ended_learning_and_architecture_search.md` — kind `multi_agent_competition_v0` — mechanism `two_population_interaction` — route R-BACKEND; R-WORLD; R-SUBSTRATE — capabilities C-4; C-1 — proxy class **interacting_landscapes**
...
- **Must survive.** a task distribution with shared structure and support/query splits, adaptation as a small number of steps from a shared starting point, the control of a random initialisation given the same adaptation budget, and reporting over many held-out tasks with a CI
...
- **Class rationale.** the whole claim depends on relatedness between task landscapes (shared structure); with unrelated landscapes the experiment is vacuous by construction

#### `mcc.bipartite.v0`  

*Minimal-Criterion Coevolution* — report `02_diversity_and_illumination.md` — kind `mcc_step.v0` — mechanism `two_population_interaction` — route R-COMPOSE; R-WORLD; R-SUBSTRATE — capabilities C-1; C-5 — proxy class **interacting_landscapes**
...
- **Must survive.** Two coupled populations; the existential minimal criterion; absence of any ranking or objective; endogenously generated environments; a genuine difficulty axis; the coupled-collapse failure mode with its diagnosis; tau calibrated per population size with the eligible range reported beside the verdict.
...
- **Class rationale.** Each population is the other's landscape and the landscape is endogenous; the phenomenon is the coupling itself.

#### `poet_paired_coevolution.v0`  

*POET / Open-Ended Learning* — report `04_open_ended_learning_and_architecture_search.md` — kind `poet_loop_v0` — mechanism `two_population_interaction` — route R-BACKEND; R-SUBSTRATE; R-COMPOSE — capabilities C-1 (relatedness axis; 04_CAPABILITIES names POET explicitly; two analysts reached it independently from POET and MAML); C-4 (POET's own implementation as backend); C-5 (archive held by the producer between batches) — proxy class **interacting_landscapes**
...
- **Must survive.** a parameterised environment FAMILY with a mutation operator and a DISTANCE between environments, an archive persisting across generations, an all-pairs agents-by-environments evaluation, and the minimal criterion adjudicated against the whole agent population
...
- **Class rationale.** the landscape each agent faces is generated and admitted by reference to the other population, and the load-bearing claim is transfer between related landscapes


### Original class: numeric_calibration

#### `computational_mathematics_walk.v0`  

*Computational Mathematics* — report `10_model_and_law_discovery.md` — kind `random_walk_v0` — mechanism `accumulate_trajectory` — route R-REPAIR; R-NOW; R-COMPOSE — capabilities C-5 (mean/variance across repeats computed downstream); portfolio E2 diffusion calibration (R-REPAIR, zero new code) — proxy class **numeric_calibration**
...
- **Retained route.** Reference names no text; the proxy is the experiment. Retained as calibration only.
- **Bench proxy.** random_walk_v0 itself, as above
- **Proxy would lose.** nothing: this template IS the bench; what is lost relative to the field is any claim about physical Brownian motion, drift, first-passage times, or the generator modelling anything outside itself
...
- **Next action.** No faithful route until a reference is supplied (H-R2).
- **Parameters destroyed.** steps; step_scale
...
- **Must survive.** a labelled term set with the published feature definitions, a model with FIXED coefficients (fitting is search and must happen upstream), a held-out split, and a list-level statistic (AUC or precision at k)
- **Can change.** the corpus (replace with K simulated feature vectors and a planted linear relevance rule with the top decile labelled relevant; Herakles proposes K=1000), the ranking baseline (raw frequency)
...
- **Retained route.** Built on the dead bits axis; the honest reading is the exchangeability null (E3).
- **Bench proxy.** evaluate_bitstring at length 8 with the bits axis retained ONLY as the E3 exchangeability null (mean 0.5, sd sqrt(1/(4L)), independent of the candidate); or, inverting the reading, 256 specs at one fixed seed to recover the whole score landscape (C-0)
- **Proxy would lose.** the corpus, the labels, the features and all biomedical content; on the bench the template as written does not implement the method at all
...
- **Next action.** bitstring.exchangeability_null.v0 PROPOSED; no other route.
- **Parameters destroyed.** length
...
- **Class rationale.** one uniform draw scored on onemax is a closed-form null; the template contains no evolutionary content and its value is as the bench's calibrated baseline
...
*Automated Experimentation* — report `06_automated_discovery_systems.md` — kind `synthesize_thin_film_v0` — mechanism `search_over_candidates` — route R-BACKEND; R-COMPOSE; R-EXECUTOR — capabilities C-4; C-0; C-3; C-5 — proxy class **other**
...
- **Must survive.** The closed loop itself, a hard experiment budget, a hidden response surface learnable only by sampling, and per-experiment fossilisation of recipe and reading bound to provenance.
...
- **Must survive.** Blindness of variation (producer cannot consult the target), retention, the cumulative-versus-single-step contrast, and a matched evaluation budget.
...
*Learning-to-Optimize* — report `13_learning_to_search_and_machine_minds.md` — kind `meta_optimize_quadratic_v0` — mechanism `nested_optimization_loop` — route R-COMPOSE; R-BACKEND; R-SUBSTRATE; R-EXECUTOR — capabilities C-4; C-5; C-1; C-0 — proxy class **other**
...
- **Must survive.** The two-level structure with a training family and a genuinely different held-out family; a frozen learned rule; a matched hand-written baseline at identical budget; a fixed unroll horizon; the transfer arm pre-registered.
...
- **Class rationale.** Nested continuous optimisation over a family of related landscapes; none of the listed branches names meta-learning. interacting_landscapes would apply only if 'relatedness between training and held-out landscapes' counts as interaction, which it does not.

#### `sbse.hillclimb.search.v0`  

*Search-Based Software Engineering* — report `09_testing_and_verification.md` — kind `sbse_hill_climb_v0` — mechanism `search_over_candidates` — route R-COMPOSE; R-REPAIR; R-SUBSTRATE; R-EXECUTOR — capabilities C-5; C-3; C-6; C-0 — proxy class **other**
...
- **Must survive.** A fitness function encoding a genuine engineering objective (a rugged landscape), the local-optimum certificate, and a fossil of every evaluation, since discarded evaluations hold the ruggedness measurement.
...
- **Must survive.** Cumulative retention against single-step resampling, matched budget, the mutation-rate sweep, and the exact scoring rule.
...
### Original class: population_ecology

#### `alife.tierra.soup.v0`  

*ALife-Inspired AI* — report `13_learning_to_search_and_machine_minds.md` — kind `tierra_soup_v0` — mechanism `evolve_population` — route R-BACKEND; R-SUBSTRATE; R-WORLD — capabilities C-4 — proxy class **population_ecology**
...
- **Must survive.** A genotype-phenotype map under which a bit flip changes what a program DOES (lethal/neutral/parasitic), a persistent shared arena, inter-candidate access, scheduler and reaper.
...
- **Class rationale.** A population in a shared resource-limited arena whose phenomenon is ecological (parasitism, resistance, cheating).

#### `creativity.v0`  

*Computational Creativity* — report `05_intrinsic_drives_and_creativity.md` — kind `novelty_search.v0` — mechanism `maintain_archive` — route R-COMPOSE; R-SUBSTRATE; R-REPAIR; R-EXECUTOR — capabilities C-3; C-5 — proxy class **population_ecology**
...
- **Must survive.** Behaviour-based rather than fitness-based selection; the archive feedback that devalues visited regions; archive_size and k_nearest as live knobs; the divergence-versus-stall question with its two-cause diagnostic.
...
- **Class rationale.** Generational population with archive-relative selection; the measurable is population-level coverage and stall.

#### `map.elites.v0`  

*Quality-Diversity (MAP-Elites)* — report `02_diversity_and_illumination.md` — kind `evaluate_qd_archive.v0` — mechanism `maintain_archive` — route R-COMPOSE; R-SUBSTRATE; R-EXECUTOR — capabilities C-3; C-5 — proxy class **population_ecology**
...
- **Must survive.** Elite-per-cell retention; a descriptor genuinely independent of fitness; thousands of dependent evaluations where each candidate is chosen by reading the archive; a matched-budget objective-only control; a landscape on which diversity pays.
...
- **Class rationale.** A niche-structured archive of elites with parents drawn from across the population; the measurable is a population-level map, not a single optimum.

#### `neurodynamics.attractor.evo.v0`  

*Darwinian Neurodynamics* — report `13_learning_to_search_and_machine_minds.md` — kind `attractor_evolution_v0` — mechanism `variation_selection_loop` — route R-BACKEND; R-SUBSTRATE; R-COMPOSE — capabilities C-4 (the simulation run outside and fossilized with a per-generation two-arm trajectory); C-5 (two-arm comparison over generations downstream) — proxy class **population_ecology**
...
- **Class rationale.** a population with selection and an explicit heredity channel, adjudicated by a two-arm trajectory; the substrate question (neural heredity) is what distinguishes it from a plain GA

#### `novelty.search.v0`  

*Novelty Search* — report `02_diversity_and_illumination.md` — kind `novelty_search_step.v0` — mechanism `maintain_archive` — route R-COMPOSE; R-SUBSTRATE; R-EXECUTOR — capabilities C-3; C-5 — proxy class **population_ecology**
...
- **Must survive.** Objective-blindness in selection; k-NN novelty over a persistent archive; archive growth; a behaviour descriptor that is an indirect consequence of the genome; a deceptive domain; a fitness-driven control at identical budget on the identical domain.
...
- **Class rationale.** Selection over a population relative to an archive of past behaviours; the measurable is archive growth and coverage, a population-level quantity.

#### `pbt_hyperparam_schedule.v0`  

*Population-Based Training* — report `04_open_ended_learning_and_architecture_search.md` — kind `pbt_optimize_v0` — mechanism `evolve_population` — route R-SUBSTRATE; R-WORLD; R-BACKEND; R-COMPOSE — capabilities C-4; C-5 — proxy class **population_ecology**
...
- **Must survive.** A population inside one world, per-worker persistent state, a copy operation between workers, periodic evaluation, and a fossil recording the lineage tree.
...
### Original class: program_level_meta_science

#### `science_of_science.v0`  

*Science of Science* — report `12_knowledge_discovery_and_serendipity.md` — kind `network_cascade.v0` — mechanism `accumulate_trajectory` — route R-EXECUTOR; R-NOW; R-REPAIR — capabilities C-5 (inequality statistic from persisted walk repeats computed downstream) — proxy class **population_ecology**
...
- **Must survive.** an accumulating structure whose next step depends on its current state (persistent state INSIDE the run), a statistic computed over the whole finished object, and a state-independent accumulation null to compare against
- **Can change.** the statistic (Gini or top-1% share preferred; both have computable attainable ranges), the presence of fitness/ageing terms, graph size
...
- **Retained route.** Mechanism cited to a review; needs a primary source and a program-level population (experiments as units).
- **Bench proxy.** random_walk_v0 with state=persist and sha256_index repeats: the inequality statistic of a state-independent accumulation process, i.e. the measured null the rich-get-richer model must beat
- **Proxy would lose.** the state-dependence itself: random_walk_v0 accumulates but its increments do not depend on its state, so it is precisely the state-independent null and not the model; also fitness, ageing, citing-vs-cited, time-resolved trajectories, and any contact with bibliometric data
...
### Original class: simulation_based_study_design

#### `simulate_study.v0`  

*Meta-Science* — report `07_experimental_design_and_falsification.md` — kind `simulate_study_v0` — mechanism `simulate_generative_process` — route R-COMPOSE; R-EXECUTOR; R-REPAIR — capabilities C-5 — proxy class **numeric_calibration**
...
- **Can change.** The rates, the number of studies, how bias is implemented, whether a purpose-built Bernoulli executor or the bitstring scorer supplies the draws.
...
### Original class: spatial_stateful

#### `agi.mc.aixi.ctw.v0`  

*Artificial General Intelligence (AGI)* — report `13_learning_to_search_and_machine_minds.md` — kind `aixi_ctw_eval_v0` — mechanism `agent_environment_loop` — route R-BACKEND; R-SUBSTRATE — capabilities C-4 (reference MC-AIXI-CTW implementations as an orchestrated backend with a per-cycle reward trace) — proxy class **spatial_stateful**
...
- **Must survive.** a closed loop in which the agent's action is an input to the world and the world's observation an input to the agent thousands of times inside one spec, a mutable model persisting across cycles, a per-cycle reward log, and per-environment random and optimal baselines
...
- **Class rationale.** an agent-environment loop with persistent memory over hidden state; the trajectory, not a point, is the observation

#### `artificial_curiosity.v0`  

*Artificial Curiosity* — report `05_intrinsic_drives_and_creativity.md` — kind `model_building_controller.v0` — mechanism `two_module_adversarial_learning` — route R-COMPOSE; R-WORLD; R-SUBSTRATE; R-BACKEND — capabilities C-5; C-4 — proxy class **spatial_stateful**
...
- **Must survive.** The adversarial coupling (model minimises the same error the controller maximises); the moving target; both a learnable and an irreducibly stochastic region so boredom and the noise trap can be separated; per-step logging of error and occupancy.
...
- **Class rationale.** An agent with persistent learned state acting in a world partitioned into regions; the measurable is occupancy over regions coupled to a learning curve.

#### `curiosity.v0`  

*Curiosity-Driven Exploration* — report `05_intrinsic_drives_and_creativity.md` — kind `pseudo_count_exploration.v0` — mechanism `accumulate_trajectory` — route R-SUBSTRATE; R-COMPOSE; R-BACKEND — capabilities C-4 — proxy class **spatial_stateful**
...
- **Must survive.** The density model, prediction gain, the pseudo-count derivation, and a ground-truth count to check it against; for exploration proper, the bonus must change the next action.
...
- **Class rationale.** An agent with memory of its own history navigating a state space; state and trajectory are the substance.

#### `empowerment.v0`  

*Empowerment* — report `05_intrinsic_drives_and_creativity.md` — kind `channel_capacity_evaluator.v0` — mechanism `measure_information_quantity` — route R-COMPOSE; R-EXECUTOR; R-WORLD — capabilities C-5; C-4 — proxy class **spatial_stateful**
...
- **Class rationale.** The measurable is a landscape over world states reached through actions from a resettable state; the faithful experiment is an agent in a stateful world.

#### `illumination.grid.v0`  

*Illumination Algorithms* — report `02_diversity_and_illumination.md` — kind `illuminate_space.v0` — mechanism `maintain_archive` — route R-COMPOSE; R-SUBSTRATE; R-EXECUTOR — capabilities C-5; C-3; C-0 — proxy class **spatial_stateful**
...
- **Must survive.** The archive with one elite per bin, elite selection, mutation, binning, coverage/QD statistics, and the illumination-versus-optimisation contrast; faithfully, a descriptor that is a property of behaviour not recoverable from score.
...
*Intrinsic Motivation* — report `05_intrinsic_drives_and_creativity.md` — kind `iac_exploration.v0` — mechanism `self_directed_curriculum` — route R-COMPOSE; R-SUBSTRATE; R-BACKEND — capabilities C-5 (two-condition error-slope comparison downstream); C-4 (existing intrinsic-motivation codebases as backend) — proxy class **spatial_stateful**
...
- **Must survive.** an action at step t that depends on results before t (output-to-input chaining), a persistent improving model, a fossil format that records an ORDERED trajectory, and a world containing an unlearnable region
...
- **Class rationale.** a closed agent-environment loop with persistent memory over a sensorimotor state space; the claim is about a trajectory through that space

#### `l2s.dagger.v0`  

*Learning-to-Search* — report `13_learning_to_search_and_machine_minds.md` — kind `l2s_dagger_v0` — mechanism `counterexample_feedback_loop` — route R-SUBSTRATE; R-COMPOSE; R-BACKEND; R-EXECUTOR — capabilities C-2; C-4; C-5 — proxy class **spatial_stateful**
...
- **Must survive.** Covariate shift; the on-policy versus off-policy distinction; oracle-label aggregation along the learner's own trajectory; the beta schedule; the horizon-scaling comparison; evaluation on the FINAL policy's own state distribution.
...
- **Class rationale.** An agent following a trajectory through a stateful world; the claim is about the state distribution the agent induces.

#### `machineevol.neat.v0`  

*Machine Evolution* — report `01_evolutionary_substrates.md` — kind `neat_eval_v0` — mechanism `score_fixed_candidate` — route R-NOW; R-EXECUTOR; R-REPAIR; R-COMPOSE — capabilities C-5 (minimal vs augmented comparison downstream) — proxy class **spatial_stateful**
...
- **Must survive.** a genome with separable node and connection genes, a builder from genes to a runnable network, and a task with real dynamics stepped over many timesteps; the episode as the unit of measurement; any controller claim stated relative to a measured no-controller rate
...
- **Class rationale.** closed-loop control over a stepped dynamical environment with an episode as the unit; the R-NOW walk baseline is numeric_calibration but the experiment itself needs the stateful loop

#### `openended.novelty.v0`  

*Open-Ended Evolution* — report `01_evolutionary_substrates.md` — kind `novelty_maze_v0` — mechanism `accumulate_trajectory` — route R-EXECUTOR; R-BACKEND; R-WORLD; R-COMPOSE — capabilities C-4; C-5 — proxy class **spatial_stateful**
...
- **Must survive.** Behaviour characterisation as the endpoint of a fixed-length rollout; the genome-to-behaviour map; the batch-of-endpoints artifact; the degeneracy check with positive and negative controls; the eval_steps axis.
...
- **Class rationale.** An embodied agent moving through a 2-D world with walls; the descriptor is a spatial endpoint.

#### `rbn.attractor.v0`  

*Artificial Gene Regulatory Networks* — report `03_generative_and_developmental_representations.md` — kind `boolean_network.v0` — mechanism `accumulate_trajectory` — route R-EXECUTOR; R-COMPOSE — capabilities C-5 — proxy class **spatial_stateful**
...
- **Must survive.** Iterated deterministic map on a finite state space with cycle detection, the K sweep, and the boolean returned separately from the period so censoring is visible.
...
### Original class: stateful_property_based_testing

#### `pbt.stateful.walk.v0`  

*Property-Based Testing* — report `09_testing_and_verification.md` — kind `random_walk_v0` — mechanism `accumulate_trajectory` — route R-REPAIR; R-COMPOSE; R-SUBSTRATE — capabilities C-2; C-5 — proxy class **numeric_calibration**
...
- **Must survive.** Random generation from a declared distribution, invariant checking, seed-replayable counterexamples, and a property whose failure rate is known in advance so the test has power.
- **Can change.** steps, step_scale, invariant chosen, number of repeats.
...
- **Retained route.** Property-based testing of stateful systems generates command sequences against a model; the walk proxy has no model to violate.
- **Bench proxy.** random_walk_v0 with steps in {10,100,1000}, step_scale 1.0 (Herakles NEW DESIGN CHOICE), many repeats under sha256_index; invariant \|displacement\| <= 3 * step_scale * sqrt(steps/3); read the empirical violation rate against the analytic tail downstream.
- **Proxy would lose.** Shrinking, per-command postconditions, model-based comparison, and a multi-command generator (only one command type exists).
...
### Original class: static_structured

#### `abduce_set_cover.v0`  

*Abductive Reasoning* — report `11_reasoning_modes.md` — kind `abduce_set_cover_v0` — mechanism `search_over_candidates` — route R-BACKEND; R-EXECUTOR; R-COMPOSE — capabilities C-4; C-5 — proxy class **static_structured**
...
- **Must survive.** The covering relation between a candidate subset and the presented manifestation set, and the parsimony-vs-completeness tradeoff.
...
*Case-Based Reasoning* — report `11_reasoning_modes.md` — kind `cbr_retrieval_cycle_v0` — mechanism `retrieve_and_rank_from_corpus` — route R-EXECUTOR; R-COMPOSE; R-SUBSTRATE — capabilities C-0 (fixed-seed pattern; 04_CAPABILITIES names case-based retrieval explicitly); C-5 (max over repeats computed downstream) — proxy class **static_structured**
...
- **Must survive.** a seeded case base, the similarity functions implemented separately, a labelled ground-truth neighbour so accuracy rather than raw similarity is reported, and calibration of the attainable range of the retrieval statistic before any threshold is frozen
...
*Computational Philosophy of Science* — report `14_evolutionary_epistemology.md` — kind `echo_coherence_network.v0` — mechanism `relax_constraint_network` — route R-EXECUTOR; R-BACKEND; R-COMPOSE — capabilities C-5 — proxy class **static_structured**
...
- **Must survive.** Coherence, explanation, inhibition, competition between rival hypotheses, and convergence to a fixed point; the budget must be recorded beside the verdict.
...
- **Must survive.** a nonlinear many-to-one developmental map, a genotype space too large to enumerate so sampling is required, an exact phenotype identity test (attractor matching), and a demonstrably unbiased genotype sampler
...
- **Class rationale.** the measurement is a static distribution over a fixed genotype-to-attractor map sampled without selection or interaction; the internal iteration is deterministic and closed

#### `knowledge_discovery.v0`  

*Knowledge Discovery* — report `12_knowledge_discovery_and_serendipity.md` — kind `two_node_search.v0` — mechanism `retrieve_and_rank_from_corpus` — route R-BACKEND; R-EXECUTOR; R-COMPOSE — capabilities C-4 (a bibliographic service as the external backend; EXPANSION_REQUESTS #1 asks for a two-node search executor with shared_b_terms); C-5 (significance against the random-pair null across specs) — proxy class **static_structured**
...
- **Must survive.** a pinned and dated corpus snapshot, a documented extraction/stopword pipeline, an explicit disjointness check for A and C at that date, and a matched null of randomly paired disjoint literatures; a treatment arm with a bridge that can be ablated
...
- **Must survive.** the benchmark table (or a landscape with its correlation/deception structure), strict query accounting, the algorithms unchanged, and many independent repetitions since the headline quantity is a distribution over runs
...
- **Must survive.** The version space; the committee; selection by maximal disagreement; an explicit query budget; a paired passive control on the same seed and budget; many seeds because early curves are high variance.
...
*Autocatalytic Sets* — report `03_generative_and_developmental_representations.md` — kind `detect_raf.v0` — mechanism `detect_closure_in_graph` — route R-BACKEND; R-EXECUTOR; R-COMPOSE — capabilities C-4; C-5 — proxy class **static_structured**
...
- **Must survive.** The closure algorithm exactly; the food-set reachability condition; a sparse, reproducible random catalysis assignment; the threshold sweep; the size-scaling question.
...
*Analogical Reasoning* — report `11_reasoning_modes.md` — kind `structure_mapping_v0` — mechanism `align_relational_structure` — route R-BACKEND; R-EXECUTOR; R-COMPOSE; R-REPAIR — capabilities C-4; C-5; C-3 (a planted-structure target is the closest listed capability to what the honest version needs) — proxy class **static_structured**
...
- **Must survive.** Alignment under an explicit correspondence; the one-to-one and parallel-connectivity constraints; the systematicity ablation as a constrained-versus-free contrast; the size confound made explicit and normalised; ideally candidate inference generation.
...
- **Must survive.** exact elimination rather than scoring, an exactly countable surviving set, monotone collapse, dependence of convergence on WHICH examples were chosen, and the empty-space failure mode
...
- **Class rationale.** exact elimination over a fixed finite hypothesis space with a noiseless oracle; no dynamics, no population; the bench already instantiates it


### Original class: symbolic_execution

#### `ai_scientist_training.v0`  

*AI Scientist systems* — report `06_automated_discovery_systems.md` — kind `train_llm_idea_v0` — mechanism `generate_and_execute_program` — route R-BACKEND; R-EXECUTOR; R-SUBSTRATE — capabilities C-4; C-5 — proxy class **symbolic_execution**
...
- **Must survive.** Proposal sealed BEFORE the target can be seen; executability as a hard gate; a bounded repair loop fed by the executor's own error output; a reference baseline to beat; a contamination control; fossilized diff + lockfile + full log beside the metric.
...
*Artificial Chemistry* — report `03_generative_and_developmental_representations.md` — kind `lambda_collision.v0` — mechanism `interpret_program` — route R-EXECUTOR; R-REPAIR; R-SUBSTRATE — capabilities C-4 (04 says the backend route is the only route to the symbolic modality; no capability is named for this template specifically) — proxy class **symbolic_execution**
...
- **Must survive.** Term rewriting to normal form under a budget whose axis is not inert; a symbolic product; for any claim about self-maintaining organisation, a population reactor with reinjection.
...
*Algorithm Discovery* — report `12_knowledge_discovery_and_serendipity.md` — kind `evaluate_bitstring` — mechanism `score_fixed_candidate` — route R-REPAIR; R-COMPOSE; R-EXECUTOR — capabilities C-6; C-5 — proxy class **symbolic_execution**
...
- **Must survive.** The genome must be INTERPRETED (executed on inputs) before it is judged; nothing scored on syntax.
...
*Artificial Life (ALife)* — report `01_evolutionary_substrates.md` — kind `tierra_core_v0` — mechanism `interpret_program` — route R-BACKEND; R-EXECUTOR; R-SUBSTRATE — capabilities C-4 (R-BACKEND; 04_CAPABILITIES names Tierra explicitly: route via ergon/avida2003/, not a new executor) — proxy class **symbolic_execution**
...
- **Must survive.** a mutable addressable memory that outlives any single evaluation, an interpreter that writes into it, self-copy as an executed act, position independence of the code (template addressing), and for the full method a scheduler interleaving many instruction pointers plus a death rule
...
- **Class rationale.** the template's own question is single-ancestor self-copy, i.e. interpreting a program over a writable memory; the full Tierra method would be population_ecology, but that is not what this template measures

#### `am_concept_generation.v0`  

*Machine Discovery* — report `06_automated_discovery_systems.md` — kind `evaluate_lisp_concept_v0` — mechanism `variation_selection_loop` — route R-SUBSTRATE; R-EXECUTOR; R-BACKEND — capabilities C-4 (AM-lineage theory-formation systems as backend with a concept-trace fossil); C-5 (archive-vs-random contrast downstream) — proxy class **symbolic_execution**
...
- **Must survive.** an executable concept representation, an interpreter producing examples from a definition, a heuristic-ordered agenda that rewrites concepts, a worth function, and an archive that grows and feeds later steps
...
- **Class rationale.** concepts are executable definitions whose value is found by running them; the archive and agenda are state, but the mechanism is symbolic rewriting and execution
...
- **Must survive.** Concept invention by composition; empirical evaluation of extensions against real objects; conjecture as surviving regularity; the yield-per-budget statistic; the full-versus-restrictive contrast; conditioning on trivial explanations before counting a regularity.
...
- **Must survive.** Over-approximation semantics; a witness-bearing counterexample (WHICH positions disagree, not how many); strictly monotone refinement; spurious-versus-genuine adjudication; running to fixpoint.
...
- **Must survive.** The loop in which each failure permanently narrows the next proposal; monotone accumulation of constraints; the iteration budget as outcome axis; sketch-size scaling; a verifier that decides a universally quantified property and returns a witness.
...
*Digital Evolution* — report `01_evolutionary_substrates.md` — kind `avida_cpu_v0` — mechanism `interpret_program` — route R-NOW; R-COMPOSE; R-BACKEND; R-EXECUTOR — capabilities C-4 (R-BACKEND; Avida routed via ergon/avida2003/); C-5 (comparison across genotypes downstream) — proxy class **symbolic_execution**
...
- **Class rationale.** the template as posed executes one program on a virtual CPU and checks its outputs; the population/ecology reading belongs to the paper, not this template

#### `eprover_superposition.v0`  

*Automated Theorem Proving* — report `08_synthesis_and_logic.md` — kind `formal_prover.v0` — mechanism `bounded_exhaustive_refutation` — route R-BACKEND; R-COMPOSE; R-EXECUTOR — capabilities C-4; C-5 — proxy class **symbolic_execution**
...
- **Must survive.** A fixed problem, a fixed budget, and a heuristic ablation over the order of identical sound operations with operation count as the score; the property that all heuristics agree on the answer so only cost varies; many problems.
...
- **Must survive.** An exhaustive bounded query (the universal quantifier over executions up to k); a witness-bearing result; k as an axis on a model whose reachable set actually grows with depth; the distinction proved-within-bound versus not-found-within-budget; k-induction for any claim beyond the bound.
...
*Genetic Programming* — report `03_generative_and_developmental_representations.md` — kind `evaluate_lgp.v0` — mechanism `interpret_program` — route R-EXECUTOR; R-COMPOSE — capabilities C-5 — proxy class **symbolic_execution**
...
- **Must survive.** Decoding bits to instructions under a declared convention and the backward liveness marking; effectiveness computed from structure alone with no fitness.
...
- **Must survive.** Budgeted search against a sound, independent verifier that cannot be fooled; a wrong proposal costs budget and nothing else; recorded (not recomputable) sampled outputs at temperature > 0.
...
*Inductive Logic Programming* — report `08_synthesis_and_logic.md` — kind `metagol_induce.v0` — mechanism `search_over_candidates` — route R-BACKEND; R-COMPOSE; R-EXECUTOR — capabilities C-4; C-5 — proxy class **symbolic_execution**
...
- **Must survive.** Entailment as the judged property, the metarule bias, and depth-bounded completeness so that failure at k is a lower-bound theorem.
...
- **Must survive.** the bounded specification, the given-part vs synthesised-part distinction, the refinement loop with its iteration count as cost, the possibility of an unsatisfiability proof, and a fossil recording solver version, bounds, seed and timeout
...
- **Class rationale.** a program with holes is completed and executed against a symbolic specification; the loop is refinement by counterexample, not population dynamics

#### `symbolic_regression_gp.v0`  

*Symbolic Regression* — report `10_model_and_law_discovery.md` — kind `evaluate_symbolic_tree_v0` — mechanism `search_over_candidates` — route R-EXECUTOR; R-COMPOSE; R-SUBSTRATE — capabilities C-5 — proxy class **symbolic_execution**
...
- **Must survive.** Evaluation of a serialised expression on world-supplied fitness cases returning normalised error; for the search half, selection and variation between batches at matched budget against a random-search comparator.
