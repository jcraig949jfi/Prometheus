...
- Status: **DONE by Proteus 2026-09-08** (`proteus/`, on main; 27 B1 tests, 198 total; 64/64 specimens agree with the arena path over 3 input sets). **Contract decisions recorded:** budget exhaustion is a DISTINCT STATUS and not a counterexample by default (lowering a budget would manufacture witnesses, the quantity Branch B prices); opt-in `BudgetPolicy.COUNTEREXAMPLE` scans the same declared order and can only move the witness earlier; `trace_limit=0` legitimately reports truncation; no invalid opcode exists (every uint32 decodes). **Boundary for WP-B2's opcode-bijection null:** relabelling invariance holds only for programs that do not read their own code as data — the genome is copied to the tape, so a relabelled word is also a different datum; B2-a must declare that scope. Next: Vivarium wraps as `program_eval_v0`.
