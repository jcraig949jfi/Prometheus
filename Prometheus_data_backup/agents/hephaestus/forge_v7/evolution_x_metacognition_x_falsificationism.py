"""v7 Evolution-Metacognition-Falsificationism: judgment calibrator, NCD tiebreaker only.
Frame D: epistemic honesty — models own uncertainty, confident when computable, honest when not.
Primary: evolutionary fitness scoring, metacognitive self-monitoring, falsification-first evaluation.
"""
...
class ReasoningTool:
    ...
    # ── GAP: Evolutionary fitness + falsification ─────────────────
    def _evolve_falsify(self, p, c):
        pl, cl = p.lower(), c.lower().strip()
        # Falsification-first: detect unfalsifiable claims
        if re.search(r'unfalsifiable|cannot\s+be\s+(?:tested|proven\s+wrong|disproven|refuted)', pl):
            if 'yes' in cl or 'unfalsifiable' in cl or 'not scientific' in cl: return 1.0, "F:unf"
            return -0.5, "F:unf_miss"
        if re.search(r'is\s+(?:this|the)\s+(?:claim|hypothesis|theory)\s+(?:falsifiable|testable|scientific)', pl):
            if re.search(r'prediction|observable|measurable|test', pl):
                return _yn(cl, True), "F:testable"
            if re.search(r'vague|unclear|untestable|no\s+prediction', pl):
                return _yn(cl, False), "F:untestable"
        # Fitness landscape reasoning
        if re.search(r'(?:fitness|survival|reproductive)\s+(?:advantage|benefit|cost)', pl):
            if re.search(r'trade-?off|cost.*benefit|benefit.*cost', pl):
                if 'trade' in cl or 'both' in cl or 'depends' in cl: return 1.0, "E:tradeoff"
                return -0.3, "E:tradeoff_miss"
        # Natural selection vs drift
        if 'natural selection' in pl and 'genetic drift' in pl:
            if re.search(r'small\s+population|bottleneck|founder', pl):
                if 'drift' in cl: return 1.0, "E:drift"
                return -0.5, "E:drift_miss"
            if re.search(r'large\s+population|consistent\s+advantage', pl):
                if 'selection' in cl: return 1.0, "E:selection"
                return -0.5, "E:sel_miss"
        # Metacognitive calibration
        if re.search(r'how\s+(?:sure|certain|confident)\s+(?:are\s+you|should\s+(?:you|one|we)\s+be)', pl):
            if re.search(r'no\s+(?:data|evidence|information)', pl):
                if 'low' in cl or 'uncertain' in cl or 'cannot' in cl: return 1.0, "M:nodata"
                return -0.5, "M:nodata_miss"
            if re.search(r'strong\s+evidence|replicated|multiple\s+studies', pl):
                if 'high' in cl or 'confident' in cl: return 1.0, "M:strong"
                return -0.3, "M:strong_miss"
        # Confirmation bias detection
        if re.search(r'only\s+(?:looked|searched|considered)\s+(?:for|at)\s+(?:evidence|data|cases)\s+(?:that|which)\s+(?:support|confirm)', pl):
            if 'confirmation bias' in cl or 'disconfirm' in cl or 'falsif' in cl: return 1.0, "F:confbias"
            return -0.5, "F:confbias_miss"
        return None, None
    ...
    def _cs(self, p, c):
        ...
        es, er = self._evolve_falsify(p, c)
        if es is not None: return es, er
    ...
    def _sec(self, p, c):
        return sum(1 for w in ['falsif', 'evolv', 'fitness', 'metacognit', 'calibrat',
                                'therefore', 'disprove', 'refute']
