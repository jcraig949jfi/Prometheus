"""v7 Causal-Counterfactual-ModelChecking: constructive computer, NCD tiebreaker only.
Gap target: Causal-interventional (counterfactual, Pearl rung 3).
Primary: counterfactual evaluator, necessary vs sufficient conditions,
post hoc detection, causal chain tracing, plus full standard parser battery.
NCD <= 15%.
"""
import re, math, zlib
import numpy as np
...
class ReasoningTool:
    ...
    # ── GAP: Counterfactual evaluator (Pearl rung 3) ───────────────
    def _parse_causal_edges(self, prompt):
        pl = prompt.lower()
        edges = []
        for pat in [r'(\w+)\s*(?:causes?|leads?\s+to|produces?|results?\s+in)\s*(\w+)',
                    r'(\w+)\s*(?:->|-->|=>|→)\s*(\w+)',
                    r'if\s+(\w+),?\s*(?:then\s+)?(\w+)\s+(?:will|would|happens?)']:
            edges += [(a.lower(), b.lower()) for a, b in re.findall(pat, pl)]
        return edges

    def _reachable(self, edges, start):
        reached = set()
    ...
    def _counterfactual_solve(self, p, c):
        ...
        cf_match = re.search(
            r"(?:if\s+(?:\w+\s+)?(?:hadn't|had\s+not|didn't|did\s+not|weren't|was\s+not|were\s+not)\s+"
            r"(?:happen|occur|exist|take\s+place|been\s+born|been\s+made|rain|start|come|arrive|go))"
            r"|(?:what\s+(?:would|could)\s+have\s+happened\s+(?:if|without))"
            r"|(?:without\s+\w+,?\s+would)"
            r"|(?:suppose\s+\w+\s+(?:hadn't|had\s+not|never))", pl)

        if cf_match:
            edges = self._parse_causal_edges(p)
            # Identify the negated event
            neg_event = re.search(
                r"(?:hadn't|had\s+not|didn't|did\s+not|without)\s+(\w+)", pl)

            if neg_event and edges:
                removed = neg_event.group(1).lower()
                # Remove edges originating from the negated event
                remaining_edges = [(a, b) for a, b in edges if a != removed]
                # Query: would Y still happen?
                query = re.search(r'(?:would|could)\s+(\w+)\s+(?:still|have)', pl)
                if query:
                    qv = query.group(1).lower()
                    # Check if qv is reachable without the removed event
                    all_sources = set(a for a, b in remaining_edges)
                    still_reachable = False
                    for src in all_sources:
                        if qv in self._reachable(remaining_edges, src):
                            still_reachable = True
                            break
                    if still_reachable:
                        return _yn(cl, True), "cf:still_reachable"
                    else:
                        return _yn(cl, False), "cf:blocked_by_removal"

            # General counterfactual without explicit graph
            # "If X hadn't happened, Y wouldn't have happened" => X necessary for Y
            if re.search(r'necessary|needed|required|essential', pl):
                if 'yes' in cl or 'necessary' in cl or 'would not' in cl:
                    return 1.0, "cf:necessary_condition"
                return -0.5, "cf:necessary_miss"

            # "If X hadn't happened, Y would still have happened" => X not necessary
            if re.search(r'still\s+(?:would|could)\s+have|would\s+have\s+happened\s+anyway', pl):
                if 'yes' in cl or 'still' in cl or 'anyway' in cl:
                    return 1.0, "cf:not_necessary"
                return -0.5, "cf:not_necessary_miss"

            # Default counterfactual: check for causal dependency
            if edges:
                return 0.3, "cf:partial_graph"
            return 0.0, "cf:no_signal"

        # -- Necessary vs sufficient conditions --
        if re.search(r'\bnecessary\b.*\bsufficient\b|\bsufficient\b.*\bnecessary\b', pl):
            # "Is X necessary for Y?" vs "Is X sufficient for Y?"
            if re.search(r'is\s+\w+\s+(?:a\s+)?necessary\s+(?:condition\s+)?(?:for|to)', pl):
                if re.search(r'but\s+not\s+sufficient|not\s+(?:a\s+)?sufficient', pl):
                    if 'necessary but not sufficient' in cl:
                        return 1.0, "cf:nec_not_suf"
                    return -0.5, "cf:nec_not_suf_miss"
            if re.search(r'is\s+\w+\s+(?:a\s+)?sufficient\s+(?:condition\s+)?(?:for|to)', pl):
                if re.search(r'but\s+not\s+necessary|not\s+(?:a\s+)?necessary', pl):
                    if 'sufficient but not necessary' in cl:
                        return 1.0, "cf:suf_not_nec"
                    return -0.5, "cf:suf_not_nec_miss"

        # Necessary condition alone
        if 'necessary' in pl and re.search(r'guarantee|definitely|occur|ensure', pl):
            if 'no' in cl or 'not' in cl or 'does not guarantee' in cl:
                return 1.0, "cf:nec_no_guarantee"
            if cl.startswith('yes'):
                return -1.0, "cf:nec_guarantee_trap"
            return 0.0, "cf:nec_ambig"

        # Sufficient condition
        if 'sufficient' in pl and re.search(r'necessary|always\s+true|must', pl):
            if re.search(r'does\s+not\s+mean.*necessary|not\s+necessarily', cl):
                return 1.0, "cf:suf_not_nec_general"
            if 'yes' in cl and 'necessary' in cl:
                return -1.0, "cf:suf_nec_trap"

        # -- Post hoc ergo propter hoc --
        if ('preceded' in pl or 'afterwards' in pl or 'shortly after' in pl or
            'before' in pl and 'therefore' in pl) and 'caus' in pl:
            if 'no' in cl or 'not necessarily' in cl or 'post hoc' in cl or 'fallacy' in cl:
                return 1.0, "cf:post_hoc"
            return -0.8, "cf:post_hoc_trap"

        # -- "But for" test (legal causation) --
        if re.search(r'but\s+for|without\s+\w+.*would.*not', pl):
            if 'yes' in cl or 'but for' in cl or 'cause' in cl:
                return 1.0, "cf:but_for"
            return -0.3, "cf:but_for_ambig"
    ...
    def _cs(self, p, c):
        ...
        # Try counterfactual gap solver first
        cs, cr = self._counterfactual_solve(p, c)
        if cs is not None:
            return cs, cr
    ...
    def _sec(self, p, c):
        return sum(1 for w in ['counterfactual', 'necessary', 'sufficient', 'cause',
                                'because', 'therefore', 'would have', 'hadn\'t']
