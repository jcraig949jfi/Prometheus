"""v7 Causal-Bayesian-InfoTheory: constructive computer, NCD as tiebreaker only.
Gap target: Causal-interventional (do-calculus, Pearl rung 2).
Primary: causal chain tracer, intervention detector, confounding detection,
correlation!=causation, Simpson's paradox (numeric reversal), plus full
standard parser battery (58-cat).  NCD <= 15%.
"""
import re, math, zlib
import numpy as np
...
class ReasoningTool:
    ...
    # ── GAP: Causal chain tracer (Pearl rung 2) ────────────────────
    def _causal_chain(self, prompt):
        """Extract causal chain edges A->B from text."""
        pl = prompt.lower()
        edges = []
        # "A causes B", "A leads to B", "A -> B", "if A then B" (causal)
        for pat in [r'(\w+)\s*(?:causes?|leads?\s+to|produces?|results?\s+in)\s*(\w+)',
                    r'(\w+)\s*(?:->|-->|=>|→)\s*(\w+)',
                    r'if\s+(\w+)(?:\s+increases?)?,?\s*(?:then\s+)?(\w+)\s+(?:increases?|decreases?|changes?)']:
            edges += [(a.lower(), b.lower()) for a, b in re.findall(pat, pl)]
        return edges

    def _downstream(self, edges, start):
        """BFS from start through causal edges."""
        reached = set()
    ...
    def _causal_interventional(self, p, c, candidates):
        """Causal-interventional solver: do-calculus, confounding, Simpson's."""
        pl = p.lower()
        cl = c.lower().strip()
        cn = _ns(c)
        ...
        if intv:
            ...
            if query:
                ...
                if qv in ds:
                    ...
                    # Not downstream => no effect (unless confounding)
                    if 'no effect' in cl or 'unchanged' in cl or 'unaffected' in cl:
        ...
        # -- Causal chain transitivity: A->B->C, does A cause C? --
        edges = self._causal_chain(p)
        ...
        # -- Confounding: "both X and Y are caused by Z" --
        ...
        if ('preceded' in pl or 'afterwards' in pl or 'shortly after' in pl or 'then' in pl) and 'caus' in pl:
            if 'no' in cl or 'not necessarily' in cl or 'post hoc' in cl:
                return 1.0, "causal:post_hoc"
            return -0.8, "causal:post_hoc_trap"

        # -- Simpson's paradox: numeric table reversal --
    ...
    def _cs(self, p, c):
        ...
        cs, cr = self._causal_interventional(p, c, None)
        if cs is not None:
            return cs, cr
