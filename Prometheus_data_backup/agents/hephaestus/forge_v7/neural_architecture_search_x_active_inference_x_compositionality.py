"""CAITL v7 Frame D — NAS-Active-Inference-Compositional Judgment Calibrator.
NAS dynamic module selection, active inference free energy, compositional decomposition.
struct>=50% comp>=25% ncd<=15%. Deterministic (numpy+stdlib)."""
import re,math,zlib
import numpy as np
...
_CAU=re.compile(r'\b(because|causes|leads to|due to|since|therefore|thus|hence|consequently)\b',re.I)
def _pn(t):
    o=[]
...
def _decompose(prompt):
    sents=re.split(r'(?<=[.!?])\s+',prompt.strip());parts=[]
    for s in sents:
        clauses=re.split(r'\b(?:and|but|however|whereas|while)\b',s,flags=re.I)
        parts.extend([c.strip() for c in clauses if len(c.strip())>5])
    return parts if parts else[prompt]
def _classify(prompt):
    pl=prompt.lower();tags=set()
    if _NUM.search(pl) and re.search(r'\b(sum|total|add|differ|subtract|multiply|product|remainder|how many|how much)\b',pl):tags.add('arith')
    if _CMP.search(pl) or _ORD.search(pl) or _ORD_R.search(pl):tags.add('cmp')
    if _CND.search(pl):tags.add('cnd');
    if _TMP.search(pl):tags.add('tmp')
    if _QNT.search(pl):tags.add('qnt');
    if _N.search(pl):tags.add('neg')
    if _CAU.search(pl):tags.add('cau')
    return tags if tags else{'gen'}
def _comp(p,c):
...
def _struct(p,c,mods=None):
    pl,cl=p.lower(),c.lower();sc,wt,R=0.0,0.0,[]
    if mods is None:mods=_classify(p)
    def _a(s,w,n):nonlocal sc,wt;sc+=s*w;wt+=w;R.append(f"{n}={s:.2f}")
    wn=0.14 if'neg'in mods else 0.08;wt_=0.12 if'tmp'in mods else 0.07;wc=0.12 if'cnd'in mods else 0.07
    ...
    if _CND.search(pl):
        ...
        _a(min(1.0,s),wc,'cnd')
    if _QNT.search(pl):_a(0.6 if _QNT.search(cl)else 0.3,0.08 if'qnt'in mods else 0.04,'qnt')
    if _CMP.search(pl):
        cn_,pn_=_pn(c),_pn(p);s=0.7 if pn_ and cn_ else(0.5 if _NUM.search(c)or _CMP.search(c)else 0.3)
        _a(s,0.10 if'cmp'in mods else 0.06,'cmp')
    if _CAU.search(pl):s=0.4+(0.3 if _CAU.search(cl)else 0);_a(min(1.0,s),0.08,'cau')
    if wt<0.01:
...
def _ai_score(cands,raw):
    if len(cands)<2:return raw
    s=np.clip(np.array(raw,dtype=np.float64),0.01,0.99)
    ex=np.exp(3.0*(s-np.max(s)));post=ex/(np.sum(ex)+1e-10)
    surp=-np.log(s+1e-10);ent=-np.sum(post*np.log(post+1e-10))
    fe=surp-0.1*ent;fen=(fe-np.min(fe))/(np.max(fe)-np.min(fe)+1e-10)
    return(0.7*s+0.3*(1.0-fen)).tolist()
def _meta_confidence(prompt,answer):
    ...
    return 1.0
class ReasoningTool:
    """NAS-Active-Inference-Compositional Judgment Calibrator v7."""
    TAG="NASAIC-v7"
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        if not candidates:return[]
        mods=_classify(prompt);parts=_decompose(prompt);raw=[];reasons=[]
        for c in candidates:
            ss=[_struct(part,c,mods)[0]for part in parts]
            st=float(np.mean(ss))if ss else 0.3;cs,cr=_comp(prompt,c);nc=max(0,1-_ncd(prompt,c))
            f=0.55*st+0.25*cs+0.10*nc+0.10*0.5;raw.append(max(0.01,min(0.99,f)))
            reasons.append(f"[{self.TAG}] st={st:.3f} cp={cs:.3f} nc={nc:.3f} mods={','.join(sorted(mods))} | {cr} | parts={len(parts)}")
        adj=_ai_score(candidates,raw);res=[]
