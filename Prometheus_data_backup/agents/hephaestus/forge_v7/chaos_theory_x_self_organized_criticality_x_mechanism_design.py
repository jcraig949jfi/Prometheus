"""Chaos Theory x Self-Organized Criticality x Mechanism Design. Frame C: Dynamics Tracker.
Edge-of-chaos dynamics (critical threshold scoring), SOC (power-law score distribution),
incentive-compatible VCG-style scoring.
...
class ReasoningTool:
    def _edge_of_chaos(self,score,r=3.56995):
        x=max(.01,min(.99,score));traj=[x]
        for _ in range(20): x=r*x*(1-x);traj.append(x)
        return 1.0/(1.0+5.0*np.std(traj[10:]))

    def _soc_normalize(self,scores):
        if len(scores)<2: return scores
        a=np.clip(np.array(scores,dtype=float),.01,.99);rk=np.argsort(np.argsort(-a))+1
        pw=rk.astype(float)**(-0.5);pw/=pw.max();return(0.7*a+0.3*pw).tolist()

    def _vcg_score(self,others,si):
        if not others: return si
        return si*.85+max(0,si-max(others))*.15
    ...
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        meta=self._meta_confidence(prompt);raw_scores=[]
        for cand in candidates:
            cs,sn=self._s(prompt,cand);tp=self._type_check(prompt,cand)
            pw=set(re.findall(r'\b[a-z]{3,}\b',prompt.lower()));cw=set(re.findall(r'\b[a-z]{3,}\b',cand.lower()))
            st=len(pw&cw)/max(len(pw),1)*.6+.2;nc=(1.0-self._ncd(prompt,cand))*.2
            base=(cs*.45+st*.35+nc) if cs is not None else(st*.55+nc+.1)
            raw_scores.append((base-tp,cs,sn))
        soc=[r[0] for r in raw_scores];soc=self._soc_normalize(soc);results=[]
        for i,(cand,ss) in enumerate(zip(candidates,soc)):
            others=[s for j,s in enumerate(soc) if j!=i];vcg=self._vcg_score(others,ss)
            rel=self._edge_of_chaos(vcg);final=vcg*(.75+.25*rel)
            results.append({'candidate':cand,'score':float(np.clip(min(final,meta),0,1)),'reasoning':f'comp:{raw_scores[i][2]} soc:{ss:.2f} vcg:{vcg:.2f}'})
