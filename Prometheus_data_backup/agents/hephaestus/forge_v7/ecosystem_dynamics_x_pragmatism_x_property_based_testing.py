"""Ecosystem Dynamics x Pragmatism x Property-Based Testing. Frame C: Dynamics Tracker.
Population dynamics (predator-prey reasoning), pragmatic truth evaluation (what works),
property invariants (conservation laws, monotonicity, idempotence).
Score: computation(50%) + structural(30%) + NCD tiebreaker(20% max)."""
...
class ReasoningTool:
    def _population_dynamics(self,p,c):
        """Predator-prey: growth/decline, equilibrium, carrying capacity."""
        pl=p.lower();cl=c.lower()
        if _h(pl,'population','species','predator','prey','ecosystem','carrying capacity','growth rate'):
            ns=_n(p);cn=_n(c)
            if _h(pl,'equilibrium','stable','balance') and _h(cl,'equilibrium','stable','balance'): return 0.15
            if _h(pl,'increase','grow') and _h(pl,'decrease','decline','shrink'):
                if _h(cl,'decrease','decline','shrink','lower','fewer'): return 0.1
            if ns and cn:
                if _h(pl,'carrying capacity') and cn: return 0.1
        return 0.0

    def _pragmatic_truth(self,p,c):
        """Pragmatic utility: does the answer actually work/resolve the prompt?"""
        pl=p.lower();cl=c.lower();pw=set(re.findall(r'\b[a-z]{3,}\b',pl));cw=set(re.findall(r'\b[a-z]{3,}\b',cl))
        overlap=len(pw&cw)/max(len(pw),1)
        ents=set(re.findall(r'\b[A-Z][a-z]+\b',p));ent_hit=1 if any(e.lower() in cl for e in ents) else 0
        pn,cn=_n(p),_n(c);num_hit=1 if(pn and cn and any(abs(a-b)<.5 for a in pn for b in cn))else 0
        length_ok=1 if 1<=len(c.split())<=50 else 0
        bits=[overlap>.15,ent_hit,num_hit,length_ok]
        return sum(bits)/len(bits)

    def _property_invariants(self,p,c,base):
        """Property-based: conservation, monotonicity, idempotence under perturbation."""
        if base is None: return 0.5
        ws=c.split()
        if len(ws)<2: return 0.8
        scores=[base]
        s1,_=self._s(p,' '.join(ws[:-1]));scores.append(s1 if s1 is not None else .5)
        s2,_=self._s(p,c.upper());scores.append(s2 if s2 is not None else .5)
    ...
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for cand in candidates:
            ...
            prag=self._pragmatic_truth(prompt,cand)
            nc=(1.0-self._ncd(prompt,cand))*.2
            inv=self._property_invariants(prompt,cand,cs)
            pop=self._population_dynamics(prompt,cand)
            if cs is not None: raw=cs*.50+prag*.30+nc+pop*.05
            else: raw=prag*.55+nc+.1+pop*.05
            raw=raw*(.7+.3*inv)
            score=float(np.clip(min(raw,meta),0,1))
            results.append({'candidate':cand,'score':score,'reasoning':f'comp:{sn} prag:{prag:.2f} inv:{inv:.2f} pop:{pop:.2f}'})
