"""Kalman Filtering x Dynamical Systems x Compositionality. Gap: temporal state estimation.
Kalman predict/update cycle: prior from structure, measurement from solver + keywords + NCD.
...
class _KF:
    """1D Kalman filter for score estimation."""
    def __init__(self,x=.5,P=.25): self.x,self.P,self.Q=x,P,.01
    def predict(self,shift=0): self.x+=shift; self.P+=self.Q
    def update(self,z,R=.04):
        K=self.P/(self.P+R); self.x+=K*(z-self.x); self.P*=(1-K); return K
    @property
    def unc(self): return math.sqrt(self.P)

class ReasoningTool:
    def _s(self,p,c):
        """Solvers return (score, meas_variance, name) or (None, .04, 'none')."""
        ...
        if _h(p,'which is','greater','larger','smaller','less than','more than','bigger') and len(ns)>=2 and cn:
            ...
            return (0.9 if abs(cn[0]-t)<1e-9 else 0.1),.01,'numeric'
        if _h(p,'cost','total','together') and _h(p,'more than') and len(ns)>=2:
            a=(ns[0]-ns[1])/2
            if cn: return (0.92 if abs(cn[0]-a)<.01 else (0.1 if abs(cn[0]-ns[1])<.01 else 0.3)),.005,'bat_ball'
        if _h(p,'all but','all except') and len(ns)>=2:
            return (0.9 if cn and abs(cn[0]-int(min(ns)))<.5 else 0.15),.01,'all_but'
        ...
        if len(pr)>=2:
            ...
            for a in an:
                if a in cl and not any(a in g.get(x,set()) for x in an if x!=a):
                    if _h(p,'tallest','fastest','oldest','biggest','heaviest','strongest'): return 0.88,.02,'trans'
            return 0.3,.06,'trans'
        m=re.search(r'if\s+(.+?)\s*,?\s*then\s+(.+?)\.',pl)
        if m and _h(p,'not','no ','never',"don't","doesn't"):
            if _h(c,'not') or fw=='no': return 0.85,.02,'mt'
            if fw in ('yes','true'): return 0.1,.02,'mt'
        if re.search(r'not\s+all\s+\w+\s+are',pl):
            if _h(c,'cannot determine','insufficient','not enough','uncertain'): return 0.88,.02,'neg'
            if fw in ('yes','no','true','false'): return 0.15,.02,'neg'
        sm=re.search(r'(\w+)\s+(gave|told|sent|passed|handed)\s+(?:\w+\s+)?(?:to\s+)?(\w+)',pl)
        if sm and _h(p,'who'):
            if _h(p,'who '+sm.group(2),'who gave','who told') and sm.group(1) in cl: return 0.88,.02,'svo'
            if _h(p,'receive','got','to whom') and sm.group(3) in cl: return 0.88,.02,'svo'
        if _h(p,'preva','base rate','1 in','sensitivity') and len(ns)>=2:
            ...
            if cn:
                ...
                if abs(cp-po)<.15: return 0.8,.03,'bayes'
            if _h(c,'low','unlikely') and po<.3: return 0.75,.04,'bayes'
        ...
        if ed:
            ...
            if tg and tg in cl: return 0.9,.01,'topo'
            if tg: return 0.2,.05,'topo'
        ...
        if rl:
            ...
            if tg and tg.group(1) in kn: return (0.92 if cn and abs(cn[0]-kn[tg.group(1)])<.5 else 0.1),.005,'age'
        if _h(p,'same time','simultaneous','parallel','together','at once'):
            ...
            if ts:
                ...
                if cn and abs(cn[0]-a2)<.5: return 0.88,.02,'conc'
        ...
        if len(ti)>=4 and _h(p,'conflict','overlap','both','attend'):
            ...
            if ov and _h(c,'yes','conflict','cannot'): return 0.88,.02,'sched'
            if not ov and _h(c,'no','can'): return 0.88,.02,'sched'
            return 0.15,.04,'sched'
        if _h(p,'accelerat','deceler','rate','increasing','decreasing') and len(ns)>=3:
            df=[ns[i+1]-ns[i] for i in range(len(ns)-1)]; dd=df[-1]-df[0]
            if dd>0 and _h(c,'accelerat','increasing','faster'): return 0.85,.02,'rate'
            if dd<0 and _h(c,'deceler','decreasing','slower'): return 0.85,.02,'rate'
            return 0.15,.04,'rate'
        if _h(p,'yesterday','tomorrow','day after','day before'):
            ...
            if dm:
                ...
                if ix>=0:
                    ...
                    if _D[(ix+of)%7] in cl: return 0.9,.01,'day'
                    return 0.15,.04,'day'
        ...
        if fm:
            ...
            return (0.9 if _C[ix] in cl else 0.1),.01,'dir'
        if _h(p,'facing','across','opposite') and _h(p,'left','right'):
            if re.search(r'(?:raise|lift|hold)s?\s+(?:his|her|their)?\s*left',pl) and 'right' in cl: return 0.88,.02,'lr'
            if re.search(r'(?:raise|lift|hold)s?\s+(?:his|her|their)?\s*right',pl) and 'left' in cl: return 0.88,.02,'lr'
        if _h(p,'fence','post','tree','pole','plant','along') and len(ns)>=2:
            ln,sp=max(ns),min(ns)
            if sp>0 and cn and abs(cn[0]-(ln/sp+1))<.5: return 0.9,.01,'fence'
        if _h(p,"o'clock",'clock','hour','modulo') and len(ns)>=2:
            a2=((int(ns[0])+int(ns[1])-1)%12)+1
            if cn and abs(cn[0]-a2)<.5: return 0.9,.01,'mod'
        if _h(p,'coin','flip','toss') and _h(p,'independent','next flip','fair') and _h(c,'50','1/2','0.5','half'): return 0.9,.01,'coin'
        if _h(p,'guarantee','worst case','minimum number','sock','drawer') and len(ns)>=2:
            if cn and abs(cn[0]-(int(max(ns))+1))<.5: return 0.88,.02,'pig'
        if _h(p,'odd','even','parity'):
            ni=[int(x) for x in re.findall(r'\b(\d+)\b',p)]
            if ni:
                v=ni[-1] if len(ni)==1 else sum(ni)
                if v%2==0 and _h(c,'even'): return 0.85,.02,'par'
                if v%2==1 and _h(c,'odd'): return 0.85,.02,'par'
        return None,.04,'none'
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        meta=self._meta_confidence(prompt); pw=set(re.findall(r'\b[a-z]{3,}\b',prompt.lower()))
        cx=min(len(pw)/20.0,1.0); results=[]
        for cand in candidates:
            kf=_KF(.4,.25); kf.predict(cx*.1)
            cs,mv,sn=self._s(prompt,cand)
            if cs is not None: kf.update(cs,mv)
            cw=set(re.findall(r'\b[a-z]{3,}\b',cand.lower())); st=len(pw&cw)/max(len(pw),1)*.8+.1
            kf.update(st,.09)
            kf.update(1.0-self._ncd(prompt,cand),.16)
            results.append({'candidate':cand,'score':float(np.clip(min(kf.x,meta),0,1)),'reasoning':f'kalman:{sn} est:{kf.x:.3f} unc:{kf.unc:.3f}'})
