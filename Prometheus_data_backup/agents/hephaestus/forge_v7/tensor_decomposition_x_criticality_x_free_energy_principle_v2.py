"""Frame B — Tensor Decomposition x Criticality x Free Energy Principle v2.
SVD on prompt-candidate feature matrix, critical threshold detection on
singular values, variational free energy (reconstruction error). Plus parsers."""
...
class ReasoningTool:
    def _fv(s,t):
        pl=t.lower(); v=np.zeros(15,dtype=np.float64)
        for pat,i in [(r'\b(not|never|no|without|neither)\b',0),(r'\b(if|then|unless|whenever)\b',1),
            (r'\b(because|causes|leads to|therefore)\b',2),(r'\b(all|every|each|any)\b',3),
            (r'\b(some|few|many|most)\b',4),(r'\b(more|less|greater|fewer|taller|shorter)\b',5),
            (r'\b(and|but|or|yet|however)\b',6)]:
            v[i]=len(re.findall(pat,pl))
        v[7]=len(_ns(t)); v[8]=1. if _h(pl,"before","after","when","year","age","old") else 0.
        v[9]=1. if _h(pl,"left","right","north","south","above","below") else 0.
        v[10]=1. if '?' in t else 0.; v[11]=1. if _h(pl,"must","can","should","would","could","might") else 0.
        wc=len(pl.split()); v[12]=1. if wc<15 else 0.; v[13]=1. if 15<=wc<40 else 0.; v[14]=1. if wc>=40 else 0.
        return v
    def _mat(s,p,c):
        sn=re.split(r'[.!?]+',p); sn=[x.strip() for x in sn if len(x.strip())>3]
        if not sn: sn=[p]
        rows=[s._fv(x) for x in sn]; rows.append(s._fv(c))
        M=np.array(rows,dtype=np.float64); nr=np.linalg.norm(M,axis=1,keepdims=True)
        nr=np.where(nr<1e-10,1.,nr); return M/nr
    def _svd(s,M):
        try: U,S,Vt=np.linalg.svd(M,full_matrices=False)
        except: return np.array([1.]),.5,.5
        return S,U,Vt
    def _crit(s,S):
        if len(S)<2 or S[0]<1e-10: return .5
        S2=S**2; tot=np.sum(S2)
        if tot<1e-10: return .5
        pr=tot**2/np.sum(S2**2); pn=pr/len(S); cr=4.*pn*(1.-pn)
        gap=S[1]/S[0] if S[0]>1e-10 else 0.; gc=4.*gap*(1.-gap) if gap<=1. else 0.
        return float(np.clip(.6*cr+.4*gc,0.,1.))
    def _fe(s,M,S,U,Vt):
        if len(S)<1: return .5
        S2=S**2; tot=np.sum(S2)
        if tot<1e-10: return .5
        cv=np.cumsum(S2)/tot; k=min(int(np.searchsorted(cv,.8))+1,len(S))
        St=S.copy(); St[k:]=0.; Mr=U[:,:len(S)]@np.diag(St)@Vt[:len(S),:]
        surp=np.mean((M-Mr)**2); fe=surp+.3*(k/len(S))
        return float(np.clip(math.exp(-2.*fe),0.,1.))
    # -- Parsers --
    def _numeric(s,p,c):
    ...
    def _score(s,p,c):
        ps,np_=s._parsers(p,c); M=s._mat(p,c); S,U,Vt=s._svd(M); cr=s._crit(S)
        fe=s._fe(M,S,U,Vt) if isinstance(U,np.ndarray) and isinstance(Vt,np.ndarray) else .5
        dom=.40*cr+.60*fe
        ...
        raw=.50*ps+.35*dom+.15*(1.-ncd)*.15
        for _ in range(3): raw=.8*raw+.2*(ps*.55+dom*.45)
        return 1./(1.+math.exp(-3.*raw)),np_
    ...
    def evaluate(s,prompt,candidates):
        ...
        for c in candidates:
            sc,np_=s._score(prompt,c); sc=min(sc,mc)
            r.append({"candidate":c,"score":float(sc),"reasoning":f"parsers={np_} svd+crit+fe cap={mc:.2f}"})
        r.sort(key=lambda x:x["score"],reverse=True); return r
    def confidence(s,prompt,answer):
        mc=s._meta_confidence(prompt,answer)
        if mc<.3: return mc
        sc,_=s._score(prompt,answer); return float(max(0.,min(.95,min(sc,mc))))
