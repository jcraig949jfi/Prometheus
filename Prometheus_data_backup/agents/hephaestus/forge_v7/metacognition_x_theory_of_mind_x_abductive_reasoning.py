"""v7 Metacognition-TheoryOfMind-AbductiveReasoning: judgment calibrator, NCD tiebreaker only.
Frame D: epistemic honesty — models own uncertainty, confident when computable, honest when not.
Primary: self-monitoring confidence calibration, perspective-taking, inference to best explanation.
"""
...
class ReasoningTool:
    ...
    # ── GAP: Metacognition + ToM + Abductive ──────────────────────
    def _meta_tom_abd(self, p, c):
        pl, cl = p.lower(), c.lower().strip()
        # Abductive reasoning: inference to best explanation
        if re.search(r'(?:best|most\s+(?:likely|plausible|probable))\s+explanation', pl):
            # Prefer simpler explanations (Occam's razor)
            if re.search(r'simpl(?:e|er|est)|parsimonious|fewer\s+assumptions', pl):
                if 'simpl' in cl or 'parsimon' in cl or 'fewer' in cl: return 1.0, "ABD:occam"
                return -0.3, "ABD:occam_miss"
            # Prefer explanations that account for more evidence
            if re.search(r'explain(?:s|ed)?\s+(?:all|more|both|every)', pl):
                if 'all' in cl or 'more' in cl or 'both' in cl: return 1.0, "ABD:coverage"
                return -0.3, "ABD:cov_miss"
        # Abductive: "given X, what most likely caused it?"
        if re.search(r'(?:what|which)\s+(?:most\s+likely|probably|best)\s+(?:caused|explains?|accounts?\s+for)', pl):
            # Check if candidate mentions common vs rare cause
            if re.search(r'common|frequent|typical|usual', cl): return 0.8, "ABD:common"
            if re.search(r'rare|unusual|unlikely|exotic', cl): return -0.3, "ABD:rare_trap"
        # Theory of Mind: false belief
        m = re.search(r'(\w+)\s+puts?\s+a?\s*(\w+)\s+in\s+the\s+(\w+).*?(\w+)\s+moves?\s+the\s+\2\s+to\s+the\s+(\w+)', pl)
        if m and re.search(r'where\s+will\s+' + m.group(1) + r'\s+look', pl):
            return (1.0 if m.group(3) in cl else -1.0), "T:fb"
        # Nested belief
        m2 = re.search(r'(\w+)\s+thinks\s+that\s+(\w+)\s+believes?\s+(.+?)\.\s+(?:according|what)', pl)
        if m2: return (1.0 if m2.group(3).strip() in cl else -.8), "T:2b"
        # Metacognitive calibration: "how confident should you be"
        if re.search(r'how\s+(?:sure|certain|confident)\s+(?:are\s+you|should)', pl):
            if re.search(r'no\s+(?:data|evidence|information|basis)', pl):
                if 'low' in cl or 'uncertain' in cl or 'cannot' in cl: return 1.0, "M:nodata"
                return -0.5, "M:nodata_miss"
            if re.search(r'strong\s+evidence|replicated|multiple\s+(?:studies|sources)', pl):
                if 'high' in cl or 'confident' in cl: return 1.0, "M:strong"
                return -0.3, "M:strong_miss"
            if re.search(r'single\s+(?:study|source|observation)', pl):
                if 'moderate' in cl or 'limited' in cl: return 1.0, "M:single"
                return -0.3, "M:single_miss"
        # Knowledge attribution errors
        if re.search(r'does\s+not\s+know|unaware|hasn\'t\s+(?:seen|heard)', pl):
            if re.search(r'what\s+(?:does|will)\s+\w+\s+(?:think|believe|expect)', pl):
                if re.search(r'(?:still|originally|initially|first)', pl):
                    return (1.0 if 'original' in cl or 'still' in cl or 'old' in cl else -.5), "T:stale"
        # Diagnostic reasoning (abductive)
        if re.search(r'symptom|diagnos|sign|indicator|evidence\s+(?:of|for|that)', pl):
            if re.search(r'(?:what|which)\s+(?:condition|disease|cause|diagnosis)', pl):
                # Bayesian reasoning about diagnostics
                if re.search(r'common|prevalent|base\s+rate', pl):
                    if 'common' in cl or 'base rate' in cl: return 1.0, "ABD:diag"
                    return -0.3, "ABD:diag_miss"
        return None, None

    # ── Standard parser battery ───────────────────────────────────
    def _cs(self, p, c):
        L, cl, cn = p.lower().strip(), c.lower().strip(), _ns(c)
        # Concept-specific first
        gs, gr = self._meta_tom_abd(p, c)
        if gs is not None: return gs, gr
    ...
    def _sec(self, p, c):
        return sum(1 for w in ['abduct', 'explain', 'belief', 'perspective', 'metacognit',
                                'therefore', 'best explanation', 'calibrat']
