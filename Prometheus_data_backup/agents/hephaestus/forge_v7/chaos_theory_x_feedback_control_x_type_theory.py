"""Chaos Theory x Feedback Control x Type Theory. Gap: temporal-sequential (state-dependent).
Feedback control adjusts scoring via cross-premise consistency. Type-checks entity consistency.
...
class ReasoningTool:
    ...
    def _feedback(self,p,c):
        ss=re.split(r'[.!?]+',p.lower()); ps=[s.strip() for s in ss if len(s.strip())>10]
        if len(ps)<2: return 0.0
        ents=[set(re.findall(r'\b[A-Z][a-z]+\b',p))|set(str(int(x)) for x in _n(s)) for s in ps]
        cl=c.lower(); return min(sum(1 for es in ents if any(e.lower() in cl for e in es))/max(len(ents),1)*.15,.2)
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        for cand in candidates:
            cs,sn=self._s(prompt,cand); tp=self._type_check(prompt,cand); fb=self._feedback(prompt,cand)
            cw=set(re.findall(r'\b[a-z]{3,}\b',cand.lower())); st=len(pw&cw)/max(len(pw),1)*.6+.2
            nc=(1.0-self._ncd(prompt,cand))*.2
            score=((cs*.45+st*.35+nc) if cs is not None else (st*.55+nc+.1))-tp+fb
            results.append({'candidate':cand,'score':float(np.clip(min(score,meta),0,1)),'reasoning':f'comp:{sn} tp:{tp:.2f} fb:{fb:.2f}'})
