"""Frame C — Gauge Theory x Immune Systems x Neuromodulation. Gauge invariance checking,
immune self/non-self discrimination, neuromodulatory gain control."""
...
class ReasoningTool:
    def __init__(s):
        np.random.seed(77); s._gp=np.random.randn(32,64)
        s._ss=np.array([hash(w)%256 for w in "the a is of and to in that it for".split()],dtype=float); s._ss/=np.linalg.norm(s._ss)+1e-9
    def _run(s,fns,p,c):
        t,n=0.,0
        for f in fns:
            try:
    ...
    def _gauge(s,p,c):
        pv,cv=s._embed(p),s._embed(c); base=np.dot(pv,cv); words=p.split()
        if len(words)<3: return base
        shifts=[np.dot(s._embed(' '.join(words[o:]+words[:o])),cv) for o in [1,len(words)//2]]
        return base/(1.+100.*np.var([base]+shifts))
    def _immune(s,p,c):
        pw,cw=set(_w(p)),_w(c)
        if not cw: return 0.
        sc=sum(1 for w in cw if w in pw)/(len(cw)+1e-9)
        fw=[w for w in cw if w not in pw]
        if fw:
            fv=np.array([hash(w)%256 for w in fw[:10]],dtype=float); fv/=np.linalg.norm(fv)+1e-9
            sc-=0.3*max(0.,1.-np.dot(fv[:len(s._ss)],s._ss[:len(fv)]))
        return sc
    def _gain(s,p,c):
        cx=min(1.,len(_ns(p))*.1+len(re.findall(r'\b(?:if|then|unless|because|therefore)\b',p.lower()))*.15+len(p.split())*.004)
        return (0.5+0.5*cx)*max(0.,1.-_ncd(p,c))
    def _numeric(s,p,c):
    ...
    def _compose(s,p,c):
        s1,n1=s._s1(p,c); gi,im,gn=s._gauge(p,c),s._immune(p,c),s._gain(p,c)
        bl=0.55*s1+0.20*gi+0.15*im+0.10*gn if n1>0 else 0.35*gi+0.35*im+0.30*gn
        return max(0.,min(1.,bl)),n1,gn
    ...
    def evaluate(s,prompt,candidates):
        ...
        for c in candidates:
            sc,n1,gn=s._compose(prompt,c); sc=float(max(0.,min(1.,min(sc,mc))))
            r.append({"candidate":c,"score":sc,"reasoning":f"S1={n1} gain={gn:.2f} cap={mc:.2f}"})
