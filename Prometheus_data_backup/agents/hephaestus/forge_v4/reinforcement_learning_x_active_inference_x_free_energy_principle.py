"""VPG Expected Free Energy v4. Concepts: RL x Active Inference x FEP"""
...
def _run(p,c):
    ...
    return 0.,'none'
class ReasoningTool:
    """Extrinsic + epistemic value with constraint filter."""
    ...
    def _wrap(self,p,c):
        pw=set(p.lower().split())-{'the','a','an','is','are','to','of','in','and','or'}
        cw=set(c.lower().split())-{'the','a','an','is','are','to','of','in','and','or'}
        ext=len(pw&cw)/max(len(pw|cw),1)
        cn=_ns(c);epist=min(.3,len(cn)*.15)+min(.3,sum(1 for t in cw if _NE.search(t)or _CD.search(t))*.1)
        return.6*ext+.4*epist
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|efe:{ws:.2f}|ncd:{nd:.2f}'})
