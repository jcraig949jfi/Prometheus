"""Load-Regulated Boltzmann Sampler v4. Concepts: Statistical Mechanics x Cognitive Load Theory x Emergence"""
import re, math, zlib
import numpy as np
...
def _ck_numcmp(p, c):
    ...
    if m:
        ...
        return (1. if c.lower().strip().startswith(ok) else -1.), True
...
class ReasoningTool:
    """Boltzmann distribution with cognitive load temperature modulation."""
    ...
    def _wrap(self, p, c):
        load = len(_CD.findall(p))*.4 + _nc(p)*.3 + len(_ns(p))*.2
        T = .5*(1.+min(load, 2.)); energy = self._struct(p, c)
        return math.exp(energy/max(T, .01))
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate': c, 'score': float(total),
                       'reasoning': f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|boltz:{ws:.2f}|ncd:{nd:.2f}'})
