"""Critical Functorial Reservoir v4. Concepts: Category Theory x Chaos Theory x SOC"""
import re, math, zlib
import numpy as np
...
class ReasoningTool:
    """Logistic map amplification with SOC avalanche penalty."""
    ...
    def _wrap(self,p,c):
        err=1.-(self._struct(p,c)+.5)
        x=max(.01,min(.99,err))
        for _ in range(5): x=3.99*x*(1.-x)
        return 1.-x
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),
                       'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|soc:{ws:.2f}|ncd:{nd:.2f}'})
