"""Chaotic Predictive Coding Network v4. Concepts: Chaos Theory x Neural Plasticity x FEP"""
import re,math,zlib
import numpy as np
...
class ReasoningTool:
    """Chaotic reservoir with free energy readout."""
    ...
    def _wrap(self,p,c):
        np.random.seed(42);W=np.random.randn(16,16)*1.2/3.
        def embed(t):
            v=np.zeros(16)
            for i,ch in enumerate(t[:32]):v[ord(ch)%16]+=1./(i+1)
            return v/(np.linalg.norm(v)+1e-9)*5.
        s=np.zeros(16)
        for _ in range(3):s=np.tanh(W@s+embed(c)*.5)
        t=np.zeros(16)
        for _ in range(3):t=np.tanh(W@t+embed(p)*.5)
        return 1./(1.+np.sum((s-t)**2))
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),
                       'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|chaos:{ws:.2f}|ncd:{nd:.2f}'})
