"""Plastic Abductive Pragmatics v4. Concepts: Neural Plasticity x Abductive Reasoning x Pragmatics"""
...
def _run(p,c):
    ...
    return 0.,'none'
class ReasoningTool:
    """Abductive constraint satisfaction with Gricean utility."""
    ...
    def _wrap(self,p,c):
        s=0.;pl,cl=p.lower(),c.lower()
        if _ns(p)and _h(_CP,pl)and _ns(c):s+=2.
        if _h(_NE,pl)and(_h(_NE,cl)or'no'in cl.split()):s+=1.5
        if _h(_CD,pl)and(_h(_CD,cl)or _h(_NE,cl)):s+=1.
        lr=len(c)/max(len(p),1)
        if lr>.8 and not _ns(c):s-=.5*lr
        else:s+=.5
        return s/5.
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|abduce:{ws:.2f}|ncd:{nd:.2f}'})
