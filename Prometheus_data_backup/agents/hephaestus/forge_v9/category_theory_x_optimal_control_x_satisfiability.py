"""Category Theory x Optimal Control x Satisfiability. Morphisms map prompt to
problem category then canonical solver. SAT for multi-step logic.
Chains solve_sat, topological_sort, bayesian_update, check_transitivity."""
import re, math, zlib, sys, os
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'src'))
from forge_primitives import (solve_sat, topological_sort, bayesian_update,
    check_transitivity, bat_and_ball, fencepost_count, pigeonhole_check)
_N = re.compile(r'-?\d+(?:\.\d+)?')
