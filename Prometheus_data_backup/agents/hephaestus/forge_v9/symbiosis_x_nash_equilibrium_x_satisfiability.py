"""Symbiosis x Nash Equilibrium x Satisfiability — Frame H. Under 200 lines."""
import re, math, zlib
from forge_primitives import (bayesian_update, bat_and_ball, all_but_n,
    fencepost_count, pigeonhole_check, check_transitivity,
    confidence_from_agreement, entropy, modus_ponens, expected_value)
...
class ReasoningTool:
    ...
    def _compute(self, p):
        ...
        if re.search(r'no\s+fire.*alarm|not.*fire.*alarm', pl): return "No", 0.92
        ...
        if re.search(r'(?:study|data|statistics)\s+finds?\s+.*(?:strong\s+)?correlation', pl):
            return "No", 0.85
        m = re.search(r'(?:takes?|requires?)\s+(\d+)\s+(?:hours?|minutes?)\s+to\s+\w+.*?(\d+)\s+(?:of\s+them|parts?|cakes?|items?)', pl)
