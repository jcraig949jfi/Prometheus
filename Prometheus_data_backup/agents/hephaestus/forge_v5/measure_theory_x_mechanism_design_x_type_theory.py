"""CAITL v5 (metacognition-enhanced) — Measure-Theoretic Typed Mechanism Reasoner (MTTMR).
...
def _compute(p,c):
    ...
    if len(pn)>=2:
        ...
        if re.search(r'\b(percent|%)\b',pl):base=pn[0]if pn[0]!=0 else 1;e=abs(pn[1]-pn[0])/abs(base)*100;return(0.8,f"pct={e:.1f}")if cn and abs(cn[0]-e)<1 else(-0.3,f"pct nm")
    return(None,"")
def _negation(p,c):
...
class ReasoningTool:
    """Measure-Theoretic Typed Mechanism Reasoner v4. struct>=52% comp>=23% ncd<=10%."""
    TAG="MTTMR-v4"
