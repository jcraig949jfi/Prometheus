"""CAITL v5 (metacognition-enhanced) — Oscillatory Free-Energy Type Checker (OFETC).
Typed constraint parsing + oscillatory precision weighting + FEP surprise minimization.
...
def _pn(t):
    ...
    seen = set()
    return [x for x in o if not (x in seen or seen.add(x))]
...
def _ncd(a, b):
    ...
    d = max(ca, cb)
    return (cc - min(ca, cb)) / d if d else 1.0
        ...
        for a in list(g):
            for b in list(g.get(a, [])):
                for c in list(g.get(b, [])):
                    if c not in g.get(a, set()):
                        g.setdefault(a, set()).add(c); changed = True
    return g
...
def _comp_score(p, c):
    ...
    if pn and re.search(r'\b(sum|total|add|combined|altogether|plus)\b', pl):
        e = sum(pn)
        if cn:
            best = min(cn, key=lambda x: abs(x - e))
            return (1.0, f"computation:sum={e},match") if abs(best - e) < 0.01 else (max(0, 1 - abs(best - e) / (abs(e) + 1)), f"computation:sum={e}")
        return (0.1, f"computation:sum={e},no_answer_num")
    if pn and re.search(r'\b(differ|subtract|minus|how many more|how much more)\b', pl) and len(pn) >= 2:
        e = abs(pn[0] - pn[1])
        if cn:
            best = min(cn, key=lambda x: abs(x - e))
            return (1.0, f"computation:diff={e},match") if abs(best - e) < 0.01 else (max(0, 1 - abs(best - e) / (abs(e) + 1)), f"computation:diff={e}")
    ...
    if pn and re.search(r'\b(product|multiply|times)\b', pl) and len(pn) >= 2:
        ...
        if cn:
            best = min(cn, key=lambda x: abs(x - e))
            return (1.0, f"computation:prod={e},match") if abs(best - e) < 0.01 else (max(0, 1 - abs(best - e) / (abs(e) + 1)), f"computation:prod={e}")
        return (0.1, f"computation:prod={e}")
    if pn and re.search(r'\b(remainder|mod|modulo)\b', pl) and len(pn) >= 2:
        a, b = int(pn[0]), int(pn[1]) if int(pn[1]) != 0 else 1
        e = a % b
        return ((1.0, f"computation:mod={e},match") if cn and e in [int(x) for x in cn] else (0.2, f"computation:mod={e}"))
    if pn and re.search(r'\b(per|rate|each|takes?)\b.*\b(together|combined|total)\b', pl) and len(pn) >= 2:
        try:
            t = 1.0 / sum(1.0 / x for x in pn if x > 0)
            if cn:
                best = min(cn, key=lambda x: abs(x - t))
                return (1.0, f"computation:rate={t:.2f},match") if abs(best - t) < 0.5 else (0.3, f"computation:rate={t:.2f}")
        except: pass
    # Numeric comparison
    if pn and len(pn) >= 2:
        if re.search(r'\b(larg|great|bigg|more|higher|tall|fast|old|heav)\w*\s+than\b', pl):
            return (0.9, "computation:cmp_yes") if ('yes' in cl or str(max(pn)) in cl) else (0.2, "computation:cmp")
        if re.search(r'\b(small|less|fewer|lower|short|slow|young|light)\w*\s+than\b', pl):
            return (0.9, "computation:cmp_yes") if ('yes' in cl or str(min(pn)) in cl) else (0.2, "computation:cmp")
    # Transitivity
    g = _transitive(p)
    if g:
        if re.search(r'\b(tallest|largest|biggest|oldest|heaviest|fastest|best|greatest|most)\b', pl):
            top = max(g, key=lambda x: len(g.get(x, set())))
            return (1.0, f"computation:trans_top={top}") if top in cl else (0.2, f"computation:trans_top={top}")
        if re.search(r'\b(shortest|smallest|youngest|lightest|slowest|worst|least)\b', pl):
            all_e = set()
            for a in g: all_e.add(a); all_e.update(g[a])
            bottoms = all_e - set(g.keys())
            for b in bottoms:
                if b in cl: return (1.0, f"computation:trans_bot={b}")
            return (0.2, f"computation:trans_bot={bottoms}")
    # All-but-N
    m = re.search(r'(?:all|every\w*)\s+(?:but|except)\s+(\d+)', pl)
    if m and re.search(r'how\s+many', pl):
        n = m.group(1)
        return (1.0, f"computation:all_but_{n}") if n in c else (0.2, f"computation:all_but_{n}")
    if not pn: return (0.5, "computation:no_nums")
    return (0.5, "computation:baseline")

def _struct(p, c):
    pl, cl = p.lower(), c.lower()
    sc, wt, R = 0.0, 0.0, []
    def _a(s, w, n):
        nonlocal sc, wt; sc += s * w; wt += w; R.append(f"{n}={s:.2f}")
    # Negation parity
    ...
    if _CMP.search(pl):
        cn_, pn_ = _pn(c), _pn(p)
        if pn_ and cn_: s = 0.7
        elif _NUM.search(c) or _CMP.search(c): s = 0.5
        else: s = 0.3
        _a(s, 0.08, 'cmp')
    # Boolean question
...
class ReasoningTool:
    """Oscillatory Free-Energy Type Checker v4. struct>=50% comp>=20% ncd<=15%."""
    TAG = "OFETC-v4"
