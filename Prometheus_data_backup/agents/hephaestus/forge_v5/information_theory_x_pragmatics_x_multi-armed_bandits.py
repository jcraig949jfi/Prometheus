"""Pragmatic Information Bandit v5. Concepts: Information Theory x Pragmatics x Multi-Armed Bandits"""
...
class ReasoningTool:
    """Information-directed sampling with pragmatic alignment."""
    ...
    def _wrap(self,p,c):
        cl=c.lower();s=1.
        if _h(_NE,p)and not _h(_NE,c):
            if cl in('yes','true','correct'):s-=.5
        if _h(_NE,p)and _h(_NE,c):s+=.2
        if _h(_CP,p)and _h(_CP,c):s+=.1
        return max(0.,s)
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        R=[]
        for c in candidates:
            cs,cn=_run(prompt,c);ss=self._struct(prompt,c);ws=self._wrap(prompt,c);nd=(1.-self._ncd(prompt,c))
            R.append({'candidate':c,'score':float((ss*.25+ws*.30)+cs*.35+nd*.10),'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|bandit:{ws:.2f}|ncd:{nd:.2f}'})
