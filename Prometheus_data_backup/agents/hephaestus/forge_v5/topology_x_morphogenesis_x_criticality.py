"""Morphogenetic Critical Network v5. Concepts: Topology x Morphogenesis x Criticality"""
import re, math, zlib
import numpy as np
...
class ReasoningTool:
    """Topological invariant checking at critical threshold."""
    ...
    def _wrap(self,p,c):
        feats=[_NE,_CP,_CD,_TM]; matches=total=0
        for f in feats:
            hp,hc=_h(f,p),_h(f,c)
            if hp or hc: total+=1
            if hp==hc: matches+=1
    ...
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),
                       'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|topo:{ws:.2f}|ncd:{nd:.2f}'})
