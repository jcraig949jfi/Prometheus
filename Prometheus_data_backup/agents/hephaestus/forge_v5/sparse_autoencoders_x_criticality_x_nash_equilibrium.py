"""CAITL v5 (metacognition-enhanced) — SAE-Criticality-Nash. Constructive computation, 58-cat parsers,
...
def _struct(p, c):
    ...
    wt = [0.22, 0.18, 0.14, 0.13, 0.12, 0.10, 0.11]
...
def _theory(prompt, cand):
    """Theory-specific: sparse codes + game-theoretic weighting."""
...
class ReasoningTool:
    """v4 SACN: constructive computation, epistemic honesty, 58-cat parsers,
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict[str, Any]]:
        ...
        for i, cand in enumerate(candidates):
            ...
            reason = f"[SACN-v4] {sr} | {cr} | ncd={ncd_raw[i]:.3f}"
