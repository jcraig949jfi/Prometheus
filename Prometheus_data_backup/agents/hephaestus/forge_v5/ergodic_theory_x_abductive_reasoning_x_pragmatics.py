"""Ergodic Abductive Pragmatic Sampler v5. Concepts: Ergodic Theory x Abductive Reasoning x Pragmatics"""
...
class ReasoningTool:
    """Ergodic stationary distribution over candidates."""
    ...
    def _struct(self,p,c):
        ...
        return s
    def _ncd(self,x,y):
        cx,cy=len(zlib.compress(x.encode(),self._lv)),len(zlib.compress(y.encode(),self._lv));cxy=len(zlib.compress((x+' '+y).encode(),self._lv));d=max(cx,cy);return(cxy-min(cx,cy))/d if d>0 else 1.
    def _wrap(self,p,c):
        abd=0.
        if _nc(p)>0:abd+=(2.if _nc(c)>0 else-1.)
        if _h(_CP,p):abd+=(1.5 if _h(_CP,c)else 0.)
        if _h(_CD,p):abd+=(1.5 if _h(_CD,c)or _ns(c)else 0.)
        pw=set(re.findall(r'\w+',p.lower()))-{'the','is','a','an','to','of'}
        cw=set(re.findall(r'\w+',c.lower()));rel=len(pw&cw)/max(len(pw),1)*3.
        return 1./(1.+math.exp(-.5*(abd+rel)))
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        R=[]
        for c in candidates:
            cs,cn=_run(prompt,c);ss=self._struct(prompt,c);ws=self._wrap(prompt,c);nd=(1.-self._ncd(prompt,c))
            R.append({'candidate':c,'score':float((ss*.25+ws*.30)+cs*.35+nd*.10),'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|ergodic:{ws:.2f}|ncd:{nd:.2f}'})
