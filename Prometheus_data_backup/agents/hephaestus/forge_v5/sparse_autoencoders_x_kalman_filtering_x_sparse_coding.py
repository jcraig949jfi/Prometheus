"""Temporal Sparse Kalman Filter v5. Concepts: SAE x Kalman Filtering x Sparse Coding"""
import re, math, zlib
import numpy as np
...
class ReasoningTool:
    """Kalman state estimation on sparse feature codes."""
    ...
    def _wrap(self,p,c):
        mu,sig=.5,1.; obs=self._struct(p,c)+.5
        sig_p=sig+.1; K=sig_p/(sig_p+.5)
        mu_n=mu+K*(obs-mu)
        return max(0.,min(1.,mu_n))
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),
                       'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|kalman:{ws:.2f}|ncd:{nd:.2f}'})
