"""CA Critical Information Engine v5. Concepts: Information Theory x Cellular Automata x Criticality"""
import re, math, zlib
import numpy as np
...
class ReasoningTool:
    """Cellular automaton entropy at criticality."""
    ...
    def _wrap(self,p,c):
        tokens=c.split()
        if not tokens: return .5
        a=np.array([sum(1 for pat in [_NE,_CP,_CD,_TM] if pat.search(t)) for t in tokens],dtype=float)
        mx=a.max(); a=a/(mx+1e-9) if mx>0 else a+.1
        for _ in range(5):
            new=np.zeros_like(a)
            for i in range(len(a)):
                nb=np.array([a[(i-1)%len(a)],a[(i+1)%len(a)]])
                new[i]=1./(1.+np.exp(-.5+2.*np.std(np.append(a[i],nb))))
            a=new
        h,_=np.histogram(a,bins=10,range=(0,1)); h=h/(h.sum()+1e-9)
        return float(-np.sum(h*np.log2(h+1e-9))/max(np.log2(10),1.))
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),
                       'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|ca_ent:{ws:.2f}|ncd:{nd:.2f}'})
