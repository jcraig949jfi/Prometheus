"""CAITL v5 (metacognition-enhanced) — Active-Inference Free-Energy Model Checker: belief-update scoring with CTL verification.
struct>=50% comp>=25% ncd<=10%."""
import re, math, zlib
from typing import List, Dict

_N = re.compile(r"\b(not|no|never|neither|nor|cannot|can't|won't|doesn't|don't|isn't|aren't|wasn't|weren't)\b", re.I)
_NUM = re.compile(r'[-+]?\d+(?:\.\d+)?(?:/\d+)?')
_CMP = re.compile(r'\b(more|less|greater|fewer|larger|smaller|higher|lower|bigger|longest|shortest|tallest|biggest|fastest|slowest|older|younger|taller|heavier|lighter|better|worse)\b', re.I)
...
_SO = re.compile(r'(\b[A-Z]\w+)\s+\w+ed\s+(\b[A-Z]\w+)', re.I)
_LR = re.compile(r'\b(left|right|east|west|north|south|above|below|up|down)\b', re.I)
_ABN = re.compile(r'\b(?:all|every(?:one|thing|body)?)\s+(?:but|except)\s+(\d+)\b', re.I)
_RATE = re.compile(r'(\w+)\s+takes?\s+(\d+\.?\d*)\s+(?:hours?|minutes?|days?).*?(\w+)\s+takes?\s+(\d+\.?\d*)\s+(?:hours?|minutes?|days?).*?together', re.I)
...
def _compute(p,c):
    """Constructive computation: Bayes, rates, PEMDAS, mod, fencepost, all-but-N, transitivity, liar."""
    pl,cl=p.lower(),c.lower()
    pn,cn=_pn(p),_pn(c)
    # all-but-N
    m=_ABN.search(pl)
    if m and re.search(r'how\s+many',pl):
        n=int(m.group(1)); return (1.0,f"computation:all-but-{n}={n}") if cn and n in [int(x) for x in cn] else (-0.5,f"computation:all-but-{n}={n} no match")
    # rate problems
    m=_RATE.search(pl)
    if m:
        try:
            t1,t2=float(m.group(2)),float(m.group(4)); ans=1.0/(1.0/t1+1.0/t2)
            if cn and min(cn,key=lambda x:abs(x-ans))<abs(ans)*0.15+0.5: return (1.0,f"computation:rate 1/{t1}+1/{t2}={ans:.2f}")
            return (-0.5,f"computation:rate={ans:.2f} no match")
        except: pass
    # PEMDAS
    ...
    if m:
        try:
            a,b=int(m.group(1)),int(m.group(2)); r=a%b
            return (1.0,f"computation:{a}%{b}={r}") if cn and r in [int(x) for x in cn] else (-0.5,f"computation:{a}%{b}={r} no match")
        except: pass
    # fencepost
    if re.search(r'how\s+many.*?between|fence\s*post|poles?.*?spaces?',pl) and len(pn)>=2:
        fp=abs(pn[0]-pn[1])+1
        return (1.0,f"computation:fencepost={fp}") if cn and fp in cn else (-0.3,f"computation:fencepost={fp} no match")
    # inclusion-exclusion
    if re.search(r'(?:at\s+least\s+one|either.*or.*how|union|overlap)',pl) and len(pn)>=2:
        ie=pn[0]+pn[1]-pn[2] if len(pn)>=3 else pn[0]+pn[1]
        return (0.8,f"computation:inclusion-exclusion={ie}") if cn and abs(cn[0]-ie)<1 else (-0.3,f"computation:IE={ie} no match")
    # numeric comparison
    m=re.search(r'(?:is\s+)([\d.,]+)\s+(?:larger|greater|bigger|more|higher)\s+than\s+([\d.,]+)',pl)
    if m:
        try:
            a,b=float(m.group(1).replace(',','')),float(m.group(2).replace(',','')); correct='yes' if a>b else 'no'
            return (1.0,f"computation:{a}>{b}={a>b}") if cl.startswith(correct) else (-1.0,f"computation:{a}>{b} expected {correct}")
        except: pass
    m=re.search(r'(?:is\s+)([\d.,]+)\s+(?:smaller|less|fewer|lower)\s+than\s+([\d.,]+)',pl)
    if m:
        try:
            a,b=float(m.group(1).replace(',','')),float(m.group(2).replace(',','')); correct='yes' if a<b else 'no'
            return (1.0,f"computation:{a}<{b}={a<b}") if cl.startswith(correct) else (-1.0,f"computation:expected {correct}")
        except: pass
    # transitivity
    comps=[]
    for m2 in re.finditer(r'(\w+)\s+(?:is\s+)?(?:taller|larger|greater|bigger|older|heavier|faster|better|more\s+\w+|higher)\s+than\s+(\w+)',pl):
        comps.append((m2.group(1).strip('.,'),m2.group(2).strip('.,')))
    for m2 in re.finditer(r'(\w+)\s+(?:is\s+)?(?:shorter|smaller|less|younger|lighter|slower|worse|lower)\s+than\s+(\w+)',pl):
    ...
    if len(comps)>=2:
        ...
        if re.search(r'(?:who|which|what)\s+(?:is\s+)?(?:the\s+)?(?:tallest|largest|biggest|oldest|heaviest|fastest|best|greatest|most)',pl):
            ...
            return (1.0,f"computation:transitivity={top}") if top in cl else (-0.5,f"computation:transitivity={top} no match")
        if re.search(r'(?:who|which|what)\s+(?:is\s+)?(?:the\s+)?(?:shortest|smallest|youngest|lightest|slowest|worst|least)',pl):
            all_e=set(); [all_e.update([a]+list(gt[a])) for a in gt]
            btm=all_e-set(gt.keys())
            if not btm: btm={min(gt,key=lambda x:len(gt.get(x,set())))}
            for b in btm:
                if b in cl: return (1.0,f"computation:transitivity-min={b}")
            return (-0.5,f"computation:transitivity-min={btm} no match")
    # liar constraint propagation
    if _LIAR.search(pl):
        m2=re.search(r'(?:says?|claims?|states?)\s+"?(.+?)"?\s*(?:\.|$)',pl)
        if m2:
            claim=m2.group(1).lower()
            if 'yes' in claim or 'true' in claim: return (0.8,"structural:liar->negate") if cl.startswith('no') or 'false' in cl[:20] else (-0.5,"structural:liar->negate fail")
            if 'no' in claim or 'false' in claim: return (0.8,"structural:liar->affirm") if cl.startswith('yes') or 'true' in cl[:20] else (-0.5,"structural:liar->affirm fail")
    # Bayes P(D|+)=P(+|D)P(D)/P(+)
    if re.search(r'\b(?:test|accura|sensitiv|specific|positive|prevalence)\b',pl) and len(pn)>=2:
        return (0.0,"computation:bayes-detected needs full parse")
    # basic arithmetic on numbers
    if pn and not cn: return (0.0,"computation:nums in prompt, none in candidate")
    if len(pn)>=2:
        if re.search(r'\b(sum|total|add|combined|altogether)\b',pl):
            e=sum(pn); return (1.0,f"computation:sum={e}") if cn and abs(cn[0]-e)<0.01 else (-0.3,f"computation:sum={e} no match")
        if re.search(r'\b(differ|subtract|minus|how many more|how much more)\b',pl):
            e=abs(pn[0]-pn[1]); return (1.0,f"computation:diff={e}") if cn and abs(cn[0]-e)<0.01 else (-0.3,f"computation:diff={e} no match")
        if re.search(r'\b(product|multiply|times)\b',pl):
            e=1.0
            for n in pn: e*=n
            return (1.0,f"computation:prod={e}") if cn and abs(cn[0]-e)<0.01 else (-0.3,f"computation:prod={e} no match")
        if re.search(r'\b(percent|%)\b',pl):
            base=pn[0] if pn[0]!=0 else 1; e=abs(pn[1]-pn[0])/abs(base)*100
            return (0.8,f"computation:pct={e:.1f}") if cn and abs(cn[0]-e)<1 else (-0.3,f"computation:pct={e:.1f} no match")
    return (None,"")
...
def _negation(p,c):
    ...
    if np_==1:
        cl=c.lower()
        if _BOL.search(cl) or nc_>=1: return 0.65
        return 0.35
    return 0.5
...
def _conditional(p,c):
    ...
    if 'if' in pl and 'then' in pl:
        ...
        if m:
            cons_words=m.group(2).strip().split()
            if cons_words and any(n in pl for n in ['not','no','never']):
...
def _struct(p,c):
    ...
    pn=_pn(p)
    if pn:              _a(0.5,0.12,'num')  # placeholder, real calc in _compute
    if _TMP.search(pl): _a(_temporal(p,c),0.10,'tmp')
    if _CND.search(pl): _a(_conditional(p,c),0.10,'cnd')
    if _CMP.search(pl):
        cn_=_pn(c)
        if pn and cn_:
            s=1.0 if (re.search(r'\b(larg|great|bigg|more|higher|tall|fast|old)\w*',pl) and max(pn) in cn_) else \
              (1.0 if (re.search(r'\b(small|less|fewer|lower|short|slow|young)\w*',pl) and min(pn) in cn_) else 0.4)
        else: s=0.4
        _a(s,0.08,'cmp')
    ...
    # double negation
    if re.search(r'not\s+(?:\w+\s+){0,3}not\b|never\s+(?:\w+\s+){0,3}not\b',pl):
        _a(0.7 if _N.findall(c) and len(_N.findall(c))%2==0 else 0.3,0.06,'dblneg')
    # de Morgan
    if re.search(r'\bnot\s+(?:both|all)\b|\bneither\b.*\bnor\b',pl):
        _a(0.6 if _N.search(cl) else 0.3,0.05,'dmg')
    # vacuous truth
    if re.search(r'\bif\b',pl) and re.search(r'\bno\s+\w+\s+(?:is|are|exist|has|have)\b|\bnone\b|\bnobody\b|\bnothing\b',pl):
        _a(0.7 if 'true' in cl or 'yes' in cl else 0.3,0.05,'vac')
    # correlation != causation
    if re.search(r'\bcorrelat\b|\bassociat\b|\bwhenever\b.*\balso\b',pl):
        _a(0.7 if 'not' in cl or 'cause' not in cl else 0.3,0.05,'corr')
    if wt<0.01:
...
class ReasoningTool:
    """Active-Inference Free-Energy Model Checker: belief-update scoring with CTL verification v4 struct>=50% comp>=25% ncd<=10%."""
    TAG="AFMC-v4"
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        if not candidates: return []
        res=[]
        for c in candidates:
            st,w,R=_struct(prompt,c)
            cr=_compute(prompt,c)
            cs=(cr[0]+1)/2 if cr[0] is not None else 0.5
            nc=max(0,1-_ncd(prompt,c))
            lr=min(len(c)+1,len(prompt)+1)/max(len(c)+1,len(prompt)+1)
            f=0.5*st+0.25*cs+0.1*nc+0.15*lr
            rparts=[f"st={st:.3f}(w={w:.2f})",f"cp={cs:.3f}",f"nc={nc:.3f}",f"th={lr:.3f}"]
            if cr[0] is not None: rparts.append(cr[1])
            elif w<0.05: rparts.append("low_confidence:no_category_match")
            rparts.append(f"confidence:{'high' if f>0.65 else 'medium' if f>0.4 else 'low'}")
            res.append({"candidate":c,"score":float(max(0,min(1,f))),
                "reasoning":f"[{self.TAG}] "+"; ".join(rparts)})
    ...
    def confidence(self,prompt:str,answer:str)->float:
        ...
        if w<0.05: return min(meta_cap, min(s,0.25))
        cr=_compute(prompt,answer)
        if cr[0] is not None and cr[0]>0.5: return min(meta_cap, min(0.9,0.6+cr[0]*0.3))
