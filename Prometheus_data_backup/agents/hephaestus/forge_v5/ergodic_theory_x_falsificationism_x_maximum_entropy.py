"""CAITL v5 (metacognition-enhanced) — Ergodic-Falsification-MaxEnt. Constructive computation, 58-cat parsers,
...
def _nums(t):
    ...
    for m in _NUM.finditer(t):
        s = m.group()
        try:
            out.append(float(s.split('/')[0]) / float(s.split('/')[1]) if '/' in s else float(s))
        except: pass
    return out
...
def _struct(p, c):
    ...
    wt = [0.22, 0.18, 0.15, 0.12, 0.12, 0.10, 0.11]
...
class ReasoningTool:
    """v4 EFME: constructive computation, epistemic honesty, 58-cat parsers,
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict[str, Any]]:
        ...
        for i, cand in enumerate(candidates):
            ...
            f = _fals(p, c); e = _erg(prompt, cand)
            raw = ss*0.55 + cs*0.25 + ncd_raw[i]*0.12 + f*0.04 + e*0.02 + 0.02
            reason = f"[EFME-v4] {sr} | {cr} | ncd={ncd_raw[i]:.3f}"
