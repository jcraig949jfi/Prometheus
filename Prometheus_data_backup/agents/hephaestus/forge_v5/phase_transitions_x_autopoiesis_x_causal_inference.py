"""Phase Transition Causal Analyzer v5. Concepts: Phase Transitions x Autopoiesis x Causal Inference"""
...
class ReasoningTool:
    """Order parameter with phase transition boost."""
    ...
    def _wrap(self,p,c):
        ss=self._struct(p,c)+.5
        if ss>=.65:return 1.
        if ss>0:return ss
        return(1.-self._ncd(p,c))*.5
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|phase:{ws:.2f}|ncd:{nd:.2f}'})
