"""Critical Functor Metacognitive v5. Concepts: Category Theory x Metacognition x Criticality"""
import re, math, zlib
import numpy as np
...
class ReasoningTool:
    """Functorial feature mapping with critical cascade penalty."""
    ...
    def _wrap(self,p,c):
        pf=np.array([_nc(p),len(_CP.findall(p)),len(_ns(p)),min(len(p)/500.,1.)])
        cf=np.array([_nc(c),len(_CP.findall(c)),len(_ns(c)),min(len(c)/500.,1.)])
        d=np.linalg.norm(pf-cf); return 1./(1.+d)
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),
                       'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|functor:{ws:.2f}|ncd:{nd:.2f}'})
