"""CAITL v5 (metacognition-enhanced) — Tensor-Criticality-FreeEnergy. Constructive computation, 58-cat parsers,
...
def _struct(p, c):
    ...
    wt = [0.20, 0.18, 0.16, 0.13, 0.12, 0.10, 0.11]
...
def _theory(prompt, cand):
    """Theory-specific: tensor decomposition + criticality gain."""
...
class ReasoningTool:
    """v4 TDCF: constructive computation, epistemic honesty, 58-cat parsers,
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict[str, Any]]:
        ...
        for i, cand in enumerate(candidates):
            ...
            raw = ss*0.54 + cs*0.26 + ncd_raw[i]*0.12 + f*0.04 + e*0.02 + 0.02
            reason = f"[TDCF-v4] {sr} | {cr} | ncd={ncd_raw[i]:.3f}"
