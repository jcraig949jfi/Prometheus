"""CAITL v5 (metacognition-enhanced) — Renormalized Differentiable Proof Engine (RDPE).
Multi-scale constraint coarsening + differentiable scoring + proof-theoretic validation.
...
def _comp(p,c):
    ...
    if pn and re.search(r'\b(remainder|mod)\b',pl)and len(pn)>=2:
...
class ReasoningTool:
    """Renormalized Differentiable Proof Engine v4. struct>=50% comp>=20% ncd<=15%."""
    TAG="RDPE-v4"
