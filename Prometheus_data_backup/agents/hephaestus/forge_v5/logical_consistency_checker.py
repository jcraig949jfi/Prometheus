"""CAITL v4 — Logical Consistency Checker (Proof Theory x Constraint Satisfaction).
Constructive computation + constraint-graph consistency scoring."""
import re, math, zlib
...
class ReasoningTool:
    ...
    def _sec(self, p, c):
        """Constraint-graph consistency: extract relational facts, check violations."""
        gt=re.findall(r'(\w+)\s+(?:is|are)\s+(?:larger|greater|taller|faster|more)\s+than\s+(\w+)',p.lower())
        lt=re.findall(r'(\w+)\s+(?:is|are)\s+(?:smaller|less|shorter|slower)\s+than\s+(\w+)',p.lower())
        g={}
        for a,b in gt: g.setdefault(a,set()).add(b)
        for a,b in lt: g.setdefault(b,set()).add(a)
        viol=0
        for a in g:
            for b in g.get(a,set()):
                if a in g.get(b,set()): viol+=1
        cw=set(self._w(c)); pw=set(self._w(p))
        ov=len(pw&cw)/max(len(pw|cw),1)
        return max(0, 0.5+0.3*ov-0.2*viol)
