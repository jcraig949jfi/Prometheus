# Thermodynamics + Analogical Reasoning + Free Energy Principle

**Fields**: Physics, Cognitive Science, Theoretical Neuroscience
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-26T23:47:38.097410
**Report Generated**: 2026-03-27T18:24:03.318647
...
**Algorithm**  
The tool builds two labeled directed graphs from the prompt + candidate: *Gₚ* (premises) and *G𝒸* (candidate answer). Nodes encode entities; edges encode relational predicates (e.g., *cause*, *greater‑than*, *¬*). Each node also carries a feature vector *f* ∈ ℝᵏ (entity type, numeric value, polarity).  

1. **Structure mapping (Analogical Reasoning)** – Compute a similarity matrix *S* ∈ ℝ^{|Vₚ|×|V𝒸|} where *Sᵢⱼ* = exp(−‖fᵢ−gⱼ‖₂²/σ²) · δ(typeᵢ,typeⱼ). Solve the linear sum assignment problem with the Hungarian algorithm (implemented via NumPy) to obtain a soft matching *M* (doubly‑stochastic). The analogical reward is *Rₐ* = trace(Mᵀ S).  

2. **Prediction error (Free Energy Principle)** – From *Gₚ* derive a prior distribution over edge types, *P* (e.g., frequency‑normalized adjacency). From *G𝒸* derive the observed distribution *Q*. Prediction error is the KL‑divergence *Dₖₗ(Q‖P)*, computed with NumPy log and sum.  

3. **Entropy term (Thermodynamics)** – The entropy of the matching *M* is *H* = −∑ᵢⱼ Mᵢⱼ log Mᵢⱼ  (added to encourage diverse mappings).  

4. **Energy (score)** – *E* = *Dₖₗ(Q‖P)* + α·*H* − β·*Rₐ*. Lower *E* indicates a better answer; the final score is *−E* (higher = better). All operations use only NumPy and the standard library.

**Parsed structural features**  
- Negations (¬ edges) → polarity flag on node/edge.  
- Comparatives & ordering relations → weighted “greater‑than/less‑than” edges with numeric attributes.  
- Conditionals → implication edges ( antecedent → consequent ).  
- Causal claims → directed cause‑effect edges with confidence weight.  
- Numeric values → attached scalar to node feature vector.  
- Conjunctions/disjunctions → hyper‑edges transformed into auxiliary nodes for propagation.

**Novelty**  
Pure energy‑based scoring or analogical mapping exists separately, but coupling variational free energy minimization (prediction error + entropy) with explicit structure‑mapping rewards for answer evaluation is not common in current NLP pipelines, making the combination relatively novel.

**Rating lines**  
Reasoning: 7/10 — captures relational consistency and prediction error but relies on linear assignment approximations.  
Metacognition: 5/10 — limited self‑monitoring; entropy term offers rough uncertainty awareness.  
Hypothesis generation: 6/10 — matching process yields alternative mappings, supporting hypothesis exploration.  
Implementability: 8/10 — uses only NumPy and stdlib; Hungarian algorithm and KL divergence are straightforward to code.
...
**Forge Timestamp**: 2026-03-27T17:52:09.320536
...
[View code](./Thermodynamics---Analogical_Reasoning---Free_Energy_Principle/tool.py)
