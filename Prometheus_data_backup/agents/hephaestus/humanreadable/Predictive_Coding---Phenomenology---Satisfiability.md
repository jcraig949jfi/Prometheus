# Predictive Coding + Phenomenology + Satisfiability

**Fields**: Cognitive Science, Philosophy, Logic
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T18:36:27.314264
**Report Generated**: 2026-04-02T12:33:29.233023
...
**Algorithm**  
The tool builds a hierarchical propositional model from the prompt and each candidate answer.  

1. **Parsing layer (phenomenological bracketing)** – Using regex we extract atomic propositions and annotate them with intentional structures:  
   - *Negation* (`not`, `no`) → literal `¬p`  
   - *Comparatives* (`>`, `<`, `≥`, `≤`, `more than`, `less than`) → arithmetic literals `x > 5` etc.  
   - *Conditionals* (`if … then …`) → implication `p → q`  
   - *Causal cues* (`because`, `leads to`) → treated as bidirectional implication for scoring.  
   - *Ordering/temporal* (`before`, `after`, `first`, `last`) → precedence literals `t1 < t2`.  
   Each literal receives a unique integer ID; we store a mapping `ID ↔ (predicate, args, polarity)`.  

2. **Generative model layer (predictive coding)** – We treat the prompt as a top‑down prediction: a set of weighted clauses **Cₚ** derived from the parsed literals (e.g., `p ∨ q`, `¬r → s`). Each clause gets a precision weight *w* (default 1.0; higher for explicit cues like “must”). The candidate answer generates a bottom‑up hypothesis **Cₐ** (its own clause set). Prediction error is the weighted sum of unsatisfied clauses when we jointly enforce **Cₚ ∪ Cₐ**.  

3. **Constraint‑propagation layer (satisfiability)** – We run a lightweight DPLL‑style SAT solver (pure Python, using only lists and recursion) on the combined clause set. The solver returns:  
   - *SAT* → all clauses satisfied → error = 0.  
   - *UNSAT* → we extract a minimal unsatisfiable core (by iterative clause removal) and compute error = Σ wᵢ over core clauses / Σ wᵢ over all clauses.  
   The final score for a candidate is `S = 1 – error`, ranging 0 (total mismatch) to 1 (perfect prediction).  

**Structural features parsed** – negations, comparatives, conditionals, causal language, numeric thresholds, ordering/temporal relations, and explicit quantifiers (“all”, “some”) when they appear as scope‑marking words.  

**Novelty** – While predictive coding models of language, phenomenological annotation schemes, and SAT‑based reasoning exist separately, their tight integration—using phenomenological bracketing to generate propositional literals, predictive coding to weight predictions, and SAT solving to quantify prediction error—has not been reported in the literature.  

**Ratings**  
Reasoning: 8/10 — captures logical structure and uncertainty via weighted SAT error.  
Metacognition: 6/10 — phenomenological bracketing gives a rudimentary model of the answerer’s perspective but lacks higher‑order self‑monitoring.  
Hypothesis generation: 7/10 — the solver actively searches for assignments that minimize error, forming hypotheses about truth values.  
Implementability: 9/10 — relies only on regex, lists, and a simple DPLL solver; no external libraries or neural components needed.
...
**Forge Timestamp**: 2026-04-02T11:47:42.182131
...
[View code](./Predictive_Coding---Phenomenology---Satisfiability/tool.py)
