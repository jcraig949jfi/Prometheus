# Immune Systems + Theory of Mind + Satisfiability

**Fields**: Biology, Cognitive Science, Logic
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-27T01:46:59.868859
**Report Generated**: 2026-03-27T17:21:24.691554

---

## Nous Analysis

The algorithm treats a prompt and each candidate answer as sets of logical clauses extracted by regex‑based structural parsing. Literals correspond to atomic propositions (e.g., “X is Y”, “X > Y”, “¬P”). Clauses are built for conditionals (“if A then B” → ¬A ∨ B), negations, comparatives, causal connectives (“A because B” → B ∧ A), and quantifiers (“all X are Y” → ∀x ¬X(x) ∨ Y(x)). The parsed clauses are stored in two parallel arrays: a NumPy array of literal IDs and a Python list of clause literal lists, enabling fast unit‑propagation and pure‑literal elimination.

Scoring proceeds in three stages. First, a lightweight DPLL SAT solver (implemented with NumPy stacks for the assignment trail) checks satisfiability of the union of prompt clauses and answer clauses. If SAT, the base score is 1.0; if UNSAT, the solver records the first conflict clause and backtracks to extract a minimal unsatisfiable core (MUC) via iterative clause deletion, yielding a conflict size *c*. The raw score becomes 1 − (*c* / |answer clauses|). Second, to emulate immune clonal selection, the algorithm generates a mutation pool of the answer by applying stochastic edits (synonym swap, negation flip, quantifier shift) and re‑scores each mutant; the highest‑scoring mutant replaces the original if it improves the score by a threshold ε (e.g., 0.05). Third, a theory‑of‑mind layer maintains a belief distribution over possible answerer intentions: each mutant’s score updates a Dirichlet‑style weight, and the final answer score is the expectation of this distribution, effectively modeling what the answerer likely believes given the prompt.

Structural features parsed include negations, comparatives (> , < , =), conditionals (if‑then, unless), causal verbs (because, leads to), temporal ordering (before, after), and numeric constants with arithmetic relations. This combination of clonal exploration, belief‑weighted aggregation, and SAT‑based conflict localization is not found in existing pure‑logic or neural scoring tools; prior work uses either argumentation trees or similarity metrics, but not an immune‑inspired mutagenesis loop coupled with ToM belief updating and MUC‑based penalization.

Reasoning: 8/10 — captures logical structure and detects contradictions via SAT, though limited to first‑order patterns expressible by the regex grammar.  
Metacognition: 7/10 — models alternative answerer beliefs through a weighted mutant pool, but lacks recursive depth beyond one level of intention modeling.  
Hypothesis generation: 8/10 — clonal mutation and selection yields diverse answer variants; effectiveness depends on mutation operators’ coverage.  
Implementability: 9/10 — relies only on regex, NumPy arrays for literal assignment, and Python lists/sets; no external libraries or neural components needed.
...
**Forge Timestamp**: 2026-03-27T16:34:07.321600
...
[View code](./Immune_Systems---Theory_of_Mind---Satisfiability/tool.py)
