# Genetic Algorithms + Criticality + Metamorphic Testing

**Fields**: Computer Science, Complex Systems, Software Engineering
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T10:42:16.206836
**Report Generated**: 2026-04-02T11:44:50.224337
...
**Algorithm**  
1. **Parsing stage** – Using only `re` from the standard library, extract a set of atomic propositions *P* from the prompt and each candidate answer. Each proposition is a 4‑tuple `(entity₁, relation, entity₂, polarity)` where `relation` ∈ {`=`, `<`, `>`, `→`, `cause`, `and`, `or`} and `polarity` ∈ {`+`, `-}` for negation. Numeric literals are captured as separate `(value, unit)` tokens and attached to the entity they modify. The result is a list `props = [p₀, p₁, …]` stored as a NumPy structured array with fields `e1`, `rel`, `e2`, `pol`, `num`.  
2. **Metamorphic relation (MR) generation** – Define a fixed MR taxonomy as functions that transform a proposition set:  
   - *Swap*: exchange `e1` and `e2` for symmetric relations (`=`, `<`, `>`).  
   - *Negate*: flip `polarity`.  
   - *Scale*: multiply any numeric token by a constant `k∈{0.5,2}`.  
   - *Chain*: apply modus ponens on `→` propositions to derive new implied propositions.  
   Each MR yields a transformed set `P'`. Store all MRs in a list `MRs`.  
3. **Fitness evaluation** – For a candidate answer `A`, compute its proposition set `P_A`. For each MR `m` compute `P_A' = m(P_A)`. Fitness `f(A)` = Σ_{m∈MRs} `match(P_A', P_prompt)` where `match` counts propositions that appear unchanged in the prompt’s proposition set (using NumPy `in1d` on the structured fields). Add a penalty proportional to the number of contradictions detected via simple resolution (e.g., same entity pair with opposite polarity).  
4. **Genetic algorithm with criticality control** – Initialise a population of `N` candidate answers (randomly generated or supplied). Each generation:  
   - Selection: tournament size 2 based on `f`.  
   - Crossover: uniform exchange of proposition tuples between two parents.  
   - Mutation: with probability `μ` flip polarity, swap entities, or scale a numeric token.  
   - Criticality adjustment: compute the variance `σ²` of fitness across the population. If `σ²` falls below a threshold `ε` (ordered regime), increase `μ` by `Δμ`; if `σ²` exceeds `ε` (disordered regime), decrease `μ`. This drives the system toward the edge‑of‑chaos point where diversity and selection balance, mimicking self‑organized criticality.  
   Iterate for a fixed number of generations or until fitness convergence. The final best individual’s fitness score is the algorithm’s output.

**Structural features parsed**  
Negations (`not`, `n’t`), comparatives (`greater than`, `less than`, `equals`), conditionals (`if … then …`), causal verbs (`cause`, `lead to`), ordering relations (`before`, `after`, `precedes`), conjunctions/disjunctions (`and`, `or`), and numeric values with units. These are captured directly as proposition fields and used in MRs and constraint propagation.

**Novelty assessment**  
While GAs have been applied to test case generation and metamorphic testing is well‑studied, coupling GA selection pressure with a criticality‑driven mutation schedule that self‑adjusts at the edge of chaos, and using MRs as the sole fitness signal, has not been reported in the literature. The combination therefore constitutes a novel reasoning‑scoring method.

**Rating**  
Reasoning: 7/10 — The algorithm captures logical structure via proposition parsing and MR‑based fitness, enabling genuine inference rather than surface similarity.  
Metacognition: 5/10 — Criticality‑based mutation control provides basic self‑regulation but lacks explicit monitoring of search dynamics or reflective heuristics.  
Hypothesis generation: 6/10 — Crossover and mutation generate new proposition combinations, offering exploratory hypothesis formation, though guided only by fitness gradients.  
Implementability: 9/10 — All components rely exclusively on `re` and NumPy; no external libraries or APIs are required, making the method straightforward to code and run.
...
**Forge Timestamp**: 2026-04-02T11:42:09.397354
...
[View code](./Genetic_Algorithms---Criticality---Metamorphic_Testing/tool.py)
