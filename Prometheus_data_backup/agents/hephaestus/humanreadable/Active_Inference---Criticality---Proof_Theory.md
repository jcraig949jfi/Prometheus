# Active Inference + Criticality + Proof Theory

**Fields**: Cognitive Science, Complex Systems, Mathematics
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-25T17:55:12.707498
**Report Generated**: 2026-03-27T01:02:18.024937
...
**Algorithm**  
1. **Parsing** – Use regex to extract atomic propositions, negations (`not`), comparatives (`>`, `<`, `=`), conditionals (`if … then …`), causal markers (`because`, `leads to`), and ordering terms (`before`, `after`). Each atom gets an index *i*.  
2. **Clause construction** – Convert each extracted relation into a Horn clause *Cₖ*: a set of antecedent literals → consequent literal. Store clauses in a list `clauses = [(antecedent_set, consequent)]`.  
3. **Proof graph** – Build a directed acyclic graph *G* where nodes are literals and an edge *u → v* exists for each clause whose antecedent contains *u* and consequent is *v*. Represent *G* with an adjacency matrix *A* (numpy `int8` shape *n×n*).  
4. **Proof normalization** – Apply cut‑elimination by iteratively removing redundant edges: for each pair *u→w*, *w→v* replace with *u→v* if the consequent *v* is already reachable; continue until fixpoint. The resulting matrix *Â* gives the minimized proof depth *d = sum(Â)*.  
5. **Expected free energy (Active Inference)** – For each literal *i* compute a belief *bᵢ* = sigmoid(⟨w, xᵢ⟩) where *w* are random weights (numpy) and *xᵢ* is a one‑hot vector. Expected free energy *F* = Σᵢ [bᵢ log bᵢ + (1−bᵢ) log(1−bᵢ)] + λ·d (λ balances complexity). Lower *F* predicts better answer.  
6. **Criticality score** – Perturb *b* by flipping each bit with probability ε (ε=0.01) 100 times, recompute satisfaction *s = Â·b* each time, and calculate susceptibility χ = Var(s) (numpy variance). High χ indicates the system is near a critical point; we reward answers with larger χ.  
7. **Final score** – `score = -F + α·χ` (α=0.5). Rank candidates by descending score.

**Structural features parsed**  
Negations, comparatives, conditionals (`if‑then`), causal markers (`because`, `leads to`), ordering relations (`before`, `after`), and explicit numeric values.

**Novelty**  
Proof‑theoretic scoring and active‑inference based surprise have been explored separately; adding a criticality‑driven susceptibility term to jointly optimize free energy and system sensitivity is not present in current literature, making the combination novel.

**Rating**  
Reasoning: 7/10 — captures logical structure and uncertainty but relies on linear approximations.  
Metacognition: 6/10 — evaluates surprise and sensitivity, yet lacks explicit self‑monitoring of belief updates.  
Hypothesis generation: 5/10 — derives hypotheses from clause chaining but does not propose alternative parses.  
...
**Forge Timestamp**: 2026-03-26T05:36:32.911313
...
[View code](./Active_Inference---Criticality---Proof_Theory/tool.py)
