# Mechanism Design + Multi-Armed Bandits + Metamorphic Testing

**Fields**: Economics, Game Theory, Software Engineering
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T10:56:51.973744
...
**Algorithm: Bandit‑Guided Metamorphic Consistency Scoring (BG‑MCS)**  

1. **Data structures**  
   - `answers`: list of dicts, each with keys `text`, `features` (numpy array), `pulls` (int), `value` (float).  
   - `features` is a fixed‑length vector extracted by regex:  
     * `neg` – count of negation tokens (`not`, `no`, `n’t`).  
     * `cmp` – count of comparative/superlative adjectives (`more`, `less`, `-er`, `-est`).  
     * `cond` – count of conditional markers (`if`, `unless`, `provided that`).  
     * `num` – sum of all numeric constants found.  
     * `cau` – count of causal cue words (`because`, `therefore`, `leads to`).  
     * `ord` – count of ordering relations (`greater than`, `less than`, `before`, `after`, `first`, `last`).  
   - `metamorphic_relations`: list of lambda functions that map an input transformation to an expected output change (e.g., double the numeric values → output should also double; swapping two comparatives should invert the truth value).  

2. **Operations per evaluation round**  
   - For each answer `a` not yet evaluated, compute a **consistency score** `c(a)` = proportion of metamorphic relations satisfied when the answer’s extracted features are transformed according to each relation (using numpy for vectorized arithmetic).  
   - Update the arm’s empirical mean `μ_a = (μ_a * pulls + c(a)) / (pulls + 1)`.  
   - Compute an **exploration bonus** `b_a = sqrt(2 * log(total_pulls) / (pulls + 1))` (UCB1).  
   - The **selection score** is `s_a = μ_a + b_a`.  
   - Pull the arm with highest `s_a`, increment its `pulls`, and repeat until a budget of evaluations is exhausted.  

3. **Scoring logic**  
   - Final score for each answer = its empirical mean `μ_a` (the estimated probability that it satisfies the metamorphic constraints).  
   - Because the scoring rule rewards higher `μ_a` and is strictly proper (answering truthfully maximizes expected score), it inherits incentive‑compatibility from mechanism design.  
   - The bandit component allocates effort to uncertain answers, mimicking explore‑exploit trade‑off.  
   - Metamorphic relations provide the oracle‑free consistency checks.  

4. **Structural features parsed**  
   The regex extractor targets negations, comparatives, conditionals, numeric constants, causal cue words, and ordering relations — exactly the linguistic structures that determine whether a metamorphic relation (e.g., “double all numbers → output doubles”) holds.  

5. **Novelty**  
   While each component appears separately (proper scoring rules, UCB bandits, metamorphic testing), their integration into a single answer‑scoring loop that uses structural parsing for relation checking is not documented in prior work, making the combination novel.  

**Ratings**  
Reasoning: 8/10 — The algorithm directly evaluates logical consistency via metamorphic relations and balances exploration, yielding reasoned scores.  
Metacognition: 7/10 — It monitors its own uncertainty through the UCB bonus, showing basic self‑assessment of answer quality.  
Hypothesis generation: 6/10 — Hypotheses are limited to whether a relation holds; it does not generate alternative explanations beyond consistency checks.  
Implementability: 9/10 — All steps use only numpy and the Python standard library; regex extraction and vectorized operations are straightforward to code.
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=32% cal=20% ncd_acc=42% ncd_cal=46%)

**Forge Timestamp**: 2026-04-01T20:12:41.069113
...
[View code](./Mechanism_Design---Multi-Armed_Bandits---Metamorphic_Testing/tool.py)
