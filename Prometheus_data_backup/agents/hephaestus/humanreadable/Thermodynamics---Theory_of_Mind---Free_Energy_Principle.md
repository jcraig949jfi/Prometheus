# Thermodynamics + Theory of Mind + Free Energy Principle

**Fields**: Physics, Cognitive Science, Theoretical Neuroscience
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-25T15:38:30.241534
**Report Generated**: 2026-03-27T06:37:37.743284
...
**Algorithm**  
We build a lightweight factor‑graph scorer that treats each candidate answer as a hypothesis *h* about the world state described by the prompt.  

1. **Parsing → propositions** – Using only the stdlib `re` module we extract atomic propositions:  
   - Predicates with arguments (`likes(A,B)`)  
   - Comparatives (`score(X) > score(Y)`)  
   - Negations (`¬rain`)  
   - Conditionals (`if studied then pass`)  
   - Causal links (`heat → expand`)  
   - Numeric equalities/inequalities (`temp = 23.5`).  
   Each proposition becomes a node *vᵢ* in a graph; edges connect nodes that share variables or appear in the same clause (e.g., the antecedent and consequent of a conditional).

2. **Factor potentials (Thermodynamics)** – For every node we assign an *energy* *Eᵢ* = −log P₀(vᵢ) where *P₀* is a prior probability derived from a simple frequency table built from a small corpus of commonsense facts (e.g., “water boils at 100 °C”). The joint energy of a configuration **x** (truth assignments to all nodes) is the sum of node energies plus pairwise interaction energies *Jᵢⱼ* that enforce logical constraints:  
   - Modus ponens: if A→B and A true then B must be true → large positive *J* for (A=1,B=0).  
   - Transitivity of > : (X>Y)∧(Y>Z)→(X>Z).  
   - Negation consistency: *J* penalizes (A=1,¬A=1).  
   These potentials are stored in NumPy arrays; the total energy *E(**x**) = ΣᵢEᵢ·xᵢ + ΣᵢⱼJᵢⱼ·xᵢxⱼ*.

3. **Theory of Mind layer** – We maintain a *belief* distribution *Q(**x**|h)* over world states conditioned on the candidate answer *h*. *Q* is initialized as the prior *P₀* and then updated by a single round of mean‑field belief propagation (using NumPy matrix‑vector products) that incorporates the answer as hard evidence: propositions directly asserted by *h* are clamped to 1, their negations to 0.

4. **Free‑Energy scoring (Free Energy Principle)** – The variational free energy for answer *h* is  
   \[
   F(h)=\langle E(\mathbf{x})\rangle_{Q} - \mathcal{H}[Q],
   \]  
   where the expectation and entropy are computed from the marginals obtained by mean‑field (all operations are NumPy dot‑products and log‑sums). Lower *F* indicates that *h* yields a world state that is both low‑energy (thermodynamically plausible) and high‑entropy (unsurprising to a Theory‑of‑Mind model of the questioner). The final score is *−F(h)* (higher = better).

**Structural features parsed** – negations, comparatives (> , < , =), conditionals (if‑then), causal arrows, numeric values and units, ordering relations (transitive chains), and explicit conjunction/disjunction via shared variables.

**Novelty** – The trio has been discussed separately (predictive coding ≈ free‑energy principle, Bayesian Theory‑of‑Mind models, thermodynamic cost of computation). Coupling them into a single factor‑graph free‑energy scorer that uses only numpy and stdlib is not present in existing NLP evaluation tools, making the combination novel for this purpose.

**Rating**  
Reasoning: 7/10 — The algorithm captures logical consistency and numeric constraints, but relies on shallow priors and mean‑field approximations that can miss deep inference.  
Metacognition: 6/10 — Theory‑of‑Mind is modeled via a simple belief update; higher‑order recursive modeling is absent.  
Hypothesis generation: 5/10 — The system scores given candidates; it does not propose new answers beyond the supplied set.  
Implementability: 9/10 — All components are regex extraction, NumPy linear algebra, and iterative updates; no external libraries or GPUs are needed.
...
**Forge Timestamp**: 2026-03-26T06:38:07.489211
...
[View code](./Thermodynamics---Theory_of_Mind---Free_Energy_Principle/tool.py)
