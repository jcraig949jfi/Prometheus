# Criticality + Causal Inference + Property-Based Testing

**Fields**: Complex Systems, Information Science, Software Engineering
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-27T14:21:06.546089
...
**Algorithm**  
1. **Parsing & Graph Construction** – Using regex, extract propositions of the form *P*, *¬P*, *P → Q*, *P because Q*, *X > Y*, *X = Y*, and numeric literals. Each proposition becomes a node in a directed acyclic graph (DAG). Node attributes: `type` (literal, comparative, causal), `value` (bool or float), `uncertainty` (initially 0). Edge attributes: `rel` ∈ {causes, implies, equals, greater‑than, less‑than} and a weight = 1.0 (numpy array `W`).  
2. **Constraint Propagation** – Topologically sort the DAG. For each node in order, compute a provisional truth value:  
   - literals keep their extracted value,  
   - `implies`: `v_Q = max(v_Q, v_P)`,  
   - `causes`: same as implies,  
   - `equals`: enforce `|v_X‑v_Y|≤ε` via averaging,  
   - comparatives: enforce `v_X‑v_Y≥δ` (δ extracted from text).  
   Propagation is a single pass using numpy dot‑product on the adjacency matrix to update a state vector `s`.  
3. **Property‑Based Perturbation Generation** – Treat the node vector `s` as the input to a hypothesis‑style generator. Randomly flip bits (for Boolean nodes) or add Gaussian noise (for numeric nodes) to create a perturbation `Δ`. Evaluate the resulting constraint violations (sum of unsatisfied edges). Use a shrinking loop: halve the perturbation magnitude until violations appear, recording the minimal Hamming/L2 distance `d_min` that flips the overall answer correctness. Repeat `N` times (e.g., N=200) to obtain a set `{d_i}`.  
4. **Criticality‑Based Scoring** – Compute susceptibility `χ = std(scores)/mean(|Δ|)`, where scores are 0/1 correctness after each perturbation. Estimate correlation length `λ` as the average number of nodes whose state changes per perturbation (reachability size from altered nodes, measured via BFS on the DAG). Final score = `χ * λ`, normalized to [0,1] by dividing by the empirical maximum observed over a validation set.  

**Structural Features Parsed**  
Negations (`not`, `¬`), conditionals (`if … then`, `unless`), causal verbs (`because`, `leads to`, `results in`), comparatives (`greater than`, `less than`, `equals`), ordering relations (`before`, `after`, `precedes`), numeric values with units, equality statements, and conjunctive/disjunctive conjunctions (`and`, `or`).  

**Novelty**  
While causal graph extraction and robustness testing exist separately, coupling them with a criticality‑derived susceptibility metric—using property‑based testing to find minimal counterfactual perturbations and measuring systemic sensitivity—has not been applied to answer scoring in NLP. It extends structural causal model testing with a statistical‑physics inspired criticality measure, making the combination novel.  

**Ratings**  
Reasoning: 8/10 — captures logical dependencies and sensitivity to perturbations, but relies on hand‑crafted regex patterns that may miss complex language.  
Metacognition: 6/10 — the algorithm can estimate its own uncertainty via susceptibility, yet lacks explicit self‑reflection on parsing failures.  
Hypothesis generation: 9/10 — property‑based perturbation generation directly implements hypothesis‑style testing with shrinking to minimal counterexamples.  
Implementability: 7/10 — all steps use only numpy and the Python standard library; the main effort lies in robust regex extraction and graph handling.
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=40% cal=39% ncd_acc=42% ncd_cal=46%)

**Forge Timestamp**: 2026-03-27T15:40:49.649530
...
[View code](./Criticality---Causal_Inference---Property-Based_Testing/tool.py)
