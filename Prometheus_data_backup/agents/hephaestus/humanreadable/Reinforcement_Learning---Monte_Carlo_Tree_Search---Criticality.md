# Reinforcement Learning + Monte Carlo Tree Search + Criticality

**Fields**: Computer Science, Computer Science, Complex Systems
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T11:29:36.997964
...
**Algorithm**  
We build a Monte Carlo Tree Search (MCTS) whose nodes encode *partial mappings* between the constraint graph extracted from the prompt and a candidate answer graph.  

*Data structures*  
- **Prompt graph** \(G_p = (V_p, E_p)\): each vertex is an entity or variable; edges carry a relation type from the set {¬, <, >, =, → (causal), ∧, ∨}. Numeric constraints are stored as intervals on vertices.  
- **Answer graph** \(G_a\) is constructed on‑the‑fly for each rollout by filling in missing vertices/edges.  
- **Tree node** stores: (i) a mapping \(M\subseteq V_p\times V_a\) that is currently fixed, (ii) visit count \(n\), (iii) total reward \(w\), (iv) empirical variance \(\sigma^2\) of rollout rewards from this node.  

*Operations*  
1. **Selection** – from the root, recursively pick the child maximizing  
   \[
   UCB = \frac{w}{n} + c\sqrt{\frac{\ln N_{parent}}{n}},\quad 
   c = \sqrt{1+\sigma^2}
   \]  
   The variance‑dependent term implements a *criticality* cue: high uncertainty (large \(\sigma\)) raises the exploration constant, pushing the search toward the “edge of order” where small changes can flip reward outcomes.  
2. **Expansion** – add one new mapping pair (entity‑to‑entity or entity‑to‑value) that is consistent with all hard constraints (e.g., type compatibility, negation).  
3. **Rollout** – randomly complete the remaining unmapped vertices by sampling values that satisfy numeric intervals (using numpy’s random uniform) and randomly choosing relation types that do not violate hard constraints.  
4. **Reward** – for the completed answer graph compute  
   \[
   r = \#\text{satisfied soft constraints} - \lambda \times \#\text{violations}
   \]  
   where soft constraints are comparatives, conditionals, and causal claims extracted from the prompt; \(\lambda\) weights violations.  
5. **Backpropagation** – update \(n, w,\) and \(\sigma^2\) along the path.  

After a fixed budget of simulations, the *score* of a candidate answer is the average reward \(\frac{w}{n}\) at the root node.

**Structural features parsed**  
Negations (“not”, “no”), comparatives (“higher than”, “at most”), conditionals (“if … then …”, “unless”), causal claims (“because”, “leads to”), ordering/temporal relations (“first”, “before”, “after”), numeric values with units, quantifiers (“all”, “some”, “none”), and coreference links (pronouns to nouns).

**Novelty**  
Pure MCTS + RL (e.g., AlphaZero) exists, but the *criticality‑driven adaptive UCB* that modulates exploration using the variance of rollout rewards is not standard in NLP scoring tools. Variance‑based UCB appears in bandit literature (UCB‑Tuned), yet tying it to a criticality interpretation and applying it to structured logical constraint satisfaction for answer scoring is novel.

**Rating**  
Reasoning: 8/10 — captures logical, numeric, and causal structure via constraint satisfaction and backpropagated rewards.  
Metacognition: 7/10 — self‑adjusts exploration based on observed uncertainty, showing rudimentary self‑monitoring.  
Hypothesis generation: 8/10 — MCTS enumerates many completions of partial answers, acting as a generator of candidate hypotheses.  
Implementability: 9/10 — relies only on numpy for sampling and basic arithmetic; all graph operations can be done with dicts/lists from the standard library.
...
**Forge Timestamp**: 2026-04-01T19:13:05.627220
...
[View code](./Reinforcement_Learning---Monte_Carlo_Tree_Search---Criticality/tool.py)
