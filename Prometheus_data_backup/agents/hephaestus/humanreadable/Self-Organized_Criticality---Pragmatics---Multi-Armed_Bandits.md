# Self-Organized Criticality + Pragmatics + Multi-Armed Bandits

**Fields**: Complex Systems, Linguistics, Game Theory
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-26T14:21:42.468981
**Report Generated**: 2026-03-27T16:08:11.213353
...
**Algorithm**  
We treat each candidate answer as a node in a weighted graph G = (V,E). V holds the candidates; an edge (eᵢ,ⱼ) exists when two answers share at least one extracted structural feature (see §2) and its weight is the Jaccard overlap of their feature sets. Each node stores a sand‑pile height hᵢ (initialized to a pragmatic relevance score, see below) and a threshold θ = 1.0.  

1. **Feature extraction (pragmatics layer)** – Using regex we detect:  
   * Negations (`not`, `n’t`)  
   ...
   * Ordering/temporal relations (`before`, `after`, `while`)  
   From these we compute a pragmatic score pᵢ = w_q·Q + w_w·W + w_r·R + w_m·M where Q,W,R,M are binary indicators of adherence to Grice’s maxims of Quantity, Quality, Relation, and Manner (e.g., violating Quantity → 0, otherwise 1).  

2. **Self‑Organized Criticality redistribution** – Set hᵢ←pᵢ. While any hᵢ > θ:  
   * excess e = hᵢ − θ; set hᵢ←θ;  
   * for each neighbor j with weight wᵢⱼ, add e·wᵢⱼ/Σₖwᵢₖ to hⱼ.  
   This avalanche propagates pragmatic relevance across semantically adjacent answers until a stable configuration (no hᵢ > θ) is reached. The final hᵢ is the SOC‑adjusted relevance.  

3. **Multi‑Armed Bandit selection** – Each candidate is an arm. For t = 1…T rounds:  
   * Compute UCBᵢ = \bar{h}_ᵢ + √(2 ln t / nᵢ), where \bar{h}_ᵢ is the mean observed SOC score for arm i and nᵢ its pull count.  
   * Pull arm i* = argmax UCBᵢ, observe reward r = hᵢ (the SOC‑adjusted score), update \bar{h}_ᵢ and nᵢ.  
   After T iterations the arm with highest \bar{h}_ᵢ is returned as the best answer.  

**Structural features parsed** – negations, comparatives/superlatives, conditionals, numeric values/units, causal connectives, ordering/temporal relations. These are captured via regex and turned into binary feature vectors that drive both the pragmatic initialization and the graph adjacency for SOC diffusion.  

**Novelty** – While spreading activation (graph‑based) and bandit‑driven answer selection exist separately, coupling them through a Self‑Organized Criticality avalanche that dynamically redistributes pragmatic scores based on feature overlap is not described in prior literature; the SOC layer adds a non‑linear, threshold‑driven constraint propagation step absent from existing methods.  

**Ratings**  
Reasoning: 8/10 — The algorithm captures logical structure via feature extraction and propagates relevance through a principled SOC process, offering deeper reasoning than pure similarity.  
Metacognition: 6/10 — It monitors uncertainty via bandit exploration but lacks explicit self‑reflection on its own feature extraction errors.  
Hypothesis generation: 7/10 — By exploring low‑confidence arms, it generates alternative answer hypotheses, yet hypothesis space is limited to the initial candidate set.  
Implementability: 9/10 — All components (regex, numpy arrays for heights/weights, simple loops for avalanche and UCB) rely only on numpy and the Python standard library.
...
**Status**: Scrapped
**Reason**: validation:syntax_error: '(' was never closed (line 377)

**Forge Timestamp**: 2026-03-27T16:07:56.920846
...
[View code](./Self-Organized_Criticality---Pragmatics---Multi-Armed_Bandits/tool.py)
