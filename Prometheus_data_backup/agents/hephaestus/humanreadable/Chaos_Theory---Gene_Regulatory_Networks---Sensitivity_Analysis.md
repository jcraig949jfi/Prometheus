# Chaos Theory + Gene Regulatory Networks + Sensitivity Analysis

**Fields**: Physics, Biology, Statistics
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T22:07:28.099571
**Report Generated**: 2026-04-02T11:44:50.526296
...
**Algorithm**  
1. **Parse** the prompt and each candidate answer into a signed, weighted directed graph \(G=(V,E,w,s)\) where \(V\) are entities/concepts, \(E\) are causal/influence relations extracted via regex patterns (see §2). Each edge gets a sign \(s_{ij}\in\{-1,+1\}\) (negation flips sign) and a magnitude \(w_{ij}\in[0,1]\) derived from comparative strength cues (e.g., “much more” → 0.9, “slightly” → 0.2). Numeric values in the text become edge weights or node bias terms.  
2. **State vector** \(x(t)\in\mathbb{R}^{|V|}\) holds the activation of each node at discrete time \(t\). Initialize \(x(0)\) from prompt‑derived evidence (e.g., asserted facts → 1, denied facts → 0, unknown → 0.5).  
3. **Update rule** (discrete‑time GRN‑like dynamics):  
   \[
   x_i(t+1)=\sigma\!\Big(b_i+\sum_{j} s_{ij}w_{ij}\,x_j(t)\Big),
   \]
   where \(\sigma(z)=1/(1+e^{-z})\) is a logistic squashing function and \(b_i\) is a bias from numeric constants.  
4. **Jacobian** \(J(t)=\mathrm{diag}(\sigma'(z))\cdot (S\circ W)\) with \(S\) the sign matrix and \(W\) the weight matrix; \(\circ\) denotes Hadamard product.  
5. **Sensitivity metric**: compute the largest Lyapunov‑type exponent approximation over a short trajectory (e.g., \(T=20\) steps):  
   \[
   \lambda_{\max}\approx\frac{1}{T}\sum_{t=0}^{T-1}\log\bigl\|J(t)v_t\bigr\|,
   \]
   where \(v_t\) is a unit perturbation vector propagated by \(J(t)\). Lower \(\lambda_{\max}\) indicates robust (low‑sensitivity) reasoning.  
6. **Attractor check**: after \(T\) steps, compute distance to a fixed point \(x^*\) (solved by \(x^*=\sigma(b+S\circ W x^*)\) via fixed‑point iteration). Score \(= -\lambda_{\max} - \alpha\|x(T)-x^*\|\) (with \(\alpha=0.5\)). Higher score ⇒ better answer.

**Structural features parsed**  
- Negations → flip edge sign.  
- Comparatives (“more”, “less”) → scale weight magnitude.  
- Conditionals (“if A then B”) → create directed edge A→B.  
- Numeric values → set bias \(b_i\) or weight \(w_{ij}\).  
- Causal claims (“X causes Y”) → edge X→Y.  
- Ordering relations (“X > Y”) → transitive closure adds implied edges with sign + and weight derived from magnitude of difference.

**Novelty**  
Pure logical parsers or constraint propagators exist (e.g., CLIPS, Prolog‑based reasoners). Combining them with a dynamical‑systems sensitivity analysis—using Jacobians, Lyapunov‑exponent estimates, and attractor distance to evaluate textual reasoning—is not present in current NLP evaluation tools, making the approach novel.

**Ratings**  
Reasoning: 8/10 — captures dynamical robustness and attractor consistency beyond pure logic.  
Metacognition: 6/10 — limited self‑monitoring; relies on fixed‑point convergence heuristics.  
Hypothesis generation: 5/10 — generates implicit dynamical hypotheses but does not propose new semantic structures.  
Implementability: 9/10 — uses only numpy for matrix ops and stdlib regex; straightforward to code.
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=43% cal=2% ncd_acc=42% ncd_cal=46%)

**Forge Timestamp**: 2026-04-02T11:39:21.319053
...
[View code](./Chaos_Theory---Gene_Regulatory_Networks---Sensitivity_Analysis/tool.py)
