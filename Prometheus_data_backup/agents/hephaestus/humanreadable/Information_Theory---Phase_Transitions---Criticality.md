# Information Theory + Phase Transitions + Criticality

**Fields**: Mathematics, Physics, Complex Systems
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-02T03:41:35.319469
**Report Generated**: 2026-04-02T08:39:55.038858
...
**Algorithm**  
1. **Parsing** – From each prompt and candidate answer we extract a finite set of atomic propositions *P* = {p₁,…,pₙ} using a handful of regex patterns that capture:  
   *Negation* (`not`, `no`), *comparatives* (`>`, `<`, `≥`, `≤`, `more`, `less`), *conditionals* (`if … then`, `unless`), *causal cues* (`because`, `leads to`, `results in`), *numeric literals*, and *ordering* (`before`, `after`, `first`, `last`). Each proposition stores its polarity, involved variables, and type tag.  
2. **Constraint graph** – Build a directed adjacency matrix **A** ∈ {0,1}ⁿˣⁿ where **A**[i,j]=1 iff pᵢ → pⱼ is explicitly present (e.g., a conditional). Compute the transitive closure **C** = A* (reachability) with repeated squaring using NumPy (`np.linalg.matrix_power` or Boolean Warshall).  
3. **Noisy worlds** – Define a temperature‑like noise parameter T∈[0,1]. For each T, generate M random truth assignments **x**∈{0,1}ⁿ by flipping each proposition independently with probability σ(T)=1/(1+exp(−k·(T−0.5))) (k fixed, e.g., 10).  
4. **Entropy & susceptibility** – For each T, evaluate how many of the closed constraints **C** are satisfied in **x** (a constraint is satisfied if the antecedent is true ⇒ consequent true). Record the fraction s(T) of satisfied constraints over the M samples. Approximate the distribution of s(T) by binning into B bins and compute Shannon entropy H(T)=−∑ p_b log p_b. Numerically differentiate to get susceptibility χ(T)=|dH/dT| via finite differences.  
5. **Critical point** – Locate T* where χ(T) peaks (np.argmax). Do this for the reference answer (derived from the prompt’s gold solution) obtaining T*_ref and for each candidate obtaining T*_cand.  
6. **Score** – Score = 1 − |T*_cand − T*_ref| / 1 (clipped to [0,1]). Higher scores indicate the candidate’s logical structure undergoes a phase‑transition‑like shift at the same noise level as the reference, i.e., it carries comparable information‑critical balance.

**Structural features parsed** – negations, comparatives, conditionals, causal keywords, numeric values, ordering relations (before/after, more/less), and conjunctions that generate implication edges.

**Novelty** – The blend of logical constraint propagation (as in SAT solvers/P-SL) with an information‑theoretic entropy curve and critical‑point detection is not a direct replica of existing work; while probabilistic soft logic and phase‑transition studies in SAT exist, coupling them to compute a susceptibility‑based alignment score for answer evaluation is novel.

**Rating**  
Reasoning: 8/10 — captures logical depth via constraint closure and sensitivity analysis.  
Metacognition: 6/10 — provides a self‑diagnostic (susceptibility peak) but lacks explicit reflection on uncertainty.  
Hypothesis generation: 5/10 — derives hypotheses implicitly from noisy worlds but does not propose new candidates.  
Implementability: 9/10 — relies only on regex, NumPy matrix ops, and basic loops; feasible in <200 lines.
...
**Forge Timestamp**: 2026-04-02T07:11:21.513761
...
[View code](./Information_Theory---Phase_Transitions---Criticality/tool.py)
