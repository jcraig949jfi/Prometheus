# Hebbian Learning + Multi-Armed Bandits + Compositional Semantics

**Fields**: Neuroscience, Game Theory, Philosophy
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T19:45:03.461521
**Report Generated**: 2026-04-02T10:55:58.860198

---

## Nous Analysis

The algorithm treats each prompt as a set of atomic propositions extracted by regex (e.g., “X is taller than Y”, “not P”, “if A then B”). Each proposition is mapped to a fixed‑dimensional basis vector (one‑hot or random orthogonal) stored in a NumPy array **V**. Meaning composition follows Frege’s principle: binary operators (negation, conjunction, comparative, conditional) are represented as learnable weight matrices **Wₙₑg**, **Wₐₙd**, **W_cₒmp**, **W_iₘp**. Initially these are identity matrices; they are updated with a Hebbian rule whenever the operator participates in a parse that receives positive feedback:  

  W ← W + η · (v₁ ⊗ v₂)  

where v₁, v₂ are the input vectors and η a small learning rate.  

Parsing is cast as a multi‑armed bandit problem. Each arm corresponds to choosing one of the operator matrices for a given span of tokens. Arm *i* maintains an estimated reward μᵢ (average similarity of the resulting meaning vector to a held‑out gold answer) and a pull count nᵢ. At each step the algorithm selects the arm with the highest Upper Confidence Bound:  

  a* = argmaxᵢ [ μᵢ + √(2 ln N / nᵢ) ]  

where N is total pulls so far. The chosen operator updates its weight via the Hebbian step, the resulting meaning vector is recomputed, and the bandit statistics are updated with the similarity reward.  

After a fixed number of iterations (or convergence), the final meaning vector **m** for the prompt is obtained. Candidate answers are processed through the same compositional pipeline (using the current weights) to obtain vectors **aⱼ**. Scores are cosine similarities:  

  scoreⱼ = (m·aⱼ) / (‖m‖ ‖aⱼ‖)  

Higher scores indicate better answers.  

**Structural features parsed:** negations, comparatives (“more/less than”), conditionals (“if…then”), causal connectives (“because”, “leads to”), numeric values and units, ordering relations (“greater than”, “first”, “last”), and conjunction/disjunction. Regex captures these patterns; the bandit guides which compositional rule to apply when ambiguities (e.g., scope of negation) exist.  

**Novelty:** While Hebbian learning, bandit‑based exploration, and compositional semantics each appear separately in the literature (e.g., neural Hebbian models, UCB for hyperparameter search, Montague‑style semantics), their tight integration—using bandit decisions to drive Hebbian updates of symbolic composition weights—has not been described in existing work. It bridges symbolic rule learning with online exploration, offering a distinct approach.  

**Ratings**  
Reasoning: 7/10 — captures logical structure and uncertainty but relies on shallow vector similarity.  
Metacognition: 6/10 — bandit provides some self‑monitoring of parse quality, yet limited higher‑order reflection.  
Hypothesis generation: 8/10 — explores multiple parses actively, generating alternative meaning hypotheses.  
Implementability: 9/10 — only NumPy and stdlib needed; regex, basic linear algebra, and simple bandit logic are straightforward.
...
**Forge Timestamp**: 2026-04-02T10:18:08.126848
...
[View code](./Hebbian_Learning---Multi-Armed_Bandits---Compositional_Semantics/tool.py)
