# Compositionality + Free Energy Principle + Type Theory

**Fields**: Linguistics, Theoretical Neuroscience, Logic
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-25T19:40:59.660836
**Report Generated**: 2026-03-27T06:37:39.764707
...
**Algorithm**  
1. **Parsing (Compositionality)** – Use regex‑based token extraction to build a simply‑typed λ‑calculus AST for each sentence. Tokens become typed constants (e.g., `5:Int`, `John:Entity`). Patterns detected:  
   - Negation: `not P` → `Neg(P)`  
   - Comparatives: `X > Y` → `GT(X,Y)`  
   - Conditionals: `if A then B` → `Imp(A,B)`  
   - Causal: `A because B` → `Cause(B,A)`  
   - Ordering: `X before Y` → `Before(X,Y)`  
   Each construct is assigned a function type (e.g., `GT : Int → Int → Bool`). The AST is a tree where the meaning of the whole node is determined by the meanings of its children and the combinatory rule encoded in the node type.

2. **Type Assignment (Type Theory)** – Perform Hindley‑Milner style type inference on the AST using only the standard library (no external solvers). Variables are unified; function application checks that argument types match the expected domain. If a mismatch occurs, record a *type error* = 1, else = 0. Dependent‑like refinements are approximated by attaching numeric ranges to `Int` types (e.g., `Int[0,100]`).

3. **Constraint Propagation (Free Energy Principle)** – Extract Horn‑clause constraints from the AST:  
   - From `Imp(A,B)` derive `A ⊢ B` (modus ponens).  
   - From `GT(X,Y)` and `GT(Y,Z)` derive `GT(X,Z)` (transitivity).  
   - From `Cause(B,A)` derive `B → A` as a directed edge.  
   Propagate forward until a fixed point, storing all derived literals in a set `D`.  

   Define variational free energy for a candidate answer `C` as  
   \[
   F(C)=\sum_{c\in D}\bigl[\,\text{typeError}(c)\bigr]^2
        +\lambda\sum_{v\in\text{numericVars}} \bigl(\text{violation}(v)\bigr)^2,
   \]  
   where `typeError(c)` is 0 if the derived literal respects its inferred type, otherwise 1; `violation(v)` measures how far a numeric value in `C` deviates from the allowed interval inferred from constraints (0 if inside). λ balances symbolic vs. numeric terms (set to 1.0). Lower `F` indicates higher plausibility; score = `-F`.

**Structural features parsed** – negations, comparatives, conditionals, causal claims, numeric values with bounds, ordering relations (before/after, greater/less), conjunction/disjunction (via nested `And/Or` nodes).

**Novelty** – While type‑theoretic parsing and free‑energy scoring appear separately in NLU and cognitive modeling, their direct composition—using typed λ‑calculus as the generative model and minimizing prediction error via constraint propagation—has not been instantiated as a pure‑numpy, rule‑based scorer. Existing tools either rely on statistical similarity or on separate logical reasoners; this hybrid ties syntax‑semantics composition directly to an energy‑based objective.

**Ratings**  
Reasoning: 7/10 — captures logical structure and type consistency but approximates dependent types crudely.  
Metacognition: 5/10 — no explicit self‑monitoring of inference depth; energy reflects only local constraint violations.  
Hypothesis generation: 6/10 — forward chaining yields implied literals, yet no mechanism for proposing novel hypotheses beyond closure.  
Implementability: 8/10 — relies solely on regex, basic unification, and numpy for numeric checks; straightforward to code in <200 lines.
...
**Status**: Scrapped
**Reason**: validation:runtime_error: TypeError: argument of type 'float' is not iterable

**Forge Timestamp**: 2026-03-26T20:47:55.991525
...
[View code](./Compositionality---Free_Energy_Principle---Type_Theory/tool.py)
