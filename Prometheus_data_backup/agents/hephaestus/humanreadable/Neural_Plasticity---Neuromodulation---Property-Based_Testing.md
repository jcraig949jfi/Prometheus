# Neural Plasticity + Neuromodulation + Property-Based Testing

**Fields**: Biology, Neuroscience, Software Engineering
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-02T06:50:06.338438
**Report Generated**: 2026-04-02T10:55:59.211193
...
1. **Parsing** – Using a handful of regex patterns, the extractor yields tuples *(subject, relation, object, polarity, modality)* where polarity ∈ {+1,‑1} encodes negation and modality marks comparatives, conditionals, causal links, numeric thresholds, or ordering (e.g., “>”, “<”, “because”, “if‑then”). Each tuple becomes a node; directed edges encode the relation type (e.g., *causes*, *greater‑than*, *equals*).  
2. **Graph representation** – Nodes are indexed 0…N‑1. An adjacency matrix **A** (N×N, dtype=float32) holds edge weights initialized to 1.0 for present relations, 0 otherwise. A second matrix **S** (same shape) stores a *similarity kernel* that will be updated by plasticity.  
3. **Neuromodulatory gain** – A gain vector **g** (length N) is computed from node features:  
   - base gain = 1.0  
   - +0.2 for causal or conditional modality  
   - +0.1 for numeric comparisons  
   - –0.1 for pure negations (to reduce confidence in denied statements)  
   The effective weight matrix is **W** = **A** ⊙ (**g** **g**ᵀ) (⊙ = element‑wise product). This implements gain control analogous to dopaminergic modulation of synaptic efficacy.  
4. **Constraint propagation** – Repeatedly apply transitivity and modus ponens via Boolean‑style matrix multiplication: **W** ← **W** ∨ (**W** @ **W**) (where @ is numpy dot, ∨ is logical OR treated as max). After convergence (≤5 iterations or no change), **W** encodes all derivable relations.  
5. **Plasticity update** – For a candidate answer, compute its propagated matrix **Wc**. Compare to the reference matrix **Wr** (from the gold answer) using the Frobenius norm: *e* = ‖**Wc** − **Wr**‖_F. Treat *e* as an error signal; update a plasticity trace **P** ← α **P** + (1‑α) **e** **I** (α=0.9, **I** identity) to accumulate experience‑based confidence.  
6. **Property‑based testing & shrinking** – Generate random perturbations of the candidate graph (node flips, edge additions/removals) using a simple Hypothesis‑style strategy. For each perturbed graph compute *e*. Keep the perturbation with minimal *e* that still fails a threshold τ; iteratively shrink it by removing changes until *e* rises above τ. The final *e* after shrinking is the score (lower = better).  

**Structural features parsed** – negations, comparatives (“more than”, “less than”), conditionals (“if … then …”), causal claims (“because”, “leads to”), numeric values with thresholds, and ordering relations (“before”, “after”, “greater than”).  

**Novelty** – While each component exists separately (graph‑based logical reasoning, neuromodulatory gain akin to attention, property‑based testing for test generation), their tight coupling—using gain‑modulated constraint propagation as a differentiable‑like error signal fed into a plasticity trace and then driving hypothesis‑based shrinking—has not been reported in the literature.  

**Ratings**  
Reasoning: 7/10 — captures logical structure and derives implicit relations, but limited to first‑order patterns.  
Metacognition: 6/10 — plasticity trace offers a rudimentary confidence monitor, yet no explicit self‑reflection on strategy.  
Hypothesis generation: 8/10 — property‑based style mutation + shrinking efficiently finds minimal counter‑examples.  
Implementability: 7/10 — relies only on regex, NumPy matrix ops, and simple loops; feasible within constraints.
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=34% cal=8% ncd_acc=42% ncd_cal=46%)

**Forge Timestamp**: 2026-04-02T10:29:32.207776
...
[View code](./Neural_Plasticity---Neuromodulation---Property-Based_Testing/tool.py)
