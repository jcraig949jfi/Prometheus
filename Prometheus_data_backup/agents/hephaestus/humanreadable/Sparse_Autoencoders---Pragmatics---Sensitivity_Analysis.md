# Sparse Autoencoders + Pragmatics + Sensitivity Analysis

**Fields**: Computer Science, Linguistics, Statistics
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-25T23:47:17.236373
**Report Generated**: 2026-03-27T06:37:41.338543
...
**Algorithm:**  
1. **Parse → Logical‑Form Extraction** – Using only regex and the stdlib `re` module, each prompt and candidate answer is converted into a list of atomic propositions. The parser extracts:  
   - Negations (`not`, `no`)  
   ...
   Each proposition is stored as a tuple `(predicate, args, polarity)` where polarity ∈ {+1,‑1} encodes negation.

2. **Sparse Dictionary Learning (SAE‑style)** – From a small development set of correct explanations, we build a dictionary **D** ∈ ℝ^{m×k} (m = number of distinct predicate‑arg patterns, k = latent features) using an iterative hard‑thresholding algorithm:  
   - Initialize D with random unit vectors.  
   - For each proposition vector **x** (one‑hot over m patterns), solve **z = argmin‖x−Dz‖₂² + λ‖z‖₁** via a few iterations of coordinate descent (numpy only).  
   - Update D via gradient step on the reconstruction error, then re‑normalize columns.  
   The result is a set of k sparse latent features that disentangle pragmatic aspects (e.g., implicature strength, speech‑act type) from pure semantics.

3. **Sensitivity Scoring** – For a candidate answer, we compute its proposition set **{x_i}**, encode each to sparse code **z_i**, and aggregate to **z̄ = mean_i z_i**.  
   - Perturb each input proposition by flipping its polarity or numeric value (±ε) and recompute **z̄′**.  
   - Approximate the Jacobian **J ≈ (z̄′−z̄)/ε** using numpy array operations.  
   - Define sensitivity **S = ‖J‖_F** (Frobenius norm). Lower S indicates the answer’s meaning is robust to small perturbations.  
   - Final score = α·(1−S) + β·sparsity(z̄) where sparsity = ‖z̄‖₀ / k (higher is better). α,β are fixed weights (e.g., 0.7,0.3).

**Structural features parsed:** negations, comparatives, conditionals, numeric values & units, causal markers, temporal/ordering relations, speech‑act indicators (e.g., “please”, “I suggest”), and quantifier scopes.

**Novelty:** While sparse autoencoders, pragmatics‑aware representations, and sensitivity analysis each appear separately, fusing them into a deterministic, numpy‑only pipeline that learns a disentangled dictionary of pragmatic‑semantic features and then scores answers by robustness to input perturbations is not present in existing work. Prior approaches either use bag‑of‑words similarity or neural encoders; this method stays fully algorithmic and transparent.

**Ratings:**  
Reasoning: 8/10 — captures logical structure and robustness, but relies on hand‑crafted regex which may miss complex syntax.  
Metacognition: 6/10 — the method can estimate its own uncertainty via sensitivity, yet lacks explicit self‑reflection on parsing failures.  
Hypothesis generation: 5/10 — generates implicit hypotheses via sparse codes, but does not propose alternative explanations explicitly.  
Implementability: 9/10 — all steps use only numpy and stdlib; no external libraries or training data beyond a small set for dictionary learning.
...
**Forge Timestamp**: 2026-03-26T12:02:37.286627
...
[View code](./Sparse_Autoencoders---Pragmatics---Sensitivity_Analysis/tool.py)
