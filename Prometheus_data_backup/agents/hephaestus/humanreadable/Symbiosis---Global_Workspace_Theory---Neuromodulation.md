# Symbiosis + Global Workspace Theory + Neuromodulation

**Fields**: Biology, Cognitive Science, Neuroscience
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-25T17:27:55.802899
**Report Generated**: 2026-03-27T06:37:38.526301
...
**Algorithm**  
1. **Parsing & Node Creation** – Use regex to extract atomic propositions from the prompt and each candidate answer. Each proposition becomes a node with fields:  
   - `id` (int)  
   - `type` ∈ {FACT, NEGATION, COMPARATIVE, CONDITIONAL, CAUSAL, NUMERIC, ORDER}  
   - `polarity` (+1 for affirmative, -1 for negated)  
   - `value` (float for numerics, otherwise None)  
   - `args` (list of connected node ids)  
   Store nodes in a NumPy structured array `nodes`.  

2. **Symbiosis‑based Initial Activation** – Compute a mutual‑benefit matrix `M` where `M[i,j] = exp(-‖fi‑fj‖²/σ²)` and `fi` is a TF‑IDF vector of the lexical content of node *i*. This captures long‑term inter‑species interaction: nodes that share useful context give each other benefit. Set initial activation `a0 = M·ones` (numpy dot product).  

3. **Global Workspace Ignition** – Maintain a global activation vector `g` (size = #nodes). At each iteration:  
   - Compute raw input `r = W·a` where `W` is the adjacency matrix derived from `args` (weight = 1 for supportive links, -1 for negations).  
   - Compute neuromodulatory gain `γ = 1 / (1 + exp(-β·U))`, where `U` is the node‑wise uncertainty entropy of outgoing edge signs (higher entropy → lower gain). `β` is a fixed scalar.  
   - Update activation: `a ← γ·r + (1-γ)·a0` (numpy operations).  
   - Nodes with `a > θ` (threshold) are “ignited” and their activation is broadcast to all others via `W` in the next step (global workspace).  

4. **Constraint Propagation** – After ignition, apply deterministic rules:  
   - Modus ponens: if a CONDITIONAL node *c* (if p → q) and its antecedent *p* are ignited, set *q*’s activation to max(current, aₚ).  
   - Transitivity for ORDER and COMPARATIVE edges.  
   - Numeric consistency: if a NUMERIC node states *x > y* and another states *y ≥ z*, enforce *x > z*.  

5. **Scoring** – After a fixed number of iterations (e.g., 10) or convergence, the score for a candidate answer is the average activation of its constituent nodes. Higher scores indicate better alignment with the prompt’s logical and quantitative structure.

**Structural Features Parsed**  
- Negations (“not”, “no”, “never”) → polarity flip.  
- Comparatives (“more than”, “less than”, “>”, “<”) → COMPARATIVE type with direction.  
...
- Numeric values and units → NUMERIC nodes with magnitude.  
- Ordering relations (“first”, “second”, “before”, “after”, ordinals) → ORDER nodes.

**Novelty**  
Pure spreading‑activation or bag‑of‑words scorers exist (e.g., baseline IR), and semantic‑network reasoners implement modus ponens. The novelty lies in coupling mutualistic symbiosis‑derived baseline activation with a neuromodulatory gain control that dynamically gates global broadcast, producing a context‑sensitive ignition threshold not seen in prior QA scoring methods.

**Rating**  
Reasoning: 8/10 — captures logical, comparative, and numeric structure via constraint propagation.  
Metacognition: 6/10 — provides uncertainty‑based gain but lacks explicit self‑monitoring of answer quality.  
Hypothesis generation: 7/10 — activation spreading yields alternative ignited nodes usable as candidate hypotheses.  
Implementability: 9/10 — relies only on NumPy regex and standard‑library data structures; no external models needed.
...
**Forge Timestamp**: 2026-03-27T05:09:24.766703
...
[View code](./Symbiosis---Global_Workspace_Theory---Neuromodulation/tool.py)
