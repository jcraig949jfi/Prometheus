# Analogical Reasoning + Mechanism Design + Free Energy Principle

**Fields**: Cognitive Science, Economics, Theoretical Neuroscience
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-27T13:05:56.112494
**Report Generated**: 2026-03-27T16:08:16.458669
...
**Algorithm**  
1. **Proposition extraction** – Using a handful of regex patterns we parse the prompt and each candidate answer into a set of atomic propositions. Each proposition is a tuple `(type, polarity, args)` where `type ∈ {CAUSE, COMPARE, COND, EQUAL, ORDER}` and `polarity ∈ {+1,‑1}` captures negation. Args are either entity strings or numeric values (converted to float).  
2. **Graph construction** – Propositions become nodes in a directed, labeled multigraph `G = (V, E)`. An edge `e = (u → v, r)` exists when two propositions share an argument; the label `r` records the relational role (e.g., *subject*, *object*, *threshold*). Node features are a one‑hot vector for `type` plus a normalized numeric value (if present); missing values are zero‑padded.  
3. **Analogical similarity** – We compute a relaxed graph‑matching score using the Hungarian algorithm on node cost matrix `C_ij = ‖f_i – g_j‖₂` (numpy). The optimal assignment yields node match `M_node = 1 – (sum C_ij / n)`. Edge match `M_edge` is the Jaccard similarity of edge‑label multisets after aligning nodes via the assignment. Overall structural similarity `S = α·M_node + (1‑α)·M_edge` (α=0.5).  
4. **Free‑energy‑principle scoring** – Treat `S` as the log‑likelihood of the candidate under a reference model: `log p ≈ S`. Variational free energy `F = –log p + H`, where `H` is the entropy of a uniform candidate set (constant). Thus `F = –S + const`. Lower `F` indicates better prediction error minimization.  
5. **Mechanism‑design incentive** – To make the scoring rule truthful (incentive‑compatible) we apply a proper Brier‑style score: `score = –(F – 𝔼[F])²`, where 𝔼[F] is the expected free energy over all candidates (pre‑computed). Higher scores reward answers that both structurally align and reduce surprise.  

All steps use only `numpy` for linear algebra and the Python `re`/`stdlib` for parsing; no external models or APIs are required.

**Structural features parsed**  
- Negations (via polarity flag)  
- Comparatives (`>`, `<`, `≈`, “more than”, “less than”) → `COMPARE` edges  
- Conditionals (`if … then …`) → `COND` edges  
- Causal verbs (“cause”, “lead to”, “results in”) → `CAUSE` edges  
- Numeric values and units → node feature  
- Ordering relations (“first”, “last”, “before”, “after”) → `ORDER` edges  

**Novelty**  
Graph‑based analogical mapping (e.g., Structure Mapping Engine) and proper scoring rules are known, but coupling them with a free‑energy‑principle objective to derive an incentive‑compatible scoring function for textual reasoning answers has not been reported in the literature. The combination is therefore novel.

**Rating**  
Reasoning: 8/10 — captures relational structure and prediction error, though scalability to large texts remains untested.  
Metacognition: 6/10 — the algorithm can estimate its own uncertainty via free energy but lacks explicit self‑reflection loops.  
Hypothesis generation: 5/10 — focuses on matching existing propositions; generating new hypotheses would require additional generative components.  
Implementability: 9/10 — relies solely on regex, numpy linear algebra, and the Hungarian algorithm (available via `scipy.optimize.linear_sum_assignment` or a simple numpy implementation), all feasible in a self‑contained class.
...
**Forge Timestamp**: 2026-03-27T14:53:26.324230
...
[View code](./Analogical_Reasoning---Mechanism_Design---Free_Energy_Principle/tool.py)
