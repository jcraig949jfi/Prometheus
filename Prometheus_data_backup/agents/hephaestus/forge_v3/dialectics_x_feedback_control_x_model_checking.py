...
class ReasoningTool:
    """Counterexample-Guided Dialectical Synthesis v3. PID feedback loop
    + CEGAR model checking + 14 general category parsers."""

    def __init__(self):
        self.kp, self.ki = 0.6, 0.1
        self._integral = 0.0
    ...
    def _pid(self, prompt, cand):
        e = 1.0 - (1.0 - self._ncd(prompt, cand))
        self._integral += e * 0.01
        return max(0.0, min(1.0, 1.0 - self.kp * e - self.ki * self._integral))

    def evaluate(self, prompt, candidates):
        self._integral = 0.0; res = []
        for c in candidates:
            ss = self._structural_score(prompt, c)
            if ss >= 0:
                ncd = self._ncd(prompt, c)
                score = ss * 0.80 + (1.0 - ncd) * 0.10 + self._pid(prompt, c) * 0.10
            else:
                score = self._pid(prompt, c)
