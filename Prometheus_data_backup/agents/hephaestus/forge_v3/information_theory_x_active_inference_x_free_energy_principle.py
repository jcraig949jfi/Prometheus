...
class ReasoningTool:
    """IBAI v3: Information-Bottleneck Active Inference. Structural >70%, fallback <=15%."""
    def __init__(self):
        self.dim = 64

    def _to_latent(self, text: str) -> List[float]:
        h = hashlib.sha256(text.encode('utf-8')).hexdigest()
        return [((int(h[i % len(h)], 16) / 15.0) * 2) - 1.0 for i in range(self.dim)]
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        for i, cand in enumerate(candidates):
            ...
            cos = self._cos_sim(p_latent, c_latent)

            if "fallback:ncd" in tag:
                final = 0.5 + 0.15 * cos
                reasoning = f"fallback:ncd cos_sim={cos:.4f}"
            else:
                final = 0.5 + 0.40 * math.tanh(s_score)
                final += 0.05 * cos
                reasoning = f"{tag} raw={s_score:.2f}, cos={cos:.4f}"
