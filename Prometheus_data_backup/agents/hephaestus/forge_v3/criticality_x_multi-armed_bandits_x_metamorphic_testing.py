"""Critical Bandit Metamorphic v3.
Criticality x Multi-Armed Bandits x Metamorphic Testing.
Criticality weighting + UCB exploration + metamorphic consistency, NCD<=15%.
"""
import re, zlib, math
from typing import List, Dict

class ReasoningTool:
    """CBM v3: criticality + bandit selection + metamorphic testing + full trap coverage."""
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        for c in candidates:
            ...
            reward = ss if ss >= 0 else 0.5
            pulls.append(reward)
            sc = reward * 0.85 + (1.0-ncd) * 0.15
            results.append({"candidate":c,"score":max(0.0,min(1.0,sc)),"reasoning":f"ss={ss:.2f} ucb={reward:.3f}"})
        if len(pulls) > 1:
            var = sum((x-sum(pulls)/len(pulls))**2 for x in pulls)/len(pulls)
            for r in results: r["score"] = r["score"] / (1.0 + var)
