"""Compressed Differential Falsification v3.
Compressed Sensing x Differentiable Programming x Falsificationism.
Sparse recovery + gradient-like update + falsification, NCD<=15%.
"""
import re, zlib, math
...
class ReasoningTool:
    """CDF v3: compressed sensing + differentiable scoring + falsification + full trap coverage."""
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        for c in candidates:
            ...
            active = max(ss, 0.0) if ss >= 0 else 0.5
            sparse_sc = active * (1.0 if active > 0.5 else 0.8)
            sc = sparse_sc * 0.85 + (1.0-ncd) * 0.15
            results.append({"candidate":c,"score":max(0.0,min(1.0,sc)),"reasoning":f"ss={ss:.2f} sp={sparse_sc:.3f}"})
