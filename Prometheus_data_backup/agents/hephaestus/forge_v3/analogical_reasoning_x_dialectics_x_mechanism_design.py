"""Dialectical Analogy-Driven Incentive-Compatible Hypothesis Tester v3.
Analogical Reasoning x Dialectics x Mechanism Design.
...
class ReasoningTool:
    """DADI-HT v3: structural parsers + dialectical-analogy secondary."""
    ...
    def _dialectical(self, prompt: str, cand: str) -> float:
        """Thesis-antithesis alignment: penalise negation mismatch."""
    ...
    def _score(self, prompt: str, cand: str) -> tuple:
        ...
        dial = self._dialectical(prompt, cand)

        if tags:
            raw = 0.70 * ((struct_score + 1.0) / 2.0) + 0.15 * ncd_sim + 0.15 * dial
        else:
            raw = 0.50 * ncd_sim + 0.50 * dial
    ...
    def evaluate(self, prompt: str, candidates: list[str]) -> list[dict]:
        ...
        for cand in candidates:
            ...
            results.append({
                "candidate": cand,
                "score": float(s),
                "reasoning": "; ".join(tags) if tags else "fallback:ncd+dial"
            })
