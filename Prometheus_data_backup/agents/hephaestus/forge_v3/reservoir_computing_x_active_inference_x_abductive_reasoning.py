"""Reservoir Active-Inference Abductive Engine v3.
Reservoir Computing x Active Inference x Abductive Reasoning.
...
class ReasoningTool:
    """RAIAE v3: structural parsers + reservoir-abduction secondary."""

    def __init__(self):
        self._level = 6
        np.random.seed(42)
        self._W = np.random.randn(24, 16) * 0.25
    ...
    def _reservoir_abd(self, prompt: str, cand: str) -> float:
        """Reservoir state + abductive best-explanation heuristic."""
        def _v(s):
            v = np.zeros(16)
            for i, ch in enumerate(s.encode()[:48]):
                v[i % 16] += ch / 255.0
            return np.tanh(self._W @ v)
        pv, cv = _v(prompt), _v(cand)
        dot = np.dot(pv, cv)
        n = np.linalg.norm(pv) * np.linalg.norm(cv)
        sim = (dot / n + 1.0) / 2.0 if n > 0 else 0.5
        # Abductive bonus: shorter explanations preferred (Occam)
        len_r = min(len(cand), len(prompt)) / (max(len(cand), len(prompt)) + 1)
        return 0.6 * sim + 0.4 * len_r

    def _score(self, prompt: str, cand: str) -> tuple:
        struct_score, tags = run_all_parsers(prompt, cand)
        ncd_sim = 1.0 - self._ncd(prompt, cand)
        res = self._reservoir_abd(prompt, cand)
    ...
    def evaluate(self, prompt: str, candidates: list[str]) -> list[dict]:
        ...
        for cand in candidates:
            ...
            results.append({
                "candidate": cand,
                "score": float(s),
                "reasoning": "; ".join(tags) if tags else "fallback:ncd+res_abd"
            })
