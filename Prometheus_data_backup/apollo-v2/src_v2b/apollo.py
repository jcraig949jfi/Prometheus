...
from selection import nsga3_select as nsga2_select, select_elites
...
def load_config(path: str = None) -> dict:
    ...
    # Support both top-level config and nested under "apollo" key
...
def run_apollo(smoke_test: bool = False, config_path: str = None):
    ...
    generation = 0
    ablation_cache = {}
    monitor = StagnationMonitor(window=50)
    ...
    # Also check nested paths config
    ...
    while generation < max_gen:
        ...
        if compiled_count < len(children):
            failed = len(children) - compiled_count
            log_info(
                f"Compilation: {compiled_count}/{len(children)} survived ({failed} killed)",
        ...
        if (not is_warmup and generation >= 200
            ...
            if ncd_killed > 0:
                log_info(
                    ...
                    stage="ncd_filter", generation=generation,
                )
        ...
        # ── Stagnation monitor ────────────────────────────────────
        monitor.update(selected_fitness, archive.size,
                       accepted_count=compiled_count,
                       total_count=config.get('offspring_per_generation', 50))

        if generation % 5 == 0:
            alerts = monitor.check_alerts(generation)
            for alert in alerts:
        ...
        if pool is not None and generation % 50 == 0 and not is_warmup:
            # Deposit verified atoms from load-bearing organisms
            try:
                for org, fv, source in zip(population[:5], selected_fitness[:5], compiled_sources[:5]):
                ...
                log_warning(f"Pool deposit failed: {e}", stage="pool", generation=generation)
            ...
            try:
                ...
                log_warning(f"Leaderboard update failed: {e}", stage="pool", generation=generation)

        # Withdraw foreign atoms every 100 gens
        if pool is not None and generation % 100 == 0 and generation >= 200:
            try:
                ...
                log_warning(f"Pool withdrawal failed: {e}", stage="pool", generation=generation)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--island-id", type=str, default=None, help="Override island ID")
    ...
    run_apollo(smoke_test=args.smoke_test, config_path=args.config)
