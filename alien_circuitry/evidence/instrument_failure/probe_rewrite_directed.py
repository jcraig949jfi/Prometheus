...
def build(L, relators, introduce=True):
...
def analyse(name, L, relators, introduce=True):
    t0 = time.time()
    states, idx, succ, nominal, rules = build(L, relators, introduce)
    NS = len(states)
    ...
    # trap transitions over ALL targets: (s,u,t) with D[s][t]<INF and D[u][t]==INF
    traps = 0; live_edges = 0; trap_alive = 0; comp = {}
    for s in range(NS):
        for u in succ[s]:
            for j in range(T):
                if D[s][j] < INF:
                    ...
                    if D[u][j] == INF:
                        ...
                        if succ[u]:
                            ...
                            if u not in comp:
                                ...
                                comp[u] = len(seen)
    res["trap_transitions_fraction_of_live_(s,a,t)"] = traps / max(1, live_edges)
    res["trap_count"] = traps
    res["trap_successors_that_still_have_legal_actions_fraction"] = trap_alive / max(1, traps)
    if comp:
        cs = sorted(comp.values())
        res["reachable_region_size_after_live_looking_trap_median_max"] = [cs[len(cs) // 2], cs[-1]]
...
if __name__ == "__main__":
    ...
    out = []
    AB = [("xy", "yx"), ("xY", "Yx"), ("Xy", "yX"), ("XY", "YX")]
    BR = [("xyx", "yxy"), ("XYX", "YXY")]
    out.append(analyse("ABELIAN_directed", L, AB, introduce=False))
    out.append(analyse("BRAID_B3_directed", L, BR, introduce=False))
    print(json.dumps(out, indent=1))
    json.dump(out, open(f"rewrite_directed_results_L{L}.json", "w"), indent=1)
