...
n = int(sys.argv[1]); k = int(sys.argv[2]); random.seed(0)
NS = n ** n
...
res = {"n": n, "k": k, "states": NS, "targets": T, "reachability_equals_kernel_criterion": bool(np.array_equal(crit, D >= 0)),
...
cand = np.argwhere(D >= 4); sel = cand[np.random.default_rng(0).choice(len(cand), 400, replace=False)]
...
print(json.dumps(res, indent=1)); json.dump(res, open(f"probe_tn_n{n}_k{k}.json", "w"), indent=1)
