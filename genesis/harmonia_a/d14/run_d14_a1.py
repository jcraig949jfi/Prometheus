...
Writes results/a1_sites.jsonl (per measured site) and
results/a1_children.jsonl (every mutate outcome). Verdict logic
...
PARENT_SEEDS = list(range(910001, 910101))      # 100 parents
K_MUT = 64                                       # mutation seeds 1..64
...
def main():
    ...
    for pi, pseed in enumerate(PARENT_SEEDS):
        ...
        if len(handcheck) < 5:
            handcheck.append(dict(
                artifact=pid,
                match=pid.endswith(hashlib.sha256(gp).hexdigest())))
        # parent hashes
    ...
    with (OUT / "a1_sites.jsonl").open("w") as f:
        for r in site_rows:
            f.write(json.dumps(r, sort_keys=True) + "\n")
    with (OUT / "a1_children.jsonl").open("w") as f:
        for r in child_rows:
            f.write(json.dumps(r, sort_keys=True) + "\n")
    json.dump(dict(controls=controls, n_calls=CALLS, cases=cases,
                   task_ids=task_ids, task_combined=task_comb,
                   wall_s=round(time.time() - t_start, 1)),
              open(OUT / "a1_run_meta.json", "w"), indent=1)
    journal({"kind": "a1_measurement_complete",
