...
def main():
    ...
    kids = [json.loads(l) for l in open("results/a2_children.jsonl")]
    meta = json.load(open("results/a2_run_meta.json"))
    ...
    support_ok = (len(parents) >= 100 and n >= 1000
                  and gates["varies"]
                  and True  # A2 R1: multi-diff = expected exclusion class of default sampler
    ...
    report = dict(
        ...
        verdict=verdict)
    json.dump(report, open("results/analysis_d14_a2.json", "w"),
              indent=1)
