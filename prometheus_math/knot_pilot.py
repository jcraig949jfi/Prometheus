"""prometheus_math.knot_pilot -- 3-algorithm pilot for KnotTraceFieldEnv.

Mirrors ``modular_form_pilot.py`` and ``_run_bsd_rank_pilot.py``: runs
random / REINFORCE-linear / PPO-MLP across multiple seeds and reports
both training-split mean reward AND a held-out test-split evaluation.

Total budget per the task spec: 5K episodes x 3 seeds x 3 algorithms =
45K episodes. Plus a held-out test eval of ~50 knots * 20 reps =
1000 episodes per seed per trained agent (the corpus is small, so we
sweep with replacement to drive variance down).

Outputs:
    prometheus_math/_knot_pilot.json  (full numerics)
    stdout summary suitable for KNOT_TRACE_FIELD_RESULTS.md.
"""
...
def _eval_test_split_random(test_corpus, n_episodes: int, seed: int) -> float:
    """Random baseline on held-out test set."""
    from prometheus_math.knot_trace_field_env import KnotTraceFieldEnv, N_CLASSES
    env = KnotTraceFieldEnv(corpus=test_corpus, split="all", seed=seed)
...
def _eval_test_split_reinforce(test_corpus, W, b, n_episodes: int, seed: int) -> float:
    """Deterministic argmax evaluation of trained REINFORCE policy."""
...
def _eval_test_split_ppo(test_corpus, W1, b1, Wp, bp, n_episodes: int, seed: int) -> float:
    """Deterministic argmax evaluation of trained PPO policy."""
    from prometheus_math.knot_trace_field_env import KnotTraceFieldEnv
    env = KnotTraceFieldEnv(corpus=test_corpus, split="all", seed=seed)
    rng = np.random.default_rng(seed)
    rs = []
...
def run_pilot(
    *,
    ...
    out_path: Path = Path("prometheus_math") / "_knot_pilot.json",
) -> Dict[str, Any]:
    ...
    if not ok:
        print(f"knot trace field corpus unavailable: {reason}", file=sys.stderr)
        return {"error": reason}

    print("Loading knot trace-field corpus...")
    corpus = _knot_trace_field_corpus.load_knot_trace_field_corpus(
        include_non_hyperbolic=False,
    )
    summary = _knot_trace_field_corpus.corpus_summary(corpus)
    source = _knot_trace_field_corpus.last_load_source()
    print(f"Corpus source: {source}")
    print(f"Corpus summary: {summary}")

    train, test = _knot_trace_field_corpus.split_train_test(
        corpus, train_frac=train_frac, seed=42,
    )
    ...
    for s in range(n_seeds):
        ...
        # Random.
        env = KnotTraceFieldEnv(corpus=train, split="all", seed=seed)
        t0 = time.time()
        ...
        env = KnotTraceFieldEnv(corpus=train, split="all", seed=seed)
        t0 = time.time()
        l_out = train_reinforce(env, n_episodes=n_episodes, lr=0.02, seed=seed)
        env.close()
        ...
        env = KnotTraceFieldEnv(corpus=train, split="all", seed=seed)
        t0 = time.time()
        p_out = train_ppo(env, n_episodes=n_episodes, lr=0.005, hidden=32, seed=seed)
        env.close()
        ...
        # Held-out test eval.
        t_n = max(test_n_episodes, max(200, len(test) * 4))
        rand_t = _eval_test_split_random(test, n_episodes=t_n, seed=seed + 100000)
