"""prometheus_math.oeis_sleeping_pilot -- 3-algorithm pilot harness for
the OEIS Sleeping Beauty next-term env.

Mirrors ``modular_form_pilot.py`` but with FOUR arms:
    random         -- uniform bin sampling baseline
    growth         -- heuristic: extrapolate by mean log-ratio
    reinforce      -- linear policy, contextual bandit
    ppo            -- 1-hidden-layer MLP, value head

The "growth" arm is the spec's "predict the same growth class as the
prior terms" heuristic baseline. We run it alongside the three RL
algorithms because the random baseline alone (~2% accuracy on a
50-bin action space) is too weak to provide a meaningful comparison
target for under-trained policies.

Total compute budget (per task spec): 5K episodes x 3 seeds x 3
algorithms = 45K episodes for the RL arms, plus the deterministic
random / growth-baseline arms (also at 5K x 3).

Outputs:
    prometheus_math/_oeis_sleeping_pilot.json   (full numerics)
    stdout summary suitable for OEIS_SLEEPING_RESULTS.md.
"""
...
def _eval_test_random(test_corpus, n_episodes: int, seed: int) -> float:
    from prometheus_math.oeis_sleeping_env import OeisSleepingEnv
    env = OeisSleepingEnv(corpus=test_corpus, split="all", seed=seed)
...
def _eval_test_growth(test_corpus, n_episodes: int, seed: int) -> float:
    from prometheus_math.oeis_sleeping_env import (
        OeisSleepingEnv, train_growth_baseline,
    )
    env = OeisSleepingEnv(corpus=test_corpus, split="all", seed=seed)
    rep = train_growth_baseline(env, n_episodes=n_episodes, seed=seed)
    env.close()
    return float(rep["mean_reward"])


def _eval_test_reinforce(test_corpus, W, b, n_episodes: int, seed: int) -> float:
    from prometheus_math.oeis_sleeping_env import OeisSleepingEnv
    env = OeisSleepingEnv(corpus=test_corpus, split="all", seed=seed)
    rng = np.random.default_rng(seed)
    rs = []
...
def _eval_test_ppo(test_corpus, W1, b1, Wp, bp, n_episodes: int, seed: int) -> float:
    from prometheus_math.oeis_sleeping_env import OeisSleepingEnv
    env = OeisSleepingEnv(corpus=test_corpus, split="all", seed=seed)
    rng = np.random.default_rng(seed)
    rs = []
...
def run_pilot(
    *,
    ...
    out_path: Path = Path("prometheus_math") / "_oeis_sleeping_pilot.json",
) -> Dict[str, Any]:
    ...
    if not ok:
        print(f"OEIS sleeping corpus unavailable: {reason}", file=sys.stderr)
        return {"error": reason}

    print(f"Loading corpus (n_total={n_total})...")
    corpus = _oeis_sleeping_corpus.load_oeis_sleeping_corpus(
        n_total=n_total, seed=42,
    )
    summary = _oeis_sleeping_corpus.corpus_summary(corpus)
    source = _oeis_sleeping_corpus.last_load_source()
    ...
    # The held-out test set should be ~50 sequences per spec; clamp it.
    if len(test) > 50:
        test = test[:50]
    ...
    for s in range(n_seeds):
        ...
        # Random.
        env = OeisSleepingEnv(corpus=train, split="all", seed=seed)
        t0 = time.time()
        ...
        env = OeisSleepingEnv(corpus=train, split="all", seed=seed)
        t0 = time.time()
        g_out = train_growth_baseline(env, n_episodes=n_episodes, seed=seed)
        env.close()
        growth_means.append(g_out["mean_reward"])
        pred_count_history["growth"].append(g_out["pred_counts"])
        print(f"  growth   : mean={g_out['mean_reward']:.2f}  "
              f"acc={g_out['accuracy']:.3f}  ({time.time()-t0:.1f}s)")
        ...
        # Test eval.
        t_n = max(200, min(test_n_episodes, len(test) * 20))
        rand_t = _eval_test_random(test, n_episodes=t_n, seed=seed + 100000)
        grow_t = _eval_test_growth(test, n_episodes=t_n, seed=seed + 150000)
        lrn_t = _eval_test_reinforce(
            test,
            W=l_out["policy_W_final"], b=l_out["policy_b_final"],
            n_episodes=t_n, seed=seed + 200000,
        )
        ppo_t = _eval_test_ppo(
            test,
            W1=p_out["policy_W1_final"], b1=p_out["policy_b1_final"],
            Wp=p_out["policy_Wp_final"], bp=p_out["policy_bp_final"],
            n_episodes=t_n, seed=seed + 300000,
        )
        test_random.append(rand_t)
        test_growth.append(grow_t)
        test_reinforce.append(lrn_t)
        test_ppo.append(ppo_t)
        print(f"  TEST eval: random={rand_t:.2f}  growth={grow_t:.2f}  "
              f"REINFORCE(argmax)={lrn_t:.2f}  PPO(argmax)={ppo_t:.2f}")

    rand_arr = np.asarray(random_means)
    grow_arr = np.asarray(growth_means)
    lrn_arr = np.asarray(reinforce_means)
    ppo_arr = np.asarray(ppo_means)
    t_rand_arr = np.asarray(test_random)
    t_grow_arr = np.asarray(test_growth)
    t_lrn_arr = np.asarray(test_reinforce)
    t_ppo_arr = np.asarray(test_ppo)

    rand_mean = float(rand_arr.mean())
    grow_mean = float(grow_arr.mean())
    lrn_mean = float(lrn_arr.mean())
    ppo_mean = float(ppo_arr.mean())

    p_grow_vs_rand = _welch_one_sided(grow_arr, rand_arr)
    p_lrn_vs_rand = _welch_one_sided(lrn_arr, rand_arr)
    p_ppo_vs_rand = _welch_one_sided(ppo_arr, rand_arr)
    p_grow_vs_lrn = _welch_one_sided(grow_arr, lrn_arr)
    p_test_grow_vs_rand = _welch_one_sided(t_grow_arr, t_rand_arr)
    ...
    print(f"TRAIN: growth    mean={grow_mean:.2f}  per-seed={grow_arr.tolist()}")
    ...
    print(f"  growth    vs random: lift "
          f"{(grow_mean - rand_mean) / max(abs(rand_mean), 1e-9):+.3f}x  "
          f"p={p_grow_vs_rand:.4g}")
    ...
    print(f"TEST : growth       mean={float(t_grow_arr.mean()):.2f}  "
          f"p_vs_random={p_test_grow_vs_rand:.4g}")
