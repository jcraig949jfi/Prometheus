"""prometheus_math.genus2_pilot -- 3-algorithm pilot for Genus2Env.

Mirrors ``modular_form_pilot.py`` but on the genus-2 corpus. Runs three
algorithms (random, REINFORCE-linear, PPO-MLP) across multiple seeds
and reports both training-split mean reward AND a held-out test-split
evaluation.

Total budget per the spec: 5K episodes x 3 seeds x 3 algorithms = 45K
episodes. Held-out test eval = 200 curves per seed per trained agent.

Outputs:
    prometheus_math/_genus2_pilot.json  (full numerics)
    stdout summary suitable for GENUS2_RESULTS.md.
"""
...
def _eval_test_random(test_corpus, n_episodes: int, seed: int) -> float:
    ...
    env = Genus2Env(corpus=test_corpus, split="all", seed=seed)
...
def run_pilot(
    *,
    ...
    n_total: int = 2000,
    cond_max: int = 50000,
    test_n_episodes: int = 200,
    out_path: Path = Path("prometheus_math") / "_genus2_pilot.json",
) -> Dict[str, Any]:
    ...
    if not ok:
        print(f"genus2 corpus unavailable: {reason}", file=sys.stderr)
        return {"error": reason}

    print(f"Loading corpus (cond_max={cond_max}, n_total={n_total})...")
    corpus = _genus2_corpus.load_genus2_corpus(
        cond_max=cond_max, n_total=n_total, seed=42,
    )
    summary = _genus2_corpus.corpus_summary(corpus)
    source = _genus2_corpus.last_load_source()
    ...
    for s in range(n_seeds):
        ...
        env = Genus2Env(corpus=train, split="all", seed=seed)
        t0 = time.time()
        ...
        env = Genus2Env(corpus=train, split="all", seed=seed)
        t0 = time.time()
        l_out = train_reinforce(env, n_episodes=n_episodes, lr=0.05, seed=seed)
        env.close()
    ...
    if rand_mean > 0:
        print(f"  REINFORCE vs random: lift "
              f"{(lrn_mean - rand_mean) / rand_mean:+.3f}x  "
              f"p={p_lrn_vs_rand:.4g}")
        print(f"  PPO vs random      : lift "
              f"{(ppo_mean - rand_mean) / rand_mean:+.3f}x  "
