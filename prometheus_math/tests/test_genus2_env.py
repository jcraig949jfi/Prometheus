...
def test_telemetry_step_emits_elapsed_and_oracle_calls(small_env):
    ...
    assert "true_rank_class" in info, (
        "regression: pre-existing key true_rank_class lost"
    )
...
def test_step_info_existing_keys_still_present(small_env):
    ...
    _, _, _, _, info = env.step(0)
    for key in (
        "label", "iso_class", "conductor", "abs_disc",
        "true_rank_class", "true_analytic_rank", "predicted_class",
        "hit", "running_accuracy", "elapsed_seconds", "oracle_calls",
    ):
