"""prometheus_math.modular_form_pilot -- 3-algorithm pilot for ModularFormEnv.

Mirrors ``_run_bsd_rank_pilot.py`` but runs three algorithms (random,
REINFORCE-linear, PPO-MLP) across multiple seeds and reports both the
training-split mean reward AND a held-out test-split evaluation.

Total budget per the task spec: 5K episodes x 3 seeds x 3 algorithms =
45K episodes. Plus a held-out test eval of ~1K episodes per seed per
trained agent.

Outputs:
    prometheus_math/_modular_form_pilot.json  (full numerics)
    stdout summary suitable for MODULAR_FORM_RESULTS.md.
"""
...
def _eval_test_split_random(
    ...
    env = ModularFormEnv(corpus=test_corpus, split="all", seed=seed)
...
def _eval_test_split_reinforce(
    test_corpus,
    W,
    b,
    n_episodes: int,
...
def run_pilot(
    *,
    ...
    out_path: Path = Path("prometheus_math") / "_modular_form_pilot.json",
) -> Dict[str, Any]:
    ...
    if not ok:
        print(f"modular-form corpus unavailable: {reason}", file=sys.stderr)
        return {"error": reason}

    print(f"Loading corpus (level_max={level_max}, n_total={n_total})...")
    corpus = _modular_form_corpus.load_modular_form_corpus(
        level_max=level_max, n_total=n_total, seed=42,
    )
    summary = _modular_form_corpus.corpus_summary(corpus)
    source = _modular_form_corpus.last_load_source()
    ...
    for s in range(n_seeds):
        ...
        # Random.
        env = ModularFormEnv(corpus=train, split="all", seed=seed)
        t0 = time.time()
        ...
        lrn_t = _eval_test_split_reinforce(
            test,
            W=l_out["policy_W_final"],
            b=l_out["policy_b_final"],
            n_episodes=t_n,
            seed=seed + 200000,
        )
        ppo_t = _eval_test_split_ppo(
            test,
            W1=p_out["policy_W1_final"],
            b1=p_out["policy_b1_final"],
            Wp=p_out["policy_Wp_final"],
            bp=p_out["policy_bp_final"],
            n_episodes=t_n,
            seed=seed + 300000,
        )
