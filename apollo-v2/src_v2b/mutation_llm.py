...
class LLMMutator:
    ...
    def _log_call(self, mutation_type, prompt_len, output_len, elapsed, success):
        """Log details of an LLM call."""
        log_debug(
            f"LLM call: {mutation_type} | {prompt_len} tok in | {output_len} tok out | {elapsed:.1f}s | {'ok' if success else 'fail'}",
            stage="llm",
            data={
...
Return ONLY the modified router code body (no function def, no explanation):"""
        ...
        success = code is not None and self._validate_router_code(code)
        self._log_call("mutate_route", len(prompt), len(output) if output else 0, elapsed, success)
...
No explanation, just the JSON:"""
        ...
        self._log_call("mutate_wiring", len(prompt), len(output) if output else 0, elapsed, success)
        if match:
...
Return ONLY the primitive name (e.g. "bayesian_update"), nothing else:"""
        ...
        for prim in ALL_PRIMITIVES:
            if prim in output:
                result = prim
                break
        self._log_call("swap_primitive", len(prompt), len(output) if output else 0, elapsed, result is not None)
        return result
...
Return ONLY the router code body (no function def):"""
        ...
        code = self._extract_code_body(output)
        success = code is not None and self._validate_router_code(code)
        self._log_call("combine_organisms", len(prompt), len(output) if output else 0, elapsed, success)
        if success:
            return code
        return None
