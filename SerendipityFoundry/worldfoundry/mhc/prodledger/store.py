...
ROOT = "0" * 64
ANCHOR_EVERY = 8
# H1/H2 repair: risk-bearing events are anchored SYNCHRONOUSLY before the
# caller is acknowledged. A truncation that erases an acknowledged risk
# event therefore leaves the external anchor pointing at an unreachable
# head, and restart refuses. What remains rollback-exposed is only the
# suffix of NON-risk events since the last anchor, and none of those can
# refund a purchase, lift a halt, or unconsume a block.
RISK_EVENTS = frozenset({"CANDIDATE_REGISTER", "EVIDENCE_BLOCK",
                         "ADMISSION", "HALT_SET", "HALT_LIFT",
...
class HeadAnchor:
    ...
    def latest(self):
        allof = self.all_anchors()
        return allof[-1] if allof else None
...
class LedgerStore:
    def __init__(self, path: str, anchor: HeadAnchor):
        ...
        try:
            ...
            self.close()          # a refused start must not leak the writer
            raise                 # lock or any open file handle
    ...
    def _replay(self):
        ...
        if os.path.exists(self.path):
            ...
            self.head = prev
        # rollback/fork defense: EVERY anchored head must be reachable,
        # not merely the latest (a replaced suffix re-anchored later would
        # otherwise launder the fork -- red-team fork-and-anchor finding)
        for a in self.anchor.all_anchors():
            if a["head"] not in self.known_heads:
                raise StoreError(
                    "ROLLBACK/FORK DETECTED: externally anchored head "
                    f"{a['head'][:12]}... (n={a['n']}) is not an ancestor "
                    "of this log. Refusing to start.")
    ...
    def append(self, event: dict) -> str:
        ...
        if (event.get("type") in RISK_EVENTS
                or self.state.n_events % ANCHOR_EVERY == 0):
            # H1/H2 repair: risk-bearing events are anchored BEFORE the
            # caller is acknowledged -- truncating one afterwards makes the
            # anchored head unreachable and restart refuses.
