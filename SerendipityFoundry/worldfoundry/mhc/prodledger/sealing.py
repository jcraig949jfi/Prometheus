...
def role_permutation(commit_hash: str, beacon_value: str, block: int,
                     n_slots: int) -> list:
    """Beacon-uniform permutation of slots (Fisher-Yates).

    DEFECT RECORD (caught by rankqual exact enumeration, preserved in
    failed_versions/): the v1.0 implementation advanced an LCG between
    draws and took j = x % (i+1) -- the low bits of an LCG are nearly
    periodic, so the permutation was measurably NON-uniform (rank-law
    max deviation 0.35 on slot-structured substrates), silently breaking
    Lemma 1's exactness in the production path. REPAIR: every Fisher-Yates
    draw is an independent domain-separated hash; rejection sampling
    removes the (already negligible) modulo bias exactly."""
    perm = list(range(n_slots))
    for i in range(n_slots - 1, 0, -1):
        ctr = 0
        while True:
            x = derive(commit_hash, beacon_value,
                       f"perm-{block}-{i}-{ctr}", 0)
            lim = (1 << 64) - ((1 << 64) % (i + 1))
            if x < lim:                      # exact rejection sampling
                break
            ctr += 1
        j = x % (i + 1)
