...
def guard_existence(world, n_states=400):
    """wC: does an exact guard exist as a disjunction of <= 2 atoms over the executable
    probe language, atoms of form comp_j(W(s)) == c or comp_j(W(s)) < c,
    W in {eps, r00..r03, (r01,r02)}?  Probe failure => atom false.

    Search is exact and tractable because for "<" atoms only the MAXIMAL c with no false
    positive matters (smaller c gives a subset mask), and for "==" atoms only values
    absent from all success states qualify."""
        ...
        for j in range(world.k):
            ...
            for s in states:
                ...
                for p in w:
                    ...
                    if v is None:
                        break
                feats.append(None if v is None else v[j])
            succ_vals = [f for i, f in enumerate(feats)
                         if f is not None and not (fail_mask >> i) & 1]
            # maximal threshold atom with zero false positives
            if succ_vals:
                c_max = min(succ_vals)
                mask = 0
                for i, f in enumerate(feats):
                    if f is not None and f < c_max:
                        mask |= 1 << i
                if mask:
                    atoms.append(((list(w), j, "<", c_max), mask))
            # equality atoms on values never seen in success states
            succ_set = set(succ_vals)
            fail_vals = {f for i, f in enumerate(feats)
                         if f is not None and (fail_mask >> i) & 1} - succ_set
            for c in sorted(fail_vals):
                mask = 0
                for i, f in enumerate(feats):
                    if f == c:
                        mask |= 1 << i
                atoms.append(((list(w), j, "==", c), mask))
...
def main():
    selfcheck(6, 997), selfcheck(7, 673), selfcheck(8, 809)
    t_start = time.time()
    worlds = make_worlds()
    report = {"census": "v2", "date": "2026-08-26", "m_word": list(M_WORD)}
    keep = {}
