"""census_meta_v4.py — FIRST ACTION, fifth iteration.

meta_v3 REJECTED dD at DROP 85 / SPURIOUS 8 (W3 ratio 1.405): the backward root is
always meetable, so meet-search degrades to forward-plus-waste instead of failing,
and frontier balancing bounds the waste share. A recorded sweep showed spurious
VOLUME is the lever: at SPURIOUS 24 the frontier-balancing reference pays ~2.1x and
alternating variants collapse (28x, solve-loss). dD => DROP 85 / SPURIOUS 24.

The first full 5-seed operator_genesis run FAILED D_HARM (pooled 1.42 vs gate 1.5)
and D_DETECT (anomaly rate 28.6% in two seeds vs gate 30%): dD at DROP 70 / SPURIOUS
6 was a marginal trap, and the learner's frontier-balancing operator variant
partially self-corrects. dD moved to DROP 85 / SPURIOUS 8. World change => the
census gates the world again before the experiment reruns.
...
def main():
    t0 = time.time()
    doms = make_domains()
    rep = {"census": "meta_v4", "date": "2026-08-26",
    ...
    out = os.path.join(os.path.dirname(HERE), "results", "census_meta_v4.json")
