"""census_meta_v0.py — FIRST ACTION for Incubation v2 (Operator Genesis).
...
def main():
    t0 = time.time()
    doms = make_domains()
    rep = {"census": "meta_v0", "date": "2026-08-26",
    ...
    for wid in ("dA", "dB", "dC"):
        ...
        a3_deep = [run_program(dom, t, A3, BUDGET) for t, _o in deep]
    ...
    d_a3 = [run_program(dD, t, A3, BUDGET) for t, _o in dtasks]
    ...
    v = {
        ...
        "W3_trap": (w["dD"]["a0_solved"] == w["dD"]["n"]
                    ...
                    and w["dD"]["a3_runs_with_verify_failures"]
                    >= 0.9 * w["dD"]["n"]
                    and verify_fail_clean == 0),
    ...
    out = os.path.join(os.path.dirname(HERE), "results", "census_meta_v0.json")
