"""census_lens_v0.py — FIRST ACTION for Incubation v3 (Lens Genesis).
...
def main():
    t0 = time.time()
    doms = make_domains()
    rep = {"census": "lens_v0", "date": "2026-08-27", "budget": BUDGET,
    ...
    # ── trap vD ─────────────────────────────────────────────────────────────────
    vD = doms["vD"]
    dtasks = gen(vD, 704, 8, BLOCK_DEPTHS["vD"], decoy_uses=2)
    ...
    spot_solves = sum(
        ...
        if all(run_with_lens(vE, t_, g, R2OP, PROBE_BUDGET)["solved"]
               for t_, _o in etasks[:2]))
    # extension order: assign each pid not covered by the prior lens
    new_pids = [p for p in vE.pids
                if p not in BLOCK1 + BLOCK2 and True]
    ext_rank = None
    ext_n = 0
    from itertools import product
    for assign in product(range(4), repeat=len(new_pids)):
    ...
    v = {
        ...
        "B7_trap": (rep["trap"]["r2_solved"] == rep["trap"]["n"]
                    and rep["trap"]["blind_lens_solved"] == 0
        ...
        "B8_recursion": (rep["recursion"]["two_block_lens_solves"] == 0
                         ...
                         and spot_solves == 0
                         and (ext_rank or 999) <= 50),
        "B9_no_pressure": rep["vW0"]["r2_solved"] == rep["vW0"]["n"],
        "B10_feature_not_lens": all(w["f1_deep_solved"] == 0
                                    for w in worlds.values()),
    }
    rep["verdicts"] = v
    ...
    out = os.path.join(V3, "results", "census_lens_v0.json")
