"""CAMPAIGN X-4 pass 1 — INSTRUMENT ONLY. Multi-split build, one rebuild for many campaigns. Build a benchmark large enough to
...
TARGET_PER_OP = 400                                               # CHANGED, power-derived
SCAN_CAP = TARGET_PER_OP * 2
rng = random.Random(20260825)                                     # CHANGED, disclosed
...
print(f"pairs excluded (X + X-2 + X-3): {len(EXCL)}", flush=True)
...
blind, gcache = {}, {}
def bid_(a):
    if a not in blind: blind[a] = f"M{len(blind):06d}"
    return blind[a]
def gcl(a):
...
bench['meta'] = {'seed': 20260824, 'target_per_op': TARGET_PER_OP, 'scan_cap': SCAN_CAP,
                 ...
                 'dev_count': len(dev_idx), 'frozenA_count': len(froA_idx),
                 'frozenB_count': len(froB_idx),
                 'dev_real_ops': _real(dev_idx), 'frozenA_real_ops': _real(froA_idx),
                 'frozenB_real_ops': _real(froB_idx),
                 'frozen_four_real_operator_pairs': nt_frozen,
                 'underpowered_ops': short,
                 'disjoint_from': ['campaign_X', 'campaign_X2', 'campaign_X3'],
...
(OUT / 'x4_pairs.json').write_text(json.dumps(bench, indent=1), encoding='utf-8')
(OUT / 'x4_split.json').write_text(json.dumps(
    {'dev_indices': sorted(dev_idx), 'frozenA_indices': sorted(froA_idx),
     'frozenB_indices': sorted(froB_idx),
     'note': 'frozenA is the X-4 decision split; frozenB is RESERVED and must not be opened by X-4'},
    indent=1), encoding='utf-8')
(OUT / 'x4_terms.json').write_text(json.dumps(
    {b: seqs[a] for a, b in blind.items()}, indent=1), encoding='utf-8')
(OUT / 'NOT_FOR_RETRIEVAL_x4_blindmap.json').write_text(json.dumps(
