...
Neither quantity reads task relevance: relevance comes only from the generator oracle (memo G2)."""
import numpy as np


def recoverability(arm, A, y, tau, rng, max_n=2000, n_pairs=2000):
    n = len(y)
    if n == 0:
        return dict(HR2=None, R=None, dist=None, n=0)
    ...
    return dict(HR2=hr2, R=float(ok.mean()), dist=dist, n=int(len(idx)))
