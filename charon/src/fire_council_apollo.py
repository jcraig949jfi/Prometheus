"""
Fire Council Prompts — Apollo Adjacent Research
================================================
Sends the Apollo evolutionary GP prompt to ChatGPT, Claude, DeepSeek, and Gemini.
...
GOOGLE_KEY = "AIzaSyA9oQ5AaATN6DGFjjlQaYmYrQWFAZQa2eI"

# Prompt
PROMPT = (ROOT / "docs" / "council_prompt_apollo_adjacent.md").read_text(encoding="utf-8")
...
def call_openai(prompt_text, output_file, label, model="gpt-4.1"):
    ...
    try:
        ...
        out = f"# {label} Council Response — Apollo Adjacent Research\n"
...
def call_claude(prompt_text, output_file, model="claude-sonnet-4-20250514"):
    ...
    try:
        ...
        out = f"# Claude Council Response — Apollo Adjacent Research\n"
        out += f"## Model: {model} | Time: {elapsed:.0f}s\n"
...
def call_deepseek(prompt_text, output_file, model="deepseek-chat"):
    ...
    try:
        ...
        out = f"# DeepSeek Council Response — Apollo Adjacent Research\n"
        out += f"## Model: {model} | Time: {elapsed:.0f}s\n"
        if usage:
            out += f"## Tokens: {usage.prompt_tokens} in / {usage.completion_tokens} out / {usage.total_tokens} total\n"
        out += f"\n## Response\n\n{content}\n"
...
def call_gemini(prompt_text, output_file, model="gemini-2.5-flash"):
    ...
    try:
        ...
        out = f"# Gemini Council Response — Apollo Adjacent Research\n"
        out += f"## Model: {model} | Time: {elapsed:.0f}s\n"
        out += f"\n## Response\n\n{content}\n"
...
def main():
    print("=== Firing Council — Apollo Adjacent Research ===")
    ...
    targets = [
        ("ChatGPT", lambda: call_openai(
            PROMPT,
            REPORT_DIR / f"chatgpt_apollo_adjacent_{TODAY}.md",
            "ChatGPT", "gpt-4.1")),
        ("Claude", lambda: call_claude(
            PROMPT,
            REPORT_DIR / f"claude_apollo_adjacent_{TODAY}.md")),
        ("DeepSeek", lambda: call_deepseek(
            PROMPT,
            REPORT_DIR / f"deepseek_apollo_adjacent_{TODAY}.md")),
        ("Gemini", lambda: call_gemini(
            PROMPT,
            REPORT_DIR / f"gemini_apollo_adjacent_{TODAY}.md")),
    ]
    ...
    print(f"\n=== COMPLETE ===")
    for f in sorted(REPORT_DIR.glob(f"*apollo_adjacent_{TODAY}.md")):
