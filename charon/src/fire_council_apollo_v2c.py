"""
Fire Council Prompts — Apollo v2c Design Review
================================================
Sends the Apollo v2c design document to ChatGPT, Claude, and DeepSeek.
Gemini is handled separately via Deep Research API.
"""
...
# Prompt
PROMPT = (ROOT / "docs" / "council_prompt_apollo_v2c_design.md").read_text(encoding="utf-8")

TODAY = datetime.now().strftime("%Y-%m-%d")
TOPIC = "Apollo v2c Design Review"
...
def call_openai(prompt_text, output_file, label, model="gpt-4.1"):
    ...
    try:
        ...
        out = f"# {label} Council Response — {TOPIC}\n"
...
def call_claude(prompt_text, output_file, model="claude-sonnet-4-20250514"):
    ...
    try:
        ...
        out = f"# Claude Council Response — {TOPIC}\n"
        out += f"## Model: {model} | Time: {elapsed:.0f}s\n"
...
def call_deepseek(prompt_text, output_file, model="deepseek-chat"):
    ...
    try:
        ...
        out = f"# DeepSeek Council Response — {TOPIC}\n"
        out += f"## Model: {model} | Time: {elapsed:.0f}s\n"
        if usage:
            out += f"## Tokens: {usage.prompt_tokens} in / {usage.completion_tokens} out / {usage.total_tokens} total\n"
        out += f"\n## Response\n\n{content}\n"
...
def main():
    print(f"=== Firing Council — {TOPIC} ===")
    print(f"  Prompt length: {len(PROMPT)} chars")
    print(f"  Date: {TODAY}")
    print(f"  Targets: ChatGPT, Claude, DeepSeek (Gemini via Deep Research)")
    print()

    targets = [
        ("ChatGPT", lambda: call_openai(
            PROMPT,
            REPORT_DIR / f"chatgpt_v2c_design_{TODAY}.md",
            "ChatGPT", "gpt-4.1")),
        ("Claude", lambda: call_claude(
            PROMPT,
            REPORT_DIR / f"claude_v2c_design_{TODAY}.md")),
        ("DeepSeek", lambda: call_deepseek(
            PROMPT,
            REPORT_DIR / f"deepseek_v2c_design_{TODAY}.md")),
    ]
    ...
    print(f"\n=== COMPLETE ===")
    for f in sorted(REPORT_DIR.glob(f"*v2c_design_{TODAY}.md")):
