"""Phase-Transition Autopoietic Causal Inference v3.
Phase Transitions x Autopoiesis x Causal Inference.
...
class ReasoningTool:
    """PTACI v3: structural parsers + phase-transition secondary."""
    ...
    def _phase_order(self, prompt: str, cand: str) -> float:
        """Order parameter: transition from disorder (no overlap) to order (full)."""
        pw = set(re.findall(r'\b\w+\b', prompt.lower()))
        cw = set(re.findall(r'\b\w+\b', cand.lower()))
        phi = len(pw & cw) / max(len(pw), 1)
        # Susceptibility peak near phi=0.5 adds bonus
        chi = 4.0 * phi * (1.0 - phi)
        return 0.7 * phi + 0.3 * chi

    def _score(self, prompt: str, cand: str) -> tuple:
        struct_score, tags = run_all_parsers(prompt, cand)
        ncd_sim = 1.0 - self._ncd(prompt, cand)
        phase = self._phase_order(prompt, cand)

        if tags:
            raw = 0.70 * ((struct_score + 1.0) / 2.0) + 0.15 * ncd_sim + 0.15 * phase
        else:
            raw = 0.50 * ncd_sim + 0.50 * phase
    ...
    def evaluate(self, prompt: str, candidates: list[str]) -> list[dict]:
        ...
        for cand in candidates:
            ...
            results.append({
                "candidate": cand,
                "score": float(s),
                "reasoning": "; ".join(tags) if tags else "fallback:ncd+phase"
            })
