"""Pragmatic Type-Guided Evolutionary v3.
Genetic Algorithms x Pragmatics x Type Theory.
Type checking + Gricean pragmatics + evolutionary selection, NCD<=15%.
"""
import re, zlib, math
from typing import List, Dict

class ReasoningTool:
    """PTGEPS v3: type consistency + pragmatic fitness + genetic selection + full trap coverage."""
    ...
    def _prag(self, prompt, cand):
        pw = set(prompt.lower().split()); cw = set(cand.lower().split())
        stops = {'the','is','a','an','and','or','but','in','on','at','to','for','of','it'}
        sp, sc = pw - stops, cw - stops
        rel = len(sp & sc) / max(len(sp), 1)
        ql = 1.0 if len(cand) > 1 else 0.3
        return rel * 0.5 + ql * 0.5
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        for c in candidates:
            ...
            pr = self._prag(prompt, c)
            if ss >= 0: sc = ss * 0.80 + pr * 0.05 + (1.0-ncd) * 0.15
            else: sc = 0.5 * 0.80 + pr * 0.05 + (1.0-ncd) * 0.15
            results.append({"candidate":c,"score":max(0.0,min(1.0,sc)),"reasoning":f"ss={ss:.2f} pr={pr:.2f}"})
