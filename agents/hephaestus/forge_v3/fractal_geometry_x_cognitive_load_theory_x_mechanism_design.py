...
class ReasoningTool:
    """FG-CLT-MD v3: general structural category parsers (14 categories).
    Structural score >= 70%, NCD <= 15%."""

    def __init__(self):
        np.random.seed(7461)
        self._dim = 32; self._W = np.random.randn(self._dim, self._dim) * 0.3
    ...
    def _structural_score(self, prompt, cand):
        ...
        if m2 and len(pn)>=2 and cn:
            want = m2.group(1) in ('larger','greater','bigger'); tgt = max(pn) if want else min(pn)
        ...
        if mt and md:
            total, diff = float(mt.group(1)), float(md.group(1)); cheap=(total-diff)/2.0; trap=total-diff
            if cn:
        ...
        if pm and re.search(r'what\s+(?:place|position)', p):
            ...
            prev={'2nd':'1st','second':'first','3rd':'2nd','third':'second'}; w=prev.get(o)
        ...
        if sp and re.search(r'which|who|what', p):
            ...
            is_gt = rel in ('more','greater','taller','bigger','faster','heavier'); qa=p[sp.end():]
            if re.search(r'larger|bigger|taller|greater|heavier|faster|more', qa): w,l=(subj,obj) if is_gt else (obj,subj)
            elif re.search(r'smaller|shorter|lighter|slower|less|fewer', qa): w,l=(obj,subj) if is_gt else (subj,obj)
            else: w,l=None,None
    ...
    def _embed(self, text):
        ...
        for i,ch in enumerate(text.encode('utf-8')[:256]): v[ch%self._dim]+=1.0/(i+1)
        n=np.linalg.norm(v); return v/(n+1e-9) if n>0 else v
    def _fallback(self, prompt, cand):
        pv,cv = self._embed(prompt),self._embed(cand)
        return float(1.0/(1.0+np.exp(0.5*np.sum((pv-cv)**2)+0.05*np.sum(cv**2)-2.0)))
    def evaluate(self, prompt, candidates):
        if not candidates: return []
        results = []
        for cand in candidates:
            ss,tag = self._structural_score(prompt, cand)
            if ss>=0: ncd=self._ncd(prompt,cand); score=ss*0.80+(1.0-ncd)*0.10+self._fallback(prompt,cand)*0.10
            else: score=self._fallback(prompt, cand)
            results.append({'candidate':cand,'score':max(0.0,min(1.0,score)),'reasoning':f'struct:[{tag}] s={ss:.2f}'})
