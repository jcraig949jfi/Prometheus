"""Thermodynamic Kalman Falsification v3.
Thermodynamics x Kalman Filtering x Falsificationism.
Kalman update + free energy + bold conjecture tests, NCD<=15%.
"""
import re, zlib, math
...
class ReasoningTool:
    """TKF v3: Kalman belief update + thermodynamic scoring + falsification + full trap coverage."""
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        for c in candidates:
            ...
            mu, sig = 0.0, 1.0
            z = ss if ss >= 0 else 0.0; R = 0.5
            K = sig / (sig + R); mu = mu + K * (z - mu); sig = (1 - K) * sig
            sc = mu * 0.85 + (1.0-ncd) * 0.15
            results.append({"candidate":c,"score":max(0.0,min(1.0,sc)),"reasoning":f"ss={ss:.2f} mu={mu:.3f}"})
