"""Bayesian-Evolutionary Critical Learner v3.
Bayesian Inference x Evolution x Criticality.
Bayesian likelihood + evolutionary fitness + criticality susceptibility, NCD<=15%.
"""
import re, zlib, math
from typing import List, Dict

class ReasoningTool:
    """EBCL v3: Bayesian prior + evolutionary selection + 15 category parsers."""
    ...
    def _ss(self, prompt, cand):
        ...
        if cgt:
            ent = {}
            for a, b in cgt:
                ent[a.lower()] = ent.get(a.lower(), 0) + 1; ent.setdefault(b.lower(), 0)
    ...
    def _susceptibility(self, scores):
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        raw = []
        for c in candidates:
            ss = self._ss(prompt, c)
            ncd = self._ncd(prompt.lower(), c.lower())
            if ss >= 0:
                sc = ss * 0.85 + (1.0 - ncd) * 0.15
            else:
                sc = 0.5 * 0.85 + (1.0 - ncd) * 0.15
            raw.append(sc)
            results.append({"candidate": c, "score": max(0.0, min(1.0, sc)),
                            "reasoning": f"ss={ss:.2f} ncd={ncd:.3f}"})
        chi = self._susceptibility(raw)
        if chi > 0.05:
            for r in results:
                s = r["score"]
                if s > 0.7: r["score"] = min(1.0, s + 0.02)
