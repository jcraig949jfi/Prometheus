"""Statistical Predictive Abductive Engine v3.
Statistical Mechanics x Predictive Coding x Abductive Reasoning.
...
class ReasoningTool:
    """SPAE v3: structural parsers + Boltzmann-abductive secondary."""
    ...
    def _boltzmann_abd(self, prompt: str, cand: str) -> float:
        """Boltzmann energy from structural mismatch; abductive length bonus."""
        pw = set(re.findall(r'\b\w+\b', prompt.lower()))
        cw = set(re.findall(r'\b\w+\b', cand.lower()))
        overlap = len(pw & cw) / max(len(pw), 1)
        energy = 1.0 - overlap
        boltz = math.exp(-self._beta * energy)
        # Abductive: prefer concise explanations
        lr = min(len(cand), len(prompt)) / (max(len(cand), len(prompt)) + 1)
        return 0.6 * boltz + 0.4 * lr

    def _score(self, prompt: str, cand: str) -> tuple:
        struct_score, tags = run_all_parsers(prompt, cand)
        ncd_sim = 1.0 - self._ncd(prompt, cand)
        ba = self._boltzmann_abd(prompt, cand)

        if tags:
            raw = 0.70 * ((struct_score + 1.0) / 2.0) + 0.15 * ncd_sim + 0.15 * ba
        else:
            raw = 0.50 * ncd_sim + 0.50 * ba
    ...
    def evaluate(self, prompt: str, candidates: list[str]) -> list[dict]:
        ...
        for cand in candidates:
            ...
            results.append({
                "candidate": cand,
                "score": float(s),
                "reasoning": "; ".join(tags) if tags else "fallback:ncd+boltz"
            })
