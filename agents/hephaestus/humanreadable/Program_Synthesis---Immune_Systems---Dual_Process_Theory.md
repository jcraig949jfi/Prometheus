# Program Synthesis + Immune Systems + Dual Process Theory

**Fields**: Computer Science, Biology, Cognitive Science
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T03:40:39.334996
**Report Generated**: 2026-04-01T20:30:43.519192
...
**Algorithm**  
The tool treats the prompt as an *antigen* A and each candidate answer as a potential *antibody* B.  
1. **Fast (System 1) stage** – Using only the Python `re` module we extract a typed predicate‑graph G from the text:  
   - entities → constants with inferred types (NUM, DATE, ENTITY)  
   - relations → predicates extracted from dependency patterns (e.g., “X > Y” → `gt(X,Y)`, “because” → `cause(X,Y)`, negations → `not(P)`, conditionals → `if(P,Q)`, ordering → `before(X,Y)`).  
   The graph is stored as a NumPy structured array `dtype=[('subj','U20'),('pred','U20'),('obj','U20')]`.  
2. **Slow (System 2) stage** – We synthesize small logical programs P in a DSL (variables, constants, predicates, `and`, `or`, `not`, `if‑then`, arithmetic). Program synthesis is *type‑directed*: the expected answer type (e.g., BOOL, NUM) constrains the DSL grammar.  
3. **Immune‑inspired clonal selection** –  
   - **Initial population**: randomly generate N programs that type‑check against G (using exhaustive enumeration up to depth d).  
   - **Affinity function**: `aff(P) = w1·sat(P,G) – w2·len(P) + w3·nov(P)`, where `sat` is the fraction of hard constraints in G satisfied (computed via forward chaining with NumPy boolean vectors), `len` is program length, and `nov` measures distance from the memory set (Jaccard of predicate sets).  
   - **Selection**: keep top‑k antibodies, clone them, and apply mutation operators (swap predicates, change constants, insert/delete a node).  
   - **Memory**: store any antibody with `aff > τ` in a long‑term set; future queries reuse these high‑affinity programs to seed the population, reducing search.  
4. **Scoring** – For each candidate answer string, we map it to the DSL program that produced it (via a simple lookup table built during synthesis). Its final score is the affinity of the best‑matching program; ties are broken by lower length.

**Parsed structural features** – negations, comparatives (`>`,`<,=`), conditionals (`if…then`), causal cues (`because`, `leads to`), numeric values and units, temporal/ordering expressions (`before`, `after`, `greater than`), conjunctions/disjunctions, and quantified phrases (`all`, `some`).

**Novelty** – While program synthesis, immune‑inspired optimization, and dual‑process reasoning each appear separately (e.g., genetic programming, clonal selection algorithms, fast/slow heuristics in cognitive architectures), their tight integration for answer scoring—using a typed DSL, constraint‑based affinity, and a memory‑guided clonal loop—has not been reported in existing QA or reasoning‑evaluation literature.

**Ratings**  
Reasoning: 8/10 — captures logical structure and constraint propagation well, but relies on hand‑crafted DSL and may miss deep semantic nuances.  
Metacognition: 7/10 — memory of high‑affinity programs provides a rudimentary form of self‑reflection, yet no explicit monitoring of search efficiency.  
Hypothesis generation: 9/10 — clonal selection actively diversifies and refines candidate programs, yielding rich hypothesis exploration.  
Implementability: 8/10 — all components use only `re`, `numpy`, and standard library; the main effort is building the DSL and mutation operators.
...
**Forge Timestamp**: 2026-04-01T19:27:52.585907
...
[View code](./Program_Synthesis---Immune_Systems---Dual_Process_Theory/tool.py)
