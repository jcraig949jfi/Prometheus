# Swarm Intelligence + Free Energy Principle + Sensitivity Analysis

**Fields**: Biology, Theoretical Neuroscience, Statistics
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-28T22:14:36.962212
**Report Generated**: 2026-03-31T16:21:16.491114

---

## Nous Analysis

**Algorithm: Swarm‑Free Sensitivity Scorer (SFSS)**  
The scorer treats each candidate answer as a “particle” moving in a discrete concept‑space defined by extracted logical predicates.  

1. **Parsing & Data Structures**  
   - Tokenise the prompt and each answer with `re.findall` to capture:  
     * atomic propositions (e.g., “X causes Y”, “X > Y”, “not Z”)  
     * comparatives (`>`, `<`, `=`)  
     * conditionals (`if … then …`)  
     * negations (`not`, `no`)  
     * numeric constants.  
   - Build a directed hypergraph `G = (V, E)` where each vertex `v ∈ V` is a grounded proposition (e.g., “temperature ↑”).  
   - Hyperedges encode logical relations: a conditional yields an edge from antecedent set to consequent; a comparative yields a weighted edge whose weight is the absolute difference of the parsed numbers; a negation flips the sign of the vertex’s truth value.  
   - Store vertex truth values in a NumPy array `T ∈ {‑1,0,1}^|V|` (‑1 = false, 0 = unknown, 1 = true).  

2. **Swarm Initialization**  
   - Initialise `N` particles, each with a random perturbation vector `δ ∈ ℝ^|V|` drawn from a zero‑mean Gaussian (σ = 0.1).  
   - Particle position `p_i = T + δ_i` represents a hypothesized world state.  

3. **Free Energy‑Like Energy Function**  
   - Define variational free energy `F(p) = ½‖Ap - b‖² + λ‖p‖₁`, where `A` encodes hard constraints extracted from the prompt (e.g., transitivity of “causes”, modus ponens for conditionals) and `b` is the RHS vector (0 for satisfied constraints).  
   - The quadratic term penalises constraint violations; the L1 term encourages sparsity (few unexplained fluctuations), mirroring prediction‑error minimisation.  

4. **Sensitivity‑Driven Update (Particle Swarm)**  
   - For each iteration:  
     * Compute gradient `∇F(p_i) = Aᵀ(Ap_i - b) + λ·sign(p_i)`.  
     * Update velocity `v_i ← ω·v_i - η·∇F(p_i) + φ₁·r₁·(p_best_i - p_i) + φ₂·r₂·(g_best - p_i)`.  
     * Update position `p_i ← p_i + v_i`, clip to `[-1,1]`.  
   - `ω, η, φ₁, φ₂` are scalar hyper‑parameters; `r₁, r₂ ∼ Uniform(0,1)`.  
   - Track each particle’s personal best `p_best_i` (lowest `F`) and global best `g_best`.  

5. **Scoring**  
   - After `K` iterations, the score for an answer is `S = -F(g_best)`. Lower free energy → higher score.  
   - Normalise scores across candidates to `[0,1]` for ranking.  

**Structural Features Parsed**  
Negations, comparatives, conditionals, numeric values, causal claims (“X leads to Y”), ordering relations (“X before Y”), and conjunctive/disjunctive combinations. The hypergraph captures these as typed edges, enabling constraint propagation (transitivity of causation, arithmetic of comparatives, logical modus ponens).  

**Novelty**  
The triple blend is not found in existing literature: swarm‑based optimisation of a free‑energy‑style energy function over a logically parsed hypergraph, with sensitivity analysis embodied in the gradient‑based velocity update. Prior work uses either pure logical theorem provers, Bayesian active inference, or standard PSO for numeric optimisation, but not the combined symbolic‑numeric‑probabilistic loop described.  

**Potential Ratings**  
Reasoning: 8/10 — captures logical structure and uncertainty via principled energy minimisation.  
Metacognition: 6/10 — monitors particle convergence but lacks explicit self‑reflection on search adequacy.  
Hypothesis generation: 7/10 — swarm explores multiple world states, yielding diverse candidate explanations.  
...
**Forge Timestamp**: 2026-03-31T14:47:55.670315
...
[View code](./Swarm_Intelligence---Free_Energy_Principle---Sensitivity_Analysis/tool.py)
