# Immune Systems + Multi-Armed Bandits + Free Energy Principle

**Fields**: Biology, Game Theory, Theoretical Neuroscience
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-28T09:18:40.593772
**Report Generated**: 2026-03-31T14:34:55.923914
...
**Algorithm**  
1. **Parsing** – The question and each candidate answer are converted into a flat list of propositional tuples `(rel, arg1, arg2, polarity)` using regular expressions that capture: negations (`not`), comparatives (`>`, `<`, `=`), conditionals (`if … then`), causal verbs (`cause`, `lead to`), numeric constants, and ordering relations (`before`, `after`). Each tuple is stored in a NumPy array of shape `(N,4)` where the fourth column is `+1` for affirmative and `-1` for negated propositions.  
2. **Affinity matrix** – For a candidate `c` and the question `q`, compute a similarity score  
   `A_{c} = Σ_i w_{rel_i} * 1_{rel_i^c == rel_i^q} * 1_{polarity_i^c == polarity_i^q}`  
   where `w_{rel}` are fixed weights (higher for causal and conditional relations). This yields a 1‑D NumPy array of affinities.  
3. **Complexity penalty** – `C_c = λ * |c|` where `|c|` is the number of propositions in the candidate and λ is a small constant.  
4. **Free‑energy‑like objective** – Define `F_c = -A_c + C_c`. Lower `F` corresponds to higher predicted accuracy and lower complexity.  
5. **Bandit‑driven clonal selection** – Treat each candidate as an arm. Maintain for each arm `i`: pull count `n_i` and cumulative reward `r_i = Σ_t ( -F_{i,t} )`. At iteration `t`:  
   - Compute Thompson‑sampling sample `θ_i ~ Beta(1+r_i, 1+n_i-r_i)`.  
   - Select arm `i* = argmax θ_i`.  
   - Evaluate its `F_{i*}` (affinity + complexity) and update `n_{i*}` and `r_{i*}`.  
   - If `F_{i*} < τ` (a threshold), clone the arm: increase its pull count by a factor `κ` (simulating proliferation) and add a small random mutation to its propositional set (swap a synonym, flip a polarity).  
6. **Scoring** – After a fixed number of iterations `T`, the final score for candidate `c` is the posterior mean `-F_c` (i.e., `A_c - C_c`). The highest‑scoring answer is returned.

**Structural features parsed** – Negations, comparatives, conditionals, causal claims, numeric values, ordering relations (temporal or magnitude), and explicit equality/inequality statements.

**Novelty** – The procedure fuses three well‑studied mechanisms: (i) clonal selection/mutation from artificial immune systems, (ii) explore‑exploit bandit allocation (Thompson sampling), and (iii) a variational free‑energy objective that balances prediction error against model complexity. While immune‑inspired optimizers and bandit‑guided search appear separately, their joint use with a free‑energy‑style loss for text‑based reasoning has not, to the best of my knowledge, been described in existing literature.

**Ratings**  
Reasoning: 8/10 — The algorithm explicitly evaluates logical affinity and penalizes unnecessary complexity, yielding a principled ranking of answers.  
Metacognition: 7/10 — Bandit sampling provides a built‑in mechanism for monitoring uncertainty and allocating effort, reflecting rudimentary self‑assessment.  
Hypothesis generation: 6/10 — Mutation of propositions creates new candidate hypotheses, though the space is limited to lexical swaps rather than generative synthesis.  
Implementability: 9/10 — All steps rely on regex parsing, NumPy array operations, and simple Beta‑distribution sampling, requiring only the standard library and NumPy.
...
**Forge Timestamp**: 2026-03-31T12:37:54.929115
...
[View code](./Immune_Systems---Multi-Armed_Bandits---Free_Energy_Principle/tool.py)
