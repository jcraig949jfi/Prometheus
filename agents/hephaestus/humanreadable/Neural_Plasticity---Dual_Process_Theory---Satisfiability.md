# Neural Plasticity + Dual Process Theory + Satisfiability

**Fields**: Biology, Cognitive Science, Logic
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T04:55:36.098620
**Report Generated**: 2026-04-02T12:33:29.001389
...
**Algorithm**  
The tool builds a *dynamic constraint‑saturation network* from each prompt‑answer pair. First, a lightweight parser (regex + shallow dependency patterns) extracts atomic propositions \(p_i\) and turns them into literals with polarity (positive/negative) and attaches any numeric or comparative modifiers as attached attributes (e.g., \(p_i\!:\!x>5\)). Each literal becomes a node in a bipartite graph: one side holds *System 1* heuristic weights \(w_i^{(1)}\) (initialised uniformly), the other side holds *System 2* deliberation weights \(w_i^{(2)}\) (initialised to zero). Clauses (e.g., \(p_i \lor \lnot p_j\), \(p_i \rightarrow p_j\)) are stored as rows of a NumPy boolean matrix \(C\) where \(C_{k,i}=1\) if literal \(i\) appears positively in clause \(k\), \(-1\) if negatively, and 0 otherwise.  

Scoring proceeds in two phases:  

1. **Fast (System 1) propagation** – compute a provisional truth vector \(t = \sigma(Cw^{(1)})\) where \(\sigma\) is a hard threshold (0/1) implemented with `np.where`. The heuristic score \(s_1 = \frac{1}{K}\sum_k \mathbb{I}[C_k·t \ge 1]\) measures the fraction of clauses satisfied by the fast weights.  

2. **Slow (System 2) refinement** – treat the unsatisfied clauses as a SAT problem. Using a pure‑Python DPLL‑style backtracking solver that operates on the NumPy clause matrix, we search for an assignment that maximises clause satisfaction. Each flip of a literal during search triggers a Hebbian‑like update: \(w_i^{(1)} \leftarrow w_i^{(1)} + \eta \cdot t_i \cdot (1-t_i)\) (strengthening co‑active literals) and \(w_i^{(2)} \leftarrow w_i^{(2)} + \eta \cdot \Delta s\) where \(\Delta s\) is the change in satisfied‑clause count caused by the flip. After a bounded number of conflicts (or when a fixed point is reached), the final score is \(s = \alpha s_1 + (1-\alpha) s_2\) with \(s_2\) the satisfaction ratio from the System 2 assignment and \(\alpha\in[0,1]\) a metaparameter that itself is updated via a simple reinforcement rule based on historic correctness (mimicking synaptic pruning).  

**Parsed structural features** – negations, comparatives (`>`, `<`, `=`), conditionals (`if … then`), causal cue words (`because`, `leads to`), ordering relations (`before`, `after`), and explicit numeric constants.  

**Novelty** – The combination mirrors recent neuro‑symbolic proposals that pair Hebbian‑style weight adaptation with SAT‑based reasoning, but the explicit dual‑process split (fast heuristic weights vs. slow search‑derived weights) and the use of a pure NumPy/stdlib implementation for online weight updates is not present in existing public tools.  

**Ratings**  
Reasoning: 8/10 — captures logical structure and adapts weights, but limited to propositional depth.  
Metacognition: 7/10 — System 2 search provides self‑monitoring; however, the metaparameter update is rudimentary.  
Hypothesis generation: 6/10 — generates alternative assignments during backtracking, yet lacks generative language modeling.  
Implementability: 9/10 — relies only on NumPy and stdlib; parsing and DPLL are straightforward to code.
...
**Forge Timestamp**: 2026-04-02T12:26:29.184959
...
[View code](./Neural_Plasticity---Dual_Process_Theory---Satisfiability/tool.py)
