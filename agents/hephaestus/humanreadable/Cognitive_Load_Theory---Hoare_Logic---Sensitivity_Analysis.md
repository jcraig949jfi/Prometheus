# Cognitive Load Theory + Hoare Logic + Sensitivity Analysis

**Fields**: Cognitive Science, Formal Methods, Statistics
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T18:51:44.300508
**Report Generated**: 2026-04-02T10:55:58.813199
...
1. **Parsing** – Use regex to extract atomic propositions from a candidate answer:  
   ...
   - Conditionals (`if C then D`) → stored as implication `C ⇒ D`  
   - Causal cues (`because`, `leads to`) → treated as implication.  
   Each proposition gets a unique ID and a type flag (boolean, numeric, relational).  
   Store in a list `props = [{id, type, lhs, op, rhs, polarity}]`.  

2. **Hoare‑style triple construction** – Identify the implicit precondition set `P` (all propositions not appearing in a consequent of an implication) and the postcondition set `Q` (all consequents). The answer itself is the command `C` (the sequence of statements). Represent `P` and `Q` as boolean numpy arrays of length `nProps`.  

3. **Constraint propagation** – Initialize a truth‑value array `T = P.copy()`. Iterate over implications: for each `C ⇒ D`, if `T[C]` is true, set `T[D] = true`. Continue until convergence (O(k·n) where k is number of implications). This yields the derived closure `T*`.  

4. **Cognitive load estimation** –  
   - *Intrinsic load* = `|P|` (number of distinct premises).  
   - *Extraneous load* = count of propositions in `C` that are neither in `P` nor contribute to any derived `Q` (detected by checking if removing a proposition leaves `T*` unchanged).  
   - *Germane load* = number of newly derived propositions in `T* \ P` that match a target goal set `G` (extracted from the question via same regex).  

5. **Sensitivity analysis** – For each numeric proposition, create a perturbed copy by adding ±ε (ε = 1e‑3 · |value|) and recompute `T*`. Compute the variance of the resulting truth‑value vectors across all perturbations using `np.var`. For boolean propositions, flip the value and recompute. The average variance `σ²` measures fragility.  

6. **Score** –  
   ```
   score = w_g * germane - w_e * extraneous - w_i * intrinsic - w_s * σ²
   ```  
   Weights (w_*) are fixed heuristics (e.g., 0.4, 0.2, 0.2, 0.2). Higher score = better reasoning.

**Structural features parsed** – negations, comparatives, conditionals, causal keywords, ordering relations, numeric constants, quantifiers (all/some), and logical connectives (and/or).  

**Novelty** – While Hoare logic, cognitive load metrics, and sensitivity analysis each appear separately in educational‑tech or program‑verification literature, their joint use to score natural‑language reasoning answers is not documented in prior work; the combination yields a unified verification‑plus‑robustness‑plus‑load estimator.

**Rating**  
Reasoning: 8/10 — captures logical derivation and robustness, but depends on heuristic weighting.  
Metacognition: 7/10 — explicit load estimates give insight into answer efficiency, yet limited to surface‑level counts.  
Hypothesis generation: 6/10 — the system can propose missing premises by checking which additions raise germane load, but does not rank alternatives creatively.  
Implementability: 9/10 — relies only on regex, numpy arrays, and fixed‑point iteration; no external libraries or training needed.
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=34% cal=2% ncd_acc=42% ncd_cal=46%)

**Forge Timestamp**: 2026-04-02T10:38:30.926019
...
[View code](./Cognitive_Load_Theory---Hoare_Logic---Sensitivity_Analysis/tool.py)
