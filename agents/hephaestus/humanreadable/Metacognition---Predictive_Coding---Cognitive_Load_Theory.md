# Metacognition + Predictive Coding + Cognitive Load Theory

**Fields**: Cognitive Science, Cognitive Science, Cognitive Science
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T06:39:42.245143
**Report Generated**: 2026-04-02T12:33:29.052390
...
**Algorithm**  
We build a lightweight probabilistic‑logic scorer that treats each candidate answer as a set of extracted propositions \(P=\{p_i\}\).  
1. **Structural parsing** – Using only `re` we extract:  
   - Atomic predicates (noun‑verb‑noun triples)  
   - Negations (`not`, `no`) → polarity flag \(s_i\in\{-1,+1\}\)  
   ...
   - Numeric values → typed literals (int/float)  
   Each proposition becomes a node with fields: `text`, `polarity`, `type` (fact, comparison, implication, causal), `value` (if numeric).  
2. **Prediction‑error layer (Predictive Coding)** – We maintain a hierarchical generative model of expected proposition patterns derived from the question prompt (e.g., expected relation type, expected numeric range). For each node we compute a surprise score:  
   \[
   e_i = \| \phi(p_i) - \hat{\phi}_i \|_2
   \]  
   where \(\phi\) is a simple feature vector (one‑hot for polarity/type, normalized numeric value) and \(\hat{\phi}_i\) is the prediction from the parent node in the hierarchy (question‑derived expectation).  
3. **Metacognitive confidence calibration** – Each node carries a confidence \(c_i\in[0,1]\) initialized from lexical cues (e.g., modal verbs → lower confidence). After error computation we update confidence via a Bayes‑like rule:  
   \[
   c_i' = \frac{c_i}{c_i + \lambda e_i}
   \]  
   with \(\lambda\) a small constant (0.1). The final metacognitive score for the answer is the mean of \(c_i'\).  
4. **Cognitive‑load penalty (Cognitive Load Theory)** – We estimate intrinsic load as the number of distinct predicate types, extraneous load as the count of negations and modal hedges, and germane load as the number of satisfied logical constraints (see below). Load cost:  
   \[
   L = \alpha \cdot \text{type\_count} + \beta \cdot \text{negation\_count} - \gamma \cdot \text{constraints\_satisfied}
   \]  
   (\(\alpha,\beta,\gamma\) tuned to 0.3,0.2,0.5).  
5. **Constraint propagation** – We build a directed graph from implication and causal edges, then run a fixed‑point forward‑chaining algorithm (using only Python lists and `numpy` for matrix‑style reachability) to derive implied facts. Transitivity and modus ponens are applied automatically. A candidate gets a bonus proportional to the fraction of its propositions that are entailed by the propagated graph.  
6. **Final score** –  
   \[
   \text{Score} = w_1 \cdot \text{MeanConfidence} - w_2 \cdot \text{MeanSurprise} - w_3 \cdot L + w_4 \cdot \text{EntailmentRatio}
   \]  
   with weights \(w_1=0.4, w_2=0.3, w_3=0.2, w_4=0.1\). Higher scores indicate better reasoning.

**Parsed structural features** – negations, comparatives, conditionals, causal cues, numeric literals, ordering relations, polarity, and modal hedges.

**Novelty** – The combination mirrors recent neuro‑cognitive models (predictive coding + metacognition) but couples them to a explicit logical‑constraint scorer, which is not present in pure symbolic reasoners (e.g., Prolog) nor in standard NLP metrics (BLEU, ROUGE). It aligns with work on “logic‑driven language models” and “cognitive load‑aware tutoring systems,” yet the specific triple‑layer algorithm is novel.

**Rating**  
Reasoning: 8/10 — captures logical entailment and uncertainty with transparent operations.  
Metacognition: 7/10 — confidence calibration is simple but reflects error‑monitoring.  
Hypothesis generation: 6/10 — focuses on scoring given candidates; hypothesis proposal would need extra modules.  
Implementability: 9/10 — uses only regex, numpy arrays, and basic graph algorithms; no external dependencies.
...
**Forge Timestamp**: 2026-04-02T11:56:11.545295
...
[View code](./Metacognition---Predictive_Coding---Cognitive_Load_Theory/tool.py)
