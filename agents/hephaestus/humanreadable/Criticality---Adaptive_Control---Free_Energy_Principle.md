# Criticality + Adaptive Control + Free Energy Principle

**Fields**: Complex Systems, Control Theory, Theoretical Neuroscience
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-28T01:45:31.605293
**Report Generated**: 2026-03-31T14:34:55.631585
...
**Algorithm**  
We build a lightweight reasoning scorer that treats a candidate answer as a set of beliefs over a parsed logical graph.  

1. **Parsing → Graph**  
   - Use regex‑based patterns to extract propositions and their relations:  
     *Negation* (`not`, `no`), *comparative* (`greater than`, `less than`), *conditional* (`if … then …`), *causal* (`because`, `leads to`), *ordering* (`before`, `after`), *numeric* (integers, floats).  
   - Each proposition becomes a node; each relation a typed directed edge stored in an adjacency matrix **A** (numpy `int8` for edge type, `float64` for weight).  
   - Belief vector **b** (size = #nodes) holds the current probability that each proposition is true (initialized from prior frequencies in the prompt).  

2. **Free‑Energy‑Like Score**  
   - Prediction error **e** = **b** − **b̂**, where **b̂** is the belief vector obtained by propagating constraints through **A** (constraint propagation):  
     *Modus ponens*: if `A[i,j]` encodes “i → j” and **b**[i] > θ, add to **b̂**[j];  
     *Transitivity*: repeatedly apply **b̂** ← **b̂** + α·(**A**·**b̂**) until convergence (α small, numpy dot).  
   - Variational free energy **F** = ½‖e‖²₂ + λ·H(**b**), where H is the Shannon entropy (numpy `log`). The first term is prediction error, the second penalizes complex belief distributions (complexity term).  

3. **Criticality‑Driven Sensitivity**  
   - To capture susceptibility, we compute the variance of **F** under tiny random perturbations of **b** (e.g., flip each bit with probability ε=0.01, renormalize).  
   - Susceptibility χ = Varₚₜᵤᵣ𝑏(**F**) / ε². Near a critical point χ diverges; we use χ as a scaling factor: **score** = **F** · (1 + χ).  

4. **Adaptive Control of λ**  
   - λ (complexity weight) is updated online using a simple self‑tuning rule: λₜ₊₁ = λₜ + η·(‖e‖₂ − τ), where η is a small learning rate and τ a target error. This keeps the system in the regime where prediction error is minimized without over‑fitting.  

**Structural features parsed** – negations, comparatives, conditionals, causal claims, ordering relations, numeric values, quantifiers (via regex groups).  

**Novelty** – While each component (free‑energy minimization, adaptive control, critical susceptibility) appears separately in neuroscience or control literature, their joint use to score textual reasoning via constraint propagation and entropy regularization is not described in existing NLP evaluation tools, making the combination novel.  

**Ratings**  
Reasoning: 8/10 — captures logical consistency and sensitivity to perturbations, but relies on shallow regex parsing.  
Metacognition: 7/10 — adaptive λ provides online self‑regulation, yet no explicit monitoring of internal states.  
Hypothesis generation: 6/10 — the scorer can rank alternatives but does not propose new hypotheses beyond the given candidates.  
Implementability: 9/10 — uses only numpy and std‑lib; all operations are matrix‑vector updates feasible in < 50 ms for typical prompts.
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=37% cal=48% ncd_acc=42% ncd_cal=46%)

**Forge Timestamp**: 2026-03-28T06:42:25.115017
...
[View code](./Criticality---Adaptive_Control---Free_Energy_Principle/tool.py)
