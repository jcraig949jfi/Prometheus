# Neural Plasticity + Free Energy Principle + Type Theory

**Fields**: Biology, Theoretical Neuroscience, Logic
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T05:12:14.816623
**Report Generated**: 2026-04-02T04:20:10.806151
...
**Algorithm**  
1. **Parsing & Typing** – Use regex to extract atomic propositions and logical connectives (¬, ∧, ∨, →, ↔, <, >, =, ≤, ≥, +, −, ×, ÷). Each atom receives a type tag from a simple hierarchy: `Bool`, `Nat`, `Real`, `Event`. Complex expressions are built as typed λ‑terms (e.g., `∀x:Nat. P(x) → Q(x)`).  
2. **Constraint Graph** – Create a directed graph `G = (V, E)` where each node `v ∈ V` holds a proposition and its current belief weight `w_v ∈ [0,1]`. Edges encode logical dependencies:  
   * For a conditional `A → B`, add edge `A → B` with type‑checked compatibility.  
   * For a causal claim `A causes B`, add edge `A → B` labeled *causal*.  
   * For comparatives/ordering, add edges with numeric difference attributes.  
   Store adjacency as a NumPy matrix `W` (weights) and a separate tensor `T` for edge types.  
3. **Prediction & Free Energy** – For each node, compute a prediction `p_v = σ(Σ_u W_{u,v}·w_u)` (σ = logistic). The observed truth `o_v` is 1 if the proposition is satisfied by the extracted facts, else 0. Free energy `F = Σ_v (o_v − p_v)^2 + λ·Σ_v [w_v log w_v + (1−w_v) log(1−w_v)]` (energy + entropy).  
4. **Hebbian‑Like Update & Pruning** – Iterate:  
   * **Hebbian potentiation:** `ΔW_{u,v} = η·(o_u·o_v − W_{u,v})` (increase weight when both pre‑ and post‑nodes are true).  
   * **Decay (synaptic pruning):** `W ← W·(1−α)` then set `W_{i,j}=0` if `W_{i,j}<τ`.  
   * After each sweep, recompute `p_v` and `F`. Stop when `ΔF < ε` or max iterations reached.  
5. **Scoring** – Insert the candidate answer as an additional observed node (fix `o_ans=1` if answer matches extracted facts, else 0). Run the dynamics to convergence; the final free energy `F_ans` measures mismatch. Score = `exp(−F_ans)` (higher is better).  

**Structural Features Parsed** – Negations (`not`, `¬`), conditionals (`if … then`), biconditionals, comparatives (`greater than`, `less than`), numeric constants and arithmetic, causal verbs (`causes`, `leads to`), ordering relations (`before`, `after`), quantifiers (`all`, `some`) via type‑checked dependent patterns.  

**Novelty** – Purely algorithmic tools typically use either symbolic rule chaining or neural embeddings. Combining Hebbian weight dynamics (plasticity), variational free‑energy minimization, and a type‑theoretic constraint graph has not been described in public reasoning‑evaluation literature; thus the combination is novel.  

**Ratings**  
Reasoning: 7/10 — captures logical structure and uncertainty via energy minimization, but limited to first‑order patterns.  
Metacognition: 5/10 — no explicit self‑monitoring of update dynamics beyond energy threshold.  
Hypothesis generation: 6/10 — edge‑weight potentiation can suggest new plausible relations, though not guided by exploratory search.  
Implementability: 8/10 — relies only on regex, NumPy matrix ops, and basic loops; straightforward to code in <200 lines.
...
**Forge Timestamp**: 2026-04-02T01:46:21.236337
...
[View code](./Neural_Plasticity---Free_Energy_Principle---Type_Theory/tool.py)
