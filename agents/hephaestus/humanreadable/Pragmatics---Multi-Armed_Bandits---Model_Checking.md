# Pragmatics + Multi-Armed Bandits + Model Checking

**Fields**: Linguistics, Game Theory, Formal Methods
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-26T15:21:49.821310
**Report Generated**: 2026-03-27T16:08:11.512865
...
**Algorithm**  
We build a three‑stage scorer that treats each candidate answer as an arm in a stochastic bandit.  

1. **Pragmatic parsing** – From the question and any supplied context we extract a set of *pragmatic constraints* using regex‑based pattern matching for:  
   - Negations (`not`, `never`)  
   - Conditionals (`if … then …`, `unless`)  
   - Comparatives (`more than`, `less than`, `equal to`)  
   - Causal markers (`because`, `leads to`, `results in`)  
   - Ordering/temporal terms (`before`, `after`, `while`)  
   Each match yields a propositional atom (e.g., `P: X > Y`) together with a polarity flag and a *speech‑act type* (assertion, question, request). These atoms are stored in a list `props`.  

2. **Model‑checking backend** – We translate `props` into a finite‑state Kripke structure `M` where each state encodes a truth assignment to the atoms. Transitions correspond to permissible pragmatic inferences (e.g., modus ponens from a conditional, transitivity of ordering). The specification `ϕ` is the conjunction of all atoms that must hold for an answer to be *literally* correct, augmented with pragmatic implicatures derived from Grice’s maxims (e.g., if a speaker says “Some of the X are Y”, we add the implicature “Not all X are Y”). Using a simple depth‑first search (no external libraries) we check whether `M ⊨ ϕ`. The result is a binary satisfaction value `sat ∈ {0,1}`.  

3. **Bandit scoring** – Each candidate answer `a_i` is an arm with unknown mean reward μ_i. After model checking we compute an immediate reward  
   `r_i = sat_i * (1 + λ * prag_i)`, where `prag_i` counts how many pragmatic implicatures are satisfied and λ∈[0,1] weights pragmatic relevance. We maintain estimates `μ̂_i` and confidence widths `w_i = sqrt( (2 log t) / n_i )` (UCB1). The arm selected for next evaluation is `argmax_i (μ̂_i + w_i)`. The final score for each answer is its current `μ̂_i`.  

**Structural features parsed** – negations, conditionals, comparatives, causal claims, ordering/temporal relations, quantifiers (“some”, “all”), and polarity shifts.  

**Novelty** – Model checking has been applied to QA, and bandits to active learning, but coupling them with a pragmatic‑constraint extractor that feeds a temporal‑logic verifier into a UCB‑style arm selector is not present in the literature; the trio jointly handles literal correctness, contextual implicature, and efficient exploration of answer space.  

**Ratings**  
Reasoning: 8/10 — The algorithm combines logical verification with a principled exploration‑exploitation loop, yielding scores that reflect both correctness and contextual appropriateness.  
Metacognition: 7/10 — UCB confidence bounds provide an explicit measure of uncertainty about each answer’s quality, enabling the system to reason about its own knowledge gaps.  
Hypothesis generation: 6/10 — While the parser generates hypotheses (propositions) from text, the bandit component does not propose new hypotheses beyond re‑ranking existing candidates.  
Implementability: 9/10 — All components rely only on regex, basic data structures, numpy for numerical ops, and a custom DFS model checker; no external libraries or neural models are needed.
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=48% cal=5% ncd_acc=52% ncd_cal=49%)

**Forge Timestamp**: 2026-03-27T08:45:58.924110
...
[View code](./Pragmatics---Multi-Armed_Bandits---Model_Checking/tool.py)
