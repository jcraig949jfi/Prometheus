# Monte Carlo Tree Search + Theory of Mind + Free Energy Principle

**Fields**: Computer Science, Cognitive Science, Theoretical Neuroscience
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-26T09:58:46.547464
**Report Generated**: 2026-03-27T18:24:01.240872
...
**Algorithm**  
We build a Monte Carlo Tree Search (MCTS) over a propositional graph extracted from the prompt + candidate answer. Each tree node stores:  
* `state` – a dictionary mapping proposition IDs to a 2‑element numpy array `[p_true, p_false]` representing the agent’s belief (self) and a second array for the modeled other’s belief (ToM).  
* `visits` – integer count.  
* `value` – accumulated negative free‑energy (lower free energy → higher value).  

**Selection** – UCB1: `value/visits + C*sqrt(log(parent_visits)/visits)`.  
**Expansion** – generate child nodes by applying a single logical rewrite rule to one proposition (e.g., flip negation, swap antecedent/consequent of a conditional, adjust scope of a quantifier, or perturb a numeric bound). The rewrite is deterministic; the child’s belief arrays are initialized from the parent’s via a small Dirichlet noise (np.random.dirichlet).  
**Simulation (Rollout)** – run a shallow constraint‑propagation pass:  
1. Encode hard constraints (e.g., `A → B` becomes `¬A ∨ B`).  
2. Propagate truth values using unit resolution until fixed point.  
3. Compute prediction error as the sum of squared deviations between the final belief probabilities and the observed evidence extracted from the prompt (numeric equalities, observed truth of atomic facts).  
4. Add a complexity penalty proportional to the number of rewritten nodes (approximating variational free energy).  
The rollout returns `-error - λ*complexity`.  
**Backpropagation** – update `visits` and `value` of all nodes on the path with the rollout reward.  

After a fixed budget of iterations, the root’s `value` (average negative free energy) is the score for the candidate answer; lower free energy → higher score.

**Parsed Structural Features**  
The front‑end uses regex‑based extraction to produce propositions for:  
* Negations (`not`, `no`).  
...
These are converted into logical atoms and constraint clauses fed to the MCTS.

**Novelty**  
MCTS for argument ranking exists; Theory of Mind–style belief modeling appears in multi‑agent dialog systems; the Free Energy Principle has been applied to perception and control. Combining all three—using MCTS to explore alternative mental models, ToM to maintain separate belief distributions for self and other, and minimizing variational free energy as the rollout objective—has not, to my knowledge, been instantiated as a pure‑numpy reasoning scorer. It therefore represents a novel synthesis.

**Ratings**  
Reasoning: 8/10 — The algorithm explicitly searches over logical rewrites and evaluates consistency, capturing deep relational reasoning.  
Metacognition: 7/10 — Theory of Mind gives a rudimentary model of another’s beliefs, but recursion is limited to one level in the current design.  
Hypothesis generation: 7/10 — MCTS expands hypotheses via rewrite rules; however, the hypothesis space is constrained to local propositional edits.  
Implementability: 9/10 — All components (regex parsing, numpy arrays, UCB, constraint propagation) rely only on numpy and the Python standard library, making it straightforward to code and run without external dependencies.
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=33% cal=19% ncd_acc=42% ncd_cal=46%)

**Forge Timestamp**: 2026-03-27T17:41:23.452087
...
[View code](./Monte_Carlo_Tree_Search---Theory_of_Mind---Free_Energy_Principle/tool.py)
