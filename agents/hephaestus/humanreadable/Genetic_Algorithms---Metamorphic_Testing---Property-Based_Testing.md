# Genetic Algorithms + Metamorphic Testing + Property-Based Testing

**Fields**: Computer Science, Software Engineering, Software Engineering
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T23:30:19.560002
**Report Generated**: 2026-04-02T11:44:50.547295

---

## Nous Analysis

**Algorithm: Evolving Property‑Guided Metamorphic Validator (EPGMV)**  

**Data structures**  
- *Population*: list of candidate answer strings.  
- *Chromosome*: a tuple `(parse_tree, weight_vector)` where `parse_tree` is a directed acyclic graph of extracted logical predicates (nodes = atomic propositions, edges = logical connectives) and `weight_vector` ∈ ℝⁿ assigns a real score to each predicate type (e.g., negation, comparative, causal).  
- *Metamorphic relation set*: a fixed library of binary relations Rᵢ defined over answer strings (e.g., “if X contains a comparative ‘more than Y’, then swapping X and Y should invert the truth value of the comparative predicate”).  
- *Property suite*: a set of unary properties Pⱼ derived from the question specification (e.g., “answer must contain exactly one numeric value”, “answer must not contain a double negation”).  

**Operations**  
1. **Parsing (structural extraction)** – Using regex‑based patterns, the tool extracts:  
   - numeric tokens (`\d+(\.\d+)?`),  
   - negation cues (`not`, `no`, `never`),  
   - comparatives (`more than`, `less than`, `≥`, `≤`),  
   - ordering tokens (`first`, `last`, `before`, `after`),  
   - causal markers (`because`, `therefore`, `if … then`).  
   Each token becomes a node; connectives (`and`, `or`, `if`) become edges, yielding a parse tree.  
2. **Fitness evaluation** – For each chromosome:  
   - Compute property violations: Σⱼ 𝟙[Pⱼ(parse_tree) fails]·wⱼ.  
   - Compute metamorphic consistency: generate k mutants of the answer by applying each Rᵢ (e.g., swap comparatives, double a number, reverse ordering). For each mutant, re‑parse and evaluate the same properties; count mismatches between original and mutant property vectors. Fitness = –(property_violations + λ·metamorphic_mismatches). Lower is better.  
3. **Selection** – Tournament selection on fitness.  
4. **Crossover** – Subtree swap: pick a random node in each parent’s parse tree and exchange the sub‑trees, producing offspring parse trees; weight vectors are averaged.  
5. **Mutation** – With probability pₘ:  
   - Flip a predicate type (e.g., change a comparative to its opposite).  
   - Perturb a numeric token by ±ε.  
   - Randomly adjust one weight in weight_vector by Gaussian noise.  
6. **Shrinking (property‑based)** – When a candidate fails a property, invoke a shrinking routine that iteratively simplifies the parse tree (remove conjuncts, replace comparatives with equality, drop modifiers) until a minimal failing core is found; this core is fed back as a high‑penalty mutant to guide the GA toward satisfying the property.  

**Scoring logic** – After a fixed number of generations, the best individual’s fitness is transformed to a score in [0,1] via `score = 1 / (1 + exp(fitness))`. Higher scores indicate answers that satisfy more properties and exhibit fewer metamorphic inconsistencies.  

**Structural features parsed** – numeric values, negation scope, comparative direction, ordering/temporal relations, causal conditionals, conjunctive/disjunctive structure, and quantifier‑like expressions (“exactly one”, “at least”).  

**Novelty** – The fusion mirrors recent work on *property‑guided fuzzing* and *metamorphic mutation testing* but adds an evolutionary search over weighted parse‑tree representations, which to my knowledge has not been applied to scoring open‑ended reasoning answers. Thus the combination is novel in this concrete formulation.  

**Ratings**  
Reasoning: 8/10 — captures logical structure and quantitative constraints via parse trees and property violations.  
Metacognition: 6/10 — the algorithm can reflect on its own failures through shrinking, but lacks explicit self‑assessment of search adequacy.  
Hypothesis generation: 7/10 — GA generates diverse answer mutants; property‑based shrinking yields minimal counter‑examples, akin to hypothesis refinement.  
Implementability: 9/10 — relies only on regex, basic graph operations, numpy for vector math, and random selection; no external libraries needed.  
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=42% cal=43% ncd_acc=42% ncd_cal=46%)

**Forge Timestamp**: 2026-04-02T11:32:36.834954
...
[View code](./Genetic_Algorithms---Metamorphic_Testing---Property-Based_Testing/tool.py)
