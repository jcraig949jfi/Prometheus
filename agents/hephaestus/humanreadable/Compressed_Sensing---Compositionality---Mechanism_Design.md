# Compressed Sensing + Compositionality + Mechanism Design

**Fields**: Computer Science, Linguistics, Economics
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-02T00:08:09.529351
**Report Generated**: 2026-04-02T10:00:36.819419
...
**Algorithm**  
We build a sparse‑representation scorer that treats each candidate answer as a measurement vector **b** and seeks the sparsest set of logical propositions **x** that could have generated it.  

1. **Parsing (Compositionality)** – Using regex‑based dependency patterns we extract atomic propositions from the question and the candidate answer. Each proposition is a triple (subject, predicate, object) enriched with flags for negation, comparatives, conditionals, causal markers, and numeric constraints. The set of all possible propositions observed across the training corpus forms a dictionary **D** of size *n*; we assign each a fixed index.  

2. **Measurement matrix (Compressed Sensing)** – For a given answer we construct a binary matrix **A** (m × n) where each row corresponds to a observed surface pattern (e.g., a specific dependency path, a numeric token, a negation cue). **A**[i, j] = 1 if proposition *j* explains pattern *i*, else 0. The observation vector **b** (m × 1) records the count or presence of each pattern in the answer (0/1 or tf‑idf weighted).  

3. **Sparse recovery** – We solve the LASSO problem  
   \[
   \hat{x} = \arg\min_x \frac{1}{2}\|A x - b\|_2^2 + \lambda \|x\|_1
   \]  
   using Iterative Soft‑Thresholding Algorithm (ISTA) with only NumPy operations (matrix multiplies, vector adds, soft‑threshold). The solution **x̂** is a sparse vector; non‑zero entries indicate propositions deemed true in the answer.  

4. **Scoring (Mechanism Design)** – To incentivize truthful answers we adopt a proper scoring rule: the reward is the negative of the objective value,  
   \[
   s = -\Big(\frac{1}{2}\|A \hat{x} - b\|_2^2 + \lambda \|\hat{x}\|_1\Big).
   \]  
   Higher *s* means the answer fits the observed patterns with few propositions, rewarding parsimonious, logically consistent responses.  

**Structural features parsed** – Negations (presence of “not”, “no”), comparatives (“more than”, “less than”, “≥”, “≤”), conditionals (“if … then”, “unless”), causal claims (“because”, “leads to”, “results in”), numeric values (integers, decimals, percentages), and ordering relations (“greater than”, “before”, “after”). Each feature creates a row in **A** linking to propositions that could satisfy it.  

**Novelty** – While compressed sensing for signal recovery and proper scoring rules for mechanism design are well known, applying sparse recovery to a logical‑form representation of text for answer scoring is not found in the literature. Existing work uses either bag‑of‑words similarity or shallow entailment checks; this approach jointly enforces sparsity (few true facts) and incentive compatibility, making it a novel combination.  

**Ratings**  
Reasoning: 8/10 — captures logical structure via sparse recovery, improving over pure similarity baselines.  
Metacognition: 6/10 — the method can estimate uncertainty via the residual error but lacks explicit self‑reflection on its own parsing failures.  
Hypothesis generation: 7/10 — by proposing multiple sparse explanations (different λ) it generates alternative factual hypotheses.  
Implementability: 9/10 — relies only on NumPy and regex; ISTA loops are straightforward to code and run efficiently.
...
**Status**: Scrapped
**Reason**: trap_battery_failed (acc=46% cal=3% ncd_acc=42% ncd_cal=46%)

**Forge Timestamp**: 2026-04-02T08:46:32.237476
...
[View code](./Compressed_Sensing---Compositionality---Mechanism_Design/tool.py)
