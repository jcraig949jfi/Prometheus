# Predictive Coding + Autopoiesis + Free Energy Principle

**Fields**: Cognitive Science, Complex Systems, Theoretical Neuroscience
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-28T17:20:06.614421
**Report Generated**: 2026-03-31T14:34:56.992081

---

## Nous Analysis

The algorithm treats each prompt and candidate answer as a set of logical propositions extracted with regex (predicate‑argument tuples, polarity, comparatives, conditionals, causal links, numeric constraints). These propositions populate a hierarchical generative model: a prior probability vector **p** is built from prompt frequencies; higher levels encode relational schemas (e.g., transitivity of “>”, modus ponens for conditionals).  

**Data structures**  
- `props`: list of dicts `{pred:str, args:tuple, polarity:bool, weight:float}`  
- `rel_mat`: numpy `n×n` matrix where `rel_mat[i,j]=1` if proposition *i* entails *j* (derived from extracted relations).  
- `error_vec`: numpy array of length *n* representing mismatch between candidate propositions and model predictions.  

**Operations**  
1. **Parsing** – regex extracts propositions and fills `rel_mat`.  
2. **Constraint propagation** – compute transitive closure of `rel_mat` with Floyd‑Warshall (numpy `k.times` loop) and apply modus ponens to derive expected propositions `exp = rel_cl @ p`.  
3. **Prediction error** – `error_vec = cand - exp` where `cand` is a binary vector of candidate propositions.  
4. **Free‑energy score** –  
   `F = 0.5 * error_vec @ error_vec + λ * complexity`  
   `complexity = sum(cand) - sum(exp.clip(0,1))` (new assumptions).  
5. **Autopoietic closure** – reject candidates that make `rel_mat` non‑acyclic (detect via numpy.linalg.eigvals; any non‑negative eigenvalue indicates inconsistency). Final score = `-F` (lower free energy → higher score).  

**Structural features parsed** – negations, comparatives (`>`, `<`, `=`), conditionals (`if … then`), causal cues (`because`, `leads to`), numeric values, ordering relations, quantifiers.  

**Novelty** – While predictive coding and the free energy principle appear in neural models, and autopoiesis is used in systems theory, combining them to drive a symbolic, constraint‑propagation‑based scoring engine for text has not been described in the literature; existing tools use either pure probabilistic soft logic or end‑to‑end neural predictors, making this hybrid approach novel.  

**Ratings**  
Reasoning: 8/10 — strong logical deduction via constraint propagation and error minimization.  
Metacognition: 6/10 — limited self‑monitoring; closure check is binary, no adaptive confidence updating.  
Hypothesis generation: 7/10 — generates alternative predictions by minimizing free energy, but hypothesis space is restricted to extracted propositions.  
...
**Forge Timestamp**: 2026-03-31T13:55:34.186500
...
[View code](./Predictive_Coding---Autopoiesis---Free_Energy_Principle/tool.py)
