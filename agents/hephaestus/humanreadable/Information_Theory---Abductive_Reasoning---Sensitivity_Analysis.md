# Information Theory + Abductive Reasoning + Sensitivity Analysis

**Fields**: Mathematics, Philosophy, Statistics
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-26T06:58:33.381718
**Report Generated**: 2026-03-27T17:21:24.215565

---

## Nous Analysis

The algorithm builds a weighted abductive hypothesis set and scores it with an information‑theoretic objective that is then regularized by a sensitivity‑analysis term.

**Data structures**  
- `Statement`: a namedtuple `(pred, args, polarity, num_val, weight)`. `polarity` is True for affirmative, False for negated; `num_val` holds extracted numbers (or None); `weight` is a confidence score derived from cue words (e.g., “certainly” → 1.0, “possibly” → 0.5).  
- `ObservationSet`: numpy array of statement weights for the prompt.  
- `Hypothesis`: a frozenset of indices into the global statement list.  
...
1. **Structural parsing** – regex patterns extract:  
   ...
   - Numeric values with optional units (`\d+(\.\d+)?\s*(kg|m|s|%)`),  
   - Ordering (`\bbefore\b|\bafter\b|\brank\b`).  
   Each match yields a `Statement` and is added to a global list; the prompt’s statements form the observation set **O**.  

2. **Abductive hypothesis generation** – start with empty hypothesis **H**. Iteratively add the statement *s* that maximizes the reduction in unexplained entropy:  
   \[
   \Delta = H_{\text{res}}(O\setminus H) - H_{\text{res}}(O\setminus (H\cup\{s\}))
   \]  
   where \(H_{\text{res}}(S) = -\sum_{i\in S} \frac{w_i}{\sum_j w_j}\log\frac{w_i}{\sum_j w_j}\). Stop when Δ falls below a threshold or a max size is reached.  

3. **Information‑theoretic scoring** – compute:  
   - Observation entropy \(H(O)\) from **O** weights.  
   - Conditional entropy \(H(O|H)\) as the entropy of the residual after removing statements explained by **H**.  
   - Mutual information \(I(H;O)=H(O)-H(O|H)\).  
   - Residual entropy \(H_{\text{res}}(O\setminus H)\).  
   Base score: \(S_{\text{base}} = I(H;O) - \lambda \, H_{\text{res}}(O\setminus H)\) (λ balances explanatory power vs. unexplained noise).  

4. **Sensitivity analysis** – create *P* perturbed observation sets by:  
   - Flipping polarity of each statement (negation ↔ affirmation) with probability 0.1,  
   - Adding Gaussian noise ε∼N(0,σ²) to each numeric value (σ=0.05·|value|).  
   For each perturbed set compute \(S_{\text{base}}^{(p)}\). Sensitivity penalty: \(\beta \cdot \text{std}\{S_{\text{base}}^{(p)}\}\).  
   Final score: \(S = S_{\text{base}} - \beta \cdot \text{std}\).  

**Structural features parsed** – negations, comparatives, conditionals, causal claims, numeric values, ordering relations.

**Novelty** – While weighted abduction (e.g., Hobbs et al.) and MDL‑style information‑theoretic scoring exist separately, coupling them with a systematic sensitivity‑analysis robustness penalty in a pure‑numpy, rule‑based tool is not described in the literature; thus the combination is novel.

**Ratings**  
Reasoning: 8/10 — The algorithm captures explanatory power via mutual information and penalizes unexplained noise, yielding principled scores for multi‑step reasoning.  
Metacognition: 6/10 — It does not explicitly monitor its own uncertainty beyond the sensitivity variance; higher‑order reflection is limited.  
Hypothesis generation: 9/10 — Greedy abductive search directly maximizes information gain, producing compact, high‑coverage hypothesis sets.  
Implementability: 7/10 — All steps rely on regex, numpy arrays, and basic loops; no external libraries are needed, though careful tuning of λ, β, and thresholds is required for stability.
...
**Forge Timestamp**: 2026-03-27T16:26:02.961602
...
[View code](./Information_Theory---Abductive_Reasoning---Sensitivity_Analysis/tool.py)
