# Theory of Mind + Sparse Coding + Pragmatics

**Fields**: Cognitive Science, Neuroscience, Linguistics
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-04-01T07:41:34.507845
**Report Generated**: 2026-04-02T04:20:10.886144
...
**Algorithm**  
We build a sparse logical‑form encoder that treats each proposition extracted from a prompt or candidate answer as a binary vector over a fixed dictionary **D** of atomic logical primitives (e.g., P, ¬P, P∧Q, P→Q, P>Q, P=<Q, cause(P,Q), time_before(P,Q)). The dictionary size |D| is modest (≈200) so vectors are naturally sparse.  

1. **Parsing** – Using only regex and the stdlib we extract:  
   * literals and their negations (`not`, `no`),  
   * comparatives (`greater than`, `less than`, `≥`, `≤`),  
   * conditionals (`if … then …`, `unless`),  
   * numeric constants and arithmetic relations,  
   * causal markers (`because`, `leads to`, `results in`),  
   * temporal/ordering cues (`before`, `after`, `while`).  
   Each extracted primitive yields a 1 in the corresponding slot of the vector; all other slots stay 0.  

2. **Sparse Coding** – The raw vector **v** is passed through a hard‑threshold sparsity step: keep the *k* largest‑magnitude entries (here magnitude is 1, so we simply keep the first *k* primitives encountered, where *k*≈5) and zero‑out the rest. This yields a sparse code **s** that enforces energy efficiency and pattern separation.  

3. **Theory of Mind Layer** – We maintain two models:  
   * **Speaker model** Mₛ: the sparse code of the reference answer (or gold standard).  
   * **Listener model** Mₗ: a recursive belief about what the evaluator expects, obtained by applying a single‑step belief update: Mₗ = Mₛ ⊕ δ, where ⊕ is a sparse‑addition (union of non‑zero slots) and δ is a small perturbation representing possible false beliefs (e.g., flipping a randomly chosen primitive with probability p=0.1).  

4. **Pragmatic Scoring** – For a candidate answer we compute its sparse code **s_c**. The score is:  

   ```
   score = -‖s_c - Mₛ‖₁          // sparsity‑aware literal match (negative L1 distance)
            + λ₁ * I_quantity   // bonus if candidate conveys no more info than needed (|s_c|₀ ≤ |Mₛ|₀)
            + λ₂ * I_relevance  // bonus if all non‑zero slots of s_c intersect Mₗ (relevance to listener’s belief)
            + λ₃ * I_manner     // penalty for redundant primitives (duplicates removed by sparsity)
   ```

   λ’s are small fixed constants (e.g., 0.2). Constraint propagation (transitivity of >, modus ponens on →) is applied to **s_c** and **Mₛ** before scoring to derive implicit primitives and fill missing slots via deterministic closure.  

**Structural Features Parsed** – negations, comparatives, conditionals, numeric constants & arithmetic, causal markers, temporal/ordering relations, and conjunctions/disjunctions inferred via closure.  

**Novelty** – Sparse encoding of logical forms appears in vector‑symbolic architectures; Theory of Mind is modeled via recursive belief updates in cognitive‑science inspired AI; Pragmatic scoring follows RSA/Gricean frameworks. The tight integration of all three—using a single sparse code as the shared representation for literal content, mental‑state modeling, and pragmatic constraint checking—has not been published as a unified, numpy‑only evaluation tool, making the combination novel in this context.  

**Ratings**  
Reasoning: 8/10 — The algorithm performs explicit logical closure and sparse matching, capturing multi‑step reasoning better than pure similarity methods.  
Metacognition: 7/10 — By modeling a listener’s belief and penalizing mismatches, it exhibits a rudimentary form of self‑other modeling, though depth is limited to one recursion.  
Hypothesis generation: 6/10 — The system can propose implicit primitives via closure, but it does not actively generate alternative hypotheses beyond those derivable from the given forms.  
Implementability: 9/10 — All steps rely on regex, numpy array operations, and simple loops; no external libraries or APIs are required, making it straightforward to code and run.
...
**Status**: Scrapped
**Reason**: validation:runtime_error: UnicodeEncodeError: 'charmap' codec can't encode character '\u2265' in position 2308: character maps to <undefined>

**Forge Timestamp**: 2026-04-01T21:55:20.821821
...
[View code](./Theory_of_Mind---Sparse_Coding---Pragmatics/tool.py)
