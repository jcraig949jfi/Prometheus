# Tensor Decomposition + Kolmogorov Complexity + Sensitivity Analysis

**Fields**: Mathematics, Information Science, Statistics
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-26T16:25:01.796627
**Report Generated**: 2026-03-27T17:21:24.437559
...
**Algorithm**  
1. **Feature extraction** – For each candidate answer, run a fixed set of regex patterns to produce a binary/categorical feature vector *f* ∈ ℝᴰ where dimensions correspond to: negation presence, comparative adjective, conditional clause (“if … then …”), numeric value (scaled), causal verb (cause, leads to, results in), ordering relation (>, <, before, after), quantifier, and token‑position index. Stack the vectors of all sentences in the answer into a 2‑D tensor *X* ∈ ℝᴺˣᴰ (N = max sentences per answer, padded with zeros).  
2. **Low‑rank CP decomposition** – Approximate *X* ≈ ∑ₖ₌₁ᴿ *aₖ* ∘ *bₖ* ∘ *cₖ* with rank R chosen by a scree‑test on the Frobenius norm. The ALS updates are performed with NumPy dot and outer products; convergence stops when ‖X‑X̂‖₂ < τ or after max_iter iterations.  
3. **Kolmogorov‑complexity proxy** – Description length L = R·(N+D)·log₂(max(N,D)) + ‖X‑X̂‖₀·log₂(2) (MDL: rank × storage + residual sparsity). Lower L indicates a more compressible, hence simpler, logical structure.  
4. **Sensitivity analysis** – Generate M perturbed copies *X⁽ᵐ⁾* by adding ε · 𝒩(0,1) to numeric entries and flipping the sign of negation/comparative features with probability p. For each copy compute the reconstruction error eₘ =‖X⁽ᵐ⁾‑X̂⁽ᵐ⁾‖₂. Sensitivity S = (1/M)∑ₘ |eₘ − e₀|, where e₀ is the error on the original *X*. High S means the answer’s logical structure is fragile to small input changes.  
5. **Score** – Score = ‑(e₀ + λ₁·L + λ₂·S). Higher (less negative) scores reward answers that are accurately reconstructed, compactly describable, and robust to perturbations.

**Structural features parsed**  
- Negation tokens (“not”, “no”)  
- Comparative adjectives (“greater”, “less”)  
...
**Novelty**  
Tensor decomposition has been used for question‑answering embeddings, and Kolmogorov‑complexity approximations appear in MDL‑based feature selection. Combining a low‑rank tensor reconstruction error, an explicit MDL‑style complexity penalty, and a sensitivity‑to‑perturbation term for scoring reasoning answers is not present in the surveyed literature; thus the combination is novel.

**Rating**  
Reasoning: 8/10 — captures logical syntax via tensor structure and quantifies fidelity, simplicity, and robustness.  
Metacognition: 6/10 — provides a self‑assessment of complexity and sensitivity but lacks explicit reflection on reasoning steps.  
Generating hypotheses: 5/10 — creates perturbed variants to test robustness, a rudimentary hypothesis‑generation mechanism.  
Implementability: 9/10 — relies only on NumPy for ALS, regex for feature extraction, and basic arithmetic; no external libraries or APIs needed.
...
**Status**: Scrapped
**Reason**: validation:runtime_error: ValueError: matmul: Input operand 1 has a mismatch in its core dimension 0, with gufunc signature (n?,k),(k,m?)->(n?,m?) (size 8 is different from 2)

**Forge Timestamp**: 2026-03-27T17:01:44.784853
...
[View code](./Tensor_Decomposition---Kolmogorov_Complexity---Sensitivity_Analysis/tool.py)
