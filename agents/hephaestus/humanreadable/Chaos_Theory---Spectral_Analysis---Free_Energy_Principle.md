# Chaos Theory + Spectral Analysis + Free Energy Principle

**Fields**: Physics, Signal Processing, Theoretical Neuroscience
**Nous Model**: nvidia/nemotron-3-super-120b-a12b
**Nous Timestamp**: 2026-03-25T15:21:45.254197
**Report Generated**: 2026-03-27T06:37:37.649285
...
**Algorithm**  
1. **Symbolic extraction** – Using a handful of regex patterns we convert each prompt and candidate answer into a ordered list of discrete symbols:  
   - `NEG` for negation cues (`not`, `no`, `never`),  
   - `CMP` for comparatives (`more`, `less`, `>-`, `</-`),  
   - `COND` for conditionals (`if`, `unless`, `then`),  
   - `NUM` for numeric tokens,  
   - `CAU` for causal verbs (`cause`, `lead to`, `result in`),  
   - `ORD` for ordering cues (`first`, `before`, `after`).  
   All other words map to a generic `TOK` token. The result is a 1‑D integer array `s ∈ {0,…,K-1}^L`.

2. **Predictive‑coding model (Free Energy Principle)** – We build a simple first‑order Markov model from the prompt sequence: transition matrix `T̂` estimated by counting symbol pairs and normalising rows. For a candidate we compute the one‑step prediction error `e_t = -log T̂[s_{t-1}, s_t]`. The variational free energy proxy is the average error `F = mean(e_t)`. Lower `F` means the candidate respects the prompt’s statistical structure.

3. **Lyapunov‑exponent‑like sensitivity (Chaos Theory)** – We perturb the candidate sequence by randomly flipping 1 % of its symbols to a different valid token (creating a nearby trajectory). For each perturbation we re‑compute the free energy `F'`. The divergence rate is approximated by  
   \[
   \lambda \approx \frac{1}{\Delta t}\frac{1}{N}\sum_{i=1}^{N}\frac{|F'_i-F|}{\epsilon},
   \]  
   where `ε` is the perturbation magnitude (0.01) and `Δt = 1`. A high λ indicates that small changes cause large prediction‑error growth → less coherent reasoning.

4. **Spectral flatness (Spectral Analysis)** – We treat the symbol sequence as a discrete signal and compute its power spectral density via `numpy.fft.fft`. Spectral flatness `SF = exp(mean(log PSD)) / mean(PSD)` measures how tone‑like (low SF) versus noise‑like (high SF) the symbolic pattern is. Well‑structured arguments exhibit periodicities (e.g., alternating condition‑conclusion) → lower SF.

5. **Combined score** –  
   \[
   \text{Score} = w_1\cdot(-F) + w_2\cdot(-\lambda) + w_3\cdot(-SF)
   \]  
   with weights `w = [0.4,0.3,0.3]`. Higher scores indicate answers that are predictable, resistant to perturbation, and exhibit regular spectral structure.

**Structural features parsed** – negations, comparatives, conditionals, numeric values, causal claims, ordering relations (via the regex‑derived symbols).  

**Novelty** – The triplet (prediction‑error minimization, Lyapunov‑exponent sensitivity, spectral flatness) has not been packaged together for text scoring; existing work uses either predictive coding or recurrence quantification, but not all three in a single algorithm.

**Ratings**  
Reasoning: 8/10 — captures logical coherence via prediction error and sensitivity to perturbations.  
Metacognition: 6/10 — the method can monitor its own error but lacks explicit self‑reflection loops.  
Hypothesis generation: 5/10 — generates implicit hypotheses (perturbations) but does not propose new candidate structures.  
Implementability: 9/10 — relies only on regex, NumPy FFT, and basic counting; no external libraries needed.
...
**Forge Timestamp**: 2026-03-26T02:17:27.784865
...
[View code](./Chaos_Theory---Spectral_Analysis---Free_Energy_Principle/tool.py)
