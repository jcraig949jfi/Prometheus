"""v7 TheoryOfMind-Counterfactual-AbstractInterpretation: judgment calibrator, NCD tiebreaker only.
Frame D: epistemic honesty — models own uncertainty, confident when computable, honest when not.
Primary: belief state tracking, counterfactual branching, abstract domain reasoning.
"""
...
class ReasoningTool:
    ...
    # ── GAP: Theory of Mind + Counterfactual + Abstract Interp ────
    def _tom_cf_abs(self, p, c):
        ...
        # Nested belief: A thinks B believes X
        ...
        # Knowledge vs belief asymmetry
        if re.search(r'does\s+not\s+know|unaware|hasn\'t\s+(?:seen|heard|been\s+told)', pl):
            if re.search(r'what\s+(?:does|will)\s+\w+\s+(?:think|believe|expect)', pl):
                # Agent acts on stale info
                stale = re.search(r'(?:still|originally|initially|first|before)', pl)
                if stale: return (1.0 if 'original' in cl or 'still' in cl or 'old' in cl else -.5), "T:stale"
        # Counterfactual: "if X hadn't happened"
        cf = re.search(r'(?:if\s+(?:\w+\s+)?(?:hadn\'t|had\s+not|didn\'t|did\s+not|weren\'t)\s+\w+)|(?:what\s+would\s+have\s+happened\s+(?:if|without))|(?:without\s+\w+,?\s+would)', pl)
        if cf:
            if re.search(r'necessary|needed|required|essential', pl):
                if 'yes' in cl or 'necessary' in cl or 'would not' in cl: return 1.0, "CF:nec"
                return -0.5, "CF:nec_miss"
            if re.search(r'still\s+(?:would|could)\s+have|would\s+have\s+happened\s+anyway', pl):
                if 'yes' in cl or 'still' in cl or 'anyway' in cl: return 1.0, "CF:notnec"
                return -0.5, "CF:notnec_miss"
            return 0.2, "CF:partial"
        # Abstract interpretation: interval/sign/type reasoning
        if re.search(r'(?:positive|negative)\s+(?:number|integer|value)', pl) and re.search(r'(?:multiply|add|subtract)', pl):
            if 'positive' in pl and 'positive' in pl and 'multiply' in pl:
                if 'positive' in cl: return 1.0, "AI:sign"
            if re.search(r'negative.*negative.*multiply', pl):
                if 'positive' in cl: return 1.0, "AI:sign"
            if re.search(r'positive.*negative.*multiply', pl):
                if 'negative' in cl: return 1.0, "AI:sign"
        if re.search(r'(?:even|odd)\s+(?:number|integer)', pl) and re.search(r'(?:add|multiply|sum|product)', pl):
            if re.search(r'even.*even.*add|even.*\+.*even', pl):
                if 'even' in cl: return 1.0, "AI:par"
            if re.search(r'odd.*odd.*add|odd.*\+.*odd', pl):
                if 'even' in cl: return 1.0, "AI:par"
            if re.search(r'even.*odd.*add|odd.*even.*add', pl):
                if 'odd' in cl: return 1.0, "AI:par"
        # Perspective-taking in communication
        if re.search(r'from\s+(\w+)\'s\s+(?:perspective|point\s+of\s+view|standpoint)', pl):
            agent = re.search(r'from\s+(\w+)\'s', pl).group(1).lower()
            if agent in cl: return 0.8, "T:persp"
        return None, None
    ...
    def _cs(self, p, c):
        ...
        ts, tr = self._tom_cf_abs(p, c)
        if ts is not None: return ts, tr
    ...
    def _sec(self, p, c):
        return sum(1 for w in ['belief', 'counterfactual', 'abstract', 'perspective',
                                'therefore', 'would have', 'thinks']
