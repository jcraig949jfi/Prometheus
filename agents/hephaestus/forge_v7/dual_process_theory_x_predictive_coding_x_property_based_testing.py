"""Frame B — Dual Process x Predictive Coding x Property-Based Testing.
S1 heuristic vs S2 analytical, prediction error signals, property invariants.
...
class ReasoningTool:
    ...
    def _rp(self,p,c):
        ...
        return o
    def _s1(self,p,c):
        pw,cw=set(_w(p)),set(_w(c)); return min(.6,.2+len(pw&cw)/max(len(pw),1)) if pw else .3
    def _s2(self,p,c):
        pn,cn=_ns(p),_ns(c)
            ...
            for j in range(len(pn)):
                if i!=j:
                    for r in[pn[i]+pn[j],pn[i]-pn[j],pn[i]*pn[j],pn[i]/pn[j] if pn[j] else None]:
                        if r is not None and abs(r-t)<.01: best=max(best,.7)
        return best
    def _pe(self,p,c):
        pn,cn=_ns(p),_ns(c)
        if pn and cn:
            for i in range(len(pn)):
                for j in range(i+1,len(pn)):
                    for op in[pn[i]+pn[j],pn[i]-pn[j],abs(pn[i]-pn[j]),pn[i]*pn[j],pn[i]/pn[j] if pn[j] else None]:
                        if op is not None and abs(op-cn[0])<.01: return .05
        stops={'the','a','an','is','are','was','were','in','on','at','to','for','of','and','or','but','not','if','then','it','this','that','with'}
        pw=[w for w in _w(p) if w not in stops];cw=[w for w in _w(c) if w not in stops]
        return max(0.,.5-len(set(pw)&set(cw))/max(len(set(cw)),1)*.3) if pw and cw else .3
    ...
    def _score(self,p,c):
        ...
        cx=min(1.,len(_ns(p))*.1+len(re.findall(r'\b(?:if|then|unless)\b',p.lower()))*.2)
        bl=(1-cx)*self._s1(p,c)+cx*self._s2(p,c);pe=self._pe(p,c);concept=.5*bl+.5*(1.-pe)
        ncd=1.-_ncd(p,c)
    ...
    def evaluate(self,prompt,candidates):
        ...
        for c in candidates:
            ...
            r.append({"candidate":c,"score":sc,"reasoning":f"dp={sc:.3f} cap={mc:.2f}"})
