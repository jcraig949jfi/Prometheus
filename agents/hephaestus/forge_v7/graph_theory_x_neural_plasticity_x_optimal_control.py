"""Frame C Dynamics — Graph Theory x Neural Plasticity x Optimal Control. Composite: 8.0.
Directed graph from premises, reachability/shortest-path/cycle detection.
Hebbian plasticity adjusts edge weights. Optimal control minimises path cost."""
...
class ReasoningTool:
    def _graph(self,text):
        pl=text.lower(); g=defaultdict(dict); e=set()
        for a,r,b in re.findall(r"(\w+)\s+is\s+(\w+(?:er|ier))\s+than\s+(\w+)",pl): g[a][b]=1.; e.update([a,b])
        for a,b in re.findall(r"(\w+)\s*(?:causes|leads to|->|→)\s*(\w+)",pl): g[a.lower()][b.lower()]=1.; e.update([a.lower(),b.lower()])
        for a,b in re.findall(r"if\s+(\w+)[,.]?\s*then\s+(\w+)",pl): g[a][b]=1.; e.update([a,b])
        return g,e
    def _spath(self,g,s,e):
        if s==e: return [s]
        v={s}; q=deque([(s,[s])])
        while q:
            n,p=q.popleft()
            for nb in g.get(n,{}):
                if nb==e: return p+[nb]
                if nb not in v: v.add(nb); q.append((nb,p+[nb]))
        return []
    def _plast(self,g,c,lr=.2):
        cl=c.lower()
        for a in g:
            for b in g[a]: g[a][b]=min(2.,g[a][b]+lr) if a in cl or b in cl else max(.1,g[a][b]-lr*.3)
        return g
    # -- Parsers --
    def _numeric(s,p,c):
    ...
    def _parsers(self,p,c):
        t,n=0.,0
        for f in [self._numeric,self._trans,self._mt,self._bb,self._abn,self._neg,self._svo,self._br,self._temp,self._fp,self._mod]:
            try:
                sc,m=f(p,c)
                if m: t+=sc; n+=1
            except: pass
    ...
    def _score(self,p,c):
        g,ents=self._graph(p); ps,np_=self._parsers(p,c); gs=0.
        if ents:
            g=self._plast(g,c); cl=c.lower(); ce=[e for e in ents if e in cl]
            tw,nc=0.,0
            for t in ce:
                for sr in ents:
                    if sr==t: continue
                    pa=self._spath(g,sr,t)
                    if pa and len(pa)>1:
                        tw+=sum(g.get(pa[i],{}).get(pa[i+1],.5) for i in range(len(pa)-1))/len(pa); nc+=1
            gs=tw/max(nc,1)
            rk=defaultdict(int)
            for a in g:
                for b in g[a]: rk[a]+=1; rk[b]-=1
            if rk:
                mx,mn=max(rk,key=rk.get),min(rk,key=rk.get); pl=p.lower()
                if _h(pl,"tallest","largest","biggest","heaviest","most","best","highest","fastest") and mx in cl: gs+=.5
                elif _h(pl,"shortest","smallest","lightest","slowest","least","worst","lowest") and mn in cl: gs+=.5
        ...
        st=.50*ps+.35*gs+.15*(1.-ncd)*.15
        for _ in range(3): st=.8*st+.2*(ps*.6+gs*.4)
        return 1./(1.+math.exp(-3.*st)),np_,len(ents)
    ...
    def evaluate(self,prompt,candidates):
        ...
        for c in candidates:
            sc,np_,ne=self._score(prompt,c); sc=min(sc,mc)
            r.append({"candidate":c,"score":float(sc),"reasoning":f"parsers={np_} ents={ne} cap={mc:.2f}"})
        r.sort(key=lambda x:x["score"],reverse=True); return r
    def confidence(self,prompt,answer):
        mc=self._meta_confidence(prompt,answer)
        if mc<.3: return mc
        sc,_,_=self._score(prompt,answer); return float(max(0.,min(.95,min(sc,mc))))
