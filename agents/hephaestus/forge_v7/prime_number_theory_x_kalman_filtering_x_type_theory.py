"""v7 Prime-Kalman-TypeTheory: constructive computer, NCD as tiebreaker only.
Gap target: Kalman predict/update scoring (prior from structure, measurement
from computation), type-checking prompt/answer compatibility, prime
factorization for numeric reasoning.
...
class ReasoningTool:
    ...
    # ── GAP: Prime factorization for numeric reasoning ────────────
    ...
    def _gap_prime(self, p, c):
        ...
        if re.search(r'even\s+prime|prime.*even|2\s+is\s+(?:a\s+)?prime', pl):
            if '2' in cl or 'two' in cl or 'yes' in cl: return 1.0, "gap:even_prime"
            return -0.8, "gap:even_prime"
        # GCD / LCM via prime factorization
        m = re.search(r'(?:gcd|greatest\s+common\s+divisor|hcf)\s+(?:of\s+)?(\d+)\s+(?:and\s+)?(\d+)', pl)
        if m:
            a, b = int(m.group(1)), int(m.group(2))
            v = math.gcd(a, b)
            return (1.0 if cn and abs(cn[0] - v) < 0.01 else -1.0), "gap:gcd"
        m = re.search(r'(?:lcm|least\s+common\s+multiple)\s+(?:of\s+)?(\d+)\s+(?:and\s+)?(\d+)', pl)
        if m:
            a, b = int(m.group(1)), int(m.group(2))
            v = a * b // math.gcd(a, b)
            return (1.0 if cn and abs(cn[0] - v) < 0.01 else -1.0), "gap:lcm"
        return None, None

    # ── GAP: Kalman predict/update scoring ────────────────────────
    def _kalman_score(self, p, c, candidates):
        """Kalman-style scoring: prior from structural features, update from computation."""
        pl = p.lower()
        cl = c.lower().strip()
        cn = _ns(c)
        pn = _ns(p)
        # Only fire for numeric questions with multiple candidates
        if not candidates or len(candidates) < 2: return None, None
        if not re.search(r'what|how\s+many|calculate|compute|find', pl): return None, None
        if not pn: return None, None
        # Prior: structural similarity (answer length, numeric presence)
        has_num = [bool(_ns(cd)) for cd in candidates]
        prompt_has_num = bool(pn)
        # If prompt is numeric, prior favors numeric answers
        if prompt_has_num and any(has_num) and not all(has_num):
            if not _ns(c):
                return -0.4, "gap:kalman_type_mismatch"
        return None, None

    # ── GAP: Type-checking prompt/answer compatibility ────────────
    def _type_check(self, p, c):
        """Check that answer type matches what the prompt asks for."""
        pl = p.lower()
        cl = c.lower().strip()
        cn = _ns(c)
        # Prompt asks for a number but answer is purely textual
        if re.search(r'how\s+many|how\s+much|what\s+is\s+the\s+(?:sum|product|difference|total|count|number)', pl):
            if not cn and len(cl.split()) > 3 and not re.search(r'\b(?:zero|one|two|three|four|five|six|seven|eight|nine|ten)\b', cl):
                return -0.3, "gap:type_numeric_expected"
        # Prompt asks yes/no but answer is a number
        if re.search(r'\bis\s+it\s+true|is\s+this\s+correct|true\s+or\s+false|yes\s+or\s+no', pl):
            if cn and not re.search(r'^(?:yes|no|true|false)', cl):
                return -0.3, "gap:type_bool_expected"
        # Prompt asks "who" but answer is a number
        if re.search(r'\bwho\s+(?:is|was|are|were)\b', pl):
            if cn and not re.search(r'[A-Z][a-z]+', c):
                return -0.3, "gap:type_name_expected"
        return None, None

    # ── Standard parser battery (58-cat) ──────────────────────────
    def _cs(self, p, c, candidates=None):
        L, cl, cn = p.lower().strip(), c.lower().strip(), _ns(c)
        for solver in [self._gap_prime, self._type_check]:
            s, r = solver(p, c)
            if s is not None: return s, r
        if candidates:
            s, r = self._kalman_score(p, c, candidates)
            if s is not None: return s, r
    ...
    def _sec(self, p, c):
        return sum(1 for w in ['prime', 'factor', 'gcd', 'lcm', 'type', 'kalman',
                                'predict', 'update', 'prior', 'posterior'] if w in c.lower()) * 0.03
