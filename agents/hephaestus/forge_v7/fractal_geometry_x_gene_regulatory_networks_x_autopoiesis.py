"""Frame B — Fractal Geometry x Gene Regulatory Networks x Autopoiesis.
Multi-scale self-similarity, GRN activation/inhibition dynamics,
autopoietic self-maintenance checking. Plus standard parsers."""
...
class ReasoningTool:
    def _feats(s,t):
        pl=t.lower(); f={}
        for pat,lb in [(r'\b(not|never|no|without|neither)\b','neg'),(r'\b(if|then|unless|whenever)\b','cond'),
            (r'\b(because|causes|leads to|therefore)\b','causal'),(r'\b(all|every|each|any)\b','univ'),
            (r'\b(some|few|many|most)\b','exist'),(r'\b(more|less|greater|fewer|taller|shorter)\b','comp'),
            (r'\b(and|but|or|yet|however)\b','conj')]:
            f[lb]=len(re.findall(pat,pl))
        f['nums']=len(_ns(t)); f['words']=len(pl.split()); return f
    def _fractal(s,t):
        w=t.lower().split()
        if len(w)<6: return .5
        sims=[]
        for sc in [2,3,4]:
            ch=[' '.join(w[i:i+sc]) for i in range(0,len(w)-sc+1,sc)]
            if len(ch)<2: continue
            for i in range(len(ch)-1):
                bg1=set(zip(ch[i].split(),ch[i].split()[1:])) if len(ch[i].split())>1 else set()
                bg2=set(zip(ch[i+1].split(),ch[i+1].split()[1:])) if len(ch[i+1].split())>1 else set()
                if bg1|bg2: sims.append(len(bg1&bg2)/max(len(bg1|bg2),1))
        if not sims: return .5
        return float(np.clip(np.mean(sims)*(1.-np.var(sims)),0.,1.))
    def _grn(s,p,c):
        pf,cf=s._feats(p),s._feats(c); act,inh=0.,0.
        if pf['causal']>0 and cf['causal']>0: act+=.3
        if pf['cond']>0 and cf['cond']>0: act+=.2
        pn=set(str(int(x)) for x in _ns(p) if x==int(x)); cn=set(str(int(x)) for x in _ns(c) if x==int(x))
        if pn and cn: act+=.3*len(pn&cn)/max(len(pn|cn),1)
        if pf['neg']>0 and cf['neg']==0: inh+=.2
        if pf['neg']==0 and cf['neg']>0: inh+=.15
        if pf['univ']>0 and cf['exist']>0: inh+=.1
        return float(1./(1.+math.exp(-3.*(act-inh))))
    def _auto(s,p,c):
        pf,cf=s._feats(p),s._feats(c)
        ap={k for k,v in pf.items() if v>0 and k!='words'}; ac={k for k,v in cf.items() if v>0 and k!='words'}
        if not ap: return .5
        mt=len(ap&ac)/len(ap); nv=ac-ap; bd=1.-min(len(nv)/max(len(ap),1),1.)*.5
        pe=set(re.findall(r'\b[A-Z][a-z]+\b',p)); ct=c.lower()
        ep=sum(1 for e in pe if e.lower() in ct)/max(len(pe),1) if pe else .5
        return float(np.clip(.4*mt+.3*bd+.3*ep,0.,1.))
    # -- Parsers --
    def _numeric(s,p,c):
    ...
    def _score(s,p,c):
        ps,np_=s._parsers(p,c); fr=s._fractal(p+" "+c); gr=s._grn(p,c); au=s._auto(p,c)
        dom=.35*fr+.35*gr+.30*au
    ...
    def evaluate(s,prompt,candidates):
        ...
        for c in candidates:
            ...
            r.append({"candidate":c,"score":float(sc),"reasoning":f"parsers={np_} frac+grn+auto cap={mc:.2f}"})
