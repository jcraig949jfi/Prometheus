"""Frame B — Category Theory x Phase Transitions x Theory of Mind.
Morphism composition (transitivity functor), phase transition threshold detection,
ToM belief state tracker. NCD tiebreaker only. Full parser battery + Tier B traps."""
...
class ReasoningTool:
    ...
    def _morph(self,p,c):
        pl=p.lower();edges=[]
        for pat in[r'(\w+)\s+(?:is a|is an|belongs to|is part of)\s+(\w+)',r'(\w+)\s+(?:contains|includes|subsumes)\s+(\w+)',r'every\s+(\w+)\s+is\s+(?:a|an)\s+(\w+)',r'(\w+)\s+(?:implies|entails|leads to)\s+(\w+)']:
            for m in re.finditer(pat,pl): edges.append((m.group(1),m.group(2)))
        ...
        while changed:
            ...
            for a in list(g):
                for b in list(g[a]):
                    for x in g.get(b,set()):
                        if x not in g[a]: g[a].add(x);changed=True
        cl=c.lower()
        for a in g:
            if a in cl and any(b in cl for b in g[a]): return .75
        if _h(pl,'is','member','type','kind','instance'):
            for a in g:
                if a in cl and any(b in pl for b in g[a]): return .7
        return .2
    def _phase(self,p,c):
        pl=p.lower();cn=_ns(c);th=re.search(r'(?:threshold|tipping|critical|boundary|limit|capacity|maximum|minimum)\s*(?:is|of|at)?\s*(\d+(?:\.\d+)?)',pl)
        if th:
            tv=float(th.group(1))
            if cn and abs(cn[0]-tv)<tv*.1+1: return .8
            if _h(pl,'above','exceed','over','past') and cn and cn[0]>tv: return .75
            if _h(pl,'below','under','within') and cn and cn[0]<tv: return .75
        if _h(pl,'phase','transition','critical','tipping','threshold'): return .6 if _h(c,'change','transition','shift','flip','switch') else .35
        return 0.
    def _tom(self,p,c):
        pl=p.lower();cl=c.lower()
        mv=re.search(r'(\w+)\s+(?:puts?|places?|hides?|leaves?)\s+(?:\w+\s+){0,3}(?:in|on|under|behind)\s+(?:the\s+)?(\w+)',pl)
        tr=re.search(r'(\w+)\s+(?:moves?|transfers?|takes?)\s+(?:\w+\s+){0,3}(?:to|into)\s+(?:the\s+)?(\w+)',pl)
        ab=re.search(r'(\w+)\s+(?:leaves?|goes?|is\s+(?:away|gone|out))',pl)
        if mv and tr and ab:
            orig,new_loc,absent=mv.group(2),tr.group(2),ab.group(1)
            qm=re.search(r'(?:where|what)\s+(?:does|will|would)\s+(\w+)\s+(?:think|believe|look|expect)',pl)
            if qm and qm.group(1).lower()==absent.lower():
                return .88 if orig in cl else(.1 if new_loc in cl else .3)
        if re.search(r"doesn.t know",pl) and re.search(r'what does \w+ (?:think|believe)',pl):
            if _h(pl,'rigged','loaded','biased','weighted'): return .85 if _h(cl,'50','fair','equal','half') else .2
        if _h(pl,'opposite','contrary','lie','deceive') and re.search(r'should \w+ say',pl):
            wm=re.search(r'wants?\s+\w+\s+to\s+(?:go\s+)?(left|right|stay|leave)',pl)
            if wm:
                opp={'left':'right','right':'left','stay':'leave','leave':'stay'}
                return .85 if opp.get(wm.group(1),'') in cl else .2
        return 0.
    ...
    def _score(self,p,c):
        ...
        concept=max(self._morph(p,c),self._phase(p,c),self._tom(p,c));ncd=1.-_ncd(p,c)
        if pmax is not None: return .70*pmax+.25*concept+.05*ncd
        return .80*concept+.20*ncd if concept>.1 else .50*.3+.50*ncd
    def evaluate(self,prompt,candidates):
        if not candidates: return []
        mc=self._meta_confidence(prompt);r=[]
        for c in candidates:
            sc=float(max(0.,min(1.,min(self._score(prompt,c),mc))))
            r.append({"candidate":c,"score":sc,"reasoning":f"ct={sc:.3f} cap={mc:.2f}"})
