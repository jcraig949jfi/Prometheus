"""Frame E Hardened v2 — computation-first + robust parsers + meta-confidence."""
import re, zlib, math
from collections import defaultdict
from itertools import permutations
_N=re.compile(r'-?\d+(?:\.\d+)?')
_D=['monday','tuesday','wednesday','thursday','friday','saturday','sunday']
...
class ReasoningTool:
    ...
    def _meta(s,p):
        ...
        for pat in [r'stopped|still|again|already|anymore',r'every.*?some|all.*?not|not.*?all',
                    r'either.*?or|must\s+be\s+one',r'successful|survivors?|winners?|made\s+it',
                    r'already\s+(?:spent|invested|paid)|too\s+late\s+to']:
    ...
    def _cm_reg(s,p):
        ...
        R={}
        for m in re.finditer(r'([A-Za-z])\s*=\s*(-?\d+(?:\.\d+)?)',rm.group(1)): R[m.group(1).upper()]=float(m.group(2))
        ...
        for o in re.split(r'[.;]\s*',pl[rm.end()-1:]):
            ...
            if m:
                if m.group(1): R[m.group(2).upper()]=R.get(m.group(2).upper(),0)+float(m.group(1))
                else: R[m.group(3).upper()]=R.get(m.group(3).upper(),0)+float(m.group(4))
                continue
    ...
    def _cm_bel(s,p):
        ...
        for w,o,l in re.findall(r'(\w+)\s+(?:moves?|relocates?|takes?)\s+(?:the\s+)?(\w+)\s+(?:\w+\s+)*?(?:to|into|in)\s+(?:the\s+)?(\w+)',pl):
            ...
            for a in B:
                if a not in absent: B.setdefault(a,{})[o]=l
        for m in re.finditer(r'(\w+)\s+arrives?\s+and\s+sees?\s+(?:the\s+)?(\w+)\s+in\s+(?:the\s+)?(\w+)',pl):
    ...
    def _cm_cst(s,p):
        ...
        P=[m.group(1) for m in re.finditer(r'([A-Z][a-z]+)',p) if m.group(1) not in skip]
        ...
        for m in re.finditer(r"(\w+)(?:'s\s+choice\s+was\s+not|\s+did\s*n[o']t\s+choose)\s+(?:the\s+)?(\w+)",pl):
            w=m.group(1).strip()
            for per in P:
                if per.lower()==w: E[per].add(m.group(2).strip())
        for m in re.finditer(r"(\w+)\s+did\s+not\s+choose\s+any\s+of\s+([\w,\s]+)",pl):
            w=m.group(1).strip()
            for per in P:
                if per.lower()==w:
                    for n in re.split(r',\s*',m.group(2)): E[per].add(n.strip())
        for m in re.finditer(r"person\s+who\s+chose\s+(\w+)\s+has\s+a\s+name\s+starting\s+with\s+'?(\w)",pl):
            for per in P:
                if per[0].upper()==m.group(2).upper(): F[per]=m.group(1)
        AC=[]
        for m in re.finditer(r"(\w+)'s\s+choice\s+comes?\s+alphabetically\s+before\s+(\w+)'s",pl):
            for p1 in P:
                if p1.lower()==m.group(1).lower():
                    for p2 in P:
                        if p2.lower()==m.group(2).lower(): AC.append((p1,p2))
        ...
        for pm in permutations(fi,len(fa)):
            ...
            ok=all(A.get(a,'') not in e for a,e in E.items()) and all(A.get(p1,'')<A.get(p2,'') for p1,p2 in AC)
            if ok: sols.append(dict(A))
    ...
    def _cm_rec(s,p):
        ...
        try:
            for i in range(n0+1,qn+1):
                e=re.sub(r'f\s*\(\s*n\s*-\s*1\s*\)',str(memo[i-1]),expr); e=e.replace('n',str(i)); memo[i]=eval(e,{"__builtins__":{}})
        except: return None
        r=memo.get(qn); return int(r) if r is not None and isinstance(r,float) and r==int(r) else r
    def _cm_cf(s,p):
        pl=p.lower()
        ch=re.findall(r'(\w[\w\s]*?)\s+(?:cause[sd]?|le[ad]d?\s+to|result(?:s|ed)?\s+in)\s+(\w[\w\s]*?)(?:[.,;]|$)',pl)
    ...
    def _cm_isf(s,p):
        ...
        if len(eqs)>=2:
            ...
            dt=a1*b2-a2*b1
            if abs(dt)>1e-9:
                x=(c1*b2-c2*b1)/dt; y=(a1*c2-a2*c1)/dt
                qm=re.search(r'(?:what\s+is|find)\s+(\w)',pl); qv=qm.group(1) if qm else 'x'
    ...
    def _cm_def(s,p):
        ...
        ep=re.findall(r'(\w+)\s+are\s+\w+\s+that\s+do\s+\w+',pl)
        ep=[t for t in ep if 'not' not in t]
    ...
    def _cm_int(s,p):
        pl=p.lower(); R=[]
        for m in re.finditer(r'(\d{1,2}):(\d{2})\s*(am|pm)\s+to\s+(\d{1,2}):(\d{2})\s*(am|pm)',pl):
            R.append((_t24(int(m.group(1)),int(m.group(2)),m.group(3)),_t24(int(m.group(4)),int(m.group(5)),m.group(6))))
    ...
    def _sp(s,p):
        ...
        if re.search(r'not\s+(?:the\s+case\s+)?(?:that\s+)?all\s+\w+\s+can',pl) and re.search(r'can\s+\w+\s+\w+\?',pl):
        ...
        if wt and re.search(r'opposite',pl):
            op={'left':'right','right':'left','north':'south','south':'north','east':'west','west':'east'}
            return ('text',op.get(wt.group(1).lower(),wt.group(1)))
        ...
        if re.search(r'tamper|rigg|load',pl) and re.search(r"doesn't|does\s+not",pl):
            return ('text','1/6' if re.search(r'die|dice',pl) else '1/52' if re.search(r'card|deck',pl) else 'fair')
        dy=re.search(r'today\s+is\s+(\w+)',pl)
        if dy and _DM.get(dy.group(1).lower()) is not None:
            d=_DM[dy.group(1).lower()]; off=0; rest=pl[dy.end():]
            _DO={'day before yesterday':-2,'day after tomorrow':2,'yesterday':-1,'tomorrow':1,'day before':-1,'day after':1}
        ...
        if tm:
            t1=_t24(int(tm.group(1)),int(tm.group(2)),tm.group(3)); t2=_t24(int(tm.group(4)),int(tm.group(5)),tm.group(6))
            if t2<=t1: t2+=1440
            d=t2-t1; return ('text',f"{d//60} hours and {d%60} minutes")
        pr=re.findall(r'(\d{4}):\s*(\d+(?:\.\d+)?)',p)
        if len(pr)>=3:
            vs=[float(v) for _,v in sorted(pr)]; d1=[vs[i+1]-vs[i] for i in range(len(vs)-1)]; d2=[d1[i+1]-d1[i] for i in range(len(d1)-1)]
    ...
    def _match(s,computed,c):
        ...
        if isinstance(computed,(int,float)):
            ...
            if cn:
                if any(abs(v-computed)<tol for v in cn): return 0.95
                if any(abs(v-computed)<0.5 for v in cn): return 0.70
            st=str(int(computed)) if isinstance(computed,float) and computed==int(computed) else str(computed)
            if st in cl: return 0.95
            return 0.08
        if isinstance(computed,dict):
            if any(ag.lower() in cl and it.lower() in cl for ag,it in computed.items()): return 0.95
            if any(it.lower() in cl for it in computed.values()): return 0.90
            return 0.08
        if isinstance(computed,str):
            ...
            M={'stops':('stop','cease','would not'),'50%':('50','1/2','0.5'),'no_cause':('confound','not necessarily','no,','correlation'),
               'not_same':('not the same','less','lower','different'),'even':('even','false'),'Cannot be determined':('cannot','determined'),
               'cannot be answered':('cannot be answered','given information','cannot be determined'),
               'same':('same','equal','neither'),'second':('second','2nd'),'inconsistent':('inconsistent','no, ','no,','contradiction'),
               'consistent':('consistent','yes, ','yes,'),'Yes':('yes',),'No':('no',)}
