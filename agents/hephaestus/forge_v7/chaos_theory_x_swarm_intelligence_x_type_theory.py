"""Chaos Theory x Swarm Intelligence x Type Theory. Frame C: Dynamics Tracker.
Logistic map sensitivity (perturb inputs, measure divergence), swarm consensus
(multiple scoring agents vote), type compatibility checking.
...
class ReasoningTool:
    def _logistic_sensitivity(self,score,r=3.9,n=5,eps=0.01):
        x0=max(.01,min(.99,score)); traj=[]
        for k in range(n):
            x=max(.01,min(.99,x0+(k-n//2)*eps))
            for _ in range(8): x=r*x*(1-x)
            traj.append(x)
        return 1.0-min(max(traj)-min(traj),1.0)

    def _swarm_vote(self,p,c):
        pl,cl=p.lower(),c.lower(); pw=set(re.findall(r'\b[a-z]{3,}\b',pl)); cw=set(re.findall(r'\b[a-z]{3,}\b',cl))
        v1=len(pw&cw)/max(len(pw),1); ents=set(re.findall(r'\b[A-Z][a-z]+\b',p))
        v2=min(sum(1 for e in ents if e.lower() in cl)/max(len(ents),1),1.0)
        pn,cn=_n(p),_n(c); v3=1.0 if(pn and cn and any(abs(a-b)<.01 for a in pn for b in cn))else(.3 if pn and cn else .5)
    ...
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for cand in candidates:
            cs,sn=self._s(prompt,cand);tp=self._type_check(prompt,cand);sw=self._swarm_vote(prompt,cand)
            nc=(1.0-self._ncd(prompt,cand))*.2
            if cs is not None:
                stab=self._logistic_sensitivity(cs);raw=cs*.45+sw*.35+nc;raw=raw*(.7+.3*stab)
            else: raw=sw*.55+nc+.1
            score=raw-tp
            results.append({'candidate':cand,'score':float(np.clip(min(score,meta),0,1)),'reasoning':f'comp:{sn} sw:{sw:.2f} tp:{tp:.2f}'})
