"""Frame C Dynamics — Dual Process x Optimal Control x Free Energy. Composite: 8.3.
S1 (fast pattern match) + S2 (deliberate computation), blended by complexity.
Free energy minimisation as objective. State evolves to equilibrium."""
...
class ReasoningTool:
    ...
    def _numeric(s,p,c):
        ...
        if ig and not il: ok=a>b
            ...
            v=max(a,b) if ig else min(a,b); return (1. if cn and abs(cn[0]-v)<1e-9 else 0.),True
    ...
    def _tom(s,p,c):
        ...
        if _h(pl,'opposite','contrary') and re.search(r'should \w+ say',pl):
            ...
            if wm:
                o={'left':'right','right':'left','stay':'leave','leave':'stay'}
                return (1. if o.get(wm.group(1),'') in c.lower() else -.5),True
        return 0.,False
    def _fp(s,p,c):
    ...
    def _s1(self,p,c): return self._run([self._numeric,self._trans,self._mt,self._bb,self._abn,self._neg,self._svo,self._br,self._temp,self._causal,self._tom,self._fp,self._mod],p,c)
    def _s2(self,p,c):
        pn,cn=_ns(p),_ns(c)
        if not pn or not cn: return 0.
        t,sc=cn[0],0.
        for i in range(len(pn)):
            for j in range(len(pn)):
                if i!=j:
                    for r in[pn[i]+pn[j],pn[i]-pn[j],pn[i]*pn[j],pn[i]/pn[j] if pn[j] else None]:
                        if r is not None and abs(r-t)<.01: sc=max(sc,.7)
        return sc
    def _evolve(self,p,c):
        s1,n1=self._s1(p,c); s2=self._s2(p,c)
        cx=min(1.,len(_ns(p))*.1+len(re.findall(r'\b(?:if|then|unless)\b',p.lower()))*.2+len(p.split())*.005)
        a=.3+.5*cx; bl=(1-a)*s1+a*s2; fe=max(0.,1.-(_ncd(p,c)-bl))
        return .55*bl+.30*fe+.15*s1, n1, cx
    ...
    def evaluate(self,prompt,candidates):
        ...
        for c in candidates:
            sc,n1,cx=self._evolve(prompt,c); sc=float(max(0.,min(1.,min(sc,mc))))
            r.append({"candidate":c,"score":sc,"reasoning":f"S1={n1} cx={cx:.2f} cap={mc:.2f}"})
        r.sort(key=lambda x:x["score"],reverse=True); return r
    def confidence(self,prompt,answer):
        mc=self._meta_confidence(prompt,answer)
        if mc<.3: return mc
        sc,_,_=self._evolve(prompt,answer); return float(max(0.,min(.95,min(sc,mc))))
