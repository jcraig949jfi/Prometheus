"""v7 Tensor-Ecosystem-ECC: constructive computer, NCD as tiebreaker only.
Gap target: SVD on feature vectors for dimensionality reduction, ecosystem
predation model (constraints eliminate weak candidates), Hamming-distance
redundancy checking across multiple scoring signals.
...
class ReasoningTool:
    ...
    # ── GAP: SVD feature reduction for candidate ranking ──────────
    def _svd_rank(self, prompt, candidates):
        """Build feature vectors for candidates, use SVD to find dominant signal."""
        ...
        for c in candidates:
            ...
            cn = _ns(c)
            f = [
                len(cl.split()) / 20.0,                              # length
                len(cn) / 5.0 if cn else 0.0,                        # numeric density
                1.0 if any(w in cl for w in ['yes', 'no', 'true', 'false']) else 0.0,  # boolean
                len(set(cl.split()) & set(pl.split())) / max(len(pl.split()), 1),  # overlap
                1.0 - self._ncd(prompt, c),                           # compression sim
                sum(1 for w in ['because', 'therefore', 'since', 'thus'] if w in cl) / 4.0,
            ]
            feats.append(f)
    ...
    # ── GAP: Ecosystem predation (constraint elimination) ─────────
    def _ecosystem_eliminate(self, prompt, candidates):
        """Predation model: hard constraints eliminate unfit candidates."""
        pl = prompt.lower()
        cn_prompt = _ns(prompt)
        survivors = []
        for i, c in enumerate(candidates):
            cl = c.lower().strip()
            cn = _ns(c)
            alive = True
            # Constraint: if prompt asks yes/no, eliminate non-boolean answers
            if re.search(r'\?$', pl) and re.search(r'is\s+it|are\s+they|does\s+it|can\s+we|true\s+or\s+false', pl):
                if not re.search(r'^(?:yes|no|true|false)', cl) and len(cl.split()) > 10:
                    alive = False
            # Constraint: if prompt has numbers and asks "how many", eliminate non-numeric
            if re.search(r'how\s+many|how\s+much|what\s+is\s+the\s+(?:total|sum|count)', pl):
                if not cn and not re.search(r'\b(?:zero|one|two|three|four|five|six|seven|eight|nine|ten|none)\b', cl):
                    alive = False
            # Constraint: absurdly long answers for simple questions
            if len(pl.split()) < 15 and len(cl.split()) > 50:
                alive = False
            survivors.append((i, alive))
        return survivors

    # ── GAP: Hamming distance redundancy check ────────────────────
    def _hamming_consensus(self, scores_list):
        """Check consensus across multiple scoring signals via Hamming distance.
        If multiple signals agree on ranking, boost confidence."""
        if len(scores_list) < 2:
            return 0.0
        n = len(scores_list[0])
        # Convert each score array to a ranking (binary: above/below median)
        rankings = []
        for scores in scores_list:
            med = np.median(scores)
            rankings.append(tuple(1 if s >= med else 0 for s in scores))
        # Compute pairwise Hamming agreement
        total_agree = 0
        pairs = 0
        for i in range(len(rankings)):
            for j in range(i + 1, len(rankings)):
                agree = sum(1 for a, b in zip(rankings[i], rankings[j]) if a == b)
                total_agree += agree / n
                pairs += 1
        return total_agree / pairs if pairs > 0 else 0.0
    ...
    def _sec(self, p, c):
        return sum(1 for w in ['svd', 'tensor', 'ecosystem', 'hamming', 'redundancy',
                                'constraint', 'eliminate', 'predation'] if w in c.lower()) * 0.03

    def evaluate(self, prompt, candidates):
        R = []
        # Get SVD scores for ensemble
        svd_scores = self._svd_rank(prompt, candidates)
        # Get ecosystem survivors
        eco_survivors = self._ecosystem_eliminate(prompt, candidates)
        for i, c in enumerate(candidates):
            s, r = self._cs(prompt, c, candidates)
            if r == "F":
                nv = self._ncd(prompt, c)
                sc = (1 - nv) * .15 + self._sec(prompt, c)
                # Blend SVD signal if available
                if svd_scores is not None and i < len(svd_scores):
                    svd_boost = float(svd_scores[i]) * 0.05
                    sc += np.clip(svd_boost, -0.1, 0.1)
            ...
            # Ecosystem penalty
            if eco_survivors:
                for idx, alive in eco_survivors:
                    if idx == i and not alive:
                        sc -= 0.3
                        cf = max(cf * 0.5, 0.1)
