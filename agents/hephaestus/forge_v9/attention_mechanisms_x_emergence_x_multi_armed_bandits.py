"""Attention Mechanisms x Emergence x Multi-Armed Bandits — Frame H (Primordial Soup).
Each parser is a bandit arm. UCB-style attention weights parsers by relevance.
Emergent ensemble outperforms any single parser. Under 200 lines.
"""
import re, math, zlib
from forge_primitives import (
    ...
    check_transitivity, topological_sort, confidence_from_agreement,
    entropy, information_sufficiency, negate, expected_value,
)
...
_FEAT = {
    'num': re.compile(r'larger|greater|bigger|less\s+than|repeating|sum.*odd', re.I),
    'alg': re.compile(r'bat.*ball|all\s+but|cost.*more|how\s+many.*left', re.I),
    'log': re.compile(r'if\s+.*then|not\s+the\s+case|all\s+\w+\s+are|therefore', re.I),
    'con': re.compile(r'must\s+two\s+share|\d+\s+people.*month|every\s+\d+\s+meters', re.I),
    'prob': re.compile(r'coin\s+flip|probability|50%|heads|tails|pound.*gold', re.I),
    'tmp': re.compile(r'facing|turn\s+(?:left|right)|today\s+is|yesterday|tomorrow', re.I),
    'pos': re.compile(r'overtake|2nd\s+place|second\s+place|chased|who\s+was', re.I),
    'trans': re.compile(r'taller\s+than|tallest|heavier\s+than|heaviest', re.I),
}
...
class ReasoningTool:
    ...
    def _attention(self, prompt):
        w = {k: len(v.findall(prompt))*2.0+(1.0 if not v.findall(prompt) else 0.0) for k,v in _FEAT.items()}
        t = sum(w.values()); return {k: v/t for k,v in w.items()} if t else w
    ...
    def _compute(self, p):
        ...
        m = re.search(r'(-?\d+\.?\d*)\s+is\s+(?:less|smaller)\s+than\s+(-?\d+\.?\d*).*which.*(?:larger|greater)', pl)
        ...
        if re.search(r'(?:study|data|statistics)\s+finds?\s+.*(?:strong\s+)?correlation', pl): return "No", 0.85
    ...
    def evaluate(self, prompt, candidates):
        ...
        attn = self._attention(prompt)
        results = []
        for cand in candidates:
            ss = 0.0; cl = cand.lower().strip()
