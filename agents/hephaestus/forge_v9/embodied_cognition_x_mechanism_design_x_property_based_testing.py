"""Embodied Cognition x Mechanism Design x Property-Based Testing.
Embodied=spatial state sim. MechDesign=parser agents, highest-conf wins.
PBT=validate invariants post-computation. Under 200 lines. Deterministic."""
import re, math, zlib
from forge_primitives import (solve_constraints, temporal_order, direction_composition,
    confidence_from_agreement, bat_and_ball, all_but_n, fencepost_count,
    modular_arithmetic, parity_check, pigeonhole_check, check_transitivity,
    bayesian_update, expected_value)
_N=re.compile(r'-?\d+(?:\.\d+)?')
...
class ReasoningTool:
    ...
    def _p(s,p):
        ...
        if re.search(r'if.*?,.*?\..*?not.*?\.\s*(?:is|does|can|will)',pl) and not re.search(r'chained|then.*then',pl): R.append(("No",.90))
        ...
        if ev:
            evs={g.upper():float(pct)/100*float(val) for g,pct,val in ev}
            if evs:
                best=max(evs,key=evs.get); R.append((f"~ev_{best}_{evs[best]:.1f}",.88))
        ...
        return R
    def _m(s,c,al):
        cl=c.lower().strip(); tags={"~fair":lambda:any(w in cl for w in ['equal','roughly','any','either']),"~nocorr":lambda:'no' in cl[:5] or 'does not' in cl or 'correlation' in cl,"~commoncause":lambda:'not necessarily' in cl or 'common' in cl or 'confound' in cl,"~stops":lambda:any(w in cl for w in ['stop','no longer','cease','puddles stop']),"~ambig":lambda:'ambiguous' in cl,"~wv_yes":lambda:'believes' in cl and re.search(r'is\s+\w+',cl) is not None,"~chain_break":lambda:'broken' in cl or 'no,' in cl[:5]}
        if al in tags: return 0.9 if tags[al]() else 0.1
        if al.startswith("~ev_"):
            _,g,v=al.split("_"); return 0.9 if f"game {g.lower()}" in cl else 0.0
    ...
    def evaluate(s,prompt,candidates):
        meta=s._mc(prompt); R=s._p(prompt)
        R.sort(key=lambda x:x[1],reverse=True); res=R[0] if R else None
        out=[]
        for c in candidates:
            ss=s._m(c,res[0].lower().strip()) if res else 0.0
    ...
    def confidence(s,prompt,answer):
        ...
        R=s._p(prompt); R.sort(key=lambda x:x[1],reverse=True); res=R[0] if R else None
        if not res: return float(max(0.1,1.0-s._ncd(prompt,answer))*0.5)
        ss=s._m(answer,res[0].lower().strip()); return min(res[1],meta) if ss>0.3 else 0.15
