"""Critical Pragmatic Error-Corrector v4. Concepts: Criticality x Error Correcting Codes x Pragmatics"""
import re, math, zlib
import numpy as np
...
class ReasoningTool:
    """Syndrome checking with Gricean pragmatic overlay at criticality."""
    ...
    def _wrap(self, p, c):
        ...
        syn = 0.
        if _h(_NE, pl) != _h(_NE, cl): syn += .4
        if _ns(p) and not _ns(c): syn += .2
        wc = len(c.split()); pw = len(p.split())
        if pw > 10 and wc < 2: syn += .3
        return 1. - min(syn, 1.)
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate': c, 'score': float(total),
                       'reasoning': f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|syndrome:{ws:.2f}|ncd:{nd:.2f}'})
