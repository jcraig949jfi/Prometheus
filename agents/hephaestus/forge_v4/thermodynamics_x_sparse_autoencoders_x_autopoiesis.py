...
class ReasoningTool:
    """Thermodynamic Autopoietic Sparse Inference v4. Free-energy minimisation
    over a sparse belief lattice with 58-category structural parsers, homeostatic
    sparsity gating, and score decomposition (struct>=50% compute>=20% ncd<=15%)."""
    ...
    def _gate(self, p):
        ...
        return 0.6 if sd <= 1 else min(1.0, 0.7 + sd * 0.03)
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict]:
        ...
        for ct in candidates:
            ...
            sc = max(0, min(1, (0.55*st + 0.30*cp + 0.15*ns) * g))
    ...
    def confidence(self, prompt: str, answer: str) -> float:
        ...
        return max(0.0, min(1.0, (0.55*st + 0.30*cp) * g))
