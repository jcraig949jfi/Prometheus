"""FE-AMP Compressed Sensing v4. Concepts: Thermodynamics x Phase Transitions x Compressed Sensing"""
...
def _run(p,c):
    ...
    return 0.,'none'
class ReasoningTool:
    """Free energy AMP with phase transition penalty."""
    ...
    def _ncd(self,x,y):
        ...
        cxy=len(zlib.compress((x+' '+y).encode(),self._lv));d=max(cx,cy);return(cxy-min(cx,cy))/d if d>0 else 1.
    def _wrap(self,p,c):
        pen=0.;pl,cl=p.lower(),c.lower()
        if _h(_NE,pl)and'yes'in cl.split()and not _h(_NE,cl):pen+=2.
        lr=len(c)/max(len(p),1)
        if lr>.8:pen+=.2*lr
        bonus=0.
        if _h(_CP,pl)and _h(_CP,cl):bonus+=.5
        if _h(_NE,pl)and _h(_NE,cl):bonus+=.5
        return 1./(1.+math.exp(-(bonus-pen)))
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|feamp:{ws:.2f}|ncd:{nd:.2f}'})
