"""v5 CAITL (metacognition-enhanced): Multi-scale epigenetic attractor with RG coarse-graining.

Primary scoring via 58-category constructive parsers with Bayesian posteriors,
PEMDAS, modular arithmetic, constraint propagation. Secondary: RG flow.
"""
import re, math, zlib
import numpy as np
...
class ReasoningTool:
    ...
    def _secondary(self, p: str, c: str) -> float:
        tokens = re.findall(r'\b\w{4,}\b', c.lower())
