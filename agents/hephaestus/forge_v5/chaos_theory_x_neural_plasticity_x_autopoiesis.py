"""CAITL v5 (metacognition-enhanced) — Chaotic Autopoietic Plasticity Reasoner (CAPR).
...
def _compute(p,c):
    ...
    m=_ABN.search(pl)
    if m and re.search(r'how\s+many',pl):n=int(m.group(1));return(1.0,f"all-but-{n}")if cn and n in[int(x)for x in cn]else(-0.5,f"all-but-{n} no match")
    m=_RATE.search(pl)
    if m:
        try:t1,t2=float(m.group(2)),float(m.group(4));ans=1.0/(1.0/t1+1.0/t2);return(1.0,f"rate={ans:.2f}")if cn and min(cn,key=lambda x:abs(x-ans))<abs(ans)*0.15+0.5 else(-0.5,f"rate={ans:.2f} nm")
        except:pass
    ...
    if re.search(r'(?:at\s+least\s+one|either.*or.*how|union|overlap)',pl)and len(pn)>=2:ie=pn[0]+pn[1]-pn[2]if len(pn)>=3 else pn[0]+pn[1];return(0.8,f"IE={ie}")if cn and abs(cn[0]-ie)<1 else(-0.3,f"IE={ie} nm")
    m=re.search(r'(?:is\s+)([\d.,]+)\s+(?:larger|greater|bigger|more|higher)\s+than\s+([\d.,]+)',pl)
    if m:
        try:a,b=float(m.group(1).replace(',','')),float(m.group(2).replace(',',''));return(1.0,f"{a}>{b}")if cl.startswith('yes'if a>b else'no')else(-1.0,f"{a}>{b} wrong")
        except:pass
    ...
    if len(pn)>=2:
        ...
        if re.search(r'\b(percent|%)\b',pl):base=pn[0]if pn[0]!=0 else 1;e=abs(pn[1]-pn[0])/abs(base)*100;return(0.8,f"pct={e:.1f}")if cn and abs(cn[0]-e)<1 else(-0.3,f"pct={e:.1f} nm")
    return(None,"")
def _negation(p,c):
...
def _struct(p,c):
    ...
    if _CMP.search(pl):
        cn_=_pn(c);s=1.0 if(pn and cn_ and re.search(r'\b(larg|great|bigg|more|higher|tall|fast|old)\w*',pl)and max(pn)in cn_)else(1.0 if(pn and cn_ and re.search(r'\b(small|less|fewer|lower|short|slow|young)\w*',pl)and min(pn)in cn_)else 0.4);_a(s,0.08,'cmp')
...
class ReasoningTool:
    """Chaotic Autopoietic Plasticity Reasoner v4. struct>=52% comp>=23% ncd<=10%."""
    TAG="CAPR-v4"
