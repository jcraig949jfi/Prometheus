"""Typed Incentive GRN Circuit v5. Concepts: Gene Regulatory Networks x Mechanism Design x Type Theory"""
...
class ReasoningTool:
    """GRN attractor stability with incentive scoring."""
    ...
    def _wrap(self,p,c):
        s=.5
        if _h(_NE,p)and _h(_NE,c):s+=.2
        elif _h(_NE,p)and not _h(_NE,c):s-=.1
        if _h(_CP,p)or _h(_CD,p):
            if _h(_CP,c)or _h(_CD,c):s+=.25
            else:s-=.15
        ns=1.;pw=len(p.split());aw=len(c.split())
        if pw>10 and aw<3:ns=.6
        return s*.8+ns*.2
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        R=[]
        for c in candidates:
            cs,cn=_run(prompt,c);ss=self._struct(prompt,c);ws=self._wrap(prompt,c);nd=(1.-self._ncd(prompt,c))
            R.append({'candidate':c,'score':float((ss*.25+ws*.30)+cs*.35+nd*.10),'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|grn:{ws:.2f}|ncd:{nd:.2f}'})
