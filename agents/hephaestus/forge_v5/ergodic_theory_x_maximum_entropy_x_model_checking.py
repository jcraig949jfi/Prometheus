"""CAITL v5 (metacognition-enhanced) — Ergodic Maximum-Entropy Model Checker (EMEMC).
...
def _compute(p,c):
    ...
    if m and re.search(r'how\s+many',pl):
        n=int(m.group(1));return(1.0,f"all-but-{n}")if cn and n in[int(x)for x in cn]else(-0.5,f"all-but-{n} no match")
    m=_RATE.search(pl)
    if m:
        try:
    ...
    if len(comps)>=2:
        ...
        if re.search(r'(?:who|which|what)\s+(?:is\s+)?(?:the\s+)?(?:tallest|largest|biggest|oldest|heaviest|fastest|best|greatest|most)',pl):
            top=max(gt,key=lambda x:len(gt.get(x,set())));return(1.0,f"trans={top}")if top in cl else(-0.5,f"trans={top} no match")
...
def _struct(p,c):
    ...
    if _CMP.search(pl):
        cn_=_pn(c)
        s=1.0 if(pn and cn_ and re.search(r'\b(larg|great|bigg|more|higher|tall|fast|old)\w*',pl)and max(pn)in cn_)else(1.0 if(pn and cn_ and re.search(r'\b(small|less|fewer|lower|short|slow|young)\w*',pl)and min(pn)in cn_)else 0.4)
        _a(s,0.08,'cmp')
...
class ReasoningTool:
    """Ergodic Maximum-Entropy Model Checker v4. struct>=52% comp>=23% ncd<=10%."""
    TAG="EMEMC-v4"
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        if not candidates:return[]
        res=[]
        for c in candidates:
            st,w,R=_struct(prompt,c);cr=_compute(prompt,c);cs=(cr[0]+1)/2 if cr[0]is not None else 0.5
