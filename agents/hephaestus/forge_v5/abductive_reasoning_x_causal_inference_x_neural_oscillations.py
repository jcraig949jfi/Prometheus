"""CAITL v5 (metacognition-enhanced) — Oscillatory Causal Abductive Reasoner (OCAR).
...
def _compute(p,c):
    ...
    if m and re.search(r'how\s+many',pl):
        n=int(m.group(1)); return (1.0,f"all-but-{n}={n}") if cn and n in [int(x) for x in cn] else (-0.5,f"all-but-{n} no match")
    m=_RATE.search(pl)
    if m:
        try:
            t1,t2=float(m.group(2)),float(m.group(4)); ans=1.0/(1.0/t1+1.0/t2)
            if cn and min(cn,key=lambda x:abs(x-ans))<abs(ans)*0.15+0.5: return (1.0,f"rate={ans:.2f}")
            return (-0.5,f"rate={ans:.2f} no match")
        except: pass
    ...
    if m:
        ...
        if re.match(r'^[\d\s\+\-\*/\(\)\.]+$',expr) and len(expr)>2:
            try:
                r=eval(expr,{"__builtins__":{}},{}); return (1.0,f"PEMDAS={r}") if cn and abs(cn[0]-r)<0.01 else (-0.5,f"PEMDAS={r} no match")
            except: pass
    m=re.search(r'(?:remainder|mod).*?(\d+).*?(?:divided\s+by|mod)\s*(\d+)',pl)
    if m:
        try:
            a,b=int(m.group(1)),int(m.group(2)); r=a%b
            return (1.0,f"mod:{a}%{b}={r}") if cn and r in [int(x) for x in cn] else (-0.5,f"mod:{a}%{b}={r} no match")
        except: pass
    if re.search(r'how\s+many.*?between|fence\s*post|poles?.*?spaces?',pl) and len(pn)>=2:
        fp=abs(pn[0]-pn[1])+1; return (1.0,f"fencepost={fp}") if cn and fp in cn else (-0.3,f"fencepost={fp} no match")
    if re.search(r'(?:at\s+least\s+one|either.*or.*how|union|overlap)',pl) and len(pn)>=2:
        ie=pn[0]+pn[1]-pn[2] if len(pn)>=3 else pn[0]+pn[1]
        return (0.8,f"IE={ie}") if cn and abs(cn[0]-ie)<1 else (-0.3,f"IE={ie} no match")
    ...
    if m:
        try:
            ...
            return (1.0,f"{a}>{b}") if cl.startswith(correct) else (-1.0,f"{a}>{b} expected {correct}")
        except: pass
    m=re.search(r'(?:is\s+)([\d.,]+)\s+(?:smaller|less|fewer|lower)\s+than\s+([\d.,]+)',pl)
    if m:
        try:
            a,b=float(m.group(1).replace(',','')),float(m.group(2).replace(',','')); correct='yes' if a<b else 'no'
            return (1.0,f"{a}<{b}") if cl.startswith(correct) else (-1.0,f"expected {correct}")
        except: pass
    comps=[]
    ...
    if len(comps)>=2:
        ...
        if re.search(r'(?:who|which|what)\s+(?:is\s+)?(?:the\s+)?(?:tallest|largest|biggest|oldest|heaviest|fastest|best|greatest|most)',pl):
            ...
            return (1.0,f"trans={top}") if top in cl else (-0.5,f"trans={top} no match")
        if re.search(r'(?:who|which|what)\s+(?:is\s+)?(?:the\s+)?(?:shortest|smallest|youngest|lightest|slowest|worst|least)',pl):
            ...
            return (-0.5,f"trans-min={btm} no match")
    if _LIAR.search(pl):
        m2=re.search(r'(?:says?|claims?|states?)\s+"?(.+?)"?\s*(?:\.|$)',pl)
        if m2:
            claim=m2.group(1).lower()
            if 'yes' in claim or 'true' in claim: return (0.8,"liar->negate") if cl.startswith('no') or 'false' in cl[:20] else (-0.5,"liar fail")
            if 'no' in claim or 'false' in claim: return (0.8,"liar->affirm") if cl.startswith('yes') or 'true' in cl[:20] else (-0.5,"liar fail")
    if re.search(r'\b(?:test|accura|sensitiv|specific|positive|prevalence)\b',pl) and len(pn)>=2: return (0.0,"bayes-detected")
    if pn and not cn: return (0.0,"nums in prompt, none in candidate")
    if len(pn)>=2:
        if re.search(r'\b(sum|total|add|combined|altogether)\b',pl):
            e=sum(pn); return (1.0,f"sum={e}") if cn and abs(cn[0]-e)<0.01 else (-0.3,f"sum={e} no match")
        if re.search(r'\b(differ|subtract|minus|how many more|how much more)\b',pl):
            e=abs(pn[0]-pn[1]); return (1.0,f"diff={e}") if cn and abs(cn[0]-e)<0.01 else (-0.3,f"diff={e} no match")
        if re.search(r'\b(product|multiply|times)\b',pl):
            e=1.0
            for n in pn: e*=n
            return (1.0,f"prod={e}") if cn and abs(cn[0]-e)<0.01 else (-0.3,f"prod={e} no match")
        if re.search(r'\b(percent|%)\b',pl):
            base=pn[0] if pn[0]!=0 else 1; e=abs(pn[1]-pn[0])/abs(base)*100
            return (0.8,f"pct={e:.1f}") if cn and abs(cn[0]-e)<1 else (-0.3,f"pct={e:.1f} no match")
    return (None,"")
def _negation(p,c):
...
def _struct(p,c):
    ...
    if _CMP.search(pl):
        cn_=_pn(c)
        s=1.0 if (pn and cn_ and re.search(r'\b(larg|great|bigg|more|higher|tall|fast|old)\w*',pl) and max(pn) in cn_) else \
          (1.0 if (pn and cn_ and re.search(r'\b(small|less|fewer|lower|short|slow|young)\w*',pl) and min(pn) in cn_) else 0.4)
        _a(s,0.08,'cmp')
...
class ReasoningTool:
    """Oscillatory Causal Abductive Reasoner v4. struct>=52% comp>=23% ncd<=10%."""
    TAG="OCAR-v4"
