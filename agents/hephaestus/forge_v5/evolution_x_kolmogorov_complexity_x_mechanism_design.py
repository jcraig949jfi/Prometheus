"""CAITL v5 (metacognition-enhanced) — Evolution-Kolmogorov-Mechanism. Constructive computation, 58-cat parsers,
...
def _struct(p, c):
    ...
    wt = [0.22, 0.17, 0.15, 0.13, 0.12, 0.10, 0.11]
...
def _theory(prompt, cand):
    """Theory-specific: evolutionary fitness + complexity penalty."""
...
class ReasoningTool:
    """v4 EKCM: constructive computation, epistemic honesty, 58-cat parsers,
    ...
    def evaluate(self, prompt: str, candidates: List[str]) -> List[Dict[str, Any]]:
        ...
        for i, cand in enumerate(candidates):
            ...
            reason = f"[EKCM-v4] {sr} | {cr} | ncd={ncd_raw[i]:.3f}"
