"""v5 (metacognition-enhanced) Spectral falsification criticality SOC — 58-category constructive computation.
Secondary: Spectral falsification criticality SOC. Primary: structural+computational parsers.
"""
import re, zlib
import numpy as np
...
class ReasoningTool:
    def __init__(self): self._g=1.0;self._buf=[0.]*16
    def _ncd(self,a,b):
    ...
    def _cs(self,p,c):
        ...
        return 0.,"F"
    def _sec(self,p,c):
        vs=[ord(ch) for ch in c[:16]];ft=np.abs(np.fft.rfft(vs)) if len(vs)>=4 else np.array([1.])
        return 1.0/(1.0+float(np.sum(ft[len(ft)//2:]))*0.001)*0.08
