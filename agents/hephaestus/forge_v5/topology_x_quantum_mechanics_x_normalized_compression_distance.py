"""CAITL v5 (metacognition-enhanced) — Topological Quantum NCD Reasoner (TQNR).
...
def _compute(p,c):
    ...
    if len(pn)>=2:
        ...
        if re.search(r'\b(product|multiply|times)\b',pl):e=1.0;[e:=e*n for n in pn];return(1.0,f"prod={e}")if cn and abs(cn[0]-e)<0.01 else(-0.3,f"prod={e} nm")
...
class ReasoningTool:
    """Topological Quantum NCD Reasoner v4. struct>=52% comp>=23% ncd<=10%."""
    TAG="TQNR-v4"
