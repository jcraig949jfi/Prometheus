"""Holographic Tensor Coder v5. Concepts: Tensor Decomposition x Holography Principle x FEP"""
...
class ReasoningTool:
    """Structural tensor with free energy boundary encoding."""
    ...
    def _wrap(self,p,c):
        energy=0.;pn,cn=_nc(p),_nc(c)
        if pn>0 and cn==0:energy+=2.
        if _h(_CP,p)and not _h(_CP,c):energy+=1.5
        if _h(_CD,p)and not _h(_CD,c):energy+=1.
        return 1./(1.+energy)
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            R.append({'candidate':c,'score':float(total),'reasoning':f'REASON: comp:{cn}={cs:.2f}|struct:{ss:.2f}|holo:{ws:.2f}|ncd:{nd:.2f}'})
