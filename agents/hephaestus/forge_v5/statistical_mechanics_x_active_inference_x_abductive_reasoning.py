"""CAITL v5 (metacognition-enhanced) — Statistical-Mechanical Active Abductive Reasoner (SMAAR).
...
def _compute(p,c):
    ...
    if m:
        try:
            ...
            if cn and min(cn,key=lambda x:abs(x-ans))<abs(ans)*0.15+0.5: return (1.0,f"computation:rate={ans:.2f}")
    ...
    if re.search(r'how\s+many.*?between|fence\s*post|poles?.*?spaces?',pl) and len(pn)>=2:
        fp=abs(pn[0]-pn[1])+1; return (1.0,f"computation:fencepost={fp}") if cn and fp in cn else (-0.3,f"computation:fencepost={fp} no match")
    if re.search(r'(?:at\s+least\s+one|either.*or.*how|union|overlap)',pl) and len(pn)>=2:
        ie=pn[0]+pn[1]-pn[2] if len(pn)>=3 else pn[0]+pn[1]
        return (0.8,f"computation:IE={ie}") if cn and abs(cn[0]-ie)<1 else (-0.3,f"computation:IE={ie} no match")
    ...
    if m:
        try:
            ...
            return (1.0,f"computation:{a}>{b}") if cl.startswith(correct) else (-1.0,f"computation:{a}>{b} expected {correct}")
        except: pass
    m=re.search(r'(?:is\s+)([\d.,]+)\s+(?:smaller|less|fewer|lower)\s+than\s+([\d.,]+)',pl)
    if m:
        try:
            a,b=float(m.group(1).replace(',','')),float(m.group(2).replace(',','')); correct='yes' if a<b else 'no'
            return (1.0,f"computation:{a}<{b}") if cl.startswith(correct) else (-1.0,f"computation:expected {correct}")
        except: pass
    comps=[]
    ...
    if _LIAR.search(pl):
        ...
        if m2:
            ...
            if 'yes' in claim or 'true' in claim: return (0.8,"liar->negate") if cl.startswith('no') or 'false' in cl[:20] else (-0.5,"liar->negate fail")
            if 'no' in claim or 'false' in claim: return (0.8,"liar->affirm") if cl.startswith('yes') or 'true' in cl[:20] else (-0.5,"liar->affirm fail")
    if re.search(r'\b(?:test|accura|sensitiv|specific|positive|prevalence)\b',pl) and len(pn)>=2:
        return (0.0,"computation:bayes-detected")
...
def _negation(p,c):
    ...
    if np_==1:
        if _BOL.search(c.lower()) or nc_>=1: return 0.65
        return 0.35
    return 0.5
...
class ReasoningTool:
    """Statistical-Mechanical Active Abductive Reasoner v4. struct>=52% comp>=23% ncd<=10%."""
    TAG="SMAAR-v4"
    def evaluate(self,prompt:str,candidates:List[str])->List[Dict]:
        ...
        for c in candidates:
            ...
            rparts=[f"st={st:.3f}(w={w:.2f})",f"cp={cs:.3f}",f"nc={nc:.3f}"]
            if cr[0] is not None: rparts.append(cr[1])
            elif w<0.05: rparts.append("low_confidence:no_category_match")
            res.append({"candidate":c,"score":float(max(0,min(1,f))),"reasoning":f"[{self.TAG}] "+"; ".join(rparts)})
