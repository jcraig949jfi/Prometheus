...
| retaining state | `MAINTENANCE_COST`, paid by every cell's stored energy every tick, whether active or inert -- holding energy anywhere is not free once `MAINTENANCE_COST>0` |
...
| maintaining gradients/boundaries | not a first-class primitive in AETH-01; only indirectly obtainable (a ring of cells could repeatedly re-write a boundary value) -- flagged as an absence, not solved here |
...
## Regime B -- "Metabolic ecology"
    ...
    REPLENISH_NUMER  > 0  (sparse, e.g. ~1/1000 per cell per tick)
...
- **Decay makes hoarding a losing default.** Any cell sitting on energy
  ...
  outcompeting a neighbor in a future transfer contest) exceeds the
  decay paid while waiting.
- **Replenishment variance is what creates the payoff.** `REPLENISH_NUMER`
  is a per-tick Bernoulli draw per cell -- inflow is bursty and
  unpredictable at the single-cell level even though its long-run rate
  is fixed and known. A cell that immediately spends everything it
  receives is exposed to any gap between replenishment events; a cell
  that retains a buffer survives such gaps and keeps acting
...
- **Stateless wins** when replenishment is dense/reliable relative to
  ...
  reserve is pure loss -- spend-everything cells should outcompete
  hoarders.
- **Dormant wins** when replenishment is extremely sparse and
