...
| E18 | archaeon | kind-generic spec builder: template declares `outcome_rule` (+ `repeat` for stateful kinds); builder validates names, types, drawability, cross-axis constraints, outcome-field compatibility against the kind's contract | 7 of 69 inbox templates name an implemented kind; 0 could be built | the 7, every future kind, E17 | **DONE** (WP-0e, `archaeon/producer/kindspec.py`; `falsification_walk.v1` buildable; v0's nulls stay null) |
| E19 | daedalus+vivarium | C-1 relatedness axis | targets are hashed, so any two worlds are unrelated by construction | transfer templates | **DEFERRED 2026-09-07**: transfer is a property of an organism with state, not of two worlds (SELECTION_RULES R6); flipped-hash targets give a curve pinned analytically at both ends; reopen behind WP-B4 (a stateful organism) as WP-A4 shared-table relatedness |
...
| E22 | vivarium | C-4 orchestrated external backend; grade per OBSERVATION by double-run at admission then sampled re-execution (D-4) | 22 R-BACKEND templates | symbolic, spatial, programmatic modalities | PROPOSED, **CORRECTED 2026-09-07**: `ergon/avida2003/` holds NO runnable Avida or Tierra (unbuilt 2005 tarball, wrong version, frozen by ruling); the Avida route is a spike (WP-P0), not an asset |
...
| E28 | all (see WORK_PACKAGES.md) | **Diversity and expansion roadmap**: four branches (NK landscapes, program evaluation on the Proteus VM, EvCA cellular automaton, population spike), eight selection rules, twelve decisions, 29 work packages | 1 qualified world; 0 of 69 inbox templates build; no runnable population world; only one spatial substrate | what SFE can newly host, and how diversity survives selection | **FILED** 2026-09-07 `archaeon/docs/ROADMAP.md` §D + `archaeon/docs/expansion/` |
| E29 | harmonia | D3 null fire rate: Archaeon reported 0.000, Harmonia measured 0.106 per region | reconciled: sample size, not coupling — pure_null had n=80/320 (F-tail 2e-8); at the floor n=8/16 the exact F(7,15) tail is 0.1088; floor corpora through D3: 0.088±0.006 per region (exact 0.083), 0.487±0.029 per corpus | M-SIGNAL false-discovery figures now quote floor-geometry rates | **RECONCILED** (WP-0d, `archaeon/docs/D3_NULL_RECONCILIATION.md`); Harmonia's scoped ruling pending |
| E30 | harmonia | grant instance: a read scope over harmonia-m2's worlds granted to `cli_1029e9255a074157a1b3ba1e` (`integration/sfe_read_grant_example.py --grant`) | probe 2026-09-07: `/v2/read/observations` 200 with 0 rows | the whole signal campaign; release condition step 2 | PROPOSED (Harmonia runs it with her own token) |
