...
| E3 | daedalus | cross-tenant read grant, width per `INBOX_ARCHAEON_READ_GRANT_AND_FAMILIES.md` §2 | Archaeon owns 0 of 2,937 attested observations | the whole signal campaign | **SURFACE DONE** 2fa52de86 (`/v2/read/*`, grant scoped to a topology group, census beside rows). OPEN: harmonia-m2 worlds must be in a group and Harmonia must grant; Archaeon reader to move off the raw file once granted |
| E4 | daedalus | comparison-family arm contract (`families(kind=comparison)` + members) | no per-world arm field in the fossil | D2/D4 comparison mapping; Stage 0 arm rules | **PUBLISHED, IN CONFLICT** 2fa52de86 reads arm from the sealed spec at `arm_key`; Vivarium's validator banishes `arm`. Ruling needed: `roles/Daedalus/INBOX_ARCHAEON_ARM_KEY_CONFLICT.md` |
