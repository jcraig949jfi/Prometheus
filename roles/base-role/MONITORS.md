...
Hermes portfolio brief mailer | cron job running scripts/send_brief_email.py after metis_portfolio.py | Eos/Hermes lineage (legacy); OWNER UNCLAIMED | M3 or M4 (operator, 2026-09-11); not M1 | dashboard/portfolio_brief.md on that host | the email itself (no freshness record on the sending host) | daily | the operator's inbox | LOCATED, UNOWNED -- still sending; needs a seat to claim it, a last-success record on its host, and a decision to keep or stop; until then it is a loop whose input is a legacy portfolio step nobody maintains
