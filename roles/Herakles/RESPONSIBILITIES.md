...
| `herakles/specimens/` (created on first recovery) | Recovered historical artifacts. Originals are immutable; every file hashed and listed in the manifest (deliverable J). Large binaries go to `Z:\` per two-machine doctrine; the manifest holds hash and location. **[SUPERSEDED 2026-09-11 by the base role: "Never hardcode a drive letter: paths are repository-relative or configuration-driven." The literal `Z:\` is superseded; the large-binary location is a configured path and the manifest still holds hash and location. Same class as the Apollo charter line the base role already ruled on.]** |
