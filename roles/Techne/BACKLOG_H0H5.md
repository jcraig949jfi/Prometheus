...
TECHNE-43 | Write the stitch backend's section-1 DECLARATION and the seven required boundary fixtures | ENGINE | 1.0 | M | none | a declaration with exact keys and no defaults (binary_sha256 re-checked before each launch, argv_template + argv_from_payload allowlist, env allowlist, mounts with modes, pinned RNG/threads, output schema, limits; container: null as a declared value) plus F1 F6 F7 F8 F13 F14 F15 passing -- F15 and the thread arm already done at N=5, invariant 1..16 threads
