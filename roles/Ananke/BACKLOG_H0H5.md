...
ANANKE-26 | Write PREREG PTE-C2 ("weather in the habitable zone"): three SEPARATE pressure axes inside C1's confirmed viable region -- traffic density (irrelevant packets per site-tick), number of independent concurrent cue processes, and informational conflict (equally credible contradictory streams) -- never collapsed into one load number; target rung L5 | ENGINE | 1.0 | M | PTE-C1 DONE + A0 analysis | PREREG_PTE_C2.md committed before any C2 row
ANANKE-27 | Measure FLIP's distal-influence rung net of the local teacher path (zero-comm twin contrast subtracted) | EVIDENCE | beta | S | a post-C1 census rerun of FLIP A0 cells with a zero-comm twin (A0 did not record one) | analysis table + rerun rows
