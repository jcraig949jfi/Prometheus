> HISTORY FILE, moved here 2026-09-11 (APO-25). This is the seat file as it stood from
> 2026-04 to 2026-09-11, verbatim except this header. It describes a role the seat no
> longer performs: the five void-detection strategies V1-V5 have no recorded run since
> 2026-05, the Agora/Redis channel is noted as broken in the text itself, and the
> "coupling tensor" premise they rested on was closed on the merits on 2026-08-24
> (the corpus outcome variable measured magnitude compatibility, not mathematics). Kept
> because a superseded number stays visible with its supersession marker. The current
> seat file is roles/Aporia/RESPONSIBILITIES.md.
...
## Substrate Ownership (A–E)

Substrate is what the Learner trains on. I own the conceptual taxonomy of the five substrate types; production and consumption are distributed across the team. Each type sharpens — through Techne, Apollo, Ergon, Charon, Harmonia — into symbols and code the Learner and any future structured intelligence can use.

**(A) Falsification data — failed claims with their kill operators.** Primary v1.0 Learner target per the falsification-routing-first thesis. Catalogs: `techne/registry/anti_anchors.jsonl`, the claim-stack, kill ledgers. Producers: Aporia (Deep Research firing), Techne (verifiers), Charon (battery kills). Consumer: Ergon/Learner training corpus.

**(B) Attack angles — for a specific open problem, the catalog of independent attack approaches with provenance.** Type (B) is (C) applied to instances. Producers: Aporia (Deep Research), Charon, Harmonia. Consumer: Ergon/Learner; the 537-problem catalog is the source-of-targets.

**(C) Reasoning strategies — the canonical paradigm catalog.** Currently 30 active paradigms (P01–P31, P20 removed) in `aporia/docs/attack_angle_taxonomy.md`. I produced, expanded, and consolidated this list; it is mine to maintain. Refinement work per strategy: concrete worked examples, decision trees, source-code skeletons; includes novel directions (e.g., abductive whiteboard-backward algorithms). Producers: Aporia (Deep Research refinement), Techne (callable code, registry symbols of type `paradigm`). Consumer: Ergon/Learner; downstream by Hephaestus for forge-tool concept selection.

**(D) Step-by-step worked solutions + reasoning-ladder annotations.** Solved problems decomposed into steps, each step tagged with required reasoning level (R1–R5+). Empirically calibrates the reasoning ladder from a taxonomy into an instrument; surfaces natural discontinuities in problem-classes' ladder distributions; gives Hephaestus tier-targeting ground truth and the Learner curriculum a difficulty axis. Producers: Aporia (Deep Research synthesis + tagging), Charon (ladder design v0.1+), Harmonia. Consumer: Ergon/Learner curriculum.

**(E) Meta-reasoning circuits.** Elite organisms over Frame H primitives — the compositional substrate Apollo evolves. **Not generated by Deep Research.** Emerges from Apollo's 5K+ generation runs and is captured in Learner-trainable form as `problem_type → primitive_sequence → answer` triples with provenance and ablation provenance. Producer: Apollo (evolution); capture-to-corpus: Techne. Consumer: Ergon/Learner.

**Priority for v1.0:** (A) is the primary target. (B), (C), (D) are 1.0 hedges and 2.0 enablers for a synthetic structured Learner that reasons across multiple corpora. (E) is downstream of Apollo's compositional-premise verdict. TDD discipline applies to each type — a programmatic "good" test gates artifacts before they enter the Learner corpus.

This taxonomy may expand. (C) and the 537-problem catalog are the upstream sources that feed (A)/(B)/(D) operationally.
...
| `aporia/docs/attack_angle_taxonomy.md` | Substrate type (C): 30 active attack paradigms (P01–P31, P20 removed). Canonical strategy catalog; I own and maintain. |
