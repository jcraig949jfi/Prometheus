...
class Ctx:
    ...
    def __init__(self, mem, base, length, policy=OWN, inputs=(), rng=None,
                 ...
                 free_lo=0, free_hi=0, out_gate_reads=0):
        self.mem = mem
        self.size = len(mem)
        self.base = base
        self.length = length
        ...
        # H1 output gate. OUT is a no-op until this many task inputs have been consumed.
...
def run(ctx, pc, budget, ops_enabled=0xFF):
    ...
    while steps < budget:
        ...
        if op == 0xD3:                      # OUT (n),A: emit an answer byte
            # H1 GATE. When out_gate_reads is set, an answer emitted before the cue has
            # been consumed is discarded: the instruction costs its cycles and advances
            # pc exactly as before, but no output is recorded. This is the only
            # difference between the gated and ungated arms - inputs, expected answers,
            # transform, bridge and every other opcode are identical - so the arms are
            # the same task measured under two output disciplines rather than two
            # different tasks.
