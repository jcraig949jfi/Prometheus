...
allowed toward a corpus.
    ...
    HYPATIA-21 | Rule whether a season 2 runs, and whether a gate-corrected re-run counts as season 1 or season 2 | program | program | XL | operator decision NEW: "Hypatia season 2 scope; seat recommends a narrow one and does not self-authorize" | the ruling recorded in DECISIONS.md or in this file
    HYPATIA-22 | Re-run the season against QUANTITATIVE evidence (ledger rows, censuses, run logs) to test whether G5 becomes a real gate rather than an abstaining one | EVIDENCE | alpha | M | operator decision HYPATIA-21 | a committed run whose specific density clears 0.50 on 3+ cases, or a recorded finding that it cannot
    HYPATIA-23 | Build a DISCRIMINATING cheat: a packet supporting a nearby-but-wrong conclusion, so refusal requires choosing rather than abstaining | EVIDENCE | alpha | M | operator decision HYPATIA-21 | the packet, the ladder and the gate row, with the refusal clause recorded
    HYPATIA-24 | Put an independent decomposer (another model family or another seat) against the frozen season-1 packets | EVIDENCE | alpha | M | operator decision HYPATIA-21 and a second seat or model | a committed comparison of load-bearing sets, which either survives or kills the 32-percent utilization finding
    HYPATIA-25 | Length-match the refusal controls so "refused because insufficient" is separable from "refused because short" | EVIDENCE | alpha | S | operator decision HYPATIA-21 | a length-matched control pair with their gate rows
    HYPATIA-26 | Propose stall_check.py to Archaeon as the WORKING_CONTRACT s7 predicate for the L-04 invariant | TOOLS | program | S | Archaeon | a comms message id and either the amended s7 or a recorded ruling that it stands
    HYPATIA-27 | Report the blanket **/results/ gitignore defect to Archaeon: three seats have now hit one rule that hides the mandated rows | TOOLS | program | S | Archaeon | a comms message id and either a central fix or a ruling
