...
| C2 | Heartbeat DURING a long row, not only between stages. Fixes the same defect from the other side. | READY | none |
...
| C6 | Record the CODE REVISION in the worker heartbeat. Had the running SHA been visible, today's 404 would have been diagnosed in one query instead of a traceback and a process-start-time comparison. | READY | none |
