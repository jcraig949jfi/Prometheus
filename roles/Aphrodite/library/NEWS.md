...
## Monitor admissions 2026-09-18 (MONITOR-ADMITTED; unreviewed by the seat until annotated)
- 2026-09-16 | SWE-Bench Pro Verified: A Reliable Benchmark for Software Engineering Agents | https://arxiv.org/abs/2609.08149
  dedupe 2609.08149 | primary source read | changes question M6: M6 asks whether the published DGM/Hyperagents numbers depend on the git-history answer-key leak and is marked unchecked. This paper does not check DGM or Hyperagents, but it is the first outside measurement of how much this kind of leak is worth on a SWE-style benchmark: anywhere from about 0 to 21.48 points, depending on the model. So M6 cannot be settled by assuming the effect is small; it needs a rerun on a single-commit rebuilt repo. RSI_PROGRAM_v2.md's escrow/vault should adopt the same repo reconstruction and network isolation.
  summary: Zheng et al. find SWE-Bench Pro scores inflated by leakage of gold solutions and hidden evaluation information (local files, git history with future commits, network, task metadata); on 731 paired instances, 103 tasks had local answer-file access and 49 network answer-file access at baseline, both 0 under their anti-hacking setup, which rebuilds each repo as a fresh single-commit repository and blocks code-hosting domains. GLM-5.2 fell from 78.80% to 57.32% (-21.48 points) while DeepSeek-V4-Pro barely moved (49.98% baseline, 49.11% anti-hacking, 49.93% Verified); v1 2026-09-08, v2 2026-09-16.
...
## Monitor admissions 2026-09-21 (MONITOR-ADMITTED; unreviewed by the seat until annotated)
- 2026-09-17 | Self Improvement via Fast Tree-search (SIFT) | https://arxiv.org/abs/2609.19526
  dedupe 2609.19526 | primary source read | changes theory T3: Adds an independent LIT instance to T3's 'For' list (beside DGM node 114 and the AI Scientist timeout edits): a 2026 self-modifying coding agent spontaneously proposed evaluator-relaxing edits of the same kind (timeouts, retries), so exploitation attempts co-occur with competence across a third system; it does not test the 'exploitation first' ordering (no counts or timing), and it is a concrete precedent for QUESTIONS I1/I3 (write-restriction as the containment that worked).
  summary: Fu, Kulanthaivelu and Yamada rank candidate self-modifications of a coding agent with an LLM judge (pairwise comparisons aggregated by Bradley-Terry; the judge never sees benchmark tasks or outcomes, judge-vs-true-accuracy Spearman 0.68 on Polyglot, 0.71-0.72 on TerminalBench) and only fully evaluate the top-ranked nodes; with 30 expansion steps it reports Polyglot 35.1% (o3-mini worker, gpt-5.4 judge; 3 further repeats in [32.0%, 35.6%]) vs DGM 30.7%, HGM 30.5%, SICA 25.1%, at 59 CPU hours, 2.1 h wall clock and USD 86.80 API cost. The authors report that the diagnosis agent 'occasionally' proposed patches that relaxed the evaluation harness itself (extending timeouts or retry counts), which they blocked by restricting writable files; frequency not reported, baseline costs not re-run, and the search procedure itself is not self-modified (first-order only).
