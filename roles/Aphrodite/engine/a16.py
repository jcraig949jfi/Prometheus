"""AMENDMENT 16 (commit 46e477d81) -- four-hour recursion campaign.

Frozen before this was written. Stages:
  foundry  catalogs A, B (G4) and C (G5 stream, generated only)
  e1 / e2  the AMENDMENT 15 assay on catalogs A / B
  e4       S1 necessity
...
WORKERS = int(os.environ.get("A16_WORKERS", "7"))
LET = "abcdefghijklmnopqrstuvwxyz"
T0 = "2026-09-24T18:35:51Z"


def log(m):
    print("[a16 %s] %s" % (time.strftime("%H:%M:%S", time.gmtime()), m), flush=True)
...
def draws(cat):
    ...
    rng = random.Random(I._seed("APHRODITE/A16/CATALOG_%s/v1" % cat))
    ...
    for op in STRATA:
        ...
        while len(lst) < DRAWS and guard < 5000:
            ...
            seen.add(bid)
            k = len(lst)
            lst.append({"name": "f%s_%s_%s%s" % (cat, op, LET[k // 26], LET[k % 26]),
...
def job_q23(args):
    ...
    M.use_provider(prov)
    size = qualify(prov, d["name"], "A16-%s" % cat)
    q3 = None
    if size is not None:
...
def foundry(cats=("A", "B")):
    ...
    write("A16_DRAWS_2026-09-24.json", {c: {"sha256": sha(st[c]), "draws": st[c]} for c in st})
    ...
    while True:
        ...
        for k, v in pool_map(job_q23, batch):
            q[k] = v
        pj = []
        for c, d in batch:
            v = q[(c, d["name"])]
            if v["Q2_size"] is not None and v["Q3"]:
                ...
                pj += [(d["name"], "PRISTINE", FR.pristine().entries, i, v["Q2_size"],
                        "A16-%s-pilot" % c, specs) for i in range(N)]
        rows = dict(pool_map(run_recipient, pj)) if pj else {}
        for c, d in batch:                                # decide in DRAW ORDER
            v = q[(c, d["name"])]
            if v["Q2_size"] is not None and v["Q3"]:
                v["Q4_pristine_pilot"] = sum(rows[(d["name"], "PRISTINE", "A16-%s-pilot" % c, i)]["qualified"]
                                             for i in range(N))
    ...
    for c in cats:
        ...
        for op in STRATA:
            ...
            random.Random(I._seed("APHRODITE/A16/ASSIGN/%s/%s" % (c, op))).shuffle(acc)
...
def donor(args):
    ...
    for fam in obs_f:
        for c in range(R_OBS):
            cell = FR.Cell(prov, fam, r * R_OBS + c, size[fam], label="A16-%s-obs/r%d" % (cat, r))
            esc = E.Escrow(ESCROW)
    ...
    cells = [FR.Cell(prov, f, r * R_VAL + j, size[f], label="A16-%s-val/r%d" % (cat, r))
...
def shams_for(cat):
    out = []
    for k in range(8):
        rng = random.Random(I._seed("APHRODITE/A16-%s/SHAM/v1/%d" % (cat, k)))
        while True:
...
def assay(cat, cataloginfo):
    ...
    write("A16_%s_DONORS_2026-09-24.json" % cat, donors)
    ...
    jobs = [(f["name"], a, arms[a], i, f["qualified_dev_size"], "A16-%s-rx" % cat, specs)
            for f in trans for a in arms for i in range(N)]
    rows = dict(pool_map(run_recipient, jobs))
    fnames = [f["name"] for f in trans]
    det = {f: {a: [rows[(f, a, "A16-%s-rx" % cat, i)] for i in range(N)] for a in arms} for f in fnames}
...
def e4_job(args):
    ...
    cands = candidates_from(derived, start, classes)
    cells = [FR.Cell(T3, f, j, 4, label="A16-E4-val/%s" % fset)
    ...
    for cl in art["classes"]:
        for p in cl["observed_programs"]:
            ...
            f1.append({"family": fam, "program": prog,
                       "parsed": parsed_for(T3, fam, quals[fam]["qualified_dev_size"], "A16-E4-F1-")})
    c3 = read("TIER3C_ARTIFACT_2026-09-22.json")
    f2 = [{"family": c3["observe_used"][k // 3], "program": p,
           "parsed": parsed_for(T3C, c3["observe_used"][k // 3], 8, "A16-E4-F2-")}
          for k, p in enumerate(c3["observed_programs"])]
    a3 = read("TIER3A_ARTIFACT_2026-09-22.json")
    f3 = [{"family": a3["meta_dev_families"][k // 2], "program": p,
           "parsed": parsed_for(T3A, a3["meta_dev_families"][k // 2], 8, "A16-E4-F3-")}
...
def conformance_gate():
    std = CF.check()
    rng = random.Random(I._seed("APHRODITE/A16/CONF/v1"))
...
if __name__ == "__main__":
    ...
    if stage == "foundry":
        ...
        write("A16_CATALOGS_2026-09-24.json", r)
        ...
        cats = read("A16_CATALOGS_2026-09-24.json")
        if cats[cat]["DISPOSITION"] != "CATALOG_TESTABLE":
            write("A16_%s_RESULT_2026-09-24.json" % stage.upper(), {"BOUNDED_RSI": "UNTESTABLE",
            ...
            r["conformance_before"] = g
            write("A16_%s_RESULT_2026-09-24.json" % stage.upper(), r)
            log("%s BOUNDED_RSI=%s %s" % (stage, r["BOUNDED_RSI"], r.get("failed") or r.get("failure_link")))
    elif stage == "e4":
        r = e4()
        write("A16_E4_RESULT_2026-09-24.json", r)
