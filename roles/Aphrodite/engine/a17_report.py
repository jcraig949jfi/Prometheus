"""Assemble APHRODITE_AMENDMENT16_CAMPAIGN_REPORT_2026-09-26.md from the
...
OUT = HERE.parent / "pivot" / "APHRODITE_AMENDMENT16_CAMPAIGN_REPORT_2026-09-26.md"
...
def donor_lines(cat):
    d = rd("A17_%s_DONORS_2026-09-26.json" % cat)
    if not d:
        return []
...
def main(elapsed, extra):
    cats = rd("A17_CATALOGS_2026-09-26.json")
    e1, e2, e4 = rd("A17_E1_RESULT_2026-09-26.json"), rd("A17_E2_RESULT_2026-09-26.json"), rd("A17_E4_RESULT_2026-09-26.json")
    L = ["+" + "=" * 78 + "+",
         "|  APHRODITE -- AMENDMENT 16 CAMPAIGN REPORT, second execution (AMENDMENT 17)" + " " * 1 + "|",
         "|  Author: Aphrodite (RSI seat), host harry1 / M4.  Date: 2026-09-26" + " " * 11 + "|",
         "|  For: James (HITL) and external reviewers. Self-contained." + " " * 19 + "|",
         "+" + "=" * 78 + "+", "",
         "Prereg: AMENDMENT_16_2026-09-26.md @ 46e477d81 (T0 18:35:51Z). Code a16.py @ 5c74e4c58.",
    ...
    L.append(extra.get("catalog_c", "  CATALOG_C = generated and hashed only (E3 conditional)"))
    L.append("")
    ...
    if e4:
        ...
        for x in e4.get("F3_star_substitutions", []):
            L.append("    F3* substitution: obs %d %s -> %s (certificate PASS %s)" % (
                x["index"], x["original"], x["replacement"], x["certificate_PASS"]))
    ...
    L += ["", "-" * 78, "2b. GATES", "-" * 78] + ["  " + x for x in extra.get("gates", [])]
