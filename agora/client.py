...
class AgoraClient:
    ...
    def start_heartbeat(self, interval_sec: Optional[float] = None):
        """Start background heartbeat thread (default HEARTBEAT_INTERVAL_SEC)."""
        interval = interval_sec or HEARTBEAT_INTERVAL_SEC
        self._running = True
        def _beat():
            while self._running:
                try:
