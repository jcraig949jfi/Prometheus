...
def main(minutes: str = "15", out: str = "roles/Bellerophon/science/SOAK_SEARCH_2026-09-19.json") -> dict:
    ...
    tracemalloc.start(); gens = []; gen = 0; sel = ref("selector.map_elites.v1", n=8, representation="statemachine.v2", rank="net")
    stop = None
    while time.perf_counter() - t_start < budget_s:
        t0 = time.perf_counter()
        res = SR.evolve(template(), sel, generations=gen + 1, workdir=wd, seed=77, registry=reg)      # one more generation, RESUMING from the rows
        ...
        cells = SR.elites_by_cell(committed, rank="net")
        files = sorted(wd.glob("gen_*_a*.jsonl")); last = scan(files[-1])
        cur, peak = tracemalloc.get_traced_memory(); gc.collect()
        gens.append({"gen": gen, "wall_s": round(dt, 3), "rows": len(rows), "committed": len(committed), "cells": len(cells),
        ...
        gen += 1
    result = {"started_utc": started, "finished_utc": datetime.now(timezone.utc).isoformat(), "minutes_budget": float(minutes), "generations": gen,
