"""Planted family pa (self-contained)."""
...
def _margin(scenario, c, n, k, g, u):
    ...
    if scenario == "artifact":
        return 0.1 + (0.30 - c) * 1.5
    if scenario == "broken":
        return {"pa": 0.1 + (0.2 - c) * 1.5, "pb": 0.1 + (c - 0.5) * 1.5, "pc": 0.1 + (0.3 - n) * 1.5}["pa"]
    if scenario == "null":
...
class Planted(Family):
    version = "1"
    lineage = ("prometheus/cosmos/planted/pa.py",)

    def __init__(self, scenario: str):
        self.scenario = scenario
        self.name = "pa:" + scenario
