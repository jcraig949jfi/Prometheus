...
note=Canonical descriptor of a distilled failure invariant. Two anchors (was three; F012 dropped — see anchor revision below): (1) PROMOTED-AND-VALIDATED through sigma_kernel as boundary_dominated_octant_walk_obstruction@v1 — the 5 anchor sequences (A149074/081/082/089/090) hold ranks 1-5 globally across the entire 1534-sequence asymptotic_deviations corpus by abs(delta_pct), with a clean 25.4pp gap to rank 6 (A151261 at +53.48 vs anchor-floor A149074 at +78.89); independently verified by Iter-6 OBSTRUCTION_SHAPE_EVIDENCE_VERIFICATION (sync 1777463045948 + auditor re-check 2026-04-29). Predicts unanimous F1+F6+F9+F11 kill within A149* family at 5/5 (100%) vs 1/54 (1.9%) on A149* non-matches, 54x predictive lift; cross-family transfer to A148/A150/A151 FAILS per sigma_kernel/a150_a151_validation_results.json (Mnemosyne extended corpus, 2026-04-29) — candidate is family-specific in current evidence; promotion at narrower scope is defensible; (2) F1xF11 co-fire cluster from curvature_experiment Source C. Anchor revision (2026-04-29 cross-thread audit, sync 1777464242945): the original draft cited F012 (Möbius retraction zero-population shape) as anchor #3 but F012's actual kill mechanism is measurement non-reproduction (Pattern 19, substrate data-hygiene) not a structural mathematical failure invariant — citing F012 as an OBSTRUCTION_SHAPE anchor conflates two categories of failure. Dropped to 2 anchors pending a third. Natural third candidate: diagonal-symmetric sister-obstruction with refined signature n_steps=5, n_full_diag=5, has_diag_neg AND has_diag_pos — resolved per Ask 2 as a distinct sister to anchor #1 (A149499 currently single-anchor; relaxed (neg_x=3, pos_x=2) variant falsified by A150349-351; tighter signature consistent with all data but cross-family-untestable for lack of matches in available corpus). First forward-path use of the schema landed via sigma_kernel/a149_obstruction.py 2026-04-28. Composes with PATTERN_30, EXHAUSTION; SISTERS with LENS_MISMATCH per Ask 1 cross-resolution (sync 1777455709802; rationale at stoa/discussions/2026-04-29-sigma-kernel-mvp-Ask1-resolution.md). Prerequisite for any future DISTILL opcode. Proposed_by=sigma_kernel_MVP_Claude_session_20260428. Reviewer wanted for endorsement at the narrower (A149*-family-specific) scope and 2-anchor count, OR identification of a real third structural anchor (not a Pattern-19 measurement-hygiene case).
```
...
- [`harmonia/memory/retraction_registry.md`](../retraction_registry.md) v0.1.1 (entries on F043 / OBSTRUCTION_SHAPE cross-family / loose-sister falsification)

**What a reviewer should do:** read the CANDIDATES.md entry; read the kernel demo + a149_obstruction.py output; review the cross-family failure evidence in a150_a151_validation_results.json; review the F012 anchor-revision rationale (cross-thread audit sync 1777464242945); either (a) endorse promotion at the A149*-family-specific scope and 2-anchor count, (b) propose a real third structural anchor (not a Pattern-19 measurement-hygiene case), or (c) DISSENT with rationale. Asks 1, 2, and 4 are RESOLVED; the remaining promotion question is scope-acceptance + anchor count.
