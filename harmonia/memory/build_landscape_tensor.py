...
FEATURES = [
    ...
    {"id": "F013", "label": "Zero spacing rigidity vs rank (H06) — P028 resolves BUT downstream of F011 excised ensemble",
     ...
     "description": "Original pooled slope=-0.0019 was a Pattern-20 MIXTURE ARTIFACT (sessionB tick 18 wsw_F013_P028, n=2M). Stratified by Katz-Sarnak: SO_even slope=+0.01284 (variance INCREASES with rank); SO_odd slope=-0.00216. Slope diff z=13.68, p=1.3e-42. BLOCK-SHUFFLE VERIFIED at z_block=15.31 (sessionB audit tick 20). "
                    ...
                    "**ALGEBRAIC_LINEAGE 2026-04-20 (Pattern 30, Level 1 WEAK_ALGEBRAIC)**: P028 splits strata by root number, which is algebraically tied to rank parity via BSD; the stratum-dependent effect is expected. The empirical content is the sign pattern and z=15.31 magnitude, neither forced by the parity relation alone."},
    ...
    {"id": "F041a", "label": "Rank-2+ moment slope monotone in num_bad_primes (supersedes F041)",
     ...
     "description": "PROMOTED 2026-04-18 after 5 independent kill-test survivals. Original F041 (session's Keating-Snaith rank-dependent convergence) demoted to first-moment drift; the REAL signal is the rank-2+ interaction. "
                    ...
                    "PATTERN-30 PARTIAL annotation 2026-04-19 (methodology tightener b57f4afe): the CFKRS arithmetic factor a_E(k) is itself a function of bad-prime structure, so a CFKRS rank-2 closed-form WOULD predict an nbp-dependent slope. The Pattern 30 gate and Pattern 5 gate are therefore the same gate: when CFKRS arrives, it either predicts this nbp ladder (→ demote to calibration / algebraic consequence) or doesn't (→ CLEAN + frontier). F041a is NOT a plain algebraic identity rearrangement like F043 — the question is whether the arithmetic theory predicts the shape, not whether the shape is definitionally forced. "
                    "2026-04-22 AUDITOR EULER-DEFLATION AUDIT (Harmonia_M2_auditor, agora:harmonia_sync 1776899465123): verdict SIGN_INVERSION_AND_RESHAPE. Deflator did NOT collapse ladder amplitude (raw 0.073 → deflated 0.142) but FLIPPED the (nbp, slope) correlation sign from +0.65 to -0.83. The original F041a monotone-INCREASING framing is substantially the bad-prime Euler-factor effect; a NEW monotone-DECREASING signal emerges in the deflated quantity (z=8-18 per stratum). Pattern 30 disposition PARTIAL → SHARED_VARIABLE on the original framing. Caveat (Pattern-19 flag): auditor raw slopes (~0.25) are ~5× smaller than F041a's recorded values (1.21..2.52); methodology mismatch needs reconciliation before tensor mutation. Full audit: `cartography/docs/audit_F041a_euler_product_deflation_results.json`."},
    ...
    {"id": "F043", "label": "BSD-Sha 'anticorrelation' — RETRACTED as algebraic identity coupling",
     "tier": "killed", "n_objects": 60003,
     "description": "RETRACTED 2026-04-19 after external methodological review. The claimed corr(log Sha, log A) = -0.4343 at z_block = -348 is a DETERMINISTIC ARTIFACT of the BSD identity rearrangement, not an empirical anticorrelation. "
                    "Proof: we defined A := Omega_real * prod_p c_p. The BSD identity L = Omega * Reg * prod(c_p) * #Sha / #tors^2 rearranges as log A = log L + 2 log tors - log Sha. The term -log(Sha) appears INSIDE log(A) by definition. Therefore corr(log Sha, log A) contains a structural negative-correlation term from the algebra, not from arithmetic. "
                    "The block-shuffle-within-conductor-decile null did NOT break this coupling because the null preserves the algebraic relationship between variables (shuffles pairs, not definitions). The sha=1 restriction made it WORSE by collapsing variance in one term and amplifying the remaining coupling. "
                    "z_block = -348 is not informative evidence; it detects the BSD identity in rearranged coordinates. "
                    "Tensor INVARIANCE cells (F043, P020) and (F043, P023) demoted from +2 to -2 (provably-collapses artifact). The original T4 observation (low-L-tail sha depletion) remains valid as an empirical pattern; the decomposition into a 'period anticorrelation mechanism' was an overinterpretation that the reviewer caught. "
                    "Methodological lesson: before running correlation tests on two quantities, check whether one is algebraically defined in terms of the other. If yes, the correlation is a rearrangement of the identity, not evidence. See Pattern 30 DRAFT (algebraic-identity coupling detection)."},
    {"id": "F044", "label": "Rank-4 corridor: disc=conductor EXACTLY — additive reduction forbidden at rank ≥ 4?",
     ...
     "description": "Charon 2026-04-18 (eb6d31df): across 2086 rank-4 EC in LMFDB, 2085 have disc=conductor (prime conductor, no additive bad reduction — only multiplicative). ONLY 1 exception. Connects abc conjecture to BSD via measurable arithmetic narrowing at high rank. Open frontier questions: "
                    ...
                    "2026-04-22 AUDITOR FRAME-BASED RESAMPLE + RANK-5 SELECTION AUDITS (Harmonia_M2_auditor, agora:harmonia_sync 1776900048561 + 1776900246480): verdict RETRACTED_AS_SELECTION_ARTIFACT. At conductor ≥ 10^8, ALL ranks 2-5 in LMFDB are 100% semistable+nbp=1 (81298/16095/1281/14 curves respectively). Rank-5 complementary check: ALL 19 LMFDB rank-5 EC are 100% prime-conductor+semistable+non-CM. The 1 rank-4 exception is 234446.a1 at the low-conductor boundary. The 'rank-4 disc=conductor' pattern is a property of LMFDB's high-conductor sourcing methodology, NOT of rank-4 per se. Recommendation (pending conductor approval): tier live_specimen → killed_selection_frame; cells (P020/P023/P026) demote +1 PROVISIONAL → -2; add F044 as 4th Pattern-4 anchor. Full audits: `cartography/docs/audit_F044_framebased_resample_results.md` + `audit_F044_rank4_lmfdb_selection_results.md`."},
    {"id": "F045", "label": "Isogeny-class murmuration (5/21 primes) — novel axis, weaker than rank",
     ...
     "description": "Ergon 2026-04-18 (7e68116c): murmuration-style stratification by isogeny class size shows 5 of 21 tested primes significant (vs ~1 expected under chance). Headline: p=79 F=6.6. Effect magnitude 5-10x weaker than classical rank-based murmurations. Wachs 2026 literature confirms Sha-direction structure but NOT the isogeny axis — isogeny-class stratification is novel. PRECISION CAVEATS (adversarial review): "
                    ...
                    "2026-04-22 AUDITOR MULTIPLE-TESTING + INDEPENDENCE AUDIT (Harmonia_M2_auditor, agora:harmonia_sync 1776900402211): verdict SURVIVES_MULTIPLE_TESTING_BUT_SHARES_STRUCTURE_WITH_F041A. 5/21 raw significant → 3/25 BH FDR=0.05 (primes {5, 19, 79}) → 1/25 Bonferroni (p=79 only). F045 is NOT fully multiple-testing artifact. BUT class_size × num_bad_primes Spearman +0.455 (n=3.29M rank-0/1 EC) indicates substantial overlap with F041a's nbp axis — F045's isogeny signal partly rides F041a's nbp ladder. Proper disentanglement needs F045 stratified-within-nbp analysis. Recommendation: description edit + kill/confirm seed for stratified-within-nbp follow-up; no tensor mutation yet. Full audit: `cartography/docs/audit_F045_multiple_testing_and_independence_results.json`."},
    {"id": "F015", "label": "Szpiro vs conductor — sign-uniform, magnitude non-monotone in k — BLOCK-SHUFFLE VERIFIED",
     ...
     "description": "Szpiro-vs-conductor slope is sign-uniformly-negative within every bad-prime stratum (P042 z=-6.9..-22.7). **Block-shuffle-within-k null (sessionC audit_F014_F015_block_shuffle, 2026-04-17) VERIFIED the sign-uniform-negative claim at ALL k:** k=1 z=-24.03, k=2 z=-19.70, k=3 z=-12.69, k=4 z=-7.48, k=5 z=-4.06, k=6 z=-3.48. Every stratum z<=-3 under the rigorous null. Magnitude is NOT monotone in k (k=4 breaks the smooth trend; this remains a Pattern-20 anchor on the magnitude side). 88% of pooled -0.597 slope is k-mediated confound; 12% residual survives decontamination (P052 kill). Within-conductor bins show opposite-sign trend (szpiro increases with k). Pattern 19 variant: Ergon monotone claim partially reproduces (sign, yes; magnitude, no). F015 is the first specimen verified under the F010-methodology block-shuffle protocol — **sign claim is durable**. "
                    "PATTERN-30 PARTIAL annotation 2026-04-19 (methodology tightener b57f4afe): szpiro_ratio = log|Disc| / log(N) contains log(N) in the denominator, so a regression of szpiro vs log-conductor has a weak algebraic component in the expected slope direction. HOWEVER the sign-uniform-per-k-stratum claim and the magnitude non-monotonicity (k=4 breaking trend) are NOT forced by the algebra — only the direction-of-slope is partially expected. Sign durability and non-monotonicity remain real findings beyond the algebraic baseline."},
...
INVARIANCE = {
    ...
    "F001": {"P010": +2, "P011": +2, "P012": +2, "P020": +1},  # modularity (Wiles et al.). Re-auditor 2026-04-18: P020 retained +1 — a_p agreement = 100% by theorem; NULL_BSWCD degenerate (constant statistic, null_std=0). THEOREM, not promotable via block-shuffle.
    "F002": {"P024": +1, "P001": +1},                          # Mazur torsion (torsion in list of 15, 100%). Re-auditor 2026-04-18: retained at +1 — theorem, NULL_BSWCD degenerate. Not promotable.
    "F003": {"P020": +2, "P023": +2, "P041": +2, "P036": -1},  # BSD parity; P036 root_number = (-1)^rank by BSD → tautological collapse
    "F004": {"P043": +2},                                      # Hasse
    "F005": {"P023": +2, "P024": +1, "P036": -1, "P038": -1},  # high-Sha parity; P036 root_number and P038 sha are both constitutive of F005 (defined by sha≥9 + root_number=(-1)^rank) → tautological collapse. Re-auditor 2026-04-18: P024 retained +1 — BSD parity is conditional theorem (100% observed); NULL_BSWCD degenerate on constant statistic. THEOREM, not promotable.
    "F009": {"P024": +2, "P039": +2, "P100": +1},              # torsion primes subset of nonmax primes (Serre+Mazur lineage). P100 +1: class_size>=2 ⇒ nonmax_primes≠[] partial tautology with F009's claim; non-empty nonmax family partially overlaps the F009 anchor cohort. Re-auditor 2026-04-19: P100 retained +1 as PARTIAL_TAUTOLOGY — Serre+Mazur theorem lineage is known; the class_size overlap with F009 anchor cohort is structural but not fully tautological.

    # Live specimens — sparse +1s, many -1s in wrong projections
    "F010": {"P001": -1, "P010": -1, "P040": -2, "P042": -1, "P052": -2, "P020": -1, "P021": -1, "P028": -1, "P104": -2},  # NF backbone KILLED: decon ρ=0.27 via P052 killed by block-shuffle-within-degree null (z=-0.86). P104 ANCHOR KILL CASE: NULL_BSWCD@v1 canonical kill (sessionC 711f8325). P052 demoted +1→-2 (the decon signal is degree-marginal leakage, not real). P010 demoted +2→-1 (the object-keyed coupling doesn't survive stricter nulls). F010 joins F022 — same data, no durable signal under the correct null model. Re-auditor 2026-04-18: P020, P021, P042 demoted +1→-1 (all three inherit the KILLED status; block-shuffle null z=-0.86). Re-auditor 2026-04-19: P028 Katz-Sarnak also demoted +1→-1 — any Katz-Sarnak differential in F010's signal is spurious since F010 itself collapses under BSWCD.
    "F011": {"P050": +2, "P051": +2, "P021": +2, "P023": +2, "P024": -1, "P025": +2, "P026": +2, "P027": -1, "P028": +2, "P020": +2, "P104": +2, "P036": +2},  # GUE deficit: tier downgraded to calibration_confirmed 2026-04-18 per Aporia Report 1. P028 resolves + block-shuffle verified. P036 root_number stratification = (-1)^rank by BSD, so information-identical to P028 on EC (+2 by formal equivalence; not independent measurement). P020 +2 (conductor IS the resolving axis, Duenez-HKMS excised ensemble). P104 DURABLE at z_block=111.78. Not novel anomaly; instrument correctly detected known RMT effect. Re-auditor 2026-04-18 (NULL_BSWCD@v1): P021 z=38.58, P023 z=31.26, P025 z=6.03, P026 z=5.53, P050/P051 z=160.20 all DURABLE → promoted +1→+2. P024 torsion z=1.37 COLLAPSES (conductor-mediated) → demoted +1→-1.
    ...
    "F013": {"P023": +2, "P041": +2, "P028": +2, "P051": +2, "P104": +2},   # spacing rigidity; P028 resolves at z=13.68 (SO_even vs SO_odd opposite sign, sessionB 2026-04-17). P051 unfolding also matters (sessionD prior). P104 DURABLE at z_block=15.31 (sessionB audit_P028_findings_block_shuffle). Re-auditor 2026-04-18 (NULL_BSWCD@v1): P023 slope_mean_vs_rank z=22.39, P041 eta2 z=8211.96, P051 slope_var_vs_rank z=8.05 — all three promoted +1→+2.
    "F014": {"P053": +2, "P040": +1, "P023": +2, "P021": +2},   # Lehmer spectrum (refined, sessionB 2026-04-17): P053 Mahler + P023 degree (bound touched at deg 10, 20) + P021 num_ram monotone (touched only at num_ram=1,2; jumps at 3+). Re-auditor 2026-04-18: P040 DEFERRED — needs proper Mahler-measure-table block-shuffle audit, not run this iteration; retained at +1 with DEFERRED flag.
    "F015": {"P021": +2, "P020": +2, "P042": +2, "P051": 0, "P052": -1, "P001": -1, "P104": +2, "P023": -1},    # Szpiro sign-uniform / magnitude non-monotone in k; 88% k-mediated (sessionD wsw_F015 2026-04-17); P042 z=-6.9..-22.7; P104 DURABLE at every k-stratum, z∈[-24.03, -3.48]. P023 rank -1: Szpiro is defined on all ranks without rank-conditioning; the k-monotone structure is rank-uniform in sessionD's within-conductor tests. First specimen to pass block-shuffle protocol — durable sign. Re-auditor 2026-04-18: P020 promoted +1→+2 per existing audit_F014_F015_block_shuffle (max |z|=24.03, DURABLE at every k-stratum).
    "F041a": {"P023": +2, "P020": +2, "P021": +2, "P026": +2, "P039": -1, "P104": +2, "P025": -1, "P036": -1},   # PROMOTED 2026-04-18. Rank-2+ monotone-in-nbp slope ladder survives: block-null W2 (amp 27.6x corr 0.97), conductor-control U_A (z=3.37), P026 semistable split T3, specific-prime joint T5. P039 Galois-image ruled out (W3). P025 CM -1: rank-2+ CM is rare (coverage cliff); P036 root_number -1: F041a is defined on rank-2+ which splits both root-number classes non-trivially; stratifying by root_number doesn't isolate the monotone slope. CFKRS Pattern 5 gate pending. Re-auditor 2026-04-18: P020 and P026 promoted +1→+2 per existing wsw_F041a_block_null (corr(nbp,slope)=0.966, amplification 27.6x).
    "F042": {"P025": 0, "P020": 0, "P037": -1, "P039": 0},                                                     # CM disc=-27 quantitative precision on known qualitative structure (Gross 1980, RVZ 1993). P037 Sato-Tate → N(U(1)) by CM hypothesis (tautological). P039 Galois ℓ-adic image +1 via CM curves having characteristic nonmax_primes structure. Re-auditor 2026-04-18: P020, P025 demoted +1→0 — per sessionC literature scan (c9a7543a), F042 6.66x depression is Deuring non-maximal-order character-sum compression (Gross LNM). Re-auditor 2026-04-19: P039 also demoted +1→0 — CM curves having characteristic ℓ-adic image is a direct consequence of CM theory (Deuring, Gross-Zagier), not a novel structural claim. Consistent with F042 overall literature-downgrade.
    "F044": {"P020": +2, "P023": +2, "P026": +2, "P035": 0, "P101": -1},                                        # Rank-4 corridor disc=conductor in 2085/2086. Semistable forced. P035 Kodaira +1: semistable implies I_n types only (tautology-adjacent with P026 +2). P101 EC regulator -1: rank-4 regulator axis collapses (only 2085 rows, regulator stratification under-powered / F033 coverage cliff). Open: theorem vs LMFDB artifact. Re-auditor 2026-04-19: P035 Kodaira demoted +1→0 — direct tautology with P026 +2 (semistable ↔ I_n types by definition). No independent structural content.
    "F045": {"P023": +1, "P100": -1},                                                                           # Isogeny-class murmuration 5/21 primes significant. Novel axis per Wachs 2026. Effect 5-10x weaker than rank. P100 -1: isogeny class size IS the F045 test-axis (definitional tautology). Pattern 5 gate + multiple-testing rigor pending. Re-auditor 2026-04-18: P023 DEFERRED — needs per-prime per-rank block-shuffle audit against conductor, not run this iteration; retained at +1 with DEFERRED flag.
    "F043": {"P020": -2, "P023": -2, "P038": -2, "P036": -1},                                                  # RETRACTED 2026-04-19. The claimed corr(log Sha, log A) = -0.43 at z_block = -348 is a deterministic artifact of the BSD identity rearrangement: A = L*tors^2/Sha means log(A) contains -log(Sha) by definition, so corr(log Sha, log A) has built-in negative coupling. Block-shuffle null did not break it because the null preserves algebraic relationships. All three cells demoted to -2 (provably-collapses artifact). Was prior +2/+2/0.
    ...
    "F023": {"P020": -2, "P040": -1},                           # spectral tail: kills under conductor. Re-auditor 2026-04-18: P040 demoted +1→-1 — conductor-conditioning kills all 4 bins (p>0.05); signal is conductor-mediated, not structural.
    ...
    "F028": {"P020": -2, "P041": -2, "P001": -1},              # Szpiro-Faltings: identity not coupling. Re-auditor 2026-04-18: P001 demoted +1→-1 — both sides encode log|Disc|, so ρ=0.97 is near-identity, not cross-domain coupling. Fails non-tautological admission gate.
...
FEATURE_EDGES = [
    ...
    # ===== edge-weaver batch 2 (Mnemosyne, 2026-04-18) =====
    {"from": "F009", "to": "F004", "relation": "calibration_peer",
     "note": "F009 description quote: 'Load-bearing calibration anchor; joins F001-F005.' "
             "F004 Hasse bound is part of the calibration family F009 joins."},
    {"from": "F042", "to": "F011", "relation": "sub_finding_of",
     "note": "F011 description names the T4 sub-family finding: 'rank-0 low-tail Pr[L/M_1<0.25] enriched for CM (1.73x, cm=-27: 6.66x), class_size=3 (1.78x); sha>1 depleted.' F042 description quantifies the same enrichment: '6.66x enrichment factor, 2.5x leading_term depression between cm=-3 and cm=-27, 71.4% low-L tail participation.' The 6.66x number matches both descriptions; F042 is the standalone write-up of the CM sub-finding within F011’s T4 probe."},
    {"from": "F043", "to": "F011", "relation": "mechanistically_explains",
     "note": "F043 description quote: 'Mechanistically EXPLAINS T4 (cbe7b623) low-L-tail sha depletion: the period drops faster than Sha rises, so large-Sha curves are BUFFERED out of the low-L tail relative to sha-independent baseline.' T4 is the sub-family probe inside F011; F043 provides a period-vs-Sha mechanism for the sha depletion F011 documents."},
    {"from": "F044", "to": "F003", "relation": "constrains",
