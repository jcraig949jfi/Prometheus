...
### 2026-05-27 — "Rank-1 KillVector basis" (topological_falsification_engine §6.2)

**Was:** The KillVector basis "MEASURED 2026-05-27: rank-1 ... effective dimensionality = 1.0 ... a single band-distance ruler wearing a 12-D costume," with a proposed test ("if LLM-mutation raises eff-dim above 1, the thesis has legs").
**Status:** DOWNGRADED (verdict was an instrumentation artifact; basis dimensionality UNMEASURED, not measured-as-1).
**Mechanism:** Every one of the 24,000 vectors in `_native_kill_vector_pilot.json` carries exactly ONE component (`out_of_band`) — including the in-band Lehmer hit (ep 622, M=1.17628, reward 100). The other 11 were never *recorded* (phase-0 short-circuit `discovery_pipeline.py:341-368` × pilot emitting `pipeline_record=None`). A correlation matrix over 1-component vectors is algebraically forced to rank-1. Legacy 315k corpus is ≤2-component by construction (`KILL_VECTOR_SPEC.md:95-97`). The proposed "dim>1" test is confounded: it measures in-band yield (= survival rate), not basis geometry, and is blind to orthogonality.
**What survives:** First actual in-band measurement (control arm, `gen_inband_basis_demonstrator.py`): eff-dim = **2.30** — multi-dimensional, refuting rank-1 — BUT with `out_of_band` ≡ `F9` perfectly collinear (corr 1.0): duplicated rulers, so PR alone misleads; read off-diagonals. The thesis itself is untested, not refuted. Env finding: `cypari` absent on M2 Windows → real battery can't run here.
**Anchor:** `harmonia/memory/architecture/reaudit_killvector_rank1_2026-05-27.md`; validators `harmonia/tmp/kv_basis_dim_inband.py`, `harmonia/runners/gen_inband_basis_demonstrator.py`.
**Lesson:** A dimensionality / rank verdict measured on data where N−1 of the N coordinates are never populated measures the *population's gate-pass rate*, not the basis. Before citing a basis as rank-r, confirm every coordinate was computed on a shared candidate set; read off-diagonal correlations, not just participation ratio (collinear rulers inflate neither past their true count nor reveal themselves in PR alone). Same family as the Pattern-20 "pooled statistic is a projection" failure.

### 2026-05-30 — "Sqrt-construction-specific over-refusal quirk; does NOT generalize"

**Was:** After the 2-arm n=40 stress test (sqrt-ph0 + rational_extra, 2026-05-29), I retracted the broader "Opus 4.8 legality-execution axis vs Sonnet 4.6" claim and kept only a narrow "sqrt-canonical over-refusal quirk that does not generalize beyond sqrt." The retraction rested on Opus matching Sonnet on rational_extra (0.97 ≈ 1.00) in that 2-arm draw, so the supposed gap was inferred to be sqrt-memorization-adjacent.
**Status:** UN-RETRACTED → POSSIBLE (re-opened, not promoted). The 4-arm replication on the same day (sqrt + rational + abs + log, n=40 each, task `bad3ay4wk`) showed over-refusals climb monotonically with operation legality-load (1 → 2 → 7 → 15), with a large, **non-overlapping-CI** Opus<Sonnet gap on log_extra (0.62 vs 1.00, 15/15 errors = over_refused). That falsified "does not generalize." Two separate caveats keep it at POSSIBLE (not promoted): (a) sqrt itself replicated UNSTABLY in the same 4-arm draw — 0.85 (earlier) → 0.97 (now), same seed — so single n=40 estimates are demonstrably noisy at ±0.12; (b) the "domain-constraint load" gradient was assigned **post-hoc** from the result, not predicted; it becomes predictive only when a pre-registered dcl=3 arm produces a larger gap than dcl=2.
**Mechanism of the original retraction failure:** **Falsifier coverage was too narrow.** Two operations (sqrt, rational) cannot test a claim about generalization across operation families; a single non-confirming arm out of two is *ambiguous*, not falsifying. I read it as falsifying because I weighed "the rational arm didn't show it" as evidence against generalization, when in fact rational is one of the *lowest*-dcl operations and would be expected to show the smallest gap under the (then-undeclared) load hypothesis. The retraction was epistemically premature: it punished an unrefined hypothesis instead of expanding the falsifier set.
**What survives:**
- The POSSIBLE-tier reading of the 4-arm result, **gated on a pre-registered 3-arm replication** (`abs_extra_clean` dcl=1, `log_extra` dcl=2, `log_extra_3arg` dcl=3) with binding decision rules — see `D:\Prometheus\harmonia\memory\architecture\legality_overrefusal_4arm_writeup_2026-05-30.md` §3.
- A *predictive* (not descriptive) statement of H1: log-3arg over-refusal will exceed log-2arg over-refusal, CIs non-overlapping. Falsifier built into `gen_log_extra_3arg` + `run_legality_replication.py`.
- The framing that a confirmed double dissociation (Sonnet>Opus on legality vs Opus>Sonnet on R8 lemma-selection) would be small-N-legitimate evidence against a rank-1 ladder — *conditional on both halves replicating*.
**Anchor:** `harmonia/memory/architecture/legality_overrefusal_4arm_writeup_2026-05-30.md` (repo-relative so the validator resolves it; the file exists and always did — the entry had written it with a `D:\` prefix, which the validator reduces to a basename and then fails. Path form corrected 2026-08-31 by Harmonia C; no content changed); memory hub `C:\Users\James\.claude\projects\D--Prometheus\memory\project_topological_falsification_engine.md` (the 4-ARM entry with `[SUPERSEDED]` marker on the prior retraction).
**Lesson:** Before retracting a claim about generalization, expand the falsifier set across the dimension you're generalizing along — not orthogonal to it. Two adjacent test cases (sqrt + rational, both heavily-memorized canonical forms) cannot falsify a "scales with X" claim if X varies along an axis those two cases don't span. Symptom: the retraction reads a *small* gap (0.97 vs 1.00) as a "no effect" when it's actually consistent with the not-yet-stated load hypothesis. Apply: when about to retract on a single non-confirming arm, ask "what does the parameter space of the *suspected* mechanism predict for this arm, before I declare it falsifying?" — if the suspected mechanism predicts a small effect there too, the arm is *consistent with*, not *against*, the hypothesis.

### 2026-08-31 — "The 57.8% action-divergence statistic has a chance floor of ~50%; the excess is the effect size"

**Was:** My own filing of 2026-08-25 (`D:\Prometheus\pivot\NOTE_2026-08-25_action_divergence_chance_floor.md`, handed to Charon as `D:\Prometheus\pivot\HANDOFF_2026-08-25_harmonia_C_to_charon.md` §1–§2). Claimed that under "the action is irrelevant", divergence has a **floor** of `2p(1-p) ≈ 0.50`; that the observed 0.578 therefore carried a **+7.8pp excess** which **is the effect size**; and that this excess at ~34 SE made regret "very likely non-vacuous" and the pre-registered replacement experiment well-motivated at a rescaled effect.
**Status:** RETRACTED (the floor framing, the excess-as-effect-size reading, and the non-vacuity conclusion). The instrument `harmonia/diagnostics/action_divergence_floor.py` is SUPERSEDED, banner added, code retained.
**Mechanism:** Two independent defects that compound.
- **(a) The bound points the other way.** With per-state success probabilities `q_s`, an irrelevant action gives `D = E[2 q_s (1-q_s)] <= 2 E[q_s](1-E[q_s]) = 2p(1-p)` by Jensen. `2p(1-p)` is a **ceiling** on action-irrelevant divergence, not a floor; state heterogeneity always pushes `D` down. A fully state-determined world gives `D = 0.0000` against a ceiling of `0.5000` with no action effect at all.
- **(b) The excess is not an effect size.** Exact identity, verified to Monte-Carlo error: `D - 2p(1-p) = 2 (E[d_s²] - Var(q_s))` — a *difference* of the action-effect term and the state-heterogeneity term, neither identified without the other. My filing implicitly set `Var(q_s) = 0`.
- **And the input was independently wrong.** Charon's exact 369.5 GB scan (`charon/SESSION_2026-08-25_post_reset.md`) replaced the 47,389-state headline of 57.8% with **41.1% over 932,852 both-action parents** — which sits **0.085 BELOW** the ceiling, i.e. exactly what an irrelevant action over heterogeneous states produces. Same magnitude as my claimed excess, opposite sign. The sample was unrepresentative *in the one direction that makes an action look decisive*.
**What survives:**
- **Exceeding `2p(1-p)` IS a sound one-sided test** — it implies `E[d²] > Var(q) >= 0`, so the actions genuinely differ. Conservative but valid. The corpus does not exceed it, and falling below licenses nothing: `D` is silent here in **both** directions. The overcorrection ("the framing was useless") is also wrong and is recorded as such.
- Three measured limits on the statistic: `D` is invariant under swapping the action labels (so it can never say *which* action is better, nor whether the difference is predictable — **non-vacuity is not navigability**); `D` is **maximised by a fair coin** at every state (the least navigable world scores highest); and the oracle-vs-coin gap is exactly `D/2`, so **oracle gap is room, not signal**.
- The identifying move: `Var(q_s)` needs repeated observations at the same `(state, action)`. Charon has them — 1,251,927 `(P,A)` groups with ≥2 rows, 38.55% multi-outcome — but they vary the *replacement object* (`(P,A⁺)` cells are singletons, 0 / 5,871,696), so they estimate replacement-induced variance. That is Charon's **UNDER-SPECIFIED ACTION** ruling arriving through a second instrument.
- The exchangeability caveat I filed as a footnote (§3 of the handoff) is now the load-bearing open question: **selection in which parents get a second action logged** would anti-correlate the pair and inflate `D` above the ceiling with no action effect whatsoever.
**Anchor:** `pivot/CORRECTION_2026-08-31_action_divergence_withdrawn.md`; instrument `harmonia/diagnostics/divergence_decomposition.py` (`--test` → 4/4, identity verified over four regimes); superseded instrument `harmonia/diagnostics/action_divergence_floor.py`; the corrected filings `pivot/NOTE_2026-08-25_action_divergence_chance_floor.md` and `pivot/HANDOFF_2026-08-25_harmonia_C_to_charon.md`. (Repo-relative here so the registry validator can resolve them; absolute paths in the correction doc's prose.)
**Lesson:** This is failure-mode #1 (marginal/structural confound) wearing the uniform of the discipline meant to catch it. `feedback_measurement_carries_its_answer` says every metric needs a published chance floor — **I published a chance floor that was a ceiling.** Applying a discipline is not the same as applying it correctly, and the discipline's own output is not exempt from the discipline. Concretely, before publishing any chance floor: (1) write the null as an *expectation over the heterogeneity you are averaging across*, not as a single-parameter draw, and check Jensen's direction on the statistic's curvature; (2) check whether the statistic is invariant under a symmetry of the thing you claim it measures — if swapping the treatment labels leaves it unchanged, it cannot be estimating a treatment effect's sign; (3) ask what world *maximises* the statistic, and if the answer is "pure noise", the metric measures room, not signal. Also: the note went out the same day it was computed, favouring a live experiment's continuation, with zero independent falsification families — exactly what `feedback_positive_results_are_provisional` forbids — and it quoted a 47,389-state sample as a population, which is my own rule (`feedback_base_rate_null_for_pattern_claims`) broken by the seat that carries it.
